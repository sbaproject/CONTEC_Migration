...
Friend Class FR_SSSMAIN
    ...
    '=== 当画面の全情報を格納 =================
    Private Const FM_PANEL3D1_CNT As Short = 31 'パネルコントロール数
    ...
    Private Function Init_Def_Dsp() As Short
        ...
        '    Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Str_Typ = IN_STR_TYP_X
        'CHG START FKS)INABA 2006/11/27 ***************************************
        '''' UPD 2009/02/24  FKS) S.Nakajima    Start
        '    Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.MaxLengthB = 8
        '    Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.Dsp_MaxLengthB = 8
        Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.MaxLengthB = 10
        Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.Dsp_MaxLengthB = 10
        '''' UPD 2009/02/24  FKS) S.Nakajima    End
        Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.Num_Int_Fig = 0
        Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.Num_Fra_Fig = 0
        Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.Num_Sign_Fig = IN_NUM_ELSE
        Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.Fil_Chr = Space(1)
    ...
    Private Function Ctl_Item_Click(ByRef pm_Ctl As System.Windows.Forms.Control) As Short
        ...
        Select Case Trg_Index
            ...
            Case CShort(CS_UODSU.Tag)
                'SRAET62の引数
                '/RPTCLTID:CLTID /PGID:IDOET62 /SBNNO:RA02HF /HINCD:LRBQ671 /URISU:100 /DATNO:
            ...
            Case CShort(btnF1.Tag)
                ...
                'add 20190829 start hou
            Case CShort(btnF3.Tag)
                Call Ctl_MN_Execute_Click()
                'add 20190829 end hou
    ...
    Private Function Ctl_MN_DeleteCM_Click() As Short
        'delete 20190829 start hou
        ''add 20190828 start hou
        'Dim intRet As Short
        ...
        '    Call Ctl_MN_APPENDC_Click()
        'End If
        ''add 20190828 end hou
        'delete 20190829 end hou
    End Function
    ...
    Private Function Set_Body_Location() As Short
        ...
        BD_HINCD_top = VB6.FromPixelsUserY(BD_HINCD(1).Top, 0, 8832.42, 527)
        BD_HINCD_height = VB6.FromPixelsUserHeight(BD_HINCD(1).Height, 8832.42, 527) + Hosei_Value

        '１行目｢No｣から｢品名｣までの相対位置を取得
        BD_HINNMB_Top = VB6.FromPixelsUserY(BD_HINNMB(1).Top, 0, 8832.42, 527) - BD_HINCD_top
        '１行目｢No｣から｢備考２｣までの相対位置を取得
        BD_LINCMB_Top = VB6.FromPixelsUserY(BD_LINCMB(1).Top, 0, 8832.42, 527) - BD_HINCD_top
        ...
        For Bd_Index = 1 To Main_Inf.Dsp_Base.Dsp_Body_Cnt
            If Bd_Index >= 2 Then
                '２行目以降から
                '配置
                BD_HINCD(Bd_Index).Top = VB6.ToPixelsUserY(BD_HINCD_top + BD_HINCD_height * (Bd_Index - 1), 0, 8832.42, 527)
                BD_HINNMA(Bd_Index).Top = VB6.ToPixelsUserY(BD_HINCD_top + BD_HINCD_height * (Bd_Index - 1), 0, 8832.42, 527)
                BD_HINNMB(Bd_Index).Top = VB6.ToPixelsUserY(BD_HINCD_top + BD_HINCD_height * (Bd_Index - 1) + BD_HINNMB_Top, 0, 8832.42, 527)
                BD_UODSU(Bd_Index).Top = VB6.ToPixelsUserY(BD_HINCD_top + BD_HINCD_height * (Bd_Index - 1), 0, 8832.42, 527)
                BD_UNTNM(Bd_Index).Top = VB6.ToPixelsUserY(BD_HINCD_top + BD_HINCD_height * (Bd_Index - 1), 0, 8832.42, 527)
                BD_LINCMA(Bd_Index).Top = VB6.ToPixelsUserY(BD_HINCD_top + BD_HINCD_height * (Bd_Index - 1), 0, 8832.42, 527)
                BD_LINCMB(Bd_Index).Top = VB6.ToPixelsUserY(BD_HINCD_top + BD_HINCD_height * (Bd_Index - 1) + BD_LINCMB_Top, 0, 8832.42, 527)
    ...
    'add 20190828 start hou
    Private Sub CS_UODSU_Click(sender As Object, e As EventArgs) Handles CS_UODSU.Click
        Call Ctl_Item_Click(CS_UODSU)
    End Sub
    'add 20190827 end hou
