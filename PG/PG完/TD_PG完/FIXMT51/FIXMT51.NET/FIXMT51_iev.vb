...
Module FIXMT51_IEV
	...
	Sub SCR_FromMfil(ByVal De As Short) 'Generated.
        ...
        Call DP_SSSMAIN_CTLCD(De, DB_FIXMTA2.CTLCD)
        Call DP_SSSMAIN_CTLNM(De, DB_FIXMTA2.CTLNM)
        Call DP_SSSMAIN_FIXVAL(De, DB_FIXMTA2.FIXVAL)
        Call DP_SSSMAIN_REMARK(De, DB_FIXMTA2.REMARK)
        '20190801 chg end
        ...
        M_FIXMT_A_inf(De).OPEID = DB_FIXMTA2.OPEID '最終作業者コード
        M_FIXMT_A_inf(De).CLTID = DB_FIXMTA2.CLTID 'クライアントＩＤ
        M_FIXMT_A_inf(De).UOPEID = DB_FIXMTA2.UOPEID '最終作業者コード（バッチ）
        M_FIXMT_A_inf(De).UCLTID = DB_FIXMTA2.UCLTID 'クライントＩＤ（バッチ）
        M_FIXMT_A_inf(De).WRTDT = DB_FIXMTA2.WRTDT '更新日付
        M_FIXMT_A_inf(De).WRTTM = DB_FIXMTA2.WRTTM '更新時刻
        M_FIXMT_A_inf(De).UWRTDT = DB_FIXMTA2.UWRTDT 'バッチ更新日付
        M_FIXMT_A_inf(De).UWRTTM = DB_FIXMTA2.UWRTTM 'バッチ更新時刻
        '20190801 chg end
	...
	Sub Mfil_FromSCR(ByVal De As Short) 'Generated.
        ...
        DB_FIXMTA2.CTLCD = RD_SSSMAIN_CTLCD(De)
        ...
        DB_FIXMTA2.CTLNM = RD_SSSMAIN_CTLNM(De)
        ...
        DB_FIXMTA2.FIXVAL = RD_SSSMAIN_FIXVAL(De)
        ...
        DB_FIXMTA2.REMARK = RD_SSSMAIN_REMARK(De)
        DB_FIXMTA2.OPEID = SSS_OPEID.Value
        DB_FIXMTA2.CLTID = SSS_CLTID.Value
        If Trim(DB_ORATM) = "" Or Trim(DB_ORADT) = "" Then
            'DB_FIXMTA.WRTTM = VB6.Format(Now, "hhmmss")
            'DB_FIXMTA.WRTDT = VB6.Format(Now, "YYYYMMDD")
            DB_FIXMTA2.WRTTM = VB6.Format(Now, "hhmmss")
            DB_FIXMTA2.WRTDT = VB6.Format(Now, "YYYYMMDD")
        Else
            'DB_FIXMTA.WRTTM = DB_ORATM
            '         DB_FIXMTA.WRTDT = DB_ORADT
            DB_FIXMTA2.WRTTM = DB_ORATM
            DB_FIXMTA2.WRTDT = DB_ORADT
            '2010801 chg end
        End If
	End Sub
