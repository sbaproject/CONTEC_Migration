...
Module SSSMAIN0001
	...
	Public gv_bolTOKMT56_INIT As Boolean '画面初期化フラグ（True:変更あり）
	' === 20060801 === INSERT S - エンターキー連打による不具合修正・検索W表示時の不具合対応
	Public gv_bolTOKMT56_LF_Enable As Boolean 'LF処理実行フラグ(False：実行しない)
    ...
    '20190724 ADD END
    Public Structure TOKMT56_TYPE_TOKMTA
	...
	End Structure
	'得意先マスタ情報
	Public TOKMT56_TOKMTA_Inf As TOKMT56_TYPE_TOKMTA
	
	Public Structure TOKMT56_TYPE_MEIMTA
		Dim DATKB As String '削除区分
		Dim MEICDA As String 'コード１
		Dim MEINMA As String '名称１
	End Structure
	'名称マスタ情報
	Public TOKMT56_MEIMTA_Inf As TOKMT56_TYPE_MEIMTA
	
	Public Structure TOKMT56_TYPE_HINMTA
		Dim DATKB As String '削除区分
		Dim DSPKB As String '検索表示区分
	...
	End Structure
	'商品マスタ情報
	Public TOKMT56_HINMTA_Inf As TOKMT56_TYPE_HINMTA
	
	Public Structure TOKMT56_TYPE_TKRMTA
	...
	End Structure
	'得意先別取扱商品マスタ情報
	Public TOKMT56_TKRMTA_Inf As TOKMT56_TYPE_TKRMTA
	...
	'インデックス情報
	Public Current_Hincd_Index As Short '製品コードの操作対象行
	
	'得意先コード
	Public pv_TOKMT56_TOKCD As String
	'仕切用商品群
	Public pv_TOKMT56_SKHINGRP As String
	'製品コード
	Public pv_TOKMT56_HINCD As String
	...
	Private Const pc_COL_HINCD As Short = 2 '製品コード
	Private Const pc_COL_HINNMA As Short = 3 '型式
	' 2006/11/15  ADD START  KUMEDA
	Private Const pc_COL_UPDATE As Short = 4 '更新フラグ
	' 2006/11/15  ADD END
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_GET_TKR_SQL
	'   概要：  データ取得ＳＱＬ生成
	'   引数：　なし
	'   戻値：　生成SQL
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_GET_TKR_SQL() As String
		...
		strSQL = strSQL & "     TKRMTA.DATKB " '伝票削除区分
		strSQL = strSQL & "    ,TKRMTA.TOKCD " '得意先コード
		strSQL = strSQL & "    ,TKRMTA.SKHINGRP " '仕切用商品群
		strSQL = strSQL & "    ,TKRMTA.SKWRKKB " '仕切処理区分
		strSQL = strSQL & "    ,TKRMTA.HINCD " '製品コード
		strSQL = strSQL & "    ,HINMTA.HINNMA " '型式
		' === 20080911 === UPDATE S - RISE)Izumi
		''2007/12/22 add-str T.KAWAMUKAI
		'    strSQL = strSQL & "    ,TKRMTA.WRTDT "              '更新日付
		'    strSQL = strSQL & "    ,TKRMTA.WRTTM "              '更新時刻
		'    strSQL = strSQL & "    ,TKRMTA.UWRTDT "             'バッチ更新日付
		'    strSQL = strSQL & "    ,TKRMTA.UWRTTM "             'バッチ更新時刻
		''2007/12/22 add-end T.KAWAMUKAI
		strSQL = strSQL & "    ,TKRMTA.OPEID " '最終作業者コード
		strSQL = strSQL & "    ,TKRMTA.CLTID " 'クライアントＩＤ
		strSQL = strSQL & "    ,TKRMTA.WRTDT " '更新日付
		strSQL = strSQL & "    ,TKRMTA.WRTTM " '更新時刻
		strSQL = strSQL & "    ,TKRMTA.UOPEID " '最終作業者コード（バッチ）
		strSQL = strSQL & "    ,TKRMTA.UCLTID " 'クライアントＩＤ（バッチ）
		strSQL = strSQL & "    ,TKRMTA.UWRTDT " 'バッチ更新日付
		strSQL = strSQL & "    ,TKRMTA.UWRTTM " 'バッチ更新時刻
		' === 20080911 === UPDATE E - RISE)Izumi
		strSQL = strSQL & " From "
		strSQL = strSQL & "     TKRMTA "
		strSQL = strSQL & "    ,( "
		strSQL = strSQL & "         Select "
		strSQL = strSQL & "             Substr(HINCD,1,6) As HINCD "
		strSQL = strSQL & "            ,Max(HINNMA) As HINNMA "
		strSQL = strSQL & "         From "
		strSQL = strSQL & "             HINMTA "
		strSQL = strSQL & "         Group By "
		strSQL = strSQL & "             Substr(HINCD,1,6) "
		strSQL = strSQL & "     ) HINMTA "
		strSQL = strSQL & " Where "
		strSQL = strSQL & "     TKRMTA.TOKCD = '" & CF_Ora_String(pv_TOKMT56_TOKCD, 10) & "' "
		strSQL = strSQL & " And TKRMTA.SKHINGRP = '" & CF_Ora_String(pv_TOKMT56_SKHINGRP, 4) & "' "
		
		'製品コードに入力がある場合
		If Trim(pv_TOKMT56_HINCD) <> "" Then
			strSQL = strSQL & " And TKRMTA.HINCD >= '" & CF_Ora_String(pv_TOKMT56_HINCD, 10) & "' "
		End If
		
		strSQL = strSQL & " And TKRMTA.HINCD = RPAD(HINMTA.HINCD, 10 ,' ') "
		strSQL = strSQL & " Order By "
		strSQL = strSQL & "     TKRMTA.HINCD "
		
		F_GET_TKR_SQL = strSQL
	...
	Public Function F_GET_BD_DATA(ByRef pm_All As Cls_All) As Short
        ...
        '			'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        '			.Bus_Inf.SKWRKKB = CF_Ora_GetDyn(Usr_Ody, "SKWRKKB", "") '仕切処理区分
        '			'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        ...
        '			Call CF_Edi_Dsp_Body_Inf(.Bus_Inf.HINCD, pm_All.Dsp_Sub_Inf(Wk_Index), intCnt, pm_All, SET_FLG_DB)
        '			pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(2).Focus_Ctl = True
        '			'型式
        '			Wk_Index = CShort(FR_SSSMAIN.BD_HINNMA(1).Tag)
        '			Call CF_Edi_Dsp_Body_Inf(.Bus_Inf.HINNMA, pm_All.Dsp_Sub_Inf(Wk_Index), intCnt, pm_All, SET_FLG_DB)
        '			' 2006/11/15  ADD START  KUMEDA
        '			'更新フラグ
        ...
        Else
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                With pm_All.Dsp_Body_Inf.Row_Inf(intCnt)
                    ...
                    '20190725 CHG START
                    '.Bus_Inf.DATKB = CF_Ora_GetDyn(Usr_Ody, "DATKB", "") '伝票削除区分
                    .Bus_Inf.DATKB = DB_NullReplace(dt.Rows(i)("DATKB"), "") '伝票削除区分
                    '20190725 CHG END
                    If .Bus_Inf.DATKB = gc_strDATKB_USE Then
                        '使用中
                        .Bus_Inf.UPDKB = UPDKB_UPD 'モード
                    ...
                    End If
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.TOKCD = DB_NullReplace(dt.Rows(i)("TOKCD"), "") '得意先コード
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.SKHINGRP = DB_NullReplace(dt.Rows(i)("SKHINGRP"), "") '仕切用商品群
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.SKWRKKB = DB_NullReplace(dt.Rows(i)("SKWRKKB"), "") '仕切処理区分
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.HINCD = DB_NullReplace(dt.Rows(i)("HINCD"), "") '製品コード
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.HINNMA = DB_NullReplace(dt.Rows(i)("HINNMA"), "") '型式

                    ' === 20080911 === INSERT S - RISE)Izumi
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.MOTO_OPEID = DB_NullReplace(dt.Rows(i)("OPEID"), "")
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.MOTO_CLTID = DB_NullReplace(dt.Rows(i)("CLTID"), "")
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.MOTO_UOPEID = DB_NullReplace(dt.Rows(i)("UOPEID"), "")
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.MOTO_UCLTID = DB_NullReplace(dt.Rows(i)("UCLTID"), "")
                    ' === 20080911 === INSERT E - RISE)Izumi
                    '2007/12/18 add-str M.SUEZAWA 訂正前に更新時間チェックを入れる
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.MOTO_WRTDT = DB_NullReplace(dt.Rows(i)("WRTDT"), "") '更新日付
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.MOTO_WRTTM = DB_NullReplace(dt.Rows(i)("WRTTM"), "") '更新時刻
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.MOTO_UWRTDT = DB_NullReplace(dt.Rows(i)("UWRTDT"), "") 'バッチ更新日付
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    .Bus_Inf.MOTO_UWRTTM = DB_NullReplace(dt.Rows(i)("UWRTTM"), "") 'バッチ更新時刻
                    '2007/12/18 add-end M.SUEZAWA
...
ERR_F_GET_BD_DATA: 
	...
	Private Function F_Body_Dsp(ByRef pm_All As Cls_All) As Short
		...
		If pm_All.Dsp_Base.Dsp_Body_Cnt > 0 Then
			...
			'得意先コードか仕切用商品群が未入力の場合
			If (Index_Of_Window = 0) And (Trim(FR_SSSMAIN.HD_TOKCD.Text) = "" Or Trim(FR_SSSMAIN.HD_SKHINGRP.Text) = "") Then
				Exit Function
			End If
			...
			If Index_Of_Window < pm_All.Dsp_Base.Dsp_Body_Cnt Then
				'データ最終行の次行のIndexを取得
				Index_Wk = CShort(FR_SSSMAIN.BD_HINCD(Index_Of_Window + 1).Tag)
				'１行のフォーカスの制御
				Call CF_Set_Item_Focus_Ctl(True, pm_All.Dsp_Sub_Inf(Index_Wk))
				
				'データ最終行のIndexを取得
				Index_Wk = CShort(FR_SSSMAIN.BD_HINCD(Index_Of_Window).Tag)
	...
	Private Function F_Jge_Focus(ByRef pm_Index_Tag As Short, ByRef pm_All As Cls_All, ByRef pm_Av_Flg As Boolean) As Boolean
		...
		'項目が「製品コード」である場合
		If pm_All.Dsp_Sub_Inf(pm_Index_Tag).Ctl.Name = FR_SSSMAIN.BD_HINCD(1).Name Then
			'対象行の状態が初期状態以外の場合
	...
	Public Function F_Set_Befe_Focus(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Move_Flg As Boolean, ByRef pm_All As Cls_All, Optional ByRef pm_Run_Flg As Boolean = True, Optional ByRef pm_Mode As Short = BEFE_FOCUS_MODE_KEYLEFT) As Short
		...
		If pm_Mode = BEFE_FOCUS_MODE_KEYUP Then
			If (pm_Dsp_Sub_Inf.Detail.Body_Index = 1) And (pm_Dsp_Sub_Inf.Ctl.Tag <> FR_SSSMAIN.BD_HINCD(1).Tag) Then
				Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(1).Tag) + 1
			Else
			...
			Case CHK_FROM_ALL_CHK
				...
				If CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf) = pm_Chk_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
					...
					If pm_Chk_Dsp_Sub_Inf.Detail.Err_Status = ERR_NOT And pm_Chk_Dsp_Sub_Inf.Detail.Not_Input_Chk_Fin_Flg = True Then
						...
						' === 20060825 === UPDATE S
						If (pm_Chk_Dsp_Sub_Inf.Ctl.Name <> FR_SSSMAIN.BD_HINCD(1).Name) Or (FR_SSSMAIN.BD_UPDKB(0).Text = UPDKB_DEL) Then
							'項目が製品コード以外、またはモードが削除の場合
							' === 20060825 === UPDATE E
							'中断
							Rtn_Cd = CHK_STOP
							'メッセージ非表示
	...
	Public Function F_Chk_HD_TOKCD(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
        ...
        If CF_Trim_Item(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf) = "" Then
			TOKMT56_TOKMTA_Inf.DATKB = Mst_Inf_Clr.DATKB
			TOKMT56_TOKMTA_Inf.DSPKB = Mst_Inf_Clr.DSPKB
			TOKMT56_TOKMTA_Inf.TOKCD = Mst_Inf_Clr.TOKCD '得意先コード
			TOKMT56_TOKMTA_Inf.TOKRN = Mst_Inf_Clr.TOKRN '得意先略称
		Else
			'未入力以外のチェック済
			...
			Else
				...
				If DSPTOKCD_SEARCH(Input_Value, Mst_Inf) = 0 Then
					...
					If Mst_Inf.DATKB = gc_strDATKB_DEL Then
						Retn_Code = CHK_ERR_ELSE
						Err_Cd = gc_strMsgTOKMT56_E_003
						'検索表示区分チェック
					ElseIf Mst_Inf.DSPKB = gc_strDSPKB_NG Then 
						Retn_Code = CHK_ERR_ELSE
						Err_Cd = gc_strMsgTOKMT56_E_004
					Else
						'ＯＫ
						Retn_Code = CHK_OK
						pm_Chk_Move = True
						TOKMT56_TOKMTA_Inf.DATKB = Mst_Inf.DATKB
						TOKMT56_TOKMTA_Inf.DSPKB = Mst_Inf.DSPKB
						TOKMT56_TOKMTA_Inf.TOKCD = Mst_Inf.TOKCD '得意先コード
						TOKMT56_TOKMTA_Inf.TOKRN = Mst_Inf.TOKRN '得意先略称
					End If
					'該当データ無し
				Else
					Retn_Code = CHK_ERR_ELSE
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Chk_HD_SKHINGRP(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		
		Dim Input_Value As String
		Dim Mst_Inf As TYPE_DB_MEIMTA
		Dim Mst_Inf_Clr As TYPE_DB_MEIMTA
		Dim Retn_Code As Short
		Dim Msg_Flg As Boolean
		Dim Rtn_Cd As Short
		Dim Bd_Index As Short
        ...
        'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        If CF_Trim_Item(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf) = "" Then
			TOKMT56_MEIMTA_Inf.DATKB = Mst_Inf_Clr.DATKB
			TOKMT56_MEIMTA_Inf.MEICDA = Mst_Inf_Clr.MEICDA 'コード１（仕切用商品群）
			TOKMT56_MEIMTA_Inf.MEINMA = Mst_Inf_Clr.MEINMA '名称２
		Else
			'未入力以外のチェック済
			pm_Chk_Dsp_Sub_Inf.Detail.Not_Input_Chk_Fin_Flg = True
			...
			Else
				'マスタチェック
				If DSPMEIM_SEARCH(pv_Skhingrp_Keycode, Input_Value, Mst_Inf) = 0 Then
					'論理削除チェック
					If Mst_Inf.DATKB = gc_strDATKB_DEL Then
						Retn_Code = CHK_ERR_ELSE
						Err_Cd = gc_strMsgTOKMT56_E_003
					Else
						...
						Retn_Code = CHK_OK
						pm_Chk_Move = True
						TOKMT56_MEIMTA_Inf.DATKB = Mst_Inf.DATKB
						TOKMT56_MEIMTA_Inf.MEICDA = Mst_Inf.MEICDA 'コード１（仕切用商品群）
						TOKMT56_MEIMTA_Inf.MEINMA = Mst_Inf.MEINMA '名称２
					End If
					'該当データ無し
				Else
					Retn_Code = CHK_ERR_ELSE
	...
	Public Function F_Chk_BD_HINCD(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Dim Input_Value As String
		Dim Mst_Inf_T As TOKMT56_TYPE_TKRMTA
		Dim Mst_Inf_H As TYPE_DB_HINMTA
		Dim Retn_Code As Short
		Dim Msg_Flg As Boolean
		Dim Rtn_Cd As Short
		Dim Bd_Index As Short
		...
		'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		If CF_Trim_Item(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf) = "" Then
			TOKMT56_TKRMTA_Inf.UPDKB = "" 'モード
			TOKMT56_TKRMTA_Inf.DATKB = "" '削除区分
			TOKMT56_TKRMTA_Inf.TOKCD = Space(10) '得意先コード
			TOKMT56_TKRMTA_Inf.SKHINGRP = Space(4) '仕切用商品群
			TOKMT56_TKRMTA_Inf.SKWRKKB = Space(1) '仕切処理区分
			TOKMT56_TKRMTA_Inf.HINCD = Space(6) '製品コード
			TOKMT56_TKRMTA_Inf.HINNMA = Space(30) '型式
			
			' === 20080911 === UPEDATE S - RISE)Izumi
			''2007/12/27 upd-str M.SUEZAWA
			''''2007/12/27 add-str T.KAWAMUKAI
			'''        With pm_All.Dsp_Body_Inf.Row_Inf(pm_Chk_Dsp_Sub_Inf.Detail.Body_Index)
			'''            .Bus_Inf.MOTO_WRTDT = ""                     '更新日付
			'''            .Bus_Inf.MOTO_WRTTM = ""                     '更新時刻
			'''            .Bus_Inf.MOTO_UWRTDT = ""                    'バッチ更新日付
			'''            .Bus_Inf.MOTO_UWRTTM = ""                    'バッチ更新時刻
			'''        End With
			''''2007/12/27 add-end T.KAWAMUKAI
			'        TOKMT56_TKRMTA_Inf.MOTO_WRTDT = ""                     '更新日付
			'        TOKMT56_TKRMTA_Inf.MOTO_WRTTM = ""                     '更新時刻
			'        TOKMT56_TKRMTA_Inf.MOTO_UWRTDT = ""                    'バッチ更新日付
			'        TOKMT56_TKRMTA_Inf.MOTO_UWRTTM = ""                    'バッチ更新時刻
			''2007/12/27 upd-end M.SUEZAWA
			'タイムスタンプを削除
			With pm_All.Dsp_Body_Inf.Row_Inf(pm_Chk_Dsp_Sub_Inf.Detail.Body_Index)
				' === 20081222 === INSERT S - RISE)Izumi
				.Bus_Inf.DATKB = ""
				.Bus_Inf.UPDKB = ""
				' === 20081222 === INSERT E - RISE)Izumi
				.Bus_Inf.TOKCD = "" '得意先コード
				.Bus_Inf.SKHINGRP = "" '仕切用商品群
				.Bus_Inf.HINCD = "" '製品コード
				.Bus_Inf.MOTO_OPEID = "" '最終作業者コード
				.Bus_Inf.MOTO_CLTID = "" 'クライアントＩＤ
				.Bus_Inf.MOTO_WRTTM = "" '更新時刻
				.Bus_Inf.MOTO_WRTDT = "" '更新日付
				.Bus_Inf.MOTO_UOPEID = "" '最終作業者コード（バッチ）
				.Bus_Inf.MOTO_UCLTID = "" 'クライアントＩＤ（バッチ）
				.Bus_Inf.MOTO_UWRTTM = "" 'バッチ更新時刻
				.Bus_Inf.MOTO_UWRTDT = "" 'バッチ更新日付
			End With
			' === 20080911 === UPEDATE E - RISE)Izumi
		Else
			'未入力以外のチェック済
			pm_Chk_Dsp_Sub_Inf.Detail.Not_Input_Chk_Fin_Flg = True
			...
			Else
				Select Case pm_Chk_Dsp_Sub_Inf.Detail.Chk_From_Process
					Case CHK_FROM_ALL_CHK
						'一括チェックの場合
						'商品マスタチェック
						If DSPHINCD_SEARCH_GROUP_ALL(Input_Value, Mst_Inf_H) = 0 Then
							'論理削除チェック
							If Mst_Inf_H.DATKB = gc_strDATKB_DEL Then
								Retn_Code = CHK_ERR_ELSE
								Err_Cd = gc_strMsgTOKMT56_E_013
								'検索表示区分チェック
							ElseIf Mst_Inf_H.DSPKB = gc_strDSPKB_NG Then 
								Retn_Code = CHK_ERR_ELSE
								Err_Cd = gc_strMsgTOKMT56_E_014
								'仕切用商品群チェック
							ElseIf Mst_Inf_H.SKHINGRP <> pv_TOKMT56_SKHINGRP Then 
								Retn_Code = CHK_ERR_ELSE
								Err_Cd = gc_strMsgTOKMT56_E_014
							End If
						...
						Else
							Retn_Code = CHK_ERR_ELSE
							Err_Cd = gc_strMsgTOKMT56_E_012
						End If
						
					Case Else
						'一括チェック以外の場合
						'マスタチェック
						If TKRMTA_SEARCH_ALL(Input_Value, Mst_Inf_T) = 0 Then
							'該当データ有り
							Retn_Code = CHK_OK
							pm_Chk_Move = True
							If Mst_Inf_T.DATKB = gc_strDATKB_USE Then
								TOKMT56_TKRMTA_Inf.UPDKB = UPDKB_UPD 'モード（更新）
							Else
								TOKMT56_TKRMTA_Inf.UPDKB = UPDKB_DEL 'モード（削除）
							End If
							TOKMT56_TKRMTA_Inf.DATKB = Mst_Inf_T.DATKB '伝票削除区分
							TOKMT56_TKRMTA_Inf.TOKCD = Mst_Inf_T.TOKCD '得意先コード
							TOKMT56_TKRMTA_Inf.SKHINGRP = Mst_Inf_T.SKHINGRP '仕切用商品群
							TOKMT56_TKRMTA_Inf.SKWRKKB = Mst_Inf_T.SKWRKKB '仕切処理区分
							TOKMT56_TKRMTA_Inf.HINCD = Mst_Inf_T.HINCD '製品コード
							TOKMT56_TKRMTA_Inf.HINNMA = Mst_Inf_T.HINNMA '型式
							
							' === 20080911 === UPEDATE S - RISE)Izumi
							''2007/12/27 upd-str M.SUEZAWA
							'''''2007/12/22 add-str T.KAWAMUKAI 訂正前に更新時間チェックを入れる
							''''                        With pm_All.Dsp_Body_Inf.Row_Inf(pm_Chk_Dsp_Sub_Inf.Detail.Body_Index)
							''''                            .Bus_Inf.MOTO_WRTDT = Mst_Inf_T.WRTDT                        '更新日付
							''''                            .Bus_Inf.MOTO_WRTTM = Mst_Inf_T.WRTTM                        '更新時刻
							''''                            .Bus_Inf.MOTO_UWRTDT = Mst_Inf_T.UWRTDT                      'バッチ更新日付
							''''                            .Bus_Inf.MOTO_UWRTTM = Mst_Inf_T.UWRTTM                      'バッチ更新時刻
							''''                        End With
							'''''2007/12/22 add-end T.KAWAMUKAI
							'                        TOKMT56_TKRMTA_Inf.MOTO_WRTDT = Mst_Inf_T.MOTO_WRTDT                '更新日付
							'                        TOKMT56_TKRMTA_Inf.MOTO_WRTTM = Mst_Inf_T.MOTO_WRTTM                '更新時刻
							'                        TOKMT56_TKRMTA_Inf.MOTO_UWRTDT = Mst_Inf_T.MOTO_UWRTDT              'バッチ更新日付
							'                        TOKMT56_TKRMTA_Inf.MOTO_UWRTTM = Mst_Inf_T.MOTO_UWRTTM              'バッチ更新時刻
							''2007/12/27 upd-end M.SUEZAWA
							'タイムスタンプ情報を退避
							With pm_All.Dsp_Body_Inf.Row_Inf(pm_Chk_Dsp_Sub_Inf.Detail.Body_Index)
								' === 20081222 === INSERT S - RISE)Izumi
								.Bus_Inf.DATKB = Mst_Inf_T.DATKB
								If Mst_Inf_T.DATKB = gc_strDATKB_USE Then
									.Bus_Inf.UPDKB = UPDKB_UPD 'モード（更新）
								Else
									.Bus_Inf.UPDKB = UPDKB_DEL 'モード（削除）
								End If
								' === 20081222 === INSERT E - RISE)Izumi
								.Bus_Inf.TOKCD = Mst_Inf_T.TOKCD '得意先コード
								.Bus_Inf.SKHINGRP = Mst_Inf_T.SKHINGRP '仕切用商品群
								.Bus_Inf.HINCD = Mst_Inf_T.HINCD '製品コード
								.Bus_Inf.MOTO_OPEID = Mst_Inf_T.MOTO_OPEID '最終作業者コード
								.Bus_Inf.MOTO_CLTID = Mst_Inf_T.MOTO_CLTID 'クライアントＩＤ
								.Bus_Inf.MOTO_WRTTM = Mst_Inf_T.MOTO_WRTTM '更新日付
								.Bus_Inf.MOTO_WRTDT = Mst_Inf_T.MOTO_WRTDT '更新時刻
								.Bus_Inf.MOTO_UOPEID = Mst_Inf_T.MOTO_UOPEID '最終作業者コード（バッチ）
								.Bus_Inf.MOTO_UCLTID = Mst_Inf_T.MOTO_UCLTID 'クライアントＩＤ（バッチ）
								.Bus_Inf.MOTO_UWRTDT = Mst_Inf_T.MOTO_UWRTDT 'バッチ更新日付
								.Bus_Inf.MOTO_UWRTTM = Mst_Inf_T.MOTO_UWRTTM 'バッチ更新時刻
							End With
							' === 20080911 === UPEDATE E - RISE)Izumi
						...
						Else
							...
							If DSPHINCD_SEARCH_GROUP_ALL(Input_Value, Mst_Inf_H) = 0 Then
								...
								Else
									...
									Retn_Code = CHK_OK
									pm_Chk_Move = True
									TOKMT56_TKRMTA_Inf.UPDKB = UPDKB_INS 'モード（追加）
									TOKMT56_TKRMTA_Inf.DATKB = gc_strDATKB_USE '伝票削除区分
									TOKMT56_TKRMTA_Inf.TOKCD = FR_SSSMAIN.HD_TOKCD.Text '得意先コード
									TOKMT56_TKRMTA_Inf.SKHINGRP = FR_SSSMAIN.HD_SKHINGRP.Text '仕切用商品群
									TOKMT56_TKRMTA_Inf.SKWRKKB = "9" '仕切処理区分
									TOKMT56_TKRMTA_Inf.HINCD = Input_Value '製品コード
									TOKMT56_TKRMTA_Inf.HINNMA = Mst_Inf_H.HINNMA '型式
								End If
	...
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function TKRMTA_SEARCH_ALL(ByVal pin_strHINCD As String, ByRef pot_DB_TKRMTA As TOKMT56_TYPE_TKRMTA) As Short
		...
		On Error GoTo ERR_TKRMTA_SEARCH_ALL
		...
		strSQL = ""
		strSQL = strSQL & " Select "
		strSQL = strSQL & "     TKRMTA.DATKB " '伝表削除区分
		strSQL = strSQL & "    ,TKRMTA.TOKCD " '得意先コード
		strSQL = strSQL & "    ,TKRMTA.SKHINGRP " '仕切用商品群
		strSQL = strSQL & "    ,TKRMTA.SKWRKKB " '仕切処理区分
		strSQL = strSQL & "    ,TKRMTA.HINCD " '製品コード
		...
		''2007/12/22 add-str T.KAWAMUKAI
		'    strSQL = strSQL & "    ,TKRMTA.WRTDT "              '更新日付
		'    strSQL = strSQL & "    ,TKRMTA.WRTTM "              '更新時間
		'    strSQL = strSQL & "    ,TKRMTA.UWRTTM "             'バッチ更新日付
		'    strSQL = strSQL & "    ,TKRMTA.UWRTDT "             'バッチ更新時間
		''2007/12/22 add-end T.KAWAMUKAI
		strSQL = strSQL & "    ,TKRMTA.OPEID " '最終作業者コード
		strSQL = strSQL & "    ,TKRMTA.CLTID " 'クライアントＩＤ
		strSQL = strSQL & "    ,TKRMTA.WRTDT " '更新日付
		...
		strSQL = strSQL & "     TKRMTA.TOKCD    = '" & CF_Ora_String(pv_TOKMT56_TOKCD, 10) & "' "
		strSQL = strSQL & " And TKRMTA.SKHINGRP = '" & CF_Ora_String(pv_TOKMT56_SKHINGRP, 4) & "' "
		strSQL = strSQL & " And TKRMTA.HINCD    = '" & CF_Ora_String(pin_strHINCD, 10) & "' "
		strSQL = strSQL & " And TKRMTA.HINCD    = RPAD(HINMTA.HINCD, 10 ,' ') "
        ...
        '		' === 20080911 === INSERT S - RISE)Izumi
        '		'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        '		.MOTO_OPEID = CF_Ora_GetDyn(Usr_Ody, "OPEID", "") '最終作業者コード
        '		'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        '		.MOTO_CLTID = CF_Ora_GetDyn(Usr_Ody, "CLTID", "") 'クライアントＩＤ
        '		'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        '		.MOTO_UOPEID = CF_Ora_GetDyn(Usr_Ody, "UOPEID", "") '最終作業者コード（バッチ）
        '		'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        '		.MOTO_UCLTID = CF_Ora_GetDyn(Usr_Ody, "UCLTID", "") 'クライアントＩＤ（バッチ）
        '		' === 20080911 === INSERT E - RISE)Izumi
        '		'2007/12/21 add-str M.SUEZAWA
        '		'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        '		.MOTO_WRTDT = CF_Ora_GetDyn(Usr_Ody, "WRTDT", "") '更新日付
        '		'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        '		.MOTO_WRTTM = CF_Ora_GetDyn(Usr_Ody, "WRTTM", "") '更新時間
        '		'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        '		.MOTO_UWRTDT = CF_Ora_GetDyn(Usr_Ody, "UWRTDT", "") 'バッチ更新日付
        '		'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        '		.MOTO_UWRTTM = CF_Ora_GetDyn(Usr_Ody, "UWRTTM", "") 'バッチ更新時間
        '		'2007/12/21 add-end M.SUEZAWA
        '	End With
        'End If
        If dt.Rows.Count > 0 Then
            With pot_DB_TKRMTA
                ...
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                .SKWRKKB = DB_NullReplace(dt.Rows(0)("SKWRKKB"), "") '仕切処理区分
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                ...
                ' === 20080911 === INSERT S - RISE)Izumi
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                .MOTO_OPEID = DB_NullReplace(dt.Rows(0)("OPEID"), "") '最終作業者コード
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                .MOTO_CLTID = DB_NullReplace(dt.Rows(0)("CLTID"), "") 'クライアントＩＤ
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                .MOTO_UOPEID = DB_NullReplace(dt.Rows(0)("UOPEID"), "") '最終作業者コード（バッチ）
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                .MOTO_UCLTID = DB_NullReplace(dt.Rows(0)("UCLTID"), "") 'クライアントＩＤ（バッチ）
                ' === 20080911 === INSERT E - RISE)Izumi
                '2007/12/21 add-str M.SUEZAWA
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                .MOTO_WRTDT = DB_NullReplace(dt.Rows(0)("WRTDT"), "") '更新日付
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                .MOTO_WRTTM = DB_NullReplace(dt.Rows(0)("WRTTM"), "") '更新時間
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                .MOTO_UWRTDT = DB_NullReplace(dt.Rows(0)("UWRTDT"), "") 'バッチ更新日付
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                .MOTO_UWRTTM = DB_NullReplace(dt.Rows(0)("UWRTTM"), "") 'バッチ更新時間
                '2007/12/21 add-end M.SUEZAWA
            End With
        End If
        '20190724 CHG END
...
END_TKRMTA_SEARCH_ALL: 
		'クローズ
		Call CF_Ora_CloseDyn(Usr_Ody)
		
		Exit Function
		
ERR_TKRMTA_SEARCH_ALL: 
		GoTo END_TKRMTA_SEARCH_ALL
	...
	' === 20060825 === INSERT S
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function DSPHINCD_SEARCH_GROUP_ALL
	'   概要：  製品コード検索
	...
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function DSPHINCD_SEARCH_GROUP_ALL(ByVal pin_strHINCD As String, ByRef pot_DB_HINMTA As TYPE_DB_HINMTA) As Short
		...
		On Error GoTo ERR_DSPHINCD_SEARCH_GROUP_ALL
		
		DSPHINCD_SEARCH_GROUP_ALL = 9
		
		strSQL = ""
		strSQL = strSQL & " Select "
		strSQL = strSQL & "     NVL(HINMTA_1.DATKB, HINMTA_2.DATKB) As DATKB "
		strSQL = strSQL & "    ,NVL(HINMTA_1.DSPKB, HINMTA_2.DSPKB) As DSPKB "
		strSQL = strSQL & "    ,HINMTA_2.HINCD6 As HINCD "
		strSQL = strSQL & "    ,HINMTA_2.HINNMA As HINNMA "
		strSQL = strSQL & "    ,HINMTA_2.SKHINGRP As SKHINGRP "
		strSQL = strSQL & " From "
		strSQL = strSQL & "     ( "
		strSQL = strSQL & "         Select "
		strSQL = strSQL & "             DATKB "
		strSQL = strSQL & "            ,DSPKB "
		strSQL = strSQL & "            ,Substr(HINCD, 1, 6) As HINCD6 "
		strSQL = strSQL & "         From "
		strSQL = strSQL & "             HINMTA "
		strSQL = strSQL & "         Where "
		strSQL = strSQL & "             DATKB = '" & gc_strDATKB_USE & "' "
		strSQL = strSQL & "         And DSPKB = '" & gc_strDSPKB_OK & "' "
		strSQL = strSQL & "         And HINCD Like '" & pin_strHINCD & "%' "
		strSQL = strSQL & "         Group By "
		strSQL = strSQL & "             DATKB "
		strSQL = strSQL & "            ,DSPKB "
		strSQL = strSQL & "            ,Substr(HINCD, 1, 6) "
		strSQL = strSQL & "     ) HINMTA_1 "
		strSQL = strSQL & "    ,( "
		strSQL = strSQL & "         Select "
		strSQL = strSQL & "             Max(DATKB) As DATKB "
		strSQL = strSQL & "            ,Max(DSPKB) As DSPKB "
		strSQL = strSQL & "            ,Substr(HINCD,1,6) As HINCD6 "
		strSQL = strSQL & "            ,Max(HINNMA) As HINNMA "
		strSQL = strSQL & "            ,Max(SKHINGRP) As SKHINGRP "
		strSQL = strSQL & "         From "
		strSQL = strSQL & "             HINMTA "
		strSQL = strSQL & "         Where "
		strSQL = strSQL & "             HINCD Like '" & pin_strHINCD & "%' "
		strSQL = strSQL & "         Group By "
		strSQL = strSQL & "             Substr(HINCD, 1, 6) "
		strSQL = strSQL & "     ) HINMTA_2 "
		strSQL = strSQL & " Where "
		strSQL = strSQL & "     HINMTA_2.HINCD6 = HINMTA_1.HINCD6(+) "

        '    strSQL = ""
        '    strSQL = strSQL & " Select "
        '    strSQL = strSQL & "     Min(DATKB) As DATKB "
        '    strSQL = strSQL & "    ,Min(DSPKB) As DSPKB "
        '    strSQL = strSQL & "    ,Substr(HINCD,1,6) As HINCD "
        '    strSQL = strSQL & "    ,Max(HINNMA) As HINNMA "
        '    strSQL = strSQL & "    ,Max(SKHINGRP) As SKHINGRP "
        '    strSQL = strSQL & " From "
        ...
        'If CF_Ora_EOF(Usr_Ody) = True Then
        '	'取得データなし
        '	DSPHINCD_SEARCH_GROUP_ALL = 1
        '	GoTo END_DSPHINCD_SEARCH_GROUP_ALL
        'End If
        Dim dt As DataTable = DB_GetTable(strSQL)
        ...
        DSPHINCD_SEARCH_GROUP_ALL = 0
		
END_DSPHINCD_SEARCH_GROUP_ALL: 
		'クローズ
		Call CF_Ora_CloseDyn(Usr_Ody)
		
		Exit Function
		
ERR_DSPHINCD_SEARCH_GROUP_ALL: 
		GoTo END_DSPHINCD_SEARCH_GROUP_ALL
	...
	Public Function F_Chk_All_Input_Serch(ByRef pm_All As Cls_All) As Boolean
		...
		With FR_SSSMAIN
			...
			If Trim(.HD_TOKCD.Text) = "" Or Trim(.HD_SKHINGRP.Text) = "" Then
				
				Err_Cd = gc_strMsgTOKMT56_E_015
				'メッセージ出力
				Call AE_CmnMsgLibrary(SSS_PrgNm, Err_Cd, pm_All)
				bolAll = True
	...
	Public Function F_Chk_All_Input(ByRef pm_All As Cls_All) As Boolean
		...
		'明細行に入力がないならエラー
		With FR_SSSMAIN
			If Trim(.BD_HINCD(1).Text) = "" And Trim(.BD_HINCD(2).Text) = "" And Trim(.BD_HINCD(3).Text) = "" And Trim(.BD_HINCD(4).Text) = "" And Trim(.BD_HINCD(5).Text) = "" And Trim(.BD_HINCD(6).Text) = "" And Trim(.BD_HINCD(7).Text) = "" And Trim(.BD_HINCD(8).Text) = "" And Trim(.BD_HINCD(9).Text) = "" And Trim(.BD_HINCD(10).Text) = "" And Trim(.BD_HINCD(11).Text) = "" And Trim(.BD_HINCD(12).Text) = "" And Trim(.BD_HINCD(13).Text) = "" And Trim(.BD_HINCD(14).Text) = "" And Trim(.BD_HINCD(15).Text) = "" Then
				
				Err_Cd = gc_strMsgTOKMT56_E_005
				'メッセージ出力
				Call AE_CmnMsgLibrary(SSS_PrgNm, Err_Cd, pm_All)
				bolAll = True
	...
	Public Function F_Dsp_Item_Detail(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			...
			Case FR_SSSMAIN.HD_TOKCD.Name
				...
				'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				pv_TOKMT56_TOKCD = CF_Get_Item_Value(pm_Dsp_Sub_Inf)
				
			Case FR_SSSMAIN.HD_SKHINGRP.Name
				'仕切用商品群(コード)による画面表示
				Call F_Dsp_HD_SKHINGRP_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
				
				'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				pv_TOKMT56_SKHINGRP = CF_Get_Item_Value(pm_Dsp_Sub_Inf)
				
			Case FR_SSSMAIN.BD_HINCD(1).Name
				'製品コードによる画面表示
				Call F_Dsp_BD_HINCD_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All, pm_Dsp_Sub_Inf.Detail.Body_Index)
	...
	Public Function F_Dsp_HD_TOKCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				Dsp_Value = CF_Cnv_Dsp_Item(TOKMT56_TOKMTA_Inf.TOKRN, pm_All.Dsp_Sub_Inf(Trg_Index), False)
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Dsp_HD_SKHINGRP_Inf
	'   概要：  仕切用商品群による画面表示
	'   引数：  pm_Dsp_Sub_Inf      :画面情報
	'           pm_Mode             :モード
	'           pm_all              :全構造体
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_SKHINGRP_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				'【仕切用商品群名】
				Trg_Index = CShort(FR_SSSMAIN.HD_SKHINRN.Tag)
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Dsp_Value = CF_Cnv_Dsp_Item(TOKMT56_MEIMTA_Inf.MEINMA, pm_All.Dsp_Sub_Inf(Trg_Index), False)
				Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All, SET_FLG_DB)
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_BD_HINCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All, ByRef pm_Index As Short) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Dsp_Value = CF_Cnv_Dsp_Item(TOKMT56_TKRMTA_Inf.UPDKB, pm_All.Dsp_Sub_Inf(Trg_Index), False)
				Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All, SET_FLG_DB)
				
				'【型式】
				Trg_Index = CShort(FR_SSSMAIN.BD_HINNMA(pm_Index).Tag)
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Dsp_Value = CF_Cnv_Dsp_Item(TOKMT56_TKRMTA_Inf.HINNMA, pm_All.Dsp_Sub_Inf(Trg_Index), False)
				Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All, SET_FLG_DB)
		...
		Else
			...
			If pm_Index = 0 Then
				'【製品コード】
				Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(pm_Index).Tag)
				Call CF_Init_Clr_Dsp(pm_All.Dsp_Sub_Inf(Trg_Index), ITM_ALL_ONLY, pm_All)
			End If
	...
	Public Function F_Set_Focus_Ctl(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All, ByRef pm_Index As Short) As Short
		...
		If pm_Index < pm_All.Dsp_Base.Dsp_Body_Cnt Then
			'次行の製品コードが空の場合
			If Trim(FR_SSSMAIN.BD_HINCD(pm_Index + 1).Text) = "" Then
				Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(pm_Index + 1).Tag)
				Call CF_Set_Item_Focus_Ctl(Fcs_Flg, pm_All.Dsp_Sub_Inf(Trg_Index))
			End If
		End If
	...
	Public Function F_Ctl_Item_Chk(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Process As String, ByRef pm_Chk_Move_Flg As Boolean, ByRef pm_All As Cls_All) As Short
        ...
        Select Case pm_Dsp_Sub_Inf.Ctl.Name
            ...
            Case FR_SSSMAIN.HD_SKHINGRP.Name
                Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
                '仕切用商品群のﾁｪｯｸ
                Rtn_Chk = F_Chk_HD_SKHINGRP(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
            ...
            Case FR_SSSMAIN.BD_HINCD(1).Name
                ...
                If CF_Trim_Item(CF_Get_Item_Value(pm_Dsp_Sub_Inf), pm_Dsp_Sub_Inf) = "" Then
                    ...
                    If Bd_Index < UBound(pm_All.Dsp_Body_Inf.Row_Inf) Then
                        '対象がデータ最終行の場合
                        If (pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Status = BODY_ROW_STATE_INPUT) And (pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index + 1).Status = BODY_ROW_STATE_LST_ROW) Then
                            '現在行を最終準備行に、次行を初期状態に戻す
                            pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Status = BODY_ROW_STATE_LST_ROW
                            pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index + 1).Status = BODY_ROW_STATE_DEFAULT
                        End If
                    End If
                End If
    ...
    Public Function F_Ctl_CS_SKHINGRP(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
        ...
        '割当ｲﾝﾃﾞｯｸｽ取得
        Trg_Index = CShort(FR_SSSMAIN.HD_SKHINGRP.Tag)
        ' === 20060702 === INSERT S
        Next_Focus = Trg_Index + 1
        ' === 20060702 === INSERT E
    ...
    '   備考：
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
    Public Function F_Ctl_CS_HINCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
        ...
        '割当ｲﾝﾃﾞｯｸｽ取得
        Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(Current_Hincd_Index).Tag)
        ' === 20060702 === INSERT S -
        Next_Focus = Trg_Index + 1
        ' === 20060702 === INSERT E
        ...
        If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
            ...
            '製品検索画面を呼び出す
            ' === 20060825 === INSERT S
            WLSHIN_BHNSEARCH = "0" '部品商品マスタ検索フラグ（検索しない）
            ' === 20060825 === INSERT E
            ' === 20061026 === INSERT S
            WLSHIN_SKHINGRP = pv_TOKMT56_SKHINGRP
            ' === 20061026 === INSERT E
            WLSHIN.ShowDialog()
            ' === 20060725 === INSERT S -
            WLSHIN.Close()
    ...
    Public Function F_Ctl_CS(ByRef pm_All As Cls_All) As Short
        ...
        Select Case Cursor_Index
            ...
            Case CShort(FR_SSSMAIN.HD_SKHINGRP.Tag)
                '仕切用商品群
                'UPGRADE_WARNING: オブジェクト FR_SSSMAIN.CS_SKHINGRP.Tag の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                Trg_Index = CShort(FR_SSSMAIN.CS_SKHINGRP.Tag)
                Call F_Ctl_CS_SKHINGRP(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(1).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(1).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(2).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(2).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(3).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(3).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(4).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(4).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(5).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(5).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(6).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(6).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(7).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(7).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(8).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(8).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(9).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(9).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(10).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(10).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(11).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(11).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(12).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(12).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(13).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(13).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(14).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(14).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)

            Case CShort(FR_SSSMAIN.BD_HINCD(15).Tag)
                '製品
                Trg_Index = CShort(FR_SSSMAIN.BD_HINCD(15).Tag)
                Call F_Ctl_CS_HINCD(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
    ...
    Public Function F_Ctl_Upd_Process(ByRef pm_All As Cls_All) As Short
        ...
        '確認メッセージ表示
        intRet = AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgTOKMT56_A_008, pm_All)
        ...
        Select Case intRet
            Case MsgBoxResult.Yes
                ...
                If pv_InpTan_TOK = False Then
                    gv_bolUpdFlg = False
                    Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgTOKMT56_E_021, pm_All)
                    GoTo End_F_Ctl_Upd_Process
                End If
                ' 2007/01/11  ADD END
                ...
                '                Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgTOKMT56_E_022, pm_All)
                ...
                '更新時間チェック
                bolRet = F_Chk_UWRTDTTM(pm_All, intCnt)
                If bolRet = False Then
                    'ロールバック
                    '20190724 CHG START
                    ...
                    '20190724 CHG END
                    bolTrn = False
                    Select Case pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(pc_COL_UPDKB).Dsp_Value
                        Case UPDKB_DEL
                            Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgTOKMT56_E_023, pm_All)
                        Case Else
                            Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgTOKMT56_E_022, pm_All)
                    End Select
                    ' === 20080911 === UPDATE E - RISE)Izumi
                    '2007/12/27 del-str T.KAWAMUKAI
                    ''               F_Ctl_Upd_Process = 0
        ...
        '正常メッセージ表示
        intRet = AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgTOKMT56_E_009, pm_All)
...
Err_F_Ctl_Upd_Process:
    ...
    Public Function F_Ctl_Upd_Process2(ByRef pm_All As Cls_All) As Short
        ...
        If gv_bolTOKMT56_INIT = True Then
            '確認メッセージ表示
            intRet = AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgTOKMT56_A_016, pm_All)
        End If
...
ERR_EXIT: 
	...
	Public Function F_Ctl_Body_RelChk(ByRef pm_intRow As Short, ByRef pm_All As Cls_All, ByRef pm_ErrIdx As Short, ByRef pm_ErrRow As Short) As Short
		...
		intHINNMA = CShort(FR_SSSMAIN.BD_HINNMA(0).Tag)
		
		bolCheck = False
		'１行に必要な情報が入力されている場合、OK
		'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		If Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intUPDKB))) <> "" And Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intHINCD))) <> "" Then
			bolCheck = True
		...
		Else
			'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			Select Case True
				Case Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intUPDKB))) = "" And Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intHINCD))) <> ""
					pm_ErrIdx = CShort(FR_SSSMAIN.BD_HINCD(1).Tag)
				Case Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intUPDKB))) <> "" And Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intHINCD))) = ""
					pm_ErrIdx = CShort(FR_SSSMAIN.BD_HINCD(1).Tag)
			End Select
		...
		'１行全部未入力の場合OK
		'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		If bolCheck = False And Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intUPDKB))) = "" And Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intHINCD))) = "" And Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intHINNMA))) = "" Then
...
F_Update_RelChk_Err: 
	...
	Public Function F_Update_Main(ByRef pm_All As Cls_All) As Short
		...
		Dim Mst_Inf_T As TOKMT56_TYPE_TKRMTA
		Dim Clr_TOKMT56_TKRMTA As TOKMT56_TYPE_TKRMTA
		...
		' === 20080911 === DELETE S - RISE)Izumi
		'    'トランザクションの開始
		'    Call CF_Ora_BeginTrans(gv_Oss_USR1)
		'    bolTrn = True
		' === 20080911 === DELETE E - RISE)Izumi
					...
					If pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(pc_COL_UPDATE).Dsp_Value = "1" Then
						...
						Select Case .Item_Detail(pc_COL_UPDKB).Dsp_Value
							Case UPDKB_INS
								...
								Mst_Inf_T = Clr_TOKMT56_TKRMTA
								
								'マスタチェック
								'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Body_Inf.Row_Inf().Item_Detail().Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
								If TKRMTA_SEARCH_ALL(.Item_Detail(pc_COL_HINCD).Dsp_Value, Mst_Inf_T) = 0 Then
									'該当データ有り
									'得意先別取扱商品マスタ更新
									intRet = F_TKRMTA_Update(intCnt, pm_All)
								Else
									'該当データ無し
									'得意先別取扱商品マスタ追加
									intRet = F_TKRMTA_Insert(intCnt, pm_All)
								End If
...
F_Update_Main_Err: 
	...
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_TKRMTA_Update(ByRef pm_intCnt As Short, ByRef pm_All As Cls_All) As Short
		
		Dim strSQL As String
		Dim bolRet As Boolean
		Dim UPD_DATKB As String
		
		On Error GoTo F_TKRMTA_Update_err
		...
		With pm_All.Dsp_Body_Inf.Row_Inf(pm_intCnt)
			...
			' 2006/11/19  ADD END
			strSQL = strSQL & "  Where TOKCD       = '" & CF_Ora_String(pv_TOKMT56_TOKCD, 10) & "' " '得意先コード
			strSQL = strSQL & "    And SKHINGRP    = '" & CF_Ora_String(pv_TOKMT56_SKHINGRP, 4) & "' " '仕切用商品群
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Body_Inf.Row_Inf().Item_Detail().Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strSQL = strSQL & "    And HINCD       = '" & CF_Ora_String(.Item_Detail(pc_COL_HINCD).Dsp_Value, 10) & "' " '製品コード
		End With
...
F_TKRMTA_Update_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgTOKMT56_E_011, pm_All, "F_TKRMTA_Update")
		GoTo F_TKRMTA_Update_End
		
	End Function
	
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_TKRMTA_Insert
	'   概要：  得意先別取扱商品マスタ追加処理
	'   引数：  pm_intCnt   : 配列番号
	'           pm_All      : 全構造体
	'   戻値：　0：正常終了　9:異常終了
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_TKRMTA_Insert(ByRef pm_intCnt As Short, ByRef pm_All As Cls_All) As Short
		
		Dim strSQL As String
		Dim bolRet As Boolean
		Dim UPD_DATKB As String
		
		On Error GoTo F_TKRMTA_Insert_err
		...
		With pm_All.Dsp_Body_Inf.Row_Inf(pm_intCnt)
			...
			strSQL = strSQL & "        ,  '" & CF_Ora_String(pv_TOKMT56_TOKCD, 10) & "' "
			strSQL = strSQL & "        ,  '" & CF_Ora_String(pv_TOKMT56_SKHINGRP, 4) & "' "
			strSQL = strSQL & "        ,  '" & CF_Ora_String("9", 1) & "' "
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Body_Inf.Row_Inf().Item_Detail().Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strSQL = strSQL & "        ,  '" & CF_Ora_String(.Item_Detail(pc_COL_HINCD).Dsp_Value, 10) & "' "
...
F_TKRMTA_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgTOKMT56_E_011, pm_All, "F_TKRMTA_Insert")
		GoTo F_TKRMTA_Insert_End
			...
			If Trg_Index = CShort(pm_Dsp_Sub_Inf.Ctl.Tag) Then
				...
				Else
					...
					If pm_All.Dsp_Body_Inf.Row_Inf(pm_Row).Status = BODY_ROW_STATE_LST_ROW Then
						...
						'同行の製品コードのｲﾝﾃﾞｯｸｽ取得
						Trg_Index_Same_Row = CShort(FR_SSSMAIN.BD_HINCD(pm_Row).Tag)
						'ﾌｫｰｶｽ移動
	...
	Public Function CF_Ctl_MN_InsertDE(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		If F_Cmn_Ctl_MN_InsertDE(Bd_Index, Ins_Bd_Index, pm_All) = True Then
			'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
			'挿入した行の製品コードをフォーカスありにする
			pm_All.Dsp_Body_Inf.Row_Inf(Ins_Bd_Index).Item_Detail(2).Focus_Ctl = True
			...
			For Bd_Index_Wk = 1 To pm_All.Dsp_Base.Dsp_Body_Cnt * MaxPageNum
				If pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index_Wk).Status = BODY_ROW_STATE_DEFAULT Then
					...
					'フォーカスの制御
					pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index_Wk).Item_Detail(2).Focus_Ctl = True
	...
	'   概要：  更新時間チェック処理
	'   引数：  pm_All        : 画面情報
	'       ：  intCnt        : 行番号（排他エラーが起こった行を返す為）
	'   戻値：　True：チェックOK　False：チェックNG
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Chk_UWRTDTTM(ByRef pm_All As Cls_All, ByRef intCnt As Short) As Boolean
		...
		For intCnt = Upd_Start To Upd_End
			...
			'2007/12/27 upd-str M.SUEZAWA
			'2007/12/27 add-str T.KAWAMUKAI
			''            If Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.HINCD) = "" Then
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Body_Inf.Row_Inf().Item_Detail().Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			...
			''
			''            '更新時間取得
			''            intRet = F_Get_UWRTDTTM("TRKMTA", _
			'''                                    pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.TOKCD, _
			'''                                    pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.SKHINGRP, _
			'''                                    pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.HINCD, _
			'''                                    strWRTDT, _
			'''                                    strWRTTM, _
			'''                                    strUWRTDT, _
			'''                                    strUWRTTM)
			...
			'2007/12/27 upd-str M.SUEZAWA
			''        If pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.UPDKB = UPDKB_INS Then
			''            '追加モード時
			''            intRet = F_Get_DATA("TKRMTA", _
			'''                                    pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.TOKCD, _
			'''                                    pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.SKHINGRP, _
			'''                                    pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.HINCD)
			''            If intRet <> 0 Then
			''                GoTo F_Chk_UWRTDTTM_End
			''            End If
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(pc_COL_UPDKB).Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			If pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(pc_COL_UPDKB).Dsp_Value = UPDKB_INS Then
				'追加モード時
				'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Body_Inf.Row_Inf().Item_Detail().Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				intRet = F_Get_DATA("TKRMTA", pv_TOKMT56_TOKCD, pv_TOKMT56_SKHINGRP, pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(pc_COL_HINCD).Dsp_Value)
				If intRet <> 9 Then
					GoTo F_Chk_UWRTDTTM_End
				End If
				'2007/12/27 upd-end M.SUEZAWA
			...
			Else
				' === 20081222 === UPDATE S - RISE)Izumi   　変更してない行はチェック対象外とする
				' === 20080911 === UPDATE S - RISE)Izumi
				...
				''
				''            '更新・削除モード時
				''            intRet = F_Get_UWRTDTTM("TKRMTA", _
				'''                                    pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.TOKCD, _
				'''                                    pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.SKHINGRP, _
				'''                                    pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.HINCD, _
				'''                                    strWRTDT, _
				...
				''            End If
				'
				'            strUWRT_MOTO = Trim$(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_OPEID) _
				''                         & Trim$(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_CLTID) _
				''                         & Trim$(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_UOPEID) _
				''                         & Trim$(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_UCLTID) _
				''                         & Trim$(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_WRTDT) _
				''                         & Trim$(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_WRTTM) _
				''                         & Trim$(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_UWRTDT) _
				''                         & Trim$(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_UWRTTM)
				'
				...
				'
				'            '更新時間チェック
				'            If Trim$(strOPEID) & Trim$(strCLTID) & Trim$(strUOPEID) & Trim$(strUCLTID) & _
				''               Trim$(strWRTDT) & Trim$(strWRTTM) & Trim$(strUWRTDT) & Trim$(strUWRTTM) <> strUWRT_MOTO Then
				'                GoTo F_Chk_UWRTDTTM_End
				'            End If
				'' === 20080911 === UPDATE E - RISE)Izumi
				'            If pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.DATKB = "1" Then
				'                strUPDKB = UPDKB_UPD
				'            Else
				'                strUPDKB = UPDKB_DEL
				'            End If
				'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(pc_COL_UPDKB).Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				If pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.UPDKB <> pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(pc_COL_UPDKB).Dsp_Value Then
					strUWRT_MOTO = Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_OPEID) & Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_CLTID) & Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_UOPEID) & Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_UCLTID) & Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_WRTDT) & Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_WRTTM) & Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_UWRTDT) & Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.MOTO_UWRTTM)
					
					'更新・削除モード時
					intRet = F_Get_UWRTDTTM("TKRMTA", pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.TOKCD, pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.SKHINGRP, pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.HINCD, strWRTDT, strWRTTM, strUWRTDT, strUWRTTM, strOPEID, strCLTID, strUOPEID, strUCLTID)
					...
					'更新時間チェック
					If Trim(strOPEID) & Trim(strCLTID) & Trim(strUOPEID) & Trim(strUCLTID) & Trim(strWRTDT) & Trim(strWRTTM) & Trim(strUWRTDT) & Trim(strUWRTTM) <> strUWRT_MOTO Then
						GoTo F_Chk_UWRTDTTM_End
					End If
				End If
				' === 20081222 === UPDATE E - RISE)Izumi
			End If
...
F_Chk_UWRTDTTM_err: 
	...
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Get_UWRTDTTM(ByVal pin_strTBLNM As String, ByVal pin_strTOKCD As String, ByVal pin_strSKHINGRP As String, ByVal pin_strHINCD As String, ByRef pot_strWRTDT As String, ByRef pot_strWRTTM As String, ByRef pot_strUWRTDT As String, ByRef pot_strUWRTTM As String, ByRef pot_strOPEID As String, ByRef pot_strCLTID As String, ByRef pot_strUOPEID As String, ByRef pot_strUCLTID As String) As Short
		...
		'引数チェック
		If Trim(pin_strTOKCD) = "" Or Trim(pin_strSKHINGRP) = "" Or Trim(pin_strHINCD) = "" Then
		...
		Str_Sql = Str_Sql & "        TOKCD    = '" & pin_strTOKCD & "'"
		Str_Sql = Str_Sql & "    AND SKHINGRP = '" & pin_strSKHINGRP & "'"
		Str_Sql = Str_Sql & "    AND HINCD    = '" & pin_strHINCD & "'"
		' === 20080911 === INSERT S - RISE)Izumi
        ...
        ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        'pot_strUWRTTM = Trim(CF_Ora_GetDyn(Usr_Ody, "UWRTTM"))SSS
        'End If
        Dim dt As DataTable = DB_GetTable(Str_Sql)
        If dt.Rows.Count > 0 Then
            ' === 20080911 === INSERT S - RISE)Izumi
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロSSSSパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            pot_strOPEID = Trim(DB_NullReplace(dt.Rows(0)("OPEID"), ""))
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            pot_strCLTID = Trim(DB_NullReplace(dt.Rows(0)("CLTID"), ""))
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            pot_strUOPEID = Trim(DB_NullReplace(dt.Rows(0)("UOPEID"), ""))
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            pot_strUCLTID = Trim(DB_NullReplace(dt.Rows(0)("UCLTID"), ""))
            ' === 20080911 === INSERT E - RISE)Izumi
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            pot_strWRTDT = Trim(DB_NullReplace(dt.Rows(0)("WRTDT"), ""))
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            pot_strWRTTM = Trim(DB_NullReplace(dt.Rows(0)("WRTTM"), ""))
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            pot_strUWRTDT = Trim(DB_NullReplace(dt.Rows(0)("UWRTDT"), ""))
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        End If
...
F_Get_UWRTDTTM_END: 
		' === 20080911 === DELETE S - RISE)Izumi 　FOR UPDATEを解除しない為
		'    Call CF_Ora_CloseDyn(Usr_Ody)
		' === 20080911 === DELETE E - RISE)Izumi
		Exit Function
...
F_Get_UWRTDTTM_ERR: 
	...
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Get_DATA(ByVal pin_strTBLNM As String, ByVal pin_strTOKCD As String, ByVal pin_strSKHINGRP As String, ByVal pin_strHINCD As String) As Short
		On Error GoTo F_Get_DATA_ERR
...
F_Get_DATA_ERR: 
    ...
    ''2007/12/26 add-end M.SUEZAWA□□□□□□□□ プログラム単位の共通処理 End □□□□□□□□□□□□□□□□
    '20190724 ADD START
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
