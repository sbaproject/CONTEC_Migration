...
Module TUKMTA_M51
	...
	Sub UPDMST()
		...
		Do While I < PP_SSSMAIN.LastDe
            ...
            sqlWhere &= " Where TUKKB ='" & DB_TUKMTA.TUKKB & "' and TEKIDT='" & DB_TUKMTA.TEKIDT & "' order by TUKKB,TEKIDT"
            Call GetRowsCommon("TUKMTA", sqlWhere)
            If DBSTAT = 0 Then
		...
		Do While I < PP_SSSMAIN.LastDe
			...
			Else
                ...
                If TUKMTA_Insert(DB_TUKMTA) = False Then
                    DB_Rollback()
                    Throw New Exception("error:TUKMTA_Insert")
                End If
...
RATMT51_MF_All_Clear_UWRTDTTM_err: 
    ...
    '20081002 ADD END   RISE)Tanimura
    '20190814 ADD START
    Function TUKMTA_Insert(ByVal pDB_TUKMTA As TYPE_DB_TUKMTA) As Boolean
        ...
        Try
            ...
            With pDB_TUKMTA
                ...
                sqlStr &= " INSERT INTO TUKMTA "
                sqlStr &= " (DATKB, RELFL, WRTFSTTM, WRTFSTDT, FOPEID,OPEID,CLTID, "
                sqlStr &= " FCLTID,WRTTM,WRTDT,TUKKB,TUKNM, TEKIDT, RATERT, "
                sqlStr &= " UOPEID,UCLTID,UWRTTM,UWRTDT,PGID) "
                sqlStr &= " VALUES ('" & .DATKB & "', '" & .RELFL & "', '" & .WRTFSTTM & "', '" & .WRTFSTDT & "', '" & .FOPEID & "','" & .OPEID & "', '" & .CLTID & "', "
                sqlStr &= " '" & .FCLTID & "', '" & .WRTTM & "', '" & .WRTDT & "', '" & .TUKKB & "', '" & .TUKNM & "','" & .TEKIDT & "','" & .RATERT & "',"
                sqlStr &= " '" & .UOPEID & "', '" & .UCLTID & "', '" & .UWRTTM & "', '" & .UWRTDT & "', '" & .PGID & "' "
        ...
        Catch ex As Exception
            MsgBox("TUKMTA_Insert" & Constants.vbCrLf & ex.Message.ToString, MsgBoxStyle.Critical, "エラー")
