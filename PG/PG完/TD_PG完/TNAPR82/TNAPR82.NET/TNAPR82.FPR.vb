...
Module SSSMAIN0001
	...
	'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
	Public Structure TNAPR82_TYPE_INPUT
		Dim TEISYOYM As String
		Dim SOUBSCD As String
		Dim SOUBSNM As String
		...
		Else
			...
			Else
				...
				If DSPSOUCD_SEARCH(Input_Value, Mst_Inf) = 0 Then
					...
					Else
						If Trim(TNAPR82_InputData.SOUBSCD) <> "" And Trim(TNAPR82_InputData.SOUBSCD) <> Trim(Mst_Inf.SOUBSCD) Then
							Retn_Code = CHK_ERR_ELSE
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Chk_HD_SOUBSCD(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Init_Clr_Dsp(ByRef pm_Index As Short, ByRef pm_All As Cls_All) As Short
		...
		For Index_Wk = Wk_Index_S To Wk_Index_E
			...
			Select Case Index_Wk
				Case CShort(FR_SSSMAIN.HD_TEISYOYM.Tag)
					'経理締日付
				...
				Case CShort(FR_SSSMAIN.HD_SOUBSCD.Tag)
					'倉庫
					'初期画面編集時、値が入っていないため編集↓
					'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Bef_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
					If Len(Trim(pm_All.Dsp_Sub_Inf(Index_Wk).Detail.Bef_Value)) = 0 Then
	...
	Public Function F_Init_Cursor_Set(ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(FR_SSSMAIN.HD_TEISYOYM.Tag)
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_SOUBSCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
				'【場所名】
				Trg_Index = CShort(FR_SSSMAIN.HD_SOUBSNM.Tag)
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Chk_HD_TEISYOYM(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
...
F_Ctl_Head_RelChk_END: 
	...
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function PrintTNAPR82_Main(ByRef pm_All As Cls_All, ByRef pm_intMode As Short) As Short
...
Error_Handler:
	...
	Private Function F_Execute_PLSQL() As Short
		...
		Dim lngPara6 As Integer 'ﾊﾟﾗﾒｰﾀ3(ﾘﾀｰﾝｺｰﾄﾞ)
		Dim lngPara7 As Integer 'ﾊﾟﾗﾒｰﾀ5(ｴﾗｰｺｰﾄﾞ)
		Dim strPara8 As String 'ﾊﾟﾗﾒｰﾀ6(ｴﾗｰ内容)
		Dim lngPara9 As Integer 'ﾊﾟﾗﾒｰﾀ7(読込件数)
		Dim lngPara10 As Integer 'ﾊﾟﾗﾒｰﾀ8(登録件数)
...
ERR_HANDLE: 
	...
	Public Function F_Get_InitYM() As String
		...
		datDT = DateAdd(Microsoft.VisualBasic.DateInterval.Day, -1, DateAdd(Microsoft.VisualBasic.DateInterval.Month, 2, datDT))
		F_Get_InitYM = CStr(datDT)
