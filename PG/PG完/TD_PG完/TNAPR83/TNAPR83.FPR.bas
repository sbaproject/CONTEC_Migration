...
'画面初期化フラグ
Public gv_bolTNAPR83_INIT           As Boolean              'True:変更あり
Public gv_bolTNAPR83_LF_Enable      As Boolean              'LF処理実行フラグ(True：実行する）
Public gv_bolKeyFlg                 As Boolean
'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
Public Type TNAPR83_TYPE_INPUT
...
'        Retn_Code = CHK_ERR_NOT_INPUT
    ...
    Else
        ...
        If CF_Chk_Item_Base(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf, Input_Value) <> CHK_BASE_OK Then
            ...
            Err_Cd = gc_strMsgTNAPR83_E_005              '入力範囲外
        Else
            'マスタチェック
...
'                    Retn_Code = CHK_ERR_ELSE
'                    Err_Cd = gc_strMsgTNAPR83_E_015       '削除済みデータ
'                Else
                    'ＯＫ
                    Retn_Code = CHK_OK
                    pm_Chk_Move = True
...
'                End If
            Else
                Retn_Code = CHK_ERR_ELSE
                Err_Cd = gc_strMsgTNAPR83_E_006          '該当データなし
            End If
        End If
...
End Function
    ...
    '   備考：  プログラム単位の共通処理
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
Public Function F_Chk_HD_ZAIOUT(pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf _
                             , pm_Chk_Move As Boolean _
                             , pm_All As Cls_All) As Integer
...
'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
        ...
        Select Case Index_Wk
            ...
            Case CInt(FR_SSSMAIN.HD_ZAIOUT.Tag)
                '初期画面編集時、値が入っていないため編集↓
                If Len(Trim(pm_All.Dsp_Sub_Inf(Index_Wk).Detail.Bef_Value)) = 0 Then
...
End Function
    ...
    '   概要：  棚卸調査表出力出力処理
    '   引数：　pm_TNAPR83Data      画面入力データ
    '           pm_intMode          1:プリンタ出力  2:画面表示  3:ファイル出力
    '   戻値：　0:正常終了  1:他で印刷中  3:該当データ無し 5:中断 9:異常終了
    '   備考：
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
Public Function PrintTNAPR83_Main(pm_All As Cls_All, pm_intMode As Integer) As Integer
...
Private Function F_Execute_PLSQL() As Integer
    ...
    Dim strPara3    As String           'ﾊﾟﾗﾒｰﾀ4(倉庫ｺｰﾄﾞ)
    Dim strPara4    As String           'ﾊﾟﾗﾒｰﾀ4(在庫数出力)
    Dim lngPara5    As Long             'ﾊﾟﾗﾒｰﾀ3(ﾘﾀｰﾝｺｰﾄﾞ)
    Dim lngPara6    As Long             'ﾊﾟﾗﾒｰﾀ5(ｴﾗｰｺｰﾄﾞ)
    Dim strPara7    As String           'ﾊﾟﾗﾒｰﾀ6(ｴﾗｰ内容)
    Dim lngPara8    As Long             'ﾊﾟﾗﾒｰﾀ7(読込件数)
    Dim lngPara9    As Long             'ﾊﾟﾗﾒｰﾀ8(登録件数)
