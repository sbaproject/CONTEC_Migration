...
Module TOKMT55_IEV
    ...
    Sub Init_Fil() 'Generated.
        ...
        '
        DBN_RNKMTA2 = 6
        DB_PARA(DBN_RNKMTA2).tblid = "RNKMTA"
        DB_PARA(DBN_RNKMTA2).DBID = "USR1"
        SSS_MFIL = DBN_RNKMTA2
        '
        DBN_UNYMTA = 7
        DB_PARA(DBN_UNYMTA).tblid = "UNYMTA"
        DB_PARA(DBN_UNYMTA).DBID = "USR1"
    ...
    Sub SCR_FromMfil(ByVal De As Short) 'Generated.
        Call DP_SSSMAIN_RNKCD(De, DB_RNKMTA2.RNKCD)
        Call DP_SSSMAIN_SIKRT(De, DB_RNKMTA2.SIKRT)
        Call DP_SSSMAIN_URISETDT(De, DB_RNKMTA2.URISETDT)

        '2007/12/14 add-str T.KAWAMUKAI 元データのタイムスタンプ退避
        '   [引数Deは画面上の行数(0～)]
        ' === 20080908 === UPDATE S - RISE)Izumi チェック項目追加
        '    M_MOTO_A_inf(De).WRTDT = DB_RNKMTA2.WRTDT            '更新日付
        '    M_MOTO_A_inf(De).WRTTM = DB_RNKMTA2.WRTTM            '更新時刻
        '    M_MOTO_A_inf(De).UWRTDT = DB_RNKMTA2.UWRTDT          'バッチ更新日付
        '    M_MOTO_A_inf(De).UWRTTM = DB_RNKMTA2.UWRTTM          'バッチ更新時刻
        M_RNKMT_A_inf(De).OPEID = DB_RNKMTA2.OPEID '最終作業者コード
        M_RNKMT_A_inf(De).CLTID = DB_RNKMTA2.CLTID 'クライアントＩＤ
        M_RNKMT_A_inf(De).UOPEID = DB_RNKMTA2.UOPEID '最終作業者コード（バッチ）
        M_RNKMT_A_inf(De).UCLTID = DB_RNKMTA2.UCLTID 'クライントＩＤ（バッチ）
        M_RNKMT_A_inf(De).WRTDT = DB_RNKMTA2.WRTDT '更新日付
        M_RNKMT_A_inf(De).WRTTM = DB_RNKMTA2.WRTTM '更新時刻
        M_RNKMT_A_inf(De).UWRTDT = DB_RNKMTA2.UWRTDT 'バッチ更新日付
        M_RNKMT_A_inf(De).UWRTTM = DB_RNKMTA2.UWRTTM 'バッチ更新時刻
        ' === 20080908 === UPDATE E - RISE)Izumi
        '2007/12/14 add-end T.KAWAMUKAI
    End Sub
    ...
    Sub Mfil_FromSCR(ByVal De As Short) 'Generated.
        'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_RNKCD() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        DB_RNKMTA2.RNKCD = RD_SSSMAIN_RNKCD(De)
        'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_SIKRT() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        DB_RNKMTA2.SIKRT = RD_SSSMAIN_SIKRT(De)
        'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_URISETDT() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        DB_RNKMTA2.URISETDT = RD_SSSMAIN_URISETDT(De)
        DB_RNKMTA2.OPEID = SSS_OPEID.Value
        DB_RNKMTA2.CLTID = SSS_CLTID.Value
        If Trim(DB_ORATM) = "" Or Trim(DB_ORADT) = "" Then
            DB_RNKMTA2.WRTTM = VB6.Format(Now, "hhmmss")
            DB_RNKMTA2.WRTDT = VB6.Format(Now, "YYYYMMDD")
        Else
            DB_RNKMTA2.WRTTM = DB_ORATM
            DB_RNKMTA2.WRTDT = DB_ORADT
        End If
    End Sub
