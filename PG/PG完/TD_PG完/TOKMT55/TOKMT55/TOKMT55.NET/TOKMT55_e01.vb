...
Module TOKMT55_E01
    ...
    Function DSPMST() As Short
        ...
        wkSKHINGRP = Left(SSS_LASTKEY.Value, Len(DB_RNKMTA2.SKHINGRP)) & Space(Len(DB_MEIMTA.MEICDA) - Len(DB_RNKMTA2.SKHINGRP))
        Call DB_GetEq(DBN_MEIMTA, 2, "043" & wkSKHINGRP, BtrNormal)
        If DBSTAT = 0 Then
            Call SCR_FromMEIMTA(i)
        End If
        ...
        SSS_FASTKEY.Value = SSS_LASTKEY.Value
        ''''Call DB_GetGrEq(DBN_RNKMTA2, 1, SSS_LASTKEY, BtrNormal)
        strSQL = ""
        strSQL = strSQL & "SELECT *"
        strSQL = strSQL & "  FROM   ("
        strSQL = strSQL & "             SELECT RNK.DATKB, RNK.SKHINGRP, RNK.RNKCD,RNK.URISETDT,RNK.SIKRT,RNK.RELFL"
        ...
        strSQL = strSQL & "             ) TBL"
        strSQL = strSQL & " WHERE   TBL.SKHINGRP || TBL.RNKCD || TBL.WRTFSTDT >= " & "'" & RTrim(SSS_FASTKEY.Value) & "'"
        strSQL = strSQL & "   AND   TBL.SKHINGRP = " & "'" & Left(SSS_FASTKEY.Value, Len(DB_RNKMTA2.SKHINGRP)) & "'"
        strSQL = strSQL & " ORDER BY TBL.SKHINGRP,TBL.RNKCD,TBL.WRTFSTDT"

        Call DB_GetSQL2(DBN_RNKMTA2, strSQL)
        ...
        If DBSTAT = 0 Then
            Do While (DBSTAT = 0) And (i < (PP_SSSMAIN.MaxDspC + 1))
                Call SCR_FromMfil(i)
                Call DP_SSSMAIN_V_DATKB(i, DB_RNKMTA2.DATKB) '2006.11.07
                Call DP_SSSMAIN_V_SIKRT(i, DB_RNKMTA2.SIKRT) '2006.11.07
                ...
                End If
                i = i + 1
                Call DB_GetNext(DBN_RNKMTA2, BtrNormal)
            Loop
        Else
            'DB上に指定キーのものが存在しないとき
            Call Dsp_Prompt("RNOTFOUND", 0) '新規レコードです。
        ...
        '
        If DBSTAT = 0 Then
            SSS_LASTKEY.Value = DB_RNKMTA2.SKHINGRP & DB_RNKMTA2.RNKCD & DB_RNKMTA2.WRTFSTDT
        Else
            'UPGRADE_WARNING: オブジェクト LenWid() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            SSS_LASTKEY.Value = Left(SSS_FASTKEY.Value, Len(DB_RNKMTA2.SKHINGRP)) & HighValue(LenWid(DB_RNKMTA2.RNKCD)) & HighValue(LenWid(DB_RNKMTA2.WRTFSTDT))
        End If
        DSPMST = i
    End Function
    ...
    Function MST_PREV() As Short
        ...
        i = SET_GAMEN_KEY()
        i = 0
        ''''Call DB_GetLs(DBN_RNKMTA2, 1, SSS_FASTKEY, BtrNormal)
        strSQL = ""
        strSQL = strSQL & "SELECT *"
        strSQL = strSQL & "  FROM   ("
        strSQL = strSQL & "             SELECT RNK.DATKB, RNK.SKHINGRP, RNK.RNKCD,RNK.URISETDT,RNK.SIKRT,RNK.RELFL"
        ...
        Do While (DBSTAT = 0) And (i < (PP_SSSMAIN.MaxDspC))
            i = i + 1
            DB_PARA(DBN_RNKMTA2).nDirection = 2
            Call DB_GetPre(DBN_RNKMTA2, BtrNormal)
        Loop
        If DBSTAT <> 0 And i = 0 Then
            '        Call DB_GetFirst(DBN_RNKMTA2, 1, BtrNormal)
            SSS_LASTKEY.Value = Left(SSS_FASTKEY.Value, Len(DB_RNKMTA2.SKHINGRP)) & Space(Len(DB_RNKMTA2.RNKCD)) & VB6.Format(DB_RNKMTA2.WRTFSTDT, "00000000")
        Else
            SSS_LASTKEY.Value = Left(SSS_FASTKEY.Value, Len(DB_RNKMTA2.SKHINGRP)) & DB_RNKMTA2.RNKCD & DB_RNKMTA2.WRTFSTDT
        End If
        i = DSPMST()
        MST_PREV = i
    End Function
    ...
    Function SET_GAMEN_KEY() As Short
        '
        'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_SKHINGRP() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        DB_RNKMTA2.SKHINGRP = RD_SSSMAIN_SKHINGRP(0)
        'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_RNKCD() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        DB_RNKMTA2.RNKCD = RD_SSSMAIN_RNKCD(0)
        'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_URISETDT() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        If Trim(RD_SSSMAIN_URISETDT(0)) = "" Then
            DB_RNKMTA2.URISETDT = "00000000"
        Else
            'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_URISETDT() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            DB_RNKMTA2.URISETDT = VB6.Format(99999999 - Val(RD_SSSMAIN_URISETDT(0)), "00000000")
        End If
        SSS_LASTKEY.Value = DB_RNKMTA2.SKHINGRP & DB_RNKMTA2.RNKCD & DB_RNKMTA2.URISETDT
