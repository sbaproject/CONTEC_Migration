...
Module SSSMAIN0001
	...
	'画面初期化フラグ
	Public gv_bolTNAPR81_INIT As Boolean 'True:変更あり
	Public gv_bolTNAPR81_LF_Enable As Boolean 'LF処理実行フラグ(True：実行する）
	Public gv_bolKeyFlg As Boolean
	'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
	Public Structure TNAPR81_TYPE_INPUT
		Dim SOUCD As String '倉庫ｺｰﾄﾞ
		Dim SOUNM As String '倉庫名
	End Structure
    '画面情報
    Public TNAPR81_InputData As TNAPR81_TYPE_INPUT
    ...
    '出力帳票ＩＤ
    Private Const mc_strLIST_ID As String = "TNAPR81"
	'印刷中フラグ
	...
	Public Function F_Chk_HD_SOUCD(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
        ...
        If CF_Trim_Item(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf) = "" Then
			...
			TNAPR81_InputData.SOUCD = ""
			TNAPR81_InputData.SOUNM = ""
			Retn_Code = CHK_ERR_ELSE
			Err_Cd = gc_strMsgTNAPR81_E_015 '入力範囲外
		Else
			'未入力以外のチェック済
			...
			If CF_Chk_Item_Base(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf, Input_Value) <> CHK_BASE_OK Then
				Retn_Code = CHK_ERR_ELSE
				Err_Cd = gc_strMsgTNAPR81_E_005 '入力範囲外
			Else
				'マスタチェック
				If DSPSOUCD_SEARCH(Input_Value, Mst_Inf) = 0 Then
					...
					'                    Retn_Code = CHK_ERR_ELSE
					'                    Err_Cd = gc_strMsgTNAPR81_E_015       '削除済みデータ
					'                Else
					'ＯＫ
					Retn_Code = CHK_OK
					pm_Chk_Move = True
					...
					'取得項目格納
					TNAPR81_InputData.SOUCD = Trim(Mst_Inf.SOUCD)
					TNAPR81_InputData.SOUNM = Trim(Mst_Inf.SOUNM)
					'                End If
				Else
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgTNAPR81_E_006 '該当データなし
				End If
			End If
		...
		If Index_Wk > pm_All.Dsp_Base.Item_Cnt Then
			...
			Select Case pm_Mode
				Case NEXT_FOCUS_MODE_KEYRETURN
					'KEYRETURNの場合
					'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
					Call PrintTNAPR81_Main(pm_All, -1)
					'キーフラグを元に戻す
					gv_bolKeyFlg = False
	...
	Public Function F_Dsp_HD_SOUCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				Dsp_Value = CF_Cnv_Dsp_Item(TNAPR81_InputData.SOUNM, pm_All.Dsp_Sub_Inf(Trg_Index), False)
...
F_Ctl_Head_RelChk_END: 
	...
	'   概要：  棚卸差額一覧出力出力処理
	'   引数：　pm_TNAPR81Data      画面入力データ
	'           pm_intMode          1:プリンタ出力  2:画面表示  3:ファイル出力
	'   戻値：　0:正常終了  1:他で印刷中  3:該当データ無し 5:中断 9:異常終了
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function PrintTNAPR81_Main(ByRef pm_All As Cls_All, ByRef pm_intMode As Short) As Short
		...
		'UPGRADE_WARNING: オブジェクト FR_SSSMAIN.CM_LCANCEL.Enabled の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        ...
        '      If Not bolRet Then
        '          strMsgCd = gc_strMsgTNAPR81_E_010
        '          bolOraErr = True
        '          GoTo Error_Handler
        '      End If
		...
		'    strPrtSeq = GetPrtSeq()
		'    If strPrtSeq = "" Then
		'        strMsgCd = gc_strMsgTNAPR81_E_007
		'        bolOraErr = True
		'        Exit Function
		'    End If
		strLIST_ID = "TNAPR81"
		...
		If Not bolRet Then
			strMsgCd = gc_strMsgTNAPR81_E_008
			bolOraErr = True
			GoTo Error_Handler
		End If
		...
		If SSS_LSTOP = False Then
			...
			If intRet <> 0 Then
				PrintTNAPR81_Main = intRet
				Select Case intRet
					Case 1 '他で印刷中
						'メッセージ出力済み
					Case 2 'キャンセル
						strMsgCd = gc_strMsgTNAPR81_I_004
					Case 3 'データなし
						strMsgCd = gc_strMsgTNAPR81_E_006
					Case Else 'それ以外
						strMsgCd = gc_strMsgTNAPR81_E_011
				End Select
				GoTo Error_Handler
			End If
...
Exit_Handler: 
		...
		If strMsgCd <> "" Then
			If bolOraErr Then
				Call AE_CmnMsgLibrary(SSS_PrgNm, strMsgCd, pm_All, "PrintTNAPR81_Main")
			Else
...
Error_Handler: 
	...
	Private Function F_Execute_PLSQL() As Short
		...
		Dim strPara3 As String 'ﾊﾟﾗﾒｰﾀ4(調整年月)
		
		
		Dim lngPara4 As Integer 'ﾊﾟﾗﾒｰﾀ3(ﾘﾀｰﾝｺｰﾄﾞ)
		Dim lngPara5 As Integer 'ﾊﾟﾗﾒｰﾀ5(ｴﾗｰｺｰﾄﾞ)
		Dim strPara6 As String 'ﾊﾟﾗﾒｰﾀ6(ｴﾗｰ内容)
		Dim lngPara7 As Integer 'ﾊﾟﾗﾒｰﾀ7(読込件数)
		Dim lngPara8 As Integer 'ﾊﾟﾗﾒｰﾀ8(登録件数)
		...
		Dim param(8) As OraParameter 'PL/SQLのバインド変数
		Dim bolRet As Boolean
		...
		strPara3 = TNAPR81_InputData.SOUCD
		...
		'UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		gv_Odb_USR1.Parameters.Add("P6", strPara6, ORAPARM_OUTPUT)
		'UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		...
		'PL/SQL呼び出しSQL
		strSQL = "BEGIN PRC_TNAPR81_01(:P1,:P2,:P3,:P4,:P5,:P6,:P7,:P8); End;"
		...
		If IsDbNull(param(6).Value) = False Then
			'UPGRADE_WARNING: オブジェクト param().Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strPara6 = param(6).Value
		End If
		
		'エラー情報設定
		gv_Str_OraErrText = strPara6
		
		F_Execute_PLSQL = lngPara4
