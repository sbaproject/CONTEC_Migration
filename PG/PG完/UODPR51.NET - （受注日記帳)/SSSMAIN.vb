...
Next_Proc:
    ...
    Sub SSS_LIST(ByRef LSTKB As Short)
        ...
        'StartTime = Timer
        'PointTime = Timer
        '★2019.04.01 DEL START 進捗状況が出てるやつみたいな
        'Call WORKING_VIEW(True)
        '2019.04.01 DEL END
        ' クリスタルレポートのオープン
        If CRW_INIT() = False Then
    ...
    Function SSSMAIN_BeginPrg() As Object
        ...
        CType(FR_SSSMAIN.Controls("cmd_Cancel"), Object).Visible = False
        '2019.04.05 ADD END
    End Function
    ...
    ''' 印刷処理
    ''' </summary>
    Sub EXE_PRINT(ByVal PARA_STTWRTDT As String,
                  ...
                  ByVal PARA_ENDWRTTM As String,
                  ByVal PRINT_NO As Integer)

        '中止ボタン有効
        CType(FR_SSSMAIN.Controls("cmd_Cancel"), Object).Visible = True
        CType(FR_SSSMAIN.Controls("cmd_Cancel"), Object).Enabled = True
        SSS_LSTOP = False

        'PLSQL実行
        If Execute_PLSQL_PRC_UODPR51(PARA_STTWRTDT,
                                     PARA_ENDWRTDT,
                                     PARA_STTJDNNO,
                                     PARA_ENDJDNNO,
                                     PARA_STTTOKCD,
        ...
        '中止ボタン無効化
        CType(FR_SSSMAIN.Controls("cmd_Cancel"), Object).Visible = False
        CType(FR_SSSMAIN.Controls("cmd_Cancel"), Object).Enabled = False
        ...
        'データができているか確認
        ls_sql = "  SELECT * FROM CNT_USR9." & SSS_PrgId
        ls_sql = ls_sql & " WHERE RPTCLTID = '" & SSS_CLTID.ToString & "'"
        Dim dt As DataTable = DB_GetTable(ls_sql)
        If dt.Rows.Count = 0 Then
            'データが存在しません
            AE_MsgLibrary(PP_SSSMAIN, "CURRENT")
            Exit Sub
        End If
        ...
        CR.SetDatabase("CNJ_ODBC", "CNT_USR9P", "CNT_USR9", EmpQuery, "UODPR51", Report)

        Select Case PRINT_NO
            Case 0
                '印刷
            Case 1
                'プレビュー
                CR.ReportPreview(Report, EmpQuery, "01")
        End Select
