...
Module REPORT_CMN
	...
	Public Function GetPrtSeq() As String
        ...
        strSeq = CStr(DB_NullReplace(dt.Rows(0)("PRTSEQ"), 0))
        'change end 20190826 kuwa
        'クローズ
	...
	Public Function OutPutList_Main(ByVal pin_intLSTKB As Short, ByVal pin_strLSTID As String, ByVal pin_strPrwName As String, ByVal pin_strPrtSeq As String) As Short
		...
		strSQL = strSQL & "  where PRTTANID = '" & SSS_OPEID.Value & "' "
        ...
        intCnt = DB_NullReplace(dt.Rows(0)("LISTCNT"), 0)
...
ERR_OutPutList_Main: '
	...
	Function Crw_ChgLocOra() As Boolean
		...
		'        '動的なSQLを使っている場合
		'        TblLocation.Location = SSS_DBHEAD & "_" & Trim$(DB_PARA(SSS_LSTMFIL).DBID) & "." & SSS_PrgId & "_" & SSS_CLTID & Chr$(&H0)
		'    Else
		'        '従来の方法
		'        TblLocation.Location = SSS_DBHEAD & "_" & Trim$(DB_PARA(SSS_LSTMFIL).DBID) & "." & SSS_PrgId & Chr$(&H0)
		'    End If
		...
		'        '動的なSQLを使っていない場合のみ
		'********************仮NAGASAWA
		'        newSelectionFormula = "{" & SSS_PrgId & ".RPTCLTID} = '" & SSS_CLTID & "'" + Chr$(0)
		'        newSelectionFormula = "{" & strLSTID & ".PRTTANID} = '" & SSS_CLTID & "'"
		newSelectionFormula = "{" & strLSTID & ".PRTTANID} = '" & SSS_OPEID.Value & "'"
		newSelectionFormula = newSelectionFormula & " and {" & strLSTID & ".PRTSEQ} = " & strPrtSeq & " " & Chr(0)
		'********************仮NAGASAWA
