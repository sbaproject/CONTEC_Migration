...
Module SSSMAIN0001
    ...
    Private Structure IDOET52_TYPE_SBNTRA
		...
		'20080919 ADD START RISE)Tanimura '排他処理
		...
		'20080919 ADD END   RISE)Tanimura
	End Structure
			...
			Else
				...
				Select Case pm_KeyAscii
					Case System.Windows.Forms.Keys.Back
                        ...
                        'add start 20190910 kuwa Enter押時に背景色（黄色）が残るのを修正
                    Case System.Windows.Forms.Keys.Return
                        pm_Move_Flg = True
                        pm_KeyAscii = 0
                        'add end 20190910 kuwa
			...
			Else
				...
				Else
					...
					Else
                        ...
                        ''UPGRADE_WARNING: オブジェクト pm_Dsp_Sub_Inf.Ctl.SelLength の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                        'pm_Dsp_Sub_Inf.Ctl.SelLength = Wk_SelLength
                        'change test 20190910 kuwa 二文字移動してしまう不具合を修正
                        'DirectCast(pm_Dsp_Sub_Inf.Ctl, TextBox).Select(Wk_SelStart, Wk_SelLength)
                        DirectCast(pm_Dsp_Sub_Inf.Ctl, TextBox).Select(Wk_SelStart, 1) 'なぜ1なのか不明
                ...
                If FR_SSSMAIN.HD_OPT1.Checked = True Then
                    ...
                    ElseIf Retn_Code2 = 8 Then
                        ...
                        If CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf) = pm_Chk_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
                            ...
                            Exit Function
                        End If
                        'UPD START FWEST)KOROYASU 2015/12/24 C2-20151214-03
                        '                    If AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_A_102, pm_All) = vbYes Then
                        If AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_A_104, pm_All) = MsgBoxResult.Yes Then
                            'UPD  END  FWEST)KOROYASU 2015/12/24 C2-20151214-03
                            Retn_Code = CHK_OK
                            pm_Chk_Move = True
                        Else
                            Retn_Code = CHK_ERR_ELSE
    ...
    Private Function F_Chk_Sbnno(ByVal p_SBNNO As String, ByVal p_DENDT As String) As Short
        ...
        Select Case IsNumeric(ls_SBNNO_1)
            ...
            Case False
                ...
                '                End If
                '            End If
                '20190910 IDOET52 を参考にadd,changeされた部分を修正する(すでにコメントされている部分については、日付等は省略) kuwa
                'add 20190827 start hou
                Dim cmd As New OracleCommand
                cmd.Connection = DB_START_GENKA()
                cmd.CommandType = CommandType.StoredProcedure
                'add 20190827 end hou

                'change 20190827 start hou
                ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                ...
                Dim FC_STA As OracleParameter = New OracleParameter
                FC_STA.ParameterName = "FC_STA"
                FC_STA.Direction = ParameterDirection.ReturnValue
                FC_STA.OracleDbType = OracleDbType.Decimal ' パラメーターの型をテーブルを参照して指定
                FC_STA.Value = 0
                cmd.Parameters.Add(FC_STA)
                ...
                SEIBAN_KBN.Direction = ParameterDirection.InputOutput
                ...
                FC_STA.OracleDbType = OracleDbType.Decimal
                'change 20190827 end hou
                ...
                'delete 20190827 start hou
                'If bolRet = False Then
                '    GoTo F_Chk_Sbnno_err
                'End If
                'delete 20190827 end hou
                    ...
                    If CDbl(ls_RET_STATUS) = 3 Then
                        ...
                        Else
                            ...
                            If ls_STATUS >= "40" Then
                                ...
                                '親が完了準備中、取消準備中の場合、警告
                                If ls_STATUS = "40" Or ls_STATUS = "50" Then
                                    'UPD  END  FWEST)KOROYASU 2015/12/24 C2-20151214-03
                                    F_Chk_Sbnno = 8
                                Else
                                    F_Chk_Sbnno = 5
...
ERR_EXIT:
    ...
    Public Function F_Ctl_Body_RelChk(ByRef pm_intRow As Short, ByRef pm_All As Cls_All, ByRef pm_ErrIdx As Short, ByRef pm_ErrRow As Short) As Short
        ...
        If Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intHINCD))) <> "" Then
            ...
            'DEL START FKS)INABA 2009/02/06 ****************************************************************************************
            '連絡票№XXXXXXX
            '訂正時も商品マスタより情報を取り直す
            '2008/05/12 FKS)HONDA ADD START
            '    If RunMode = RUNMODE_IDOET52 Then
            'DEL  END  FKS)INABA 2009/02/06 ****************************************************************************************
            '' 型式をテストのためクリアする
            ...
            ''MsgBox "test"
            '
            '        strSBNNO = CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_SBNNO.Tag)))
            '
            '        strSQL = ""
            ...
            '            Trg_Index = CInt(FR_SSSMAIN.BD_HINNMB(1).Tag)
            '            Dsp_Value = CF_Cnv_Dsp_Item(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.HINNMB, pm_All.Dsp_Sub_Inf(Trg_Index), False)
            '            Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
            '
            '            ' 単位
            '            pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UNTNM = CF_Ora_GetDyn(Usr_Ody, "UNTNM", "")
            '            Trg_Index = CInt(FR_SSSMAIN.BD_UNTNM(1).Tag)
            '            Dsp_Value = CF_Cnv_Dsp_Item(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UNTNM, pm_All.Dsp_Sub_Inf(Trg_Index), False)
            '            Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
...
Err_F_Ctl_Upd_Process:
    ...
    Public Function F_Update_Main(ByRef pot_strJdnNo As String, ByRef pm_All As Cls_All) As Short
        ...
        strSQL = strSQL & "WHERE"
        strSQL = strSQL & "  DATNO = '" & IDOET52_SBNTRA_Inf.DATNO & "' "
        ...
        If CF_Ora_EOF(Usr_Ody) = False Then
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            If Trim(CF_Ora_GetDyn(Usr_Ody, "OPEID", "")) <> Trim(IDOET52_SBNTRA_Inf.OPEID) Or Trim(CF_Ora_GetDyn(Usr_Ody, "CLTID", "")) <> Trim(IDOET52_SBNTRA_Inf.CLTID) Or Trim(CF_Ora_GetDyn(Usr_Ody, "WRTDT", "")) <> Trim(IDOET52_SBNTRA_Inf.WRTDT) Or Trim(CF_Ora_GetDyn(Usr_Ody, "WRTTM", "")) <> Trim(IDOET52_SBNTRA_Inf.WRTTM) Or Trim(CF_Ora_GetDyn(Usr_Ody, "UOPEID", "")) <> Trim(IDOET52_SBNTRA_Inf.UOPEID) Or Trim(CF_Ora_GetDyn(Usr_Ody, "UCLTID", "")) <> Trim(IDOET52_SBNTRA_Inf.UCLTID) Or Trim(CF_Ora_GetDyn(Usr_Ody, "UWRTDT", "")) <> Trim(IDOET52_SBNTRA_Inf.UWRTDT) Or Trim(CF_Ora_GetDyn(Usr_Ody, "UWRTTM", "")) <> Trim(IDOET52_SBNTRA_Inf.UWRTTM) Then
                Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_903, pm_All)
                GoTo F_Update_Main_err
            End If
        End If
        '20080919 ADD END   RISE)Tanimura
...
F_Update_Main_err:
    ...
    Public Function F_SBNTRA_Insert(ByVal pin_strDatNo As String, ByVal pin_strPUDLNO As String, ByRef pm_All As Cls_All) As Short
        ...
        'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        If CF_Ora_Number(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value) = 0 Then
            strSQL = strSQL & "        ,  '" & gc_strDLFLG_DEL & "' " '削除フラグ
        Else
            strSQL = strSQL & "        ,  '" & gc_strDLFLG_UPD & "' " '削除フラグ
        End If
...
ERR_DSPJDNTRA_SEARCH: 
	...
	Private Function F_DSPSBNTRA_SEARCH(ByVal pin_strDatNo As String, ByRef pm_All As Cls_All) As Short
        ...
        If dt Is Nothing OrElse dt.Rows.Count > 0 Then
            ...
            '2019/06/19 CHG END
            'CHG START FKS)INABA 2008/08/20 ***********************************************************
            If intFRDSU = 0 And intHIKSMSU <> 0 Then ' 出荷指示数=0 かつ 引当数<>0 => 引当を解除してください
                F_DSPSBNTRA_SEARCH = 3
                'メッセージ出力
                Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_066, pm_All)
                GoTo DSPSBNTRA_SEARCH_err
            ElseIf intFRDSU <> 0 And intHIKSMSU = 0 Then  ' 出荷指示数<>0 AND 引当数=0  => 出荷指示を取り消してください
                F_DSPSBNTRA_SEARCH = 3
                'メッセージ出力
                Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_067, pm_All)
                GoTo DSPSBNTRA_SEARCH_err
            ElseIf intFRDSU <> 0 And intHIKSMSU <> 0 Then  ' 出荷指示数<>0 AND 引当数<>0 => 出荷指示及び引当中です。
                F_DSPSBNTRA_SEARCH = 3
                'メッセージ出力
                Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_019, pm_All)
                GoTo DSPSBNTRA_SEARCH_err
            End If
            ...
            '''            Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_066, pm_All)
            '''
            '''DEL START FKS)INABA 2007/01/20 *********************************************************            GoTo DSPSBNTRA_SEARCH_err
            ...
            ''20080919 ADD START RISE)Tanimura '排他処理
            ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'IDOET52_SBNTRA_Inf.OPEID = CF_Ora_GetDyn(Usr_Ody, "OPEID", "") ' 最終作業者コード
            ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'IDOET52_SBNTRA_Inf.CLTID = CF_Ora_GetDyn(Usr_Ody, "CLTID", "") ' クライアントＩＤ
            ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'IDOET52_SBNTRA_Inf.WRTTM = CF_Ora_GetDyn(Usr_Ody, "WRTTM", "") ' タイムスタンプ（時間）
            ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'IDOET52_SBNTRA_Inf.WRTDT = CF_Ora_GetDyn(Usr_Ody, "WRTDT", "") ' タイムスタンプ（日付）
            ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'IDOET52_SBNTRA_Inf.UOPEID = CF_Ora_GetDyn(Usr_Ody, "UOPEID", "") ' ユーザID（バッチ）
            ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'IDOET52_SBNTRA_Inf.UCLTID = CF_Ora_GetDyn(Usr_Ody, "UCLTID", "") ' クライアントＩＤ（バッチ）
            ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'IDOET52_SBNTRA_Inf.UWRTTM = CF_Ora_GetDyn(Usr_Ody, "UWRTTM", "") ' タイムスタンプ（バッチ時間）
            ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'IDOET52_SBNTRA_Inf.UWRTDT = CF_Ora_GetDyn(Usr_Ody, "UWRTDT", "") ' タイムスタンプ（バッチ日）
            ''20080919 ADD END   RISE)Tanimura

            IDOET52_SBNTRA_Inf.DATNO = DB_NullReplace(dt.Rows(0)("DATNO"), "")
            IDOET52_SBNTRA_Inf.OPEID = DB_NullReplace(dt.Rows(0)("OPEID"), "")
            IDOET52_SBNTRA_Inf.CLTID = DB_NullReplace(dt.Rows(0)("CLTID"), "")
            IDOET52_SBNTRA_Inf.WRTTM = DB_NullReplace(dt.Rows(0)("WRTTM"), "")
            IDOET52_SBNTRA_Inf.WRTDT = DB_NullReplace(dt.Rows(0)("WRTDT"), "")
            IDOET52_SBNTRA_Inf.UOPEID = DB_NullReplace(dt.Rows(0)("UOPEID"), "")
            IDOET52_SBNTRA_Inf.UCLTID = DB_NullReplace(dt.Rows(0)("UCLTID"), "")
            IDOET52_SBNTRA_Inf.UWRTTM = DB_NullReplace(dt.Rows(0)("UWRTTM"), "")
            IDOET52_SBNTRA_Inf.UWRTDT = DB_NullReplace(dt.Rows(0)("UWRTDT"), "")
            '2019/06/19 CHG END
            ' 出庫日
            Trg_Index = CShort(FR_SSSMAIN.HD_DENDT.Tag)
            'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
