...
Module SSSMAIN0001
	...
	'//* ---------|----------|---------------|-----------------------------------------------
	'//* 1.00     |          |RISE)          |新規作成
	'//* 1.10     |2009/03/17|RISE)宮島      |・入金種別が「手形」の場合、得意先マスタからの初期表示は行わない。
	'//* 1.10     |2009/03/17|RISE)宮島      |・銀行コードの入力は入金種別が「手形」の場合のみ入力可能とする。(元々)
	'//* 1.10     |2009/03/17|RISE)宮島      |・以下、チェック内容を行わない。
	...
	'//*          |          |               |      ※仕様書3行目のチェック｡
	'//* 1.10     |2009/03/17|RISE)宮島      |・対象の受注データ（受注見出しトラン）の前受区分が「２．前受」で
	'//*          |          |               |  なければエラーとする。
	'//* 1.10     |2009/03/18|RISE)宮島      |・受注データ（受注見出しトラン）が排他チェックの対象として必要。
	...
	'//*          |2009/06/15|FKS)中田       |・ランク別処理/伝票枚数更新処理の意図不明のコメントアウトの解除
	'//*          |　　　　　|　　　　　　　 |   ※対象：TOKSSA・TOKSSB・TOKSSC・TOKSME
	'//*          |　　　　　|　　　　　　　 |   ※入金訂正ではコメントアウトされていないため
	'//*          |2009/09/03|RISE)宮島      |・入金日のチェック時、前回月次更新実行日だけでなく、前回請求締日とのチェックも必要
	...
	'//*          |2009/09/24|RISE)宮島      |・金額差額チェックの追加
	'//*          |2009/09/27|RISE)宮島      |・UDNTRA.RATERT に TUKMTA.RATERT を設定する
	'//*          |2009/09/30|RISE)宮島      |・金額差額方法の変更
	...
	'//*          |2011/07/09|TOM)森本       |・自動入金登録画面連携による入金登録の変更
	'//*          |          |               |・入金登録更新関数一元化による変更
	...
	Public gv_bolURKET51_INIT As Boolean '画面初期化フラグ（True:変更あり）
	Public gv_bolURKET51_LF_Enable As Boolean 'LF処理実行フラグ(True：実行する）
	Public gv_bolKeyFlg As Boolean
	Public gv_bolUpdFlg As Boolean
	Private intInput_Bef_RowNo As Short '空白行の先頭行№
	
	Private Structure URKET51_TYPE_HEAD
		Dim FBRFNO As String 'FBデータ(照会番号)
		Dim FBTRA As TYPE_DB_FBTRA 'FBデータ(照会番号)に紐づくFBデータ
	...
	End Structure
	'見出情報
	Private URKET51_HEAD_Inf As URKET51_TYPE_HEAD
	...
	' === 20111026 === INSERT S TOM)Morimoto
	Public gv_intMeisaiCnt As Short '入力明細数（更新時使用）(パラメータ受け渡し)
	' === 20111026 === INSERT E
	...
	'呼び出しもとの分岐判定変数
	'=== 20110511 === INSERT S TOM)Morimoto
	Public strCallMode As String
	Public Const MAIN_MODE As String = "MAIN"
	Public Const SUB_MODE As String = "SUB"
	Public Const AUTO_MODE As String = "AUTO"
	Public UPD_flg As Boolean
	'=== 20110511 === INSERT E
	...
	Public Function CF_Ctl_Item_Change(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		Select Case True
			Case TypeOf pm_Dsp_Sub_Inf.Ctl Is System.Windows.Forms.TextBox
                ...
                '2019/04/01 CHG START
                'Act_SelStr = pm_Dsp_Skcub_Inf.Ctl.SelText
	...
	Public Function CF_Ctl_MN_ClearDE(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		If CF_Cmn_Ctl_MN_ClearDE(Bd_Index, pm_All) = True Then
            ...
            '2019/04/04 DEL START (仮)ファンクションが消えてる
            'Call CF_EXCTBZ_Unlock(pm_All)
            '2019/04/04 DEL E N D
			' === 20130711 === INSERT E -
			'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
			'業務の初期値を編集
	...
	Public Function F_Init_Clr_Dsp(ByRef pm_Index As Short, ByRef pm_All As Cls_All) As Short
		...
		Dim Init_WK As URKET51_TYPE_HEAD
		...
		For Index_Wk = Wk_Index_S To Wk_Index_E
			...
			Select Case Index_Wk
				Case CShort(FR_SSSMAIN.HD_NYUKB.Tag)
					'入金区分
					Call CF_Set_Item_Direct(CF_Cnv_Dsp_Item(gc_strMAEUKKB_NML, pm_All.Dsp_Sub_Inf(Index_Wk), False), pm_All.Dsp_Sub_Inf(Index_Wk), pm_All,  , True)
					'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
					'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Bef_Chk_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
					pm_All.Dsp_Sub_Inf(Index_Wk).Detail.Bef_Chk_Value = CF_Cnv_Dsp_Item(gc_strMAEUKKB_NML, pm_All.Dsp_Sub_Inf(Index_Wk), False)
		...
		If Wk_Mode = ITM_ALL_CLR Then
			Init_WK.NYUKB = gc_strMAEUKKB_NML
			Init_WK.NYUDT = GV_UNYDate
			'UPGRADE_WARNING: オブジェクト URKET51_HEAD_Inf の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			URKET51_HEAD_Inf = Init_WK
		End If
	...
	Public Function F_Init_Cursor_Set(ByRef pm_All As Cls_All) As Short
		...
		'FBデータ取込にフォーカス設定
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(FR_SSSMAIN.HD_FBRFNO.Tag)
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Chk_HD_FBRFNO(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			If CF_Chk_Item_Base(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf, Input_Value) <> CHK_BASE_OK Then
				...
				Err_Cd = gc_strMsgURKET51_E_007 '入力範囲外
			Else
				'マスタチェック
				If DSPFBTRA_SEARCH_ALL(Input_Value, Mst_Inf_FBTRA) = 0 Then
					'論理削除チェック
					Select Case True
						Case Mst_Inf_FBTRA.DATKB = gc_strDATKB_DEL
							Retn_Code = CHK_ERR_ELSE
							Err_Cd = gc_strMsgURKET51_E_009 '削除済みデータ
							GoTo F_Chk_HD_FBRFNO_End
						Case Else
							'ＯＫ
							Retn_Code = CHK_OK
							pm_Chk_Move = True
							...
							'取得項目格納
							URKET51_HEAD_Inf.FBRFNO = Input_Value
							'UPGRADE_WARNING: オブジェクト URKET51_HEAD_Inf.FBTRA の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
							URKET51_HEAD_Inf.FBTRA = Mst_Inf_FBTRA
							With pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf '１行目のデータ
								.NYUKN = Mst_Inf_FBTRA.FBNYUKN
							End With
				...
				'バーチャル口座（＝振込依頼人コード）(FBCLTCD)で、得意マスタを見る
				strKOZNO = Left(Mst_Inf_FBTRA.FBCLTCD, 7)
				
				'マスタチェック
				If DSPTOKMTA_KOZNO_SEARCH(strKOZNO, Mst_Inf_TOKMTA) = 0 Then
					'論理削除チェック
					Select Case True
						Case Mst_Inf_TOKMTA.DATKB = gc_strDATKB_DEL
							Retn_Code = CHK_ERR_ELSE
							'Err_Cd = gc_strMsgURKET51_E_009       '削除済みデータ
							Err_Cd = gc_strMsgURKET51_E_024 'バーチャル口座が得意先に存在しません
							GoTo F_Chk_HD_FBRFNO_End
							'Case Mst_Inf_TOKMTA.DSPKB = gc_strDSPKB_NG
							'    Retn_Code = CHK_ERR_ELSE
							'    Err_Cd = gc_strMsgURKET51_E_010      '検索対象外データ
						Case Mst_Inf_TOKMTA.TOKSEICD <> Mst_Inf_TOKMTA.TOKCD
							Retn_Code = CHK_ERR_ELSE
							'Err_Cd = gc_strMsgURKET51_E_025       '≠請求先
							Err_Cd = gc_strMsgURKET51_E_024 'バーチャル口座が得意先に存在しません
							GoTo F_Chk_HD_FBRFNO_End
						Case Else
							'取得項目格納
							URKET51_HEAD_Inf.NYUDT = Mst_Inf_FBTRA.FBKJDT
							URKET51_HEAD_Inf.TOKCD = Mst_Inf_TOKMTA.TOKCD
							'UPGRADE_WARNING: オブジェクト URKET51_HEAD_Inf.TOKMTA の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
							URKET51_HEAD_Inf.TOKMTA = Mst_Inf_TOKMTA
					End Select
				Else
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgURKET51_E_024 'バーチャル口座が得意先に存在しません
				...
				'正常に読み込めた場合だけ、銀行データも取得しておく
				With pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf '１行目のデータ
					'２：振込か３：手形の場合だけ、銀行も取得する
					Select Case .DKBID
						Case pc_strDKBID_URK_HURI, pc_strDKBID_URK_TEG
							'マスタチェック
							If DSPBANK_SEARCH_ALL(Mst_Inf_FBTRA.FBBNKCD, Mst_Inf_BNKMTA) = 0 Then
								'論理削除チェック
								Select Case True
									Case Mst_Inf_BNKMTA.DATKB = gc_strDATKB_DEL
										'Retn_Code = CHK_ERR_ELSE
										'Err_Cd = gc_strMsgURKET51_E_009       '削除済みデータ
									Case Else
										.BNKCD = Mst_Inf_BNKMTA.BNKCD
...
F_Chk_HD_FBRFNO_End: 
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Dsp_HD_FBRFNO_Inf
	'   概要：  見出：FBデータ取込による画面表示
	'   引数：  pm_Dsp_Sub_Inf   :
	'           pm_Mode          : 画面表示モード
	'           pm_All           : 画面情報
	'   戻値：
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_FBRFNO_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		Dim blnUpdTOKCD As Boolean
		Dim blnUpdNYUKN As Boolean
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				Dsp_Value = CF_Cnv_Dsp_Item(URKET51_HEAD_Inf.FBRFNO, pm_All.Dsp_Sub_Inf(Trg_Index), False)
				...
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Dsp_Value = CF_Cnv_Dsp_Item(URKET51_HEAD_Inf.NYUDT, pm_All.Dsp_Sub_Inf(Trg_Index), False)
				Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All, SET_FLG_DB_ERR)
				...
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Dsp_Value = CF_Cnv_Dsp_Item(URKET51_HEAD_Inf.TOKCD, pm_All.Dsp_Sub_Inf(Trg_Index), False)
				Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All, SET_FLG_DB_ERR)
				...
				'画面に編集
				Call CF_Edi_Dsp_Body_Item(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.BNKCD, pm_All.Dsp_Sub_Inf(Wk_Index), Wk_Row, pm_All, SET_FLG_DB_ERR)
				
				'画面ボディ情報(PM_ALL.Dsp_Body_Inf)に編集
				Call CF_Edi_Dsp_Body_Inf(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.BNKCD, pm_All.Dsp_Sub_Inf(Wk_Index), Bd_Index, pm_All, SET_FLG_DB_ERR)
				
				'【銀行名称】
				Wk_Index = CShort(FR_SSSMAIN.BD_BNKNM(0).Tag)
				
				'画面に編集
				Call CF_Edi_Dsp_Body_Item(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.BNKNM, pm_All.Dsp_Sub_Inf(Wk_Index), Wk_Row, pm_All, SET_FLG_DB_ERR)
				
				'画面ボディ情報(PM_ALL.Dsp_Body_Inf)に編集
				Call CF_Edi_Dsp_Body_Inf(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.BNKNM, pm_All.Dsp_Sub_Inf(Wk_Index), Bd_Index, pm_All, SET_FLG_DB_ERR)
				
				'【支店名称】
				Wk_Index = CShort(FR_SSSMAIN.BD_STNNM(0).Tag)
				
				'画面に編集
				Call CF_Edi_Dsp_Body_Item(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.STNNM, pm_All.Dsp_Sub_Inf(Wk_Index), Wk_Row, pm_All, SET_FLG_DB_ERR)
				
				'画面ボディ情報(PM_ALL.Dsp_Body_Inf)に編集
				Call CF_Edi_Dsp_Body_Inf(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.STNNM, pm_All.Dsp_Sub_Inf(Wk_Index), Bd_Index, pm_All, SET_FLG_DB_ERR)
		...
		Else
            ...
            '情報の初期化
            URKET51_HEAD_Inf.FBRFNO = ""
            '2019/06/27 CHG START
            'Call DB_FBTRA_Clear(URKET51_HEAD_Inf.FBTRA)
            URKET51_HEAD_Inf.FBTRA = New TYPE_DB_FBTRA
            '2019/06/27 CHG E N D
            URKET51_HEAD_Inf.NYUDT = GV_UNYDate
            URKET51_HEAD_Inf.TOKCD = ""
            '2019/06/27 CHG START
            'Call DB_TOKMTA_Clear(URKET51_HEAD_Inf.TOKMTA)
            URKET51_HEAD_Inf.TOKMTA = New TYPE_DB_TOKMTA
            '2019/06/27 CHG E N D

            '明細をすべて削除する
            For Wk_Row = 6 To 1 Step -1
				If Wk_Row = 1 Then
					'１行目は、項目をクリア
		...
		'請求先が変更されていたら、それに付随するものも変更する
		If blnUpdTOKCD = True Then
			'【請求先名】
			Trg_Index = CShort(FR_SSSMAIN.HD_TOKRN.Tag)
			'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			Dsp_Value = CF_Cnv_Dsp_Item(URKET51_HEAD_Inf.TOKMTA.TOKRN, pm_All.Dsp_Sub_Inf(Trg_Index), False)
			Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All, SET_FLG_DB_ERR)
			...
			'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			Dsp_Value = CF_Cnv_Dsp_Item(URKET51_HEAD_Inf.TOKMTA.TUKKB, pm_All.Dsp_Sub_Inf(Trg_Index), False)
			Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All, SET_FLG_DB_ERR)
			
			'** ｺﾝﾄﾛｰﾙ制御 **
			Trg_Index = CShort(FR_SSSMAIN.HD_TUKKB.Tag)
			If URKET51_HEAD_Inf.TOKMTA.FRNKB = gc_strFRNKB_FRN Then
				'海外
		...
		'入金額(円)が変更されていたら、それに付随するものも変更する
		If blnUpdNYUKN = True Then
			'【入金額(円)】
			Trg_Index = CShort(FR_SSSMAIN.BD_NYUKN(0).Tag)
			...
			'画面に編集
			Call CF_Edi_Dsp_Body_Item(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.NYUKN, pm_All.Dsp_Sub_Inf(Trg_Index), Wk_Row, pm_All, SET_FLG_DB_ERR)
			
			'画面ボディ情報(PM_ALL.Dsp_Body_Inf)に編集
			Call CF_Edi_Dsp_Body_Inf(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.NYUKN, pm_All.Dsp_Sub_Inf(Trg_Index), Bd_Index, pm_All, SET_FLG_DB_ERR)
			
			'合計(円)　計算
			pv_curNYUKN_SUM = 0
			For intCnt = 1 To UBound(pm_All.Dsp_Body_Inf.Row_Inf)
				pv_curNYUKN_SUM = pv_curNYUKN_SUM + pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.NYUKN
			Next intCnt
			...
			Else
				Select Case CShort(Input_Value)
					Case 1, 2 '１：入金、２：前受入金
						...
						URKET51_HEAD_Inf.NYUKB = Input_Value
...
F_Chk_HD_NYUKB_End: 
	...
	Public Function F_Dsp_HD_NYUKB_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		'入金区分が変更されていたら、それに付随するものも変更する
		If blnUpdNYUKB = True Then
			'** ｺﾝﾄﾛｰﾙ制御 **
			'【受注番号】
			If URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_MAE Then
				'前受入金
	...
	Public Function F_Chk_HD_NYUDT(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			If CF_Chk_Item_Base(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf, Input_Value) <> CHK_BASE_OK Then
				Retn_Code = CHK_ERR_ELSE
				Err_Cd = gc_strMsgURKET51_E_008 '入力範囲外
			Else
				'システム日付より未来はエラー
				If Input_Value > GV_UNYDate Then
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgURKET51_E_015
				Else
					'''' UPD 2011/01/14  FKS) T.Yamamoto    Start    連絡票№FC11011401
					'月次本締日の条件撤廃
					'                '前回月次更新実行日より過去はエラー
					...
					If Trim(Input_Value) <= Trim(pv_strSMAUPDDT) Then
						'''' UPD 2011/01/14  FKS) T.Yamamoto    End
						Retn_Code = CHK_ERR_ELSE
						Err_Cd = gc_strMsgURKET51_E_016
						GoTo F_Chk_HD_NYUDT_End
					End If
					'2009/09/03 ADD START RISE)MIYAJIMA
					'画面.入金日 <= 前回請求日の場合はエラーを表示する
					If Trim(Input_Value) <= Trim(URKET51_HEAD_Inf.TOKMTA.TOKSMEDT) Then
						Retn_Code = CHK_ERR_ELSE
						Err_Cd = gc_strMsgURKET51_E_030
						GoTo F_Chk_HD_NYUDT_End
					End If
                    '2009/09/03 ADD E.N.D RISE)MIYAJIMA
                    ...
                    '締めを跨いでの日付はエラー
                    '2019/04/08 DEL START   --pv_strSMAUPDDTに値が入っておらずおちるためいったん削除
					...
					URKET51_HEAD_Inf.NYUDT = Input_Value
				End If
			End If
		End If
	...
	Public Function F_Chk_HD_TOKCD(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
        ...
        For intCnt = 1 To UBound(pm_All.Dsp_Body_Inf.Row_Inf)
			With pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf
				'２：振込 の銀行(銀行コード・銀行名称・支店名称)を書き換える
				If .DKBID = pc_strDKBID_URK_HURI Then
					.BNKCD = ""
					.BNKNM = ""
					.STNNM = ""
				End If
		...
		Else
			...
			Else
				...
				If DSPTOKCD_SEARCH(Input_Value, Mst_Inf) = 0 Then
					...
					'2009/09/03 ADD START RISE)MIYAJIMA
					'営業担当フラグを取得
					Call F_Util_GET_TANMTA_TANCLAKB(Mst_Inf.TANCD, strTANCLAKB)
					'2009/09/03 ADD E.N.D RISE)MIYAJIMA
					...
					Select Case True
						Case Mst_Inf.DATKB = gc_strDATKB_DEL
							...
							'    Retn_Code = CHK_ERR_ELSE
							'    Err_Cd = gc_strMsgURKET51_E_010      '検索対象外データ
						Case Mst_Inf.TOKSEICD <> Mst_Inf.TOKCD
							Retn_Code = CHK_ERR_ELSE
							Err_Cd = gc_strMsgURKET51_E_025 '≠請求先
                            '2009/09/03 ADD START RISE)MIYAJIMA
                        ...
                        Case strTANCLAKB <> "1"
                            Retn_Code = CHK_ERR_ELSE
							Err_Cd = gc_strMsgURKET51_E_031 '請求先担当者が営業でありません
							'2009/09/03 ADD E.N.D RISE)MIYAJIMA
						Case Else
							'2009/09/03 ADD START RISE)MIYAJIMA
							'画面.入金日 <= 前回請求日の場合はエラーを表示する
							If Trim(URKET51_HEAD_Inf.NYUDT) <= Trim(Mst_Inf.TOKSMEDT) Then
								Retn_Code = CHK_ERR_ELSE
								Err_Cd = gc_strMsgURKET51_E_030
							...
							URKET51_HEAD_Inf.TOKCD = Input_Value
							'UPGRADE_WARNING: オブジェクト URKET51_HEAD_Inf.TOKMTA の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
							URKET51_HEAD_Inf.TOKMTA = Mst_Inf
							
							For intCnt = 1 To UBound(pm_All.Dsp_Body_Inf.Row_Inf)
								With pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf
									'２：振込 の銀行(銀行コード・銀行名称・支店名称)を書き換える
									If .DKBID = pc_strDKBID_URK_HURI Then
										.BNKCD = URKET51_HEAD_Inf.TOKMTA.BNKCD
										If DSPBANK_SEARCH_ALL(.BNKCD, Mst_Inf_BNKMTA) = 0 Then
											.BNKNM = Mst_Inf_BNKMTA.BNKNM
											.STNNM = Mst_Inf_BNKMTA.STNNM
										End If
									End If
...
F_Chk_HD_TOKCD_End: 
	...
	Public Function F_Dsp_HD_TOKCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If blnUpdTOKCD = True Then
			...
			For intCnt = 1 To UBound(pm_All.Dsp_Body_Inf.Row_Inf)
				'２：振込 の銀行(銀行コード・銀行名称・支店名称)を書き換える
				If pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.DKBID = pc_strDKBID_URK_HURI Then
					...
					'【銀行コード】
					Wk_Index = CShort(FR_SSSMAIN.BD_BNKCD(intCnt).Tag)
					...
					'【銀行名称】
					Wk_Index = CShort(FR_SSSMAIN.BD_BNKNM(intCnt).Tag)
					...
					'【支店名称】
					Wk_Index = CShort(FR_SSSMAIN.BD_STNNM(intCnt).Tag)
	...
	Public Function F_Chk_HD_TOKRN(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			Else
				...
				URKET51_HEAD_Inf.TOKMTA.TOKRN = Input_Value
			End If
		End If
...
F_Chk_HD_TOKRN_End: 
	...
	Public Function F_Chk_HD_TUKKB(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			Else
				'マスタチェック
				If DSPMEIM_SEARCH(gc_strKEYCD_TUKKB, Input_Value, Mst_Inf) = 0 Then
					'論理削除チェック
					Select Case True
						Case Mst_Inf.DATKB = gc_strDATKB_DEL
							Retn_Code = CHK_ERR_ELSE
						...
						Case Else
							...
							URKET51_HEAD_Inf.TOKMTA.TUKKB = Input_Value
					End Select
				Else
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgURKET51_E_011 '該当データなし
				...
				If F_Util_KNJKOZ_Search(Input_Value, Mst_Inf) = 0 Then
					...
					Select Case True
						...
						Case Else
							...
							URKET51_HEAD_Inf.KNJKOZ = Input_Value
					End Select
				Else
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgURKET51_E_011 '該当データなし
	...
	Public Function F_Dsp_HD_KNJKOZ_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				If Trim(URKET51_HEAD_Inf.KNJKOZ) <> "" Then
					For intCnt = 1 To UBound(pm_All.Dsp_Body_Inf.Row_Inf)
						...
						If Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(Wk_Index))) <> "" Then
							...
							pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.KANKOZ = URKET51_HEAD_Inf.KNJKOZ
							...
							'画面に編集
							Call CF_Edi_Dsp_Body_Item(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.KANKOZ, pm_All.Dsp_Sub_Inf(Wk_Index), Wk_Row, pm_All, SET_FLG_DB_ERR)
							'画面ボディ情報(PM_ALL.Dsp_Body_Inf)に編集
							Call CF_Edi_Dsp_Body_Inf(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.KANKOZ, pm_All.Dsp_Sub_Inf(Wk_Index), Bd_Index, pm_All, SET_FLG_DB_ERR)
						End If
					Next intCnt
				End If
			...
			Else
				...
				If SYSTBD_SEARCH(pc_strDKBSB_URK, Input_Value, Mst_Inf_SYSTBD) = 0 Then
						...
						If F_Util_KNJKOZ_Search(strKNJKOZ, Mst_Inf_MEIMTA) = 0 Then
							...
							If Trim(Mst_Inf_MEIMTA.MEINMC) = pc_strKNJKOZ_TEG Then
								...
								If Input_Value <> pc_strDKBID_URK_TEG Then
									Retn_Code = CHK_ERR_ELSE
									Err_Cd = gc_strMsgURKET51_E_037 '手形の勘定口座が指定されています。
							...
							Else
								...
								If Input_Value = pc_strDKBID_URK_TEG Then
									Retn_Code = CHK_ERR_ELSE
									Err_Cd = gc_strMsgURKET51_E_036 '勘定口座の種別が手形ではありません。
									GoTo F_Chk_BD_DKBID_End
								End If
							End If
						Else
					...
					With pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf
						...
						If Trim(URKET51_HEAD_Inf.KNJKOZ) <> "" Then
							.KANKOZ = URKET51_HEAD_Inf.KNJKOZ
						End If
					...
					If Input_Value = pc_strDKBID_URK_HURI Then
						...
						'銀行を検索し、名称を取得
						If DSPBANK_SEARCH_ALL(URKET51_HEAD_Inf.TOKMTA.BNKCD, Mst_Inf_BNKMTA) = 0 Then
							With pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf
								.BNKNM = Mst_Inf_BNKMTA.BNKNM
								.STNNM = Mst_Inf_BNKMTA.STNNM
							End With
					...
					If Input_Value = pc_strDKBID_URK_TEG Then
						...
						dteNYUDT = CDate(VB6.Format(URKET51_HEAD_Inf.NYUDT, "@@@@/@@/@@"))
						
						dteNYUDT = DateSerial(Year(dteNYUDT), Month(dteNYUDT), VB.Day(dteNYUDT) + URKET51_HEAD_Inf.TOKMTA.NYUDD)
					...
					Select Case URKET51_HEAD_Inf.TOKMTA.SHAKB
						Case pc_strSHAKB_HURI, pc_strSHAKB_TEG, pc_strSHAKB_HURI_OR_TEG, pc_strSHAKB_HURI_AND_TEG
							...
							If pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.DKBID = pc_strDKBID_URK_HURIK Then
								...
								'                            Err_Cd = gc_strMsgURKET51_E_017
								Err_Cd = gc_strMsgURKET51_E_033
	...
	Public Function F_Dsp_BD_DKBID_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				'画面に編集
				Call CF_Edi_Dsp_Body_Item(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.DKBNM, pm_All.Dsp_Sub_Inf(Wk_Index), Wk_Row, pm_All, SET_FLG_DB_ERR)
				
				'画面ボディ情報(PM_ALL.Dsp_Body_Inf)に編集
				Call CF_Edi_Dsp_Body_Inf(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.DKBNM, pm_All.Dsp_Sub_Inf(Wk_Index), Bd_Index, pm_All, SET_FLG_DB_ERR)
				...
				'画面に編集
				Call CF_Edi_Dsp_Body_Item(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.TEGDT, pm_All.Dsp_Sub_Inf(Wk_Index), Wk_Row, pm_All, SET_FLG_DB_ERR)
				
				'画面ボディ情報(PM_ALL.Dsp_Body_Inf)に編集
				Call CF_Edi_Dsp_Body_Inf(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.TEGDT, pm_All.Dsp_Sub_Inf(Wk_Index), Bd_Index, pm_All, SET_FLG_DB_ERR)
				
				'【手形番号】
				Wk_Index = CShort(FR_SSSMAIN.BD_TEGNO(0).Tag)
				
				'画面に編集
				Call CF_Edi_Dsp_Body_Item(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.TEGNO, pm_All.Dsp_Sub_Inf(Wk_Index), Wk_Row, pm_All, SET_FLG_DB_ERR)
				
				'画面ボディ情報(PM_ALL.Dsp_Body_Inf)に編集
				Call CF_Edi_Dsp_Body_Inf(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.TEGNO, pm_All.Dsp_Sub_Inf(Wk_Index), Bd_Index, pm_All, SET_FLG_DB_ERR)
	...
	Public Function F_Chk_BD_JDNNO(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			Else
				...
				If intRet <> 0 Then
					...
					Select Case intRet
						Case 1
							Err_Cd = gc_strMsgURKET51_E_011 '該当データなし
						Case 2
							Err_Cd = gc_strMsgURKET51_E_035 '受注伝票日付の年月＞画面.入金日の年月
					End Select
					GoTo F_Chk_BD_JDNNO_End
				End If
				...
				' === 20130711 === INSERT S - FWEST)Koroyasu 排他制御の追加
				'排他チェック
                If Trim(strJDNNO) <> "" And strCallMode <> AUTO_MODE Then
                    '2019/04/04 DEL START (仮)変換直後と引数の数がちがう
                    ...
                    Select Case rResult
                        ...
                        Case 9
                            Retn_Code = CHK_ERR_ELSE
                            Err_Cd = gc_strMsgURKET51_E_004 '更新異常
                            GoTo F_Chk_BD_JDNNO_End
                    End Select
                End If
				' === 20130711 === INSERT E -
...
F_Chk_BD_JDNNO_End: 
        ...
        If strCallMode <> AUTO_MODE Then
            '2019/04/04 DEL START (仮)ファンクションがなくなってる
            'Call CF_EXCTBZ_Unlock(pm_All)
            '2019/04/04 DEL E N D
        End If
		' === 20130716 === INSERT E
	...
	Public Function F_Chk_BD_TEGDT(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			Else
				...
				If Trim(URKET51_HEAD_Inf.NYUDT) > Trim(Input_Value) Then
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgURKET51_E_008
					GoTo F_Chk_BD_TEGDT_End
				End If
				'2009/09/03 ADD E.N.D RISE)MIYAJIMA
...
F_Chk_BD_LINCMB_End: 
	...
	Public Function F_Dsp_Item_Detail(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
			Case FR_SSSMAIN.HD_FBRFNO.Name
				'FBデータ取込による画面表示
				Call F_Dsp_HD_FBRFNO_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
	...
	Public Function F_Ctl_Item_Chk(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Process As String, ByRef pm_Chk_Move_Flg As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			...
			Case FR_SSSMAIN.HD_FBRFNO.Name
				...
				'見出：FBデータ取込のﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_FBRFNO(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
		...
		For Index_Wk = 1 To pm_All.Dsp_Base.Head_Lst_Idx
			...
			If Rtn_Chk <> CHK_OK Then
				...
				If Rtn_Chk = CHK_ERR_NOT_INPUT Then
					Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_014, pm_All)
				End If
	...
	Public Function F_Ctl_Head_RelChk(ByRef pm_All As Cls_All, ByRef pm_ErrIdx As Short) As Short
		...
		Err_Cd = ""
		pm_ErrIdx = CShort(FR_SSSMAIN.HD_NYUKB.Tag)
...
F_Ctl_Head_RelChk_END: 
	...
	Public Function F_Ctl_Body_Chk(ByRef pm_All As Cls_All) As Short
		...
		If pv_bolMEISAI_INPUT = False Then
			...
			'エラーメッセージ表示
			Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_012, pm_All)
		...
		'        '得意先マスタ．手形支払金額＞０ のみ
		'        If URKET51_HEAD_Inf.TOKMTA.TEGSHKN > 0 Then
		'            'エラーメッセージ表示
		'            Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_018, pm_All)
		'
		'            'ﾁｪｯｸ後移動なし
	...
	Public Function F_Ctl_Body_RelChk(ByRef pm_intRow As Short, ByRef pm_All As Cls_All, ByRef pm_ErrIdx As Short, ByRef pm_ErrRow As Short) As Short
		...
		If strDKBID <> "" Then
			...
			Select Case True
				...
				Case URKET51_HEAD_Inf.TOKMTA.FRNKB = gc_strFRNKB_FRN And (Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intFNYUKN))) = "" Or SSSVal(Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intFNYUKN)))) = 0)
					pm_ErrIdx = CShort(FR_SSSMAIN.BD_FNYUKN(1).Tag)
					
					
					'2009/06/08 ADD START FKS)NAKATA
					'受注番号は、必須入力（ただし、前受のみ）
				Case URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_MAE And Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intJDNNO))) = ""
				...
				Case strDKBID = pc_strDKBID_URK_TEG And Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intTEGNO))) = ""
					...
					Err_Cd = gc_strMsgURKET51_E_022
					
					
					'2009/06/08 ADD START FKS)NAKATA
					'入金種別＝振込仮の場合
					'決済日は、必須入力（ただし、前受のみ）
				Case URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_MAE And strDKBID = pc_strDKBID_URK_HURIK And Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intTEGDT))) = ""
					pm_ErrIdx = CShort(FR_SSSMAIN.BD_TEGDT(1).Tag)
					'2009/06/08 ADD E.N.D FKS)NAKATA
					
					'2009/09/03 ADD START RISE)MIYAJIMA
				Case strDKBID = pc_strDKBID_URK_TEG And Trim(URKET51_HEAD_Inf.NYUDT) > VB6.Format(Trim(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intTEGDT))), "YYYYMMDD")
					pm_ErrIdx = CShort(FR_SSSMAIN.BD_TEGDT(1).Tag)
					Err_Cd = gc_strMsgURKET51_E_008
					'2009/09/03 ADD E.N.D RISE)MIYAJIMA
					...
					''             And URKET51_HEAD_Inf.TOKMTA.TEGSHKN > 0 _
					''             And URKET51_HEAD_Inf.TOKMTA.TEGSHKN > SSSVal(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intNYUKN)))
					'                pm_ErrIdx = CInt(FR_SSSMAIN.BD_NYUKN(1).Tag)
					'                Err_Cd = gc_strMsgURKET51_E_023
					'// V1.10↑ DEL
					...
					'            Case strDKBID = pc_strDKBID_URK_TEG _
					''             And URKET51_HEAD_Inf.TOKMTA.FRNKB = gc_strFRNKB_FRN _
					''             And URKET51_HEAD_Inf.TOKMTA.TEGSHKN > 0 _
					''             And URKET51_HEAD_Inf.TOKMTA.TEGSHKN > SSSVal(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(intFNYUKN)))
...
F_Ctl_Body_RelChk_END: 
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Ctl_CS_FBRFNO
	'   概要：  対象項目のFBデータ取込ボタンの制御
	'   引数：　なし
	'   戻値：　なし
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Ctl_CS_FBRFNO(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		Dim Trg_Index As Short
		Dim Dsp_Value As Object
		Dim Move_Flg As Boolean
		Dim Rtn_Chk As Short
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			gv_bolURKET51_LF_Enable = True
			
			If WLSFBTRA2_RTNCODE <> "" Then
				'検索ＯＫ
				'画面に編集
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Dsp_Value = CF_Cnv_Dsp_Item(WLSFBTRA2_RTNCODE, pm_All.Dsp_Sub_Inf(Trg_Index), False)
				Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Ctl_CS_TUKKB
	'   概要：  対象項目の見出：通貨ボタンの制御
	'   引数：　なし
	'   戻値：　なし
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Ctl_CS_TUKKB(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		Dim Trg_Index As Short
		Dim Dsp_Value As Object
		Dim Move_Flg As Boolean
		Dim Rtn_Chk As Short
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			'リスト選択画面の情報を設定
			Call DSPMEIMTA_SEARCH_SORTUSE(gc_strKEYCD_TUKKB, Mst_Inf_MEI, "KEYCD, MEICDA, MEICDB")
			WLS_LIST.Text = "通貨区分名称一覧"
			CType(WLS_LIST.Controls("LST"), Object).Items.Clear()
			For intCnt = 1 To UBound(Mst_Inf_MEI)
				If Mst_Inf_MEI(intCnt).DATKB <> "9" Then
					strItem = LeftWid(Mst_Inf_MEI(intCnt).MEICDA, 5) & " " & LeftWid(Mst_Inf_MEI(intCnt).MEINMA, 40)
	...
	Public Function F_Ctl_CS_DKBID(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Act_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		If Focus_Flg = True Then
			...
			WLS_LIST.Text = "入金種別"
			CType(WLS_LIST.Controls("LST"), Object).Items.Clear()
            For intCnt = 0 To UBound(Mst_Inf_TBD)
	...
	Private Function F_Util_Get_Simebi(ByVal pin_strNYUDT As String, ByVal pin_strTOKCD As String, ByRef Pot_strSMADT As String, ByRef pot_strSSADT As String, ByRef Pot_strKESDT As String) As Short
		...
		'=== 請求締め日付取得 ===
		With URKET51_HEAD_Inf.TOKMTA
			'UPGRADE_WARNING: オブジェクト SSSVal(URKET51_HEAD_Inf.TOKMTA.TOKSMEKB) の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			If SSSVal(.TOKSMEKB) = 1 Then
				'--- 月X回締め ---
				'UPGRADE_WARNING: オブジェクト SSSVal(URKET51_HEAD_Inf.TOKMTA.TOKSMECC) の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Private Function F_Util_CheckJDNNO(ByVal pin_strJDNNO As String, ByVal pin_strJDNLINNO As String, Optional ByVal pin_strNYUDT As String = "") As Short
        ...
        '2009/06/08 ADD START FKS)NAKATA
        '請求先
        strSQL = strSQL & "    AND TOKSEICD = '" & URKET51_HEAD_Inf.TOKCD & "' "
        '2009/06/08 ADD E.N.D FKS)NAKATA
        ...
        Else
            If pin_strNYUDT <> "" Then
                ...
                strJDNYM = DB_NullReplace(dt.Rows(0)("JDNDT"), "")
                '2019/04/02 CHG E N D
...
F_Util_CheckJDNNO_err:
	...
	'   引数：  受注番号，行番号
	'   戻値：　受注金額－入金額，入金額
	'   備考：　2009/09/07：入金額のみの値も戻値とする。
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'*** 2009/09/07 CHG START FKS)NAKATA
	'Private Function F_Util_Get_UODKN(ByVal pin_strJDNNO As String, ByVal pin_strJDNLINNO As String) As Currency
	Private Function F_Util_Get_UODKN(ByVal pin_strJDNNO As String, ByVal pin_strJDNLINNO As String, ByRef pin_curNYUKN As Decimal, ByVal pi_strDKBID As Object) As Decimal
		'*** 2009/09/07 ADD E.N.D FKS)NAKATA
		...
		'**** 2009/09/23 CHG STRAT FKS)NAKATA
		'    If pi_strDKBID <> "09" Then
		'        strSQL = strSQL & "  　AND   TRA.DKBID   != '09'"    '本入金は相手にしない
		'    Else
		'        strSQL = strSQL & "  　AND   TRA.DKBID   = '09'"     '画面入力が本入金の場合は、本入金のみ相手にする。
		'    End If
		strSQL = strSQL & "  　AND   TRA.DKBID   != '09'" '本入金は相手にしない
		'**** 2009/09/23 CHG STRAT FKS)NAKATA
        ...
        Else
            ...
            '2019/014/02 CHG E N D

            '*** 2009/09/07 CHG START FKS)NAKATA
            '入金額の戻値のセット
            pin_curNYUKN = curNYUKN
            '*** 2009/09/07 CHG E.N.D FKS)NAKATA
        End If
...
F_Util_Get_UODKN_err:
	...
	Public Function F_Util_GET_TANMTA_KEIBMNCD(ByRef pot_strTANCD As String, ByRef pot_strKEIBMNCD As String) As Short
		...
		strTKDT = Replace(URKET51_HEAD_Inf.NYUDT, "/", "")
...
ERR_F_Util_GET_TANMTA_KEIBMNCD:
	...
	Public Function F_Ctl_Upd_Process(ByRef pm_All As Cls_All) As Short
		...
		'    If intRet <> 0 Then
		'        Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_026, pm_All)    ' MSG内容:他端末で更新中です。
		...
		If Inp_Inf.InpJDNUPDKB <> gc_strJDNUPDKB_OK Then
			Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_003, pm_All)
			GoTo End_F_Ctl_Upd_Process
		End If
		
		'経理締日付、締日付、決済日付 取得
		intRet = F_Util_Get_Simebi(URKET51_HEAD_Inf.NYUDT, URKET51_HEAD_Inf.TOKCD, pv_strSMADT, pv_strSSADT, pv_strKESDT)
		If intRet <> 0 Then
			Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_027, pm_All)
			GoTo End_F_Ctl_Upd_Process
		End If
		...
		If F_Util_CheckSumOver(pm_All, 1) <> 0 Then
			'メッセージ出力
			Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_034, pm_All)
			'チェックＮＧの場合
			GoTo End_F_Ctl_Upd_Process
		End If
		'2009/09/24 ADD E.N.D RISE)MIYAJIMA
		...
		' === 20110714 === INSERT S TOM)Morimoto
		If strCallMode <> AUTO_MODE Then
			' === 20110714 === INSERT E
			'登録確認
			If AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_A_005, pm_All) = MsgBoxResult.No Then
				GoTo End_F_Ctl_Upd_Process
			End If
			' === 20110714 === INSERT S TOM)Morimoto
		End If
		...
		If strCallMode <> AUTO_MODE Then
			' === 20110714 === INSERT E
			'登録完了
			Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_A_006, pm_All)
			' === 20110714 === INSERT S TOM)Morimoto
		End If
		' === 20110714 === INSERT E
		F_Ctl_Upd_Process = 0
...
Err_F_Ctl_Upd_Process: 
	...
	Public Function F_Update_Main(ByRef pm_All As Cls_All) As Short
		...
		'=== 20110516 === INSERT S TOM)Morimoto
		'UPGRADE_WARNING: 構造体 objrst の配列は、使用する前に初期化する必要があります。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="814DF224-76BD-4BB4-BFFB-EA359CB9FC48"' をクリックしてください。
		Dim objrst As U_Ody
		Dim strSQL As String
		'=== 20110516 === INSERT E
		...
		'ＦＢ選択されている場合のみ実行
		If Trim(URKET51_HEAD_Inf.FBRFNO) <> "" Then
			'ＦＢトランの排他制御
			intRet = F_FBTRA_Exicz()
			If intRet <> 0 Then
				Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_026, pm_All) ' MSG内容:他端末で更新中です。
				F_Update_Main = intRet
				GoTo F_Update_Main_err
		...
		'新しい伝票№を取得
		intRet = F_SYSTBC_SaibanDENNO(pm_All, strDenNo)
		If intRet <> 0 Then
			F_Update_Main = intRet
			GoTo F_Update_Main_err
		End If
		...
		' === 20110714 === INSERT S TOM)Morimoto
		If strCallMode = AUTO_MODE Then
			' === 20111031 === UPDATE S TOM)Morimoto
			'        pv_intMeisaiCnt = 1
			pv_intMeisaiCnt = gv_intMeisaiCnt
			' === 20111031 === UPDATE E
		End If
		' === 20110714 === INSERT E
		For intCnt = 1 To pv_intMeisaiCnt
			...
			If Tbl_Inf_UDNTRA.DENKB = "8" And Tbl_Inf_UDNTRA.DKBTEGFL = "1" Then
				'受取手形トランの更新
				intRet = F_UTGTRA_Insert(pm_All, Tbl_Inf_UDNTRA)
				If intRet <> 0 Then
					F_Update_Main = intRet
					GoTo F_Update_Main_err
				End If
		...
		'=== 20110516 === INSERT S TOM)Morimoto
		If strCallMode = MAIN_MODE Then
			'=== 20110516 === INSERT E
			'ＦＢ選択されている場合のみ実行
			If Trim(URKET51_HEAD_Inf.FBRFNO) <> "" Then
				'ＦＢトランの更新
				intRet = F_FBTRA_Update_DelF(pm_All)
				If intRet <> 0 Then
					F_Update_Main = intRet
					GoTo F_Update_Main_err
				End If
		...
		If strCallMode = SUB_MODE Then
			strSQL = vbNullString
			strSQL = strSQL & "UPDATE FBTRB SET "
			strSQL = strSQL & " DATKB='9'"
			strSQL = strSQL & " ,OPEID='" & SSS_OPEID.Value & "'"
			strSQL = strSQL & " ,CLTID='" & SSS_CLTID.Value & "'"
			strSQL = strSQL & " ,WRTTM='" & CF_Ora_String(GV_SysTime, 6) & "' " 'タイムスタンプ（バッチ時間）
			strSQL = strSQL & " ,WRTDT='" & CF_Ora_String(GV_SysDate, 8) & "' " 'タイムスタンプ（バッチ日付）
            strSQL = strSQL & " WHERE DATNO='" & Trim(Link_IN.HINCD) & "'"
            '2019/04/02 CHG START
            'CF_Ora_Execute(gv_Odb_USR1, strSQL)
...
F_SYSTBA_SaibanDATNO_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_SYSTBA_SaibanDATNO")
...
F_SYSTBA_SaibanRECNO_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_SYSTBA_SaibanRECNO")
...
F_SYSTBC_SaibanDENNO_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_SYSTBC_SaibanDENNO")
	...
	Private Function F_UDNTHA_MakeInf(ByRef pm_All As Cls_All, ByVal pin_strDATNO As String, ByVal pin_strDENNO As String, ByRef pot_Tbl_Inf_UDNTHA As TYPE_DB_UDNTHA) As Short
		...
		'経理部門コードを取得
		Call F_Util_GET_TANMTA_KEIBMNCD(URKET51_HEAD_Inf.TOKMTA.TANCD, strBUMCD)
		...
		With Tbl_Inf_UDNTHA
			...
			.JDNNO = "" '受注伝票番号
			.USDNO = "" '直送伝票NO
			.UDNDT = URKET51_HEAD_Inf.NYUDT '売上伝票日付
			.DENDT = GV_UNYDate '売上日付
			.REGDT = URKET51_HEAD_Inf.NYUDT '初回伝票日付
			.TOKCD = URKET51_HEAD_Inf.TOKCD '得意先コード
			.TOKRN = URKET51_HEAD_Inf.TOKMTA.TOKRN '得意先略称
			.NHSCD = "" '納入先コード
			.NHSRN = "" '納入先略称
			.NHSNMA = "" '納入先名称１
			.NHSNMB = "" '納入先名称２
			...
			.BUMCD = "" '部門コード
			.BUMNM = "" '部門名
			.TOKSEICD = URKET51_HEAD_Inf.TOKCD '請求先コード
			.SOUCD = "" '倉庫コード
			.SOUNM = "" '倉庫名
			.NXTKB = "" '帳端区分
			.NXTNM = "" '帳端名称
			...
			.NYUCD = URKET51_HEAD_Inf.NYUKB '入金区分
			.ZKTKB = "" '取引区分
			.ZKTNM = "" '取引区分名
			.KENNMA = "" '件名１
			.KENNMB = "" '件名２
			...
			.DENCM = "" '備考
			.DENCMIN = "" '社内備考
			.TOKSMEKB = URKET51_HEAD_Inf.TOKMTA.TOKSMEKB '締区分
			.TOKSMEDD = URKET51_HEAD_Inf.TOKMTA.TOKSMEDD '締初期日付(売上)
			.TOKSMECC = URKET51_HEAD_Inf.TOKMTA.TOKSMECC '締サイクル(売上)
			.TOKSDWKB = URKET51_HEAD_Inf.TOKMTA.TOKSDWKB '締め曜日
			.TOKKESCC = URKET51_HEAD_Inf.TOKMTA.TOKKESCC '回収サイクル
			.TOKKESDD = URKET51_HEAD_Inf.TOKMTA.TOKKESDD '回収日付
			.TOKKDWKB = URKET51_HEAD_Inf.TOKMTA.TOKKDWKB '回収曜日
			.LSTID = URKET51_HEAD_Inf.TOKMTA.LSTID '伝票種別
			.TOKJUNKB = URKET51_HEAD_Inf.TOKMTA.TOKJUNKB '順位表出力区分
			.TOKMSTKB = URKET51_HEAD_Inf.TOKMTA.TOKMSTKB 'マスタ区分(得意先)
			.TKNRPSKB = URKET51_HEAD_Inf.TOKMTA.TKNRPSKB '金額端数処理桁数
			.TKNZRNKB = URKET51_HEAD_Inf.TOKMTA.TKNZRNKB '金額端数処理区分
			.TOKZEIKB = URKET51_HEAD_Inf.TOKMTA.TOKZEIKB '消費税区分
			.TOKZCLKB = URKET51_HEAD_Inf.TOKMTA.TOKZCLKB '消費税算出区分
			.TOKRPSKB = URKET51_HEAD_Inf.TOKMTA.TOKRPSKB '消費税端数処理桁数
			.TOKZRNKB = URKET51_HEAD_Inf.TOKMTA.TOKZRNKB '消費税端数処理区分
			.TOKNMMKB = URKET51_HEAD_Inf.TOKMTA.TOKNMMKB '名称ﾏﾆｭｱﾙ区分
			.NHSMSTKB = "" 'マスタ区分(納入先)
			.NHSNMMKB = "" '名称ﾏﾆｭｱﾙ区分
			.TANMSTKB = "" 'マスタ区分(担当者)
			.URIKJN = "" '売上基準
			...
			.SEIKB = "" '請求区分
			.JDNTRKB = "" '受注取引区分
			.TUKKB = URKET51_HEAD_Inf.TOKMTA.TUKKB '通貨区分
			.FRNKB = URKET51_HEAD_Inf.TOKMTA.FRNKB '海外取引区分
			.UDNPRAKB = "" '納品書発行区分
			.UDNPRBKB = "" '個別請求発行区分
			.MOTDATNO = "" '元伝票管理番号
...
F_UDNTHA_MakeInf_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_UDNTHA_MakeInf")
...
F_UDNTHA_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_UDNTHA_Insert")
	...
	Private Function F_UDNTRA_MakeInf(ByRef pm_All As Cls_All, ByVal pin_intRow As Short, ByVal pin_strDATNO As String, ByVal pin_strDENNO As String, ByVal pin_strRECNO As String, ByRef pot_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA) As Short
		...
		Select Case Trim(pm_All.Dsp_Body_Inf.Row_Inf(pin_intRow).Bus_Inf.SYSTBD.DFLDKBCD)
			...
			Case Else : strNYUKB = "1"
		End Select
		If URKET51_HEAD_Inf.TOKMTA.SHAKB = pc_strSHAKB_KIJZITU Or URKET51_HEAD_Inf.TOKMTA.SHAKB = pc_strSHAKB_FACTERING Then
		...
		With Tbl_Inf_UDNTRA
			...
			.RATERT = F_Get_RATERT(URKET51_HEAD_Inf.TOKMTA.TUKKB, URKET51_HEAD_Inf.NYUDT) '為替レート
			'2009/09/27 UPD E.N.D RISE)MIYAJIMA
			.EMGODNKB = "" '緊急出荷区分
			
			'2009/06/05 CHG START FKS)NAKATA
			'.OKRJONO = ""                                       '送り状№
			.OKRJONO = Trim(strJDNNO) & Trim(strJDNLINNO) '送り状№
			'2009/06/05 CHG E.N.D FKS)NAKATA
			...
			'2009/09/18 UPD START RISE)MIYAJIMA
			.TEGDT = strTEGDT '手形期日
			If URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_NML Then
				.TEGDT = strTEGDT '手形期日
			Else
				If URKET51_HEAD_Inf.TOKMTA.SHAKB = pc_strSHAKB_KIJZITU Or URKET51_HEAD_Inf.TOKMTA.SHAKB = pc_strSHAKB_FACTERING Then
					If .DKBID <> pc_strDKBID_URK_GENKN And .DKBID <> pc_strDKBID_URK_HURI And .DKBID <> pc_strDKBID_URK_TEG And .DKBID <> pc_strDKBID_URK_HNYU And .DKBID <> pc_strDKBID_URK_HURIK Then
						...
						'                    .TEGDT = F_GET_MaeukeTEGDT(pm_All, Trim(strJDNNO) & Trim(strJDNLINNO))  '手形期日
						.TEGDT = F_GET_MaeukeTEGDT(pm_All, Trim(strJDNNO) & Trim(strJDNLINNO), strTEGDT) '手形期日
...
F_UDNTRA_MakeInf_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_UDNTRA_MakeInf")
	...
	Private Function F_UDNTRA_Insert(ByRef pm_All As Cls_All, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA) As Short
		...
		With pin_Tbl_Inf_UDNTRA
			...
			strSQL = strSQL & "        , '" & CF_Ora_String(.DENKB, 1) & "' " '伝票区分
			strSQL = strSQL & "        , '" & CF_Ora_String(.UDNNO, 8) & "' " '売上伝票番号
			strSQL = strSQL & "        , '" & CF_Ora_String(.LINNO, 3) & "' " '行番号
			strSQL = strSQL & "        , '" & CF_Ora_String(.ZKTKB, 1) & "' " '取引区分
			strSQL = strSQL & "        , '" & CF_Ora_String(.ODNNO, 8) & "' " '出荷伝票番号
			strSQL = strSQL & "        , '" & CF_Ora_String(.ODNLINNO, 3) & "' " '行番号
			strSQL = strSQL & "        , '" & CF_Ora_String(.JDNNO, 10) & "' " '受注伝票番号
			strSQL = strSQL & "        , '" & CF_Ora_String(.JDNLINNO, 3) & "' " '受注伝票行番号
			strSQL = strSQL & "        , '" & CF_Ora_String(.RECNO, 10) & "' " 'レコード管理NO.
			strSQL = strSQL & "        , '" & CF_Ora_String(.USDNO, 8) & "' " '直送伝票NO
			strSQL = strSQL & "        , '" & CF_Ora_String(.UDNDT, 8) & "' " '売上伝票日付
			strSQL = strSQL & "        , '" & CF_Ora_String(.DKBSB, 3) & "' " '伝票取引区分種別
			strSQL = strSQL & "        , '" & CF_Ora_String(.DKBID, 2) & "' " '取引区分コード
			strSQL = strSQL & "        , '" & CF_Ora_String(.DKBNM, 6) & "' " '取引区分名称
			strSQL = strSQL & "        , '" & CF_Ora_String(.HENRSNCD, 2) & "' " '返品理由
			strSQL = strSQL & "        , '" & CF_Ora_String(.HENSTTCD, 2) & "' " '返品状態
			strSQL = strSQL & "        , '" & CF_Ora_String(.SMADT, 8) & "' " '経理締日付
			strSQL = strSQL & "        , '" & CF_Ora_String(.SSADT, 8) & "' " '締日付
			strSQL = strSQL & "        , '" & CF_Ora_String(.KESDT, 8) & "' " '決済日付
			strSQL = strSQL & "        , '" & CF_Ora_String(.TOKCD, 10) & "' " '得意先コード
			...
			strSQL = strSQL & "        , '" & CF_Ora_String(.TOKSEICD, 10) & "' " '請求先コード
			strSQL = strSQL & "        , '" & CF_Ora_String(.SOUCD, 3) & "' " '倉庫コード
			strSQL = strSQL & "        , '" & CF_Ora_String(.SBNNO, 20) & "' " '製番
			strSQL = strSQL & "        , '" & CF_Ora_String(.HINCD, 10) & "' " '製品コード
			strSQL = strSQL & "        , '" & CF_Ora_String(.TOKJDNNO, 23) & "' " '客先注文番号
			strSQL = strSQL & "        , '" & CF_Ora_String(.HINNMA, 50) & "' " '型式
			strSQL = strSQL & "        , '" & CF_Ora_String(.HINNMB, 50) & "' " '商品名１
			strSQL = strSQL & "        , '" & CF_Ora_String(.UNTCD, 2) & "' " '単位コード
			strSQL = strSQL & "        , '" & CF_Ora_String(.UNTNM, 4) & "' " '単位名
			strSQL = strSQL & "        ,  " & CStr(.IRISU) '入数
			strSQL = strSQL & "        ,  " & CStr(.CASSU) 'ケース数
			strSQL = strSQL & "        ,  " & CStr(.URISU) '売上数量
			strSQL = strSQL & "        ,  " & CStr(.URITK) '単価
			strSQL = strSQL & "        ,  " & CStr(.GNKTK) '原価単価
			strSQL = strSQL & "        ,  " & CStr(.SIKTK) '営業仕切単価
			strSQL = strSQL & "        ,  " & CStr(.FURITK) '外貨単価
			strSQL = strSQL & "        ,  " & CStr(.URIKN) '売上金額
			strSQL = strSQL & "        ,  " & CStr(.FURIKN) '外貨売上金額
			strSQL = strSQL & "        ,  " & CStr(.SIKKN) '営業仕切金額
			strSQL = strSQL & "        ,  " & CStr(.UZEKN) '消費税金額
			strSQL = strSQL & "        , '" & CF_Ora_String(.NYUDT, 8) & "' " '入金日
			strSQL = strSQL & "        ,  " & CStr(.NYUKN) '入金額
			strSQL = strSQL & "        ,  " & CStr(.FNYUKN) '外貨入金額
			strSQL = strSQL & "        ,  " & CStr(.GNKKN) '原価金額
			strSQL = strSQL & "        ,  " & CStr(.JKESIKN) '消込金額
			strSQL = strSQL & "        ,  " & CStr(.FKESIKN) '外貨消込金額
			strSQL = strSQL & "        , '" & CF_Ora_String(.KESIKB, 1) & "' " '消込区分
			strSQL = strSQL & "        , '" & CF_Ora_String(.NYUKB, 1) & "' " '入金種別
			strSQL = strSQL & "        , '" & CF_Ora_String(.TNKID, 2) & "' " '種別
			strSQL = strSQL & "        , '" & CF_Ora_String(.TUKKB, 3) & "' " '通貨区分
			strSQL = strSQL & "        ,  " & CStr(.RATERT) '為替レート
			strSQL = strSQL & "        , '" & CF_Ora_String(.EMGODNKB, 1) & "' " '緊急出荷区分
			strSQL = strSQL & "        , '" & CF_Ora_String(.OKRJONO, 15) & "' " '送り状№
			strSQL = strSQL & "        , '" & CF_Ora_String(.INVNO, 8) & "' " 'インボイス№
			strSQL = strSQL & "        , '" & CF_Ora_String(.LINCMA, 20) & "' " '明細備考１
			strSQL = strSQL & "        , '" & CF_Ora_String(.LINCMB, 20) & "' " '明細備考２
			strSQL = strSQL & "        , '" & CF_Ora_String(.BNKCD, 7) & "' " '銀行コード
			strSQL = strSQL & "        , '" & CF_Ora_String(.BNKNM, 50) & "' " '銀行名称
			strSQL = strSQL & "        , '" & CF_Ora_String(.TEGNO, 10) & "' " '手形番号
			strSQL = strSQL & "        , '" & CF_Ora_String(.TEGDT, 8) & "' " '手形期日
			strSQL = strSQL & "        , '" & CF_Ora_String(.UPDID, 2) & "' " '更新用ｲﾝﾃﾞｯｸｽ(ACNT)
			strSQL = strSQL & "        , '" & CF_Ora_String(.DFLDKBCD, 13) & "' " 'デフォルトコード
			strSQL = strSQL & "        , '" & CF_Ora_String(.DKBZAIFL, 1) & "' " '在庫関連フラグ
			strSQL = strSQL & "        , '" & CF_Ora_String(.DKBTEGFL, 1) & "' " '手形発生フラグ
			strSQL = strSQL & "        , '" & CF_Ora_String(.DKBFLA, 1) & "' " 'ダミーフラグ１
			strSQL = strSQL & "        , '" & CF_Ora_String(.DKBFLB, 1) & "' " 'ダミーフラグ２
			strSQL = strSQL & "        , '" & CF_Ora_String(.DKBFLC, 1) & "' " 'ダミーフラグ３
			strSQL = strSQL & "        , '" & CF_Ora_String(.LSTID, 7) & "' " '伝票種別
			strSQL = strSQL & "        , '" & CF_Ora_String(.HINZEIKB, 1) & "' " '商品消費税区分
			strSQL = strSQL & "        , '" & CF_Ora_String(.HINMSTKB, 1) & "' " 'マスタ区分(商品)
			strSQL = strSQL & "        , '" & CF_Ora_String(.TOKMSTKB, 1) & "' " 'マスタ区分(得意先)
			strSQL = strSQL & "        , '" & CF_Ora_String(.NHSMSTKB, 1) & "' " 'マスタ区分(納入先)
			strSQL = strSQL & "        , '" & CF_Ora_String(.TANMSTKB, 1) & "' " 'マスタ区分(担当者)
			strSQL = strSQL & "        , '" & CF_Ora_String(.ZEIRNKKB, 1) & "' " '消費税ランク
			strSQL = strSQL & "        , '" & CF_Ora_String(.HINKB, 1) & "' " '商品区分
			strSQL = strSQL & "        ,  " & CStr(.ZEIRT) '消費税率
			strSQL = strSQL & "        , '" & CF_Ora_String(.ZAIKB, 1) & "' " '在庫管理区分
			strSQL = strSQL & "        , '" & CF_Ora_String(.MRPKB, 1) & "' " '展開区分
			strSQL = strSQL & "        , '" & CF_Ora_String(.HINJUNKB, 1) & "' " '順位表出力区分
			strSQL = strSQL & "        , '" & CF_Ora_String(.MAKCD, 6) & "' " 'メーカーコード
			strSQL = strSQL & "        , '" & CF_Ora_String(.HINSIRCD, 10) & "' " '商品仕入先コード
			strSQL = strSQL & "        , '" & CF_Ora_String(.HINNMMKB, 1) & "' " '名称ﾏﾆｭｱﾙ区分(商品)
			strSQL = strSQL & "        , '" & CF_Ora_String(.HRTDD, 2) & "' " '発注リードタイム
			strSQL = strSQL & "        , '" & CF_Ora_String(.ORTDD, 2) & "' " '出荷リードタイム
			strSQL = strSQL & "        ,  " & CStr(.ZNKURIKN) '税抜課税対象額
			strSQL = strSQL & "        ,  " & CStr(.ZKMURIKN) '税込課税対象額
			strSQL = strSQL & "        ,  " & CStr(.ZKMUZEKN) '税込消費税
			strSQL = strSQL & "        , '" & CF_Ora_String(.MOTDATNO, 10) & "' " '元伝票管理番号
			strSQL = strSQL & "        , '" & CF_Ora_String(.FOPEID, 8) & "' " '初回登録ユーザID
			strSQL = strSQL & "        , '" & CF_Ora_String(.FCLTID, 5) & "' " '初回登録クライアントID
			strSQL = strSQL & "        , '" & CF_Ora_String(.WRTFSTTM, 6) & "' " 'タイムスタンプ（登録時間）
...
F_UDNTRA_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_UDNTRA_Insert")
	...
	Private Function F_UPDSMF(ByRef pm_All As Cls_All, ByVal pin_intRow As Short, ByVal pin_intSMFKB As Short, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA) As Short
		...
		If URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_NML And Trim(pin_Tbl_Inf_UDNTRA.DFLDKBCD) <> "3" Then
			'請求サマリ更新
		...
		If URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_MAE And Trim(pin_Tbl_Inf_UDNTRA.DFLDKBCD) <> "3" Then
		...
		'    If URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_NML And URKET51_HEAD_Inf.TOKMTA.FRNKB = gc_strFRNKB_FRN Then
		If URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_NML And URKET51_HEAD_Inf.TOKMTA.FRNKB = gc_strFRNKB_FRN And Trim(pin_Tbl_Inf_UDNTRA.DFLDKBCD) <> "3" Then
		...
		'更新条件：入金区分＝２：前受入金
		If (URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_NML And Trim(pin_Tbl_Inf_UDNTRA.DFLDKBCD) <> "2") Or URKET51_HEAD_Inf.NYUKB = gc_strMAEUKKB_MAE Then
			'売掛サマリ請求の更新
...
F_UPDSMF_err: 
		'Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_UPDSMF")
		GoTo F_UPDSMF_end
	...
	Private Function F_TOKSSA(ByRef pm_All As Cls_All, ByVal pin_intRow As Short, ByVal pin_intSMFKB As Short, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA) As Short
		...
		'2009/06/15 CHG START FKS)NAKATA
		'意図不明のコメントアウト解除(↓)
		'ランク別処理
		...
		End If
		'意図不明のコメントアウト解除(↑)
		'2009/06/15 CHG E.N.D FKS)NAKATA
...
F_TOKSSA_err: 
		'Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSA")
		GoTo F_TOKSSA_end
	...
	Private Function F_TOKSSA_Update(ByRef pm_All As Cls_All, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA, ByRef pin_curSSAURIKN() As Decimal, ByVal pin_curSSAUZEKN As Decimal, ByRef pin_curSSANYUKN() As Decimal, ByVal pin_curKSKZANKN As Decimal, ByRef pot_lngRowCnt As Integer) As Short
        ...
        '2019/04/10 ADD START
        strSQL = ""
        strSQL = strSQL & " SELECT * FROM TOKSSA "
...
F_TOKSSA_Update_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSA_Update")
...
F_TOKSSA_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSA_Insert")
...
F_TOKSSA_UpdateRANK_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSA_UpdateRANK")
...
F_TOKSSA_UpdateDENSU_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSA_UpdateDENSU")
...
F_TOKSSB_err: 
		'Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSB")
		GoTo F_TOKSSB_end
	...
	Private Function F_TOKSSB_Update(ByRef pm_All As Cls_All, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA, ByRef pin_curSSAURIKN() As Decimal, ByVal pin_curSSAUZEKN As Decimal, ByRef pin_curSSANYUKN() As Decimal, ByVal pin_curKSKZANKN As Decimal, ByRef pot_lngRowCnt As Integer) As Short
        ...
        '2019/04/10 ADD START
        strSQL = ""
        strSQL = strSQL & " SELECT * FROM TOKSSB "
        strSQL = strSQL & "  WHERE TOKCD = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.TOKCD, 10) & "' " '得意先コード
        strSQL = strSQL & "    AND SSADT = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.SSADT, 8) & "' " '締日付
...
F_TOKSSB_Update_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSB_Update")
...
F_TOKSSB_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSB_Insert")
...
F_TOKSSB_UpdateRANK_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSB_UpdateRANK")
...
F_TOKSSB_UpdateDENSU_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSB_UpdateDensu")
	...
	Private Function F_TOKSSC(ByRef pm_All As Cls_All, ByVal pin_intRow As Short, ByVal pin_intSMFKB As Short, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA) As Short
		...
		dblSSCURIKN(8) = 0
		dblSSCURIKN(9) = 0
		dblSSCURIKN(CInt(pin_Tbl_Inf_UDNTRA.UPDID)) = dblSSCURIKN(CInt(pin_Tbl_Inf_UDNTRA.UPDID)) + pin_Tbl_Inf_UDNTRA.URIKN * pin_intSMFKB
...
F_TOKSSC_err: 
		'Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSC")
		GoTo F_TOKSSC_end
	...
	Private Function F_TOKSSC_Update(ByRef pm_All As Cls_All, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA, ByRef pin_dblSSCURIKN() As Double, ByVal pin_dblSSCUZEKN As Double, ByRef pin_dblSSCNYUKN() As Double, ByVal pin_dblFKSZANKN As Double, ByRef pot_lngRowCnt As Integer) As Short
        ...
        '2019/04/10 ADD START
        strSQL = ""
        strSQL = strSQL & " SELECT * FROM TOKSSC "
        strSQL = strSQL & "  WHERE TOKCD = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.TOKCD, 10) & "' " '得意先コード
...
F_TOKSSC_Update_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSC_Update")
...
F_TOKSSC_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSC_Insert")
...
F_TOKSSC_UpdateRANK_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSC_UpdateRANK")
...
F_TOKSSC_UpdateDENSU_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSSC_UpdateDENSU")
...
F_TOKSME_err: 
		'Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSME")
		GoTo F_TOKSME_end
	...
	Private Function F_TOKSME_Update(ByRef pm_All As Cls_All, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA, ByRef pin_curSMAURIKN() As Decimal, ByVal pin_curSMAUZEKN As Decimal, ByRef pin_curSMAGNKKN() As Decimal, ByRef pin_curSMANYUKN() As Decimal, ByRef pot_lngRowCnt As Integer) As Short
        ...
        '2019/04/10 ADD START
        strSQL = ""
        strSQL = strSQL & " SELECT * FROM TOKSME "
        strSQL = strSQL & "  WHERE TOKCD = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.TOKCD, 10) & "' " '得意先コード
...
F_TOKSME_Update_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSME_Update")
...
F_TOKSME_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSME_Insert")
...
F_TOKSME_UpdateRANK_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_TOKSME_UpdateRANK")
...
F_UTGTRA_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_UTGTRA_Insert")
...
F_NKSSMA_err: 
		'Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_NKSSMA")
		GoTo F_NKSSMA_end
	...
	Private Function F_NKSSMA_Update(ByRef pm_All As Cls_All, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA, ByRef pin_curSSANYUKN() As Decimal, ByRef pot_lngRowCnt As Integer) As Short
        ...
        '2019/04/10 ADD START
        strSQL = ""
        strSQL = strSQL & " SELECT * FROM NKSSMA "
        strSQL = strSQL & "  WHERE TOKCD = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.TOKCD, 10) & "' " '得意先コード
        strSQL = strSQL & "    AND SMADT = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.SMADT, 8) & "' " '経理締日付
...
F_NKSSMA_Update_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_NKSSMA_Update")
...
F_NKSSMA_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_NKSSMA_Insert")
...
F_NKSSMB_err: 
		'Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_NKSSMB")
		GoTo F_NKSSMB_end
	...
	Private Function F_NKSSMB_Update(ByRef pm_All As Cls_All, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA, ByRef pin_curSSANYUKN() As Decimal, ByRef pot_lngRowCnt As Integer) As Short
        ...
        '2019/04/10 ADD START
        strSQL = ""
        strSQL = strSQL & " SELECT * FROM NKSSMB "
        strSQL = strSQL & "  WHERE TOKCD = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.TOKCD, 10) & "' " '得意先コード
        strSQL = strSQL & "    AND SMADT = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.SMADT, 8) & "' " '経理締日付
...
F_NKSSMB_Update_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_NKSSMB_Update")
...
F_NKSSMB_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_NKSSMB_Insert")
...
F_NKSSMC_err: 
		'Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_NKSSMC")
		GoTo F_NKSSMC_end
	...
	Private Function F_NKSSMC_Update(ByRef pm_All As Cls_All, ByRef pin_Tbl_Inf_UDNTRA As TYPE_DB_UDNTRA, ByRef pin_curSSANYUKN() As Decimal, ByRef pot_lngRowCnt As Integer) As Short
        ...
        '2019/04/10 ADD START
        strSQL = ""
        strSQL = strSQL & " SELECT * FROM NKSSMC "
        strSQL = strSQL & "  WHERE TOKCD = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.TOKCD, 10) & "' " '得意先コード
        strSQL = strSQL & "    AND TUKKB = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.TUKKB, 3) & "' " '通貨区分
        strSQL = strSQL & "    AND SMADT = '" & CF_Ora_String(pin_Tbl_Inf_UDNTRA.SMADT, 8) & "' " '経理締日付
...
F_NKSSMC_Update_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_NKSSMC_Update")
...
F_NKSSMC_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_NKSSMC_Insert")
		GoTo F_NKSSMC_Insert_end
		
	End Function
	
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_FBTRA_Update_DelF
	'   概要：  ＦＢトラン更新
	'   引数：
	'   戻値：  0:正常   1:データ無し  9:異常
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_FBTRA_Update_DelF(ByRef pm_All As Cls_All) As Short
		Dim strSQL As String
		Dim bolRet As Boolean
		
		On Error GoTo F_FBTRA_Update_DelF_err
		
		F_FBTRA_Update_DelF = 9
		
		'SQL
		strSQL = ""
		strSQL = strSQL & " UPDATE FBTRA "
		...
		strSQL = strSQL & "      , WRTFSTTM = '" & CF_Ora_String(GV_SysTime, 6) & "' " 'タイムスタンプ（登録時間）
		strSQL = strSQL & "      , WRTFSTDT = '" & CF_Ora_String(GV_SysDate, 8) & "' " 'タイムスタンプ（登録日）
		strSQL = strSQL & "  WHERE FBRFNO = '" & CF_Ora_String(URKET51_HEAD_Inf.FBRFNO, 6) & "' " '照会番号
		
        'SQL実行
        '2019/04/02 CHG START
        'bolRet = CF_Ora_Execute(gv_Odb_USR1, strSQL)
        'If bolRet = False Then
        '	GoTo F_FBTRA_Update_DelF_err
        'End If
        Call DB_Execute(strSQL)
        '2019/04/02 CHG E N D

		F_FBTRA_Update_DelF = 0
		
F_FBTRA_Update_DelF_end: 
		Exit Function
		
F_FBTRA_Update_DelF_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_FBTRA_Update_DelF")
		GoTo F_FBTRA_Update_DelF_end
		
	End Function
	
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_FBTRA_Exicz
	'   概要：  ＦＢトラン排他制御
	'   引数：
	'   戻値：  0:正常   1:データ無し  9:異常
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_FBTRA_Exicz() As Short
		'UPGRADE_WARNING: 構造体 Usr_Ody の配列は、使用する前に初期化する必要があります。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="814DF224-76BD-4BB4-BFFB-EA359CB9FC48"' をクリックしてください。
		Dim Usr_Ody As U_Ody
		Dim strSQL As String
        '2019/04/02 ADD START
        Dim dt As DataTable = New DataTable
        '2019/04/02 ADD E N D
		On Error GoTo F_FBTRA_Exicz_err
		
		F_FBTRA_Exicz = 9
		
		'SQL
		strSQL = ""
		strSQL = strSQL & " SELECT OPEID " '最終作業者コード
		strSQL = strSQL & "      , CLTID " 'クライアントＩＤ
		strSQL = strSQL & "      , WRTTM " 'タイムスタンプ（時間）
		strSQL = strSQL & "      , WRTDT " 'タイムスタンプ（日付）
		strSQL = strSQL & "      , WRTFSTTM " 'タイムスタンプ（登録時間）
		...
		Else
            ...
            '2019/04/02 CHG START
            'If URKET51_HEAD_Inf.FBTRA.OPEID <> CF_Ora_GetDyn(Usr_Ody, "OPEID", "") Or URKET51_HEAD_Inf.FBTRA.CLTID <> CF_Ora_GetDyn(Usr_Ody, "CLTID", "") Or URKET51_HEAD_Inf.FBTRA.WRTTM <> CF_Ora_GetDyn(Usr_Ody, "WRTTM", "") Or URKET51_HEAD_Inf.FBTRA.WRTDT <> CF_Ora_GetDyn(Usr_Ody, "WRTDT", "") Or URKET51_HEAD_Inf.FBTRA.WRTFSTTM <> CF_Ora_GetDyn(Usr_Ody, "WRTFSTTM", "") Or URKET51_HEAD_Inf.FBTRA.WRTFSTDT <> CF_Ora_GetDyn(Usr_Ody, "WRTFSTDT", "") Then
            If URKET51_HEAD_Inf.FBTRA.OPEID <> DB_NullReplace(dt.Rows(0)("OPEID"), "") Or URKET51_HEAD_Inf.FBTRA.CLTID <> DB_NullReplace(dt.Rows(0)("CLTID"), "") Or URKET51_HEAD_Inf.FBTRA.WRTTM <> DB_NullReplace(dt.Rows(0)("WRTTM"), "") Or URKET51_HEAD_Inf.FBTRA.WRTDT <> DB_NullReplace(dt.Rows(0)("WRTDT"), "") Or URKET51_HEAD_Inf.FBTRA.WRTFSTTM <> DB_NullReplace(dt.Rows(0)("WRTFSTTM"), "") Or URKET51_HEAD_Inf.FBTRA.WRTFSTDT <> DB_NullReplace(dt.Rows(0)("WRTFSTDT"), "") Then
                '2019/04/02 CHG E N D
                GoTo F_FBTRA_Exicz_end
            End If
        End If
...
F_FBTRA_Exicz_err: 
	...
	Public Function F_Set_Body_Enable(ByRef pm_All As Cls_All) As Short
		...
		'【通貨】
		Wk_Index = CShort(FR_SSSMAIN.HD_TUKKB.Tag)
		If URKET51_HEAD_Inf.TOKMTA.FRNKB = gc_strFRNKB_FRN Then
			'海外
			Call CF_Set_Item_Focus_Ctl(True, pm_All.Dsp_Sub_Inf(Wk_Index))
		Else
			'国内か、エラー
			Call CF_Set_Item_Focus_Ctl(False, pm_All.Dsp_Sub_Inf(Wk_Index))
		End If
	...
	Public Function F_Ctl_MN_APPENDC_Click(ByRef pm_All As Cls_All) As Short
		...
		'画面変更なしとする
		gv_bolURKET51_INIT = False
		gv_bolURKET51_LF_Enable = True
		' === 20110517 === INSERT S TOM)Morimoto
		If strCallMode = SUB_MODE Then
			Call set_DSP(Left(Link_IN.HINCD, 10), pm_All)
			strCallMode = SUB_MODE
			Call CF_Set_Item_SetFocus(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NYUKB.Tag)), pm_All)
		End If
		' === 20110517 === INSERT E
	End Function
	...
	Public Function F_GET_SYSTBA() As Short
        ...
        Dim dt As DataTable = DB_GetTable(strSQL)
        If dt Is Nothing OrElse dt.Rows.Count > 0 Then
            pv_strYERUPDDT = DB_NullReplace(dt.Rows(0)("YERUPDDT"), "")
            '月次本締日の条件撤廃
            pv_strSMAUPDDT = DB_NullReplace(dt.Rows(0)("SMAUPDDT"), "")

            pv_strSMADD = DB_NullReplace(dt.Rows(0)("SMADD"), "")
        End If
        '2019/04/02 CHG E N D
...
F_GET_SYSTBA_Err:
	...
	Public Function CF_Ctl_Item_LostFocus_Dummy(ByRef pm_All As Cls_All) As Short
		...
		If gv_bolURKET51_LF_Enable = False Then
			Exit Function
		End If
...
F_Chk_HAITA_JDNNO_End: 
	...
	Public Function F_Chk_UODKN_JDNNO(ByRef pm_All As Cls_All) As Short
		...
		Dim curUODKN As Decimal
		'*** 2009/09/07 ADD START FKS)NAKATA
		Dim curBefNYUKN As Decimal '過去の入金額
		...
		For intCnt = 1 To pv_intMeisaiCnt
			...
			'*** 2009/09/23 ADD START FKS)NAKATA
			'本入金はチェックを行わない
			If strDKBID <> "09" Then
				'*** 2009/09/23 ADD E.N.D FKS)NAKATA
				...
				For intCnt2 = 1 To pv_intMeisaiCnt
					...
					If intCnt <> intCnt2 Then
						...
						'*** 2009/09/23 CHG START FKS)NAKATA
						'本入金はチェックを行わない
						...
						If Trim(strJDNNO) = Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt2).Bus_Inf.JDNNO) And Trim(strJDNLINNO) = Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt2).Bus_Inf.JDNLINNO) And Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt2).Bus_Inf.DKBID) <> "09" Then
							...
							'*** 2009/09/23 CHG E.N.D FKS)NAKATA
				...
				If strJDNNO <> "" Or strJDNLINNO <> "" Then
					...
					'
					'        Msg_Flg = True
					'        Err_Cd = gc_strMsgURKET51_E_028  '受注金額を上回っています。
					...
					'
					'        Msg_Flg = True
					'        Err_Cd = gc_strMsgURKET51_E_029  '受注金額を下回っています。
					'        GoTo F_Chk_UODKN_JDNNO_End
					'
					'    End If
					
					'受注金額の取得
					curUODKN = F_Util_Get_UODKN(strJDNNO, strJDNLINNO, curBefNYUKN, strDKBID)
					
					'入力された金額と過去の入金額が、ゼロ以外の場合チェックする。(逆仕訳はエラー対象外)
					
					If curNYUKN + curBefNYUKN <> 0 Then
					...
					Else
						...
						'逆仕訳の場合は、消込済みチェック
						If F_Chk_KESIZUMI(strJDNNO, strJDNLINNO) <> 0 Then
							Msg_Flg = True
							Err_Cd = gc_strMsgURKET51_E_032 '充当済みです。更新できません。
							GoTo F_Chk_UODKN_JDNNO_End
						End If
					End If
...
F_Chk_UODKN_JDNNO_End: 
	...
	Function F_Chk_KESIZUMI(ByRef pm_JDNNO As Object, ByRef pm_JDNLINNO As Object) As Short
		...
		strSQL = strSQL & "      , (SELECT MAX(DATNO) AS MAX_DATNO "
		strSQL = strSQL & "           FROM JDNTHA "
		'UPGRADE_WARNING: オブジェクト pm_JDNNO の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSQL = strSQL & "          WHERE JDNNO = '" & pm_JDNNO & "'"
        ...
        strSQL = strSQL & "  AND    AKAKROKB     = '1'"
        'UPGRADE_WARNING: オブジェクト pm_JDNNO の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        strSQL = strSQL & "  AND    JDNNO     = '" & pm_JDNNO & "'"

        'システム・セットアップは伝票単位にて確認
        If strJDNTRKB = "11" Or strJDNTRKB = "21" Then
        Else
            'UPGRADE_WARNING: オブジェクト pm_JDNLINNO の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            strSQL = strSQL & "  AND   JDNLINNO  = '" & pm_JDNLINNO & "'"
        End If
        ...
        'If CF_Ora_EOF(Usr_Ody) = False Then
        ''UPGRADE_WARNING: オブジェクト SSSVal() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        'Cnt = SSSVal(CF_Ora_GetDyn(Usr_Ody, "CNT", ""))

        If dt Is Nothing OrElse dt.Rows.Count > 0 Then
            Cnt = SSSVal(DB_NullReplace(dt.Rows(0)("CNT"), ""))
            '2019/04/02 CHG E N D
        Else
...
ERR_F_Util_GET_TANMTA_TANCLAKB:
	...
	'2009/10/07 UPD START RISE)MIYAJIMA
	'Function F_GET_MaeukeTEGDT(pm_All As Cls_All, pmstrOKRJONO As String) As String
	Function F_GET_MaeukeTEGDT(ByRef pm_All As Cls_All, ByRef pmstrOKRJONO As String, ByRef pstrTEGDT As String) As String
		...
		For intCnt = 1 To pv_intMeisaiCnt
			...
			If strDKBID = pc_strDKBID_URK_HURIK Then
				'送り状№の格納
				strOKRJONO = Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.JDNNO) & Trim(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf.JDNLINNO)
				'受注番号を比較（同一のものを探す）
		...
		If strMAXTEGDT = "" Then
			...
			Else
				strMAXTEGDT = Trim(URKET51_HEAD_Inf.NYUDT)
				'2009/10/07 UPD E.N.D RISE)MIYAJIMA
			End If
		End If
...
F_GET_MaeukeTEGDT_err: 
	...
	Private Function F_Util_CheckSumOver(ByRef pm_All As Cls_All, ByRef pm_Mode As Short) As Short
		...
		'        '訂正時
		'        If pm_Mode = 1 Then
		'            If curAryCHGKN(I) < 0 Then
		'                If gc_NKSSMX_Inf.curZAN(I) + curAryCHGKN(I) < 0 Then
		'                    F_Util_CheckSumOver = 2
		'                    Exit Function
		'                End If
		'            End If
		...
		'        'チェック
		'        If curCHGKN < 0 Then
		'            If Abs(curCHGKN) > Abs(curZANKN) Then
		'                F_Util_CheckSumOver = 2
		'                Exit Function
		'            End If
		'        End If
...
F_Get_JDN_HAITA_err: 
	...
	Private Function F_Get_JDN_HAITA_Inf(ByVal pin_strJDNNO As String) As Boolean
        ...
        For j As Integer = 0 To dt.Rows.Count - 1
            '伝票管理NOの取得
            strDATNO = DB_NullReplace(dt.Rows(j)("DATNO"), "")
            strLINNO = DB_NullReplace(dt.Rows(j)("LINNO"), "")
            ...
            If intJDNTRAIndex = 0 Then
                ...
                With gc_JDNTRA_HAITA_Inf(intJDNTRAIndex)
                    ...
                    .DATNO = DB_NullReplace(dt.Rows(j)("DATNO"), "")

                    .JDNNO = DB_NullReplace(dt.Rows(j)("JDNNO"), "")

                    .LINNO = DB_NullReplace(dt.Rows(j)("LINNO"), "")

                    .FOPEID = DB_NullReplace(dt.Rows(j)("FOPEID"), "")

                    .FCLTID = DB_NullReplace(dt.Rows(j)("FCLTID"), "")

                    .WRTFSTTM = DB_NullReplace(dt.Rows(j)("WRTFSTTM"), "")

                    .WRTFSTDT = DB_NullReplace(dt.Rows(j)("WRTFSTDT"), "")

                    .OPEID = DB_NullReplace(dt.Rows(j)("OPEID"), "")

                    .CLTID = DB_NullReplace(dt.Rows(j)("CLTID"), "")

                    .WRTTM = DB_NullReplace(dt.Rows(j)("WRTTM"), "")

                    .WRTDT = DB_NullReplace(dt.Rows(j)("WRTDT"), "")

                    .UOPEID = DB_NullReplace(dt.Rows(j)("UOPEID"), "")

                    .UCLTID = DB_NullReplace(dt.Rows(j)("UCLTID"), "")

                    .UWRTTM = DB_NullReplace(dt.Rows(j)("UWRTTM"), "")

                    .UWRTDT = DB_NullReplace(dt.Rows(j)("UWRTDT"), "")
                End With
            End If
...
F_JDNTHA_Upd_TimeStamp_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_JDNTHA_Upd_TimeStamp")
...
F_JDNTRA_Upd_TimeStamp_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgURKET51_E_004, pm_All, "F_JDNTRA_Upd_TimeStamp")
	...
	Public Function F_Chk_NYUDT_JDNDT(ByRef pm_All As Cls_All) As Short
		...
		For intCnt = 1 To pv_intMeisaiCnt
			...
			If strJDNNO <> "" Or strJDNLINNO <> "" Then
				intRet = F_Util_CheckJDNNO(strJDNNO, strJDNLINNO, URKET51_HEAD_Inf.NYUDT)
				If intRet <> 0 Then
					Retn_Code = CHK_ERR_ELSE
					Select Case intRet
						Case 1
...
F_Chk_NYUDT_JDNDT_End: 
	...
	'=== 20110509 === INSERT S TOM)Morimoto
	'コール元からの伝票管理番号より情報を表示する。
	Public Sub set_DSP(ByVal strDATNO As String, ByRef pm_All As Cls_All)
		Dim Trg_Index As Short
		Dim Dsp_Value As Object
		Dim Move_Flg As Boolean
		Dim Rtn_Chk As Short
		...
		Dim Chk_Move_Flg As Boolean
		Dim Next_Focus As Short
		Dim strFBRFNO As String
		Dim DB_FBTRB As TYPE_DB_FBTRB
		Dim Mst_Inf_FBTRA As TYPE_DB_FBTRA
		Dim Mst_Inf_TOKMTA As TYPE_DB_TOKMTA
		Dim Mst_Inf_BNKMTA As TYPE_DB_BNKMTA
		Dim strKOZNO As String
		Dim Retn_Code As Short
		Dim Err_Cd As String
		Dim pm_Mode As Short
		Dim pm_Chk_Move_Flg As Boolean
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(FR_SSSMAIN.HD_FBRFNO.Tag)
		Next_Focus = Trg_Index + 1
		'伝票管理番号を取得する。
		DB_FBTRB_Clear(DB_FBTRB)
        DSPFBTRB_SEARCH(strDATNO, DB_FBTRB)
        '2019/06/27 CHG START
        'DB_FBTRA_Clear(Mst_Inf_FBTRA)
        Mst_Inf_FBTRA = New TYPE_DB_FBTRA
        '2019/06/27 CHG E N D
        '画面に編集
        'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        Dsp_Value = CF_Cnv_Dsp_Item(DB_FBTRB.FBRFNO, pm_All.Dsp_Sub_Inf(Trg_Index), False)
		Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
		With Mst_Inf_FBTRA
			.DATKB = DB_FBTRB.DATKB
			.FBRFNO = DB_FBTRB.FBRFNO
			.FBKSDT = DB_FBTRB.FBKSDT
			.FBNYUKN = DB_FBTRB.FBNYUKN
			.FBCLTCD = DB_FBTRB.FBCLTCD
			.FBCLTNM = DB_FBTRB.FBCLTNM
			.FBDELKB = DB_FBTRB.FBDELKB
			.FBSSDT = DB_FBTRB.FBSSDT
			.FBKJJDT = DB_FBTRB.FBKJJDT
			.FBKJIDT = DB_FBTRB.FBKJIDT
			.FBBNKCD = DB_FBTRB.FBBNKCD
			.FBBNKNK = DB_FBTRB.FBBNKNK
			.FBSTNNK = DB_FBTRB.FBSTNNK
			.FBYKNKB = DB_FBTRB.FBYKNKB
			.FBKOZNO = DB_FBTRB.FBKOZNO
			.FBKOZNM = DB_FBTRB.FBKOZNM
			.FBKJDT = DB_FBTRB.FBKJDT
		End With
		'バーチャル口座（＝振込依頼人コード）(FBCLTCD)で、得意マスタを見る
		strKOZNO = Left(Mst_Inf_FBTRA.FBCLTCD, 7)
		...
		'マスタチェック
		'    If DSPTOKMTA_KOZNO_SEARCH(strKOZNO, Mst_Inf_TOKMTA) = 0 Then
		DSPTOKMTA_KOZNO_SEARCH(strKOZNO, Mst_Inf_TOKMTA)
		'取得項目格納
		If Trim(strKOZNO) <> "" Then
			URKET51_HEAD_Inf.TOKCD = Mst_Inf_TOKMTA.TOKCD
			'UPGRADE_WARNING: オブジェクト URKET51_HEAD_Inf.TOKMTA の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			URKET51_HEAD_Inf.TOKMTA = Mst_Inf_TOKMTA
		End If
		...
		Dsp_Value = CF_Cnv_Dsp_Item(URKET51_HEAD_Inf.NYUDT, pm_All.Dsp_Sub_Inf(Trg_Index), False)
		Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All, SET_FLG_DB_ERR)
		If F_Chk_HD_NYUDT(pm_All.Dsp_Sub_Inf(Trg_Index), pm_Chk_Move_Flg, pm_All) <> CHK_OK Then
			URKET51_HEAD_Inf.NYUDT = GV_UNYDate
			'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			Dsp_Value = CF_Cnv_Dsp_Item(URKET51_HEAD_Inf.NYUDT, pm_All.Dsp_Sub_Inf(Trg_Index), False)
		...
		With pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf '１行目のデータ
			...
			Select Case .DKBID
				Case pc_strDKBID_URK_HURI, pc_strDKBID_URK_TEG
					...
					End If
			End Select
			.NYUKN = DB_FBTRB.FBNYUKN
		End With
		
set_DSP_End: 
		pm_All.Dsp_Base.LostFocus_Flg = True
		
		For Trg_Index = 1 To pm_All.Dsp_Base.Head_Lst_Idx
			Call F_Dsp_Item_Detail(pm_All.Dsp_Sub_Inf(Trg_Index), DSP_SET, pm_All)
		Next 
		'    If Trim(FR_SSSMAIN.HD_TOKCD.Text) = vbNullString Then
		'        Call CF_Set_Item_Focus_Ctl(True, pm_All.Dsp_Sub_Inf(FR_SSSMAIN.HD_TOKCD.Tag))
		'    Else
		'        Call CF_Set_Item_Focus_Ctl(False, pm_All.Dsp_Sub_Inf(FR_SSSMAIN.HD_TOKCD.Tag))
		'    End If
		If Chk_Move_Flg = True Then
			'ﾁｪｯｸ後移動あり
			Call SSSMAIN0001.F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRETURN, Move_Flg, pm_All)
