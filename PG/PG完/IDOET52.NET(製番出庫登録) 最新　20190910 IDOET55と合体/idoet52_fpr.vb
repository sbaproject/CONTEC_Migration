...
Module SSSMAIN0001
    ...
    'Public WLSTAN_RTNCODE As String
    ...
    'delete 20190822 start hou
    ''add 20190731 START hou
    ''受注情報検索戻り値
    ...
    ''add 20190731 END hou
    'delete 20190822 end hou
	...
	' === 20080815 === INSERT S - RISE)Izumi
	'シリアル№登録ワーク
	Structure M_TYPE_SRAET61_MOTO
	...
	' === 20080815 === INSERT E - RISE)Izumi
	
	' === 20080821 === INSERT S - RISE)Izumi
	'倉庫別在庫マスタ（キー、引当数）　退避用
	Structure M_TYPE_HINMTB_MOTO
		'UPGRADE_WARNING: 固定長文字列のサイズはバッファに合わせる必要があります。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="3C1E4426-0B80-443E-B943-0627CD55D48B"' をクリックしてください。
		...
		'UPGRADE_WARNING: 固定長文字列のサイズはバッファに合わせる必要があります。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="3C1E4426-0B80-443E-B943-0627CD55D48B"' をクリックしてください。
		<VBFixedString(10),System.Runtime.InteropServices.MarshalAs(System.Runtime.InteropServices.UnmanagedType.ByValArray,SizeConst:=10)> Public HIKSU() As Char ' 引当数
	End Structure
	
	Public M_HINMTB_MOTO_Inf As M_TYPE_HINMTB_MOTO
	' === 20080821 === INSERT E - RISE)Izumi
	...
	'ADD 20141119 START
	Public Const FAC_CD_GENKA As String = "CONTEC" ' 原価の工場番号
	Public Execute_Flg As Short ' 登録ボタン実行フラグ
	'ADD 20141119 END
	...
	Public Function SBNNO_Saiban(ByRef pm_All As Cls_All, ByRef Pit_strBUMCD As String, ByRef Pit_strCLMDL As String, ByRef Pit_strOUTYTDT As String, ByRef Pot_strDENNO As String) As Short
		...
		'''' ADD 2014/03/19  FWEST) Koroyasu    Start    連絡票№HAN20140220-01
		Static strJDNDT As String
		'''' ADD 2014/03/19  FWEST) Koroyasu    End
		...
		'受注日取得
		strSQL = "SELECT TRA.JDNDT"
		strSQL = strSQL & " FROM JDNTRA TRA, JDNTHC THC "
		...
		strSQL = strSQL & "  AND TRA.JDNNO = " & "'" & Left(Trim(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_JDNNO.Tag)).Detail.Dsp_Value), 6) & "'"
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSQL = strSQL & "  AND TRA.LINNO = " & "'" & "0" & Right(Trim(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_JDNNO.Tag)).Detail.Dsp_Value), 2) & "'"

        'SQL実行
        'change starat 20190909 kuwa
        'bolRet = CF_Ora_CreateDyn(gv_Odb_USR1, usrOdy, strSQL)
        'If bolRet = False Then
        Dim dt As DataTable = DB_GetTable(strSQL)
        If dt Is Nothing OrElse dt.Rows.Count > 0 Then
            'change end 20190909 kuwa
            GoTo ERR_SBNNO_Saiban
        End If

        '''' UPD 2014/04/23  FWEST) Koroyasu    Start    連絡票№HAN20140220-01
        '    If Trim$(CF_Ora_GetDyn(usrOdy, "JDNDT", "")) = "" Then
        '        GoTo ERR_SBNNO_Saiban
        '    Else
        '        strJDNDT = CF_Ora_GetDyn(usrOdy, "JDNDT", "")
        '    End If
        '
        '    'ユーザー情報管理テーブル検索
        ...
        '
        '    '当期開始日付取得
        '    If strJDNDT < Replace(Get_BGNAcedt(CInt(Mid(strDate, 1, 4)), CInt(Mid(strDate, 5, 2))), "/", "") Then
        '        '部門コード取得
        '        strSQL = "SELECT MEINMA"
        '        strSQL = strSQL & " FROM MEIMTC "
        '        strSQL = strSQL & "WHERE DATKB = '" & gc_strDATKB_USE & "' "
        '        strSQL = strSQL & "  AND KEYCD = '086' "
        '        strSQL = strSQL & "  AND MEICDA = " & "'" & Trim$(Pit_strBUMCD) & "'"
        ...
        '        End If
        '
        '        If Trim$(CF_Ora_GetDyn(usrOdy, "MEINMA", "")) = "" Then
        '            GoTo ERR_SBNNO_Saiban
        '        Else
        '            Pit_strBUMCD = CF_Ora_GetDyn(usrOdy, "MEINMA", "")
        '        End If
        '    End If
        ''''' ADD 2014/03/19  FWEST) Koroyasu    End
        '
        ...
        '        strW_JDNNO_2 = Mid(Trim$(pm_All.Dsp_Sub_Inf(FR_SSSMAIN.HD_JDNNO.Tag).Detail.Dsp_Value), 2, 1)
        ...
        ''''' ADD 2014/03/19  FWEST) Koroyasu    Start    連絡票№HAN20140220-01
        '        strSQL = strSQL & "  AND B.STTTKDT <= '" & CF_Ora_Date(strDate) & "' "
        '        strSQL = strSQL & "  AND B.ENDTKDT >= '" & CF_Ora_Date(strDate) & "' "
        ''''' ADD 2014/03/19  FWEST) Koroyasu    End
        '    Else
        '        strSQL = "SELECT MEIKBA"
        '        strSQL = strSQL & " FROM BMNMTA B "
        ...
        ''''' UPD 2012/01/18  FKS) T.Yamamoto    Start    連絡票№FC12011801
        ''取得できなかった場合は最小の地区コードの区分とする
        ...
        '        strSQL = strSQL & "WHERE KEYCD = '060' "
        '        strSQL = strSQL & "  AND MEICDA = (SELECT MIN(MEICDA) FROM MEIMTA WHERE KEYCD = '060') "
        '
        '        'SQL実行
        '        bolRet = CF_Ora_CreateDyn(gv_Odb_USR1, usrOdy, strSQL)
        '        If bolRet = False Then
        ...
        '        End If
        ''''' UPD 2012/01/18  FKS) T.Yamamoto    End
        '    Else
        '        Pot_strDENNO = Trim$(Pot_strDENNO) & CF_Ora_GetDyn(usrOdy, "MEIKBA", "")
        '    End If
        ''    Pot_strDENNO = Trim$(Pot_strDENNO) & CF_Ora_GetDyn(usrOdy, "MEIKBA", "")
        ...
        'change start 20190909 kuwa
        'If Trim(CF_Ora_GetDyn(usrOdy, "JDNDT", "")) = "" Then
        If Trim(DB_NullReplace(dt.Rows(0)("JDNDT"), "")) = "" Then
            'change end 20190909 kuwa
            '本製番以外の代替出荷の場合は、名称マスタ(MEIMTA KEYCD=「060」)のコード１が「99」を参照
            bolRet = CF_Ora_CloseDyn(usrOdy)
            If bolRet = False Then
                GoTo ERR_SBNNO_Saiban
            End If
            ...
            strSQL = strSQL & " FROM MEIMTA "
            strSQL = strSQL & "WHERE DATKB = '" & gc_strDATKB_USE & "' "
            strSQL = strSQL & "  AND KEYCD = '060' "
            strSQL = strSQL & "  AND MEICDA = '99' "
            ...
            'change start 20190909 kuwa
            'If Trim(CF_Ora_GetDyn(usrOdy, "MEIKBA", "")) = "" Then
            If Trim(DB_NullReplace(dt.Rows(0)("MEIKBA"), "")) = "" Then
                'change end 20190909 kuwa
                GoTo ERR_SBNNO_Saiban
            Else
                ...
                'change start 20190909 kuwa
                'Pot_strDENNO = Trim(Pot_strDENNO) & CF_Ora_GetDyn(usrOdy, "MEIKBA", "")
                Pot_strDENNO = Trim(Pot_strDENNO) & DB_NullReplace(dt.Rows(0)("MEIKBA"), "")
                'change end 20190909 kuwa
            End If
        Else
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'change start 20190909 kuwa
            'strJDNDT = CF_Ora_GetDyn(usrOdy, "JDNDT", "")
            strJDNDT = DB_NullReplace(dt.Rows(0)("JDNDT"), "")
            'change end 20190909 kuwa
			...
			'当期開始日付取得
			If strJDNDT < Replace(Get_BGNAcedt(CShort(Mid(strDate, 1, 4)), CShort(Mid(strDate, 5, 2))), "/", "") Then
				'部門コード取得
				strSQL = "SELECT MEINMA"
				strSQL = strSQL & " FROM MEIMTC "
				strSQL = strSQL & "WHERE DATKB = '" & gc_strDATKB_USE & "' "
                ...
                'change start 20190909 kuwa
                'If Trim(CF_Ora_GetDyn(usrOdy, "MEINMA", "")) = "" Then
                If Trim(DB_NullReplace(dt.Rows(0)("MEINMA"), "")) = "" Then
                    'change end 20190909 kuwa
                    GoTo ERR_SBNNO_Saiban
                Else
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'change start 20190909 kuwa
                    'Pit_strBUMCD = CF_Ora_GetDyn(usrOdy, "MEINMA", "")
                    Pit_strBUMCD = DB_NullReplace(dt.Rows(0)("MEINMA"), "")
                    'change end 20190909 kuwa
                End If
			End If
			'''' ADD 2014/03/19  FWEST) Koroyasu    End
			...
			Else
				...
				'MOD 20150330 START
				'            strSQL = strSQL & "     ,(SELECT * FROM MEIMTA WHERE KEYCD = '060') M "
				strSQL = strSQL & "     ,(SELECT * FROM MEIMTA WHERE DATKB = '" & gc_strDATKB_USE & "' AND KEYCD = '060') M "
				'MOD 20150330 END
				strSQL = strSQL & "WHERE B.BMNCD = '" & Pit_strBUMCD & "' "
				strSQL = strSQL & "  AND B.TIKKB = M.MEICDA "
				'''' ADD 2014/03/19  FWEST) Koroyasu    Start    連絡票№HAN20140220-01
                ...
                '取得できなかった場合、名称マスタ(MEIMTA KEYCD=「060」)のコード１が「99」を参照
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                'change start 20190909 kuwa
                'If Trim(CF_Ora_GetDyn(usrOdy, "MEIKBA", "")) = "" Then
                If Trim(DB_NullReplace(dt.Rows(0)("MEIKBA"), "")) = "" Then
			...
			End If
		End If
		'''' UPD 2014/04/23  FWEST) Koroyasu    End
		...
		If Trim(Pit_strBUMCD) = "" Then
            ...
            Pit_strCLMDL = DB_NullReplace(dt.Rows(0)("MDLCL"), "")
            'change end 20190909 kuwa
            bolRet = CF_Ora_CloseDyn(usrOdy)
        ...
        strPCOED = DB_NullReplace(dt.Rows(0)("PCODE"), "")
        'change end 20190909 kuwa
        bolRet = CF_Ora_CloseDyn(usrOdy)
		If bolRet = False Then
			GoTo ERR_SBNNO_Saiban
        ...
        strWPot_strDENNO = DB_NullReplace(dt.Rows(0)("HINGRPRM"), "")
        'change end 20190909 kuwa
        bolRet = CF_Ora_CloseDyn(usrOdy)
		If bolRet = False Then
			GoTo ERR_SBNNO_Saiban
        ...
        '''' DEL 2014/03/19  FWEST) Koroyasu    Start    連絡票№HAN20140220-01
        '        'タイムスタンプ決定
        '        strDate = ""
        '        strTime = ""
        '        If Trim(GV_SysTime) <> "" Then
        ...
        '        End If
        '''' DEL 2014/03/19  FWEST) Koroyasu    End
        ...
        Else
            ...
            strDenNo = DB_NullReplace(dt.Rows(0)("SDENNO"), "")
            'change end 20190909 kuwa
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'change start 20190909 kuwa
            'strFixCd = CF_Ora_GetDyn(usrOdy, "FIXCD", "")
            strFixCd = DB_NullReplace(dt.Rows(0)("FIXCD"), "")
            'change end 20190909 kuwa
			...
			strSQL = strSQL & ",    '" & Trim(Pot_strDENNO) & Trim(strWPot_strDENNO) & "'" '製番
			'MOD 20141222 END
...
F_Chk_HD_NHSFAX_End: 
	...
	Function F_JDNTRA_SBNNO_SEARCH(ByRef p_SBNNO As String) As Short
		...
		If Len(Trim(p_SBNNO)) = 8 Then
            ...
            'DBアクセス
            'change 20190823 start hou
            'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
            Dim dt As DataTable = DB_GetTable(strSQL)
            'change 20190823 end hou

            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'change start 20190909 kuwa
            'll_cnt = CF_Ora_GetDyn(Usr_Ody, "CNT", 0) 'カウント
            ll_cnt = DB_NullReplace(dt.Rows(0)("CNT"), "")
            If ll_cnt = 0 Then
				'取得データなし
		...
		ElseIf Len(Trim(p_SBNNO)) = 7 Then 
            ...
            ls_JDNTRKB = DB_NullReplace(dt.Rows(0)("JDNTRKB"), 0)
            'change end 20190909 kuwa
            Select Case Trim(ls_JDNTRKB)
				...
				Case Else
                    ...
                    ll_cnt2 = DB_NullReplace(dt.Rows(0)("CNT"), 0)
                    'change end 20190909 kuwa
                    If ll_cnt2 = 0 Then
						F_JDNTRA_SBNNO_SEARCH = 1
						GoTo END_F_JDNTRA_SBNNO_SEARCH
			...
			Else
				...
				Select Case pm_KeyAscii
					Case System.Windows.Forms.Keys.Back
                        ...
                        'add start 20190906 kuwa
                    Case System.Windows.Forms.Keys.Return
                        pm_Move_Flg = True
                        pm_KeyAscii = 0
                        'add end 20190906 kuwa
...
F_Set_Next_Focus_Skip01: 
					...
					Else
						...
						Else
							...
							If Act_SelLength = 0 Then
                                ...
                                '同じ項目で移動する場合に選択文字数は継続する
                                Wk_SelLength = 0
                                'add start 20190909 kuwa 2文字移動してしまう不具合を修正
                                Next_SelStart = Next_SelStart - 1
	...
	Public Function F_Init_Clr_Dsp(ByRef pm_Index As Short, ByRef pm_All As Cls_All) As Short
		...
		For Index_Wk = Wk_Index_S To Wk_Index_E
			'ADD START FKS)INABA 2008/10/30 ********************************************
			'入力者名が消える対応
			If Index_Wk = CShort(FR_SSSMAIN.HD_IN_TANCD.Tag) Or Index_Wk = CShort(FR_SSSMAIN.HD_IN_TANNM.Tag) Then
			Else
				'ADD  END  FKS)INABA 2008/10/30 ********************************************
				'共通初期化
				...
				Select Case Index_Wk
					Case CShort(FR_SSSMAIN.HD_DENDT.Tag)
						'出庫日
	...
	Public Function F_Chk_HD_DENDT(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			Else
				...
				'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				lb_KKOUT = CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(CShort(FR_SSSMAIN.TL_KKOUT.Tag)))
			...
			Else
				...
				If DSPSOUCD_SEARCH(Input_Value, Mst_Inf) = 0 Then
					...
					Else
						...
						' === 20080821 === INSERT S - RISE)Izumi 倉庫別在庫マスタの引当数を退避する
						
						strSQL = ""
						strSQL = strSQL & " SELECT HIKSU "
						strSQL = strSQL & "   FROM HINMTB "
						strSQL = strSQL & " WHERE DATKB = '1'"
						strSQL = strSQL & "   AND SOUCD = '" & IDOET52_SBNTRA_Inf.SOUCD & "'"
                        strSQL = strSQL & "   AND HINCD = '" & strHINCD & "'"
                        ...
                        If dt Is Nothing OrElse dt.Rows.Count > 0 Then
                            ...
                            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                            'change start 20190909 kuwa
                            'M_HINMTB_MOTO_Inf.HIKSU = Trim(CF_Ora_GetDyn(Usr_Ody, "HIKSU", ""))
                            M_HINMTB_MOTO_Inf.HIKSU = Trim(DB_NullReplace(dt.Rows(0)("HIKSU"), ""))
                            'change end 20190909 kuwa
                            M_HINMTB_MOTO_Inf.SOUCD = Trim(IDOET52_SBNTRA_Inf.SOUCD)
                            M_HINMTB_MOTO_Inf.HINCD = Trim(strHINCD)
                        Else
                            M_HINMTB_MOTO_Inf.HIKSU = ""
							M_HINMTB_MOTO_Inf.SOUCD = ""
							M_HINMTB_MOTO_Inf.HINCD = ""
						End If
						'クローズ
						Call CF_Ora_CloseDyn(Usr_Ody)
						' === 20080821 === INSERT E - RISE)Izumi
			...
			Else
				...
				If DSPMEIM_SEARCH("066", Input_Value, Mst_Inf) = 0 Then
					...
					Else
						...
						'2008/08/29 ADD START FKS)NAKATA
						''処理理由「50」番以上は、出荷戻りのため緊急出荷できないようにする。
						If IDOET52_SBNTRA_Inf.OUTRYKB3 = "9" Then
							FR_SSSMAIN.TL_KKOUT.Enabled = False
							FR_SSSMAIN.TL_KKOUT.CheckState = System.Windows.Forms.CheckState.Unchecked
						Else
							FR_SSSMAIN.TL_KKOUT.Enabled = True
						End If
						'2008/08/29 ADD E.N.D FKS)NAKATA
						...
						If IDOET52_SBNTRA_Inf.OUTRYKB1 = "1" Then ' 区分1='1' : 代替出荷 -> 参照受注番号必須
							...
							If RunMode = RUNMODE_IDOET52 Then
								...
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_OPT1.Tag)).Detail.Focus_Ctl = False
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_OPT1.Tag)).Detail.Locked = False
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_OPT2.Tag)).Detail.Focus_Ctl = False
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_OPT2.Tag)).Detail.Locked = False
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_OPT3.Tag)).Detail.Focus_Ctl = False
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_OPT3.Tag)).Detail.Locked = False
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_Cursol_Wk_2.Tag)).Detail.Focus_Ctl = False
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_Cursol_Wk_2.Tag)).Detail.Locked = False
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_Cursol_Wk_3.Tag)).Detail.Focus_Ctl = False
								pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_Cursol_Wk_3.Tag)).Detail.Locked = False
							End If
						Else
...
F_Chk_HD_BINCD_END: 
	...
	Public Function F_Chk_HD_SBNNO(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		'MOD 20150330 START
		'復活
		'DEL START FKS)INABA 2008/08/18 *****************************************************
		'チェック実行判定
		Rtn_Cd = F_Chk_Jge_Action(pm_Chk_Dsp_Sub_Inf, Retn_Code, Msg_Flg, pm_Chk_Move)
		If Rtn_Cd = CHK_STOP Then
			'中断の場合
		...
		Else
			...
			Else
				...
				If FR_SSSMAIN.HD_OPT1.Checked = True Then
					Retn_Code2 = F_Chk_Sbnno(Input_Value, VB6.Format(FR_SSSMAIN.HD_DENDT.Text, "YYYYMMDD")) ' 製番を製番マスタと照合する
					If Retn_Code2 = 0 Then
						'ＯＫ
						Retn_Code = CHK_OK
						pm_Chk_Move = True
					...
					ElseIf Retn_Code2 = 7 Then 
						If Trim(IDOET52_SBNTRA_Inf.OUTRYKB3) = "9" Then
							If AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_A_102, pm_All) = MsgBoxResult.Yes Then
								Retn_Code = CHK_OK
								pm_Chk_Move = True
...
F_Chk_HD_SBNNO_END: 
    ...
    Private Function F_Chk_Sbnno(ByVal p_SBNNO As String, ByVal p_DENDT As String) As Short
        ...
        'ADD 20150330 END
        'delete test start 20190904   
        'On Error GoTo F_Chk_Sbnno_err
        'delete end 20190905 kuwa 'On Error からTryに変更。（意味は特にない）
        'add start 20190905 kuwa
        Try
            'add end 20190905 kuwa
            Dyn_Open = False
            F_Chk_Sbnno = 1
            ...
            Dim li_FC_STA As Short
            Dim ls_SEIBAN_KBN As String
            Dim ls_SEIBAN_CLS As String
            Dim ls_JUDGE_CD As String
            Dim ls_RET_STATUS As String 'PL/SQLの戻り値
            Select Case IsNumeric(ls_SBNNO_1)
                Case True
                    ...
                    If dt Is Nothing OrElse dt.Rows.Count <= 0 Then
                        ...
                        F_Chk_Sbnno = 4
                    Else
                        'CHG START FKS)INABA 2008/08/18 **************************************************
                        F_Chk_Sbnno = 6
                        'change start 20190909 kuwa
                        ...
                        For Each row As DataRow In dt.Rows
                            ...
                            ls_STTTKDT = Trim(DB_NullReplace(row("STTTKDT"), ""))
                            'change end 20190909 kuwa
                            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                            'change start 20190909 kuwa
                            'ls_ENDTKDT = Trim(CF_Ora_GetDyn(Usr_Ody, "ENDTKDT", ""))
                            ls_ENDTKDT = Trim(DB_NullReplace(row("ENDTKDT"), ""))
                            'change end 20190909 kuwa
                            If ls_STTTKDT <= ls_DENDT And ls_ENDTKDT >= ls_DENDT Then
                                F_Chk_Sbnno = 0
                            End If
                            'delete start 20190909 kuwa
                            'Call CF_Ora_MoveNext(Usr_Ody)
                            'delete end 20190909 kuwa
                            'change start 20190909 kuwa
                            'Loop
                        Next
                        'change end 20190909 kuwa
                ...
                Case False
                    ...
                    '                End If
                    '            End If
                    'add start 20190904 kuwa 'ストアドの設定
                    Dim cmd As New OracleCommand
                    CON = DB_START_GENKA() '接続先はGENKA_USR1
                    cmd.Connection = CON
                    cmd.CommandType = CommandType.StoredProcedure

                    'add end 20190904 kuwa

                    'change start 20190904 kuwa

                    ''change 20190801 START hou
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Add("FC_STA", 0, ORAPARM_OUTPUT)
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Add("ls_SEIBAN_KBN", "", ORAPARM_OUTPUT)
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Add("ls_SEIBAN_CLS", "", ORAPARM_OUTPUT)
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Add("ls_JUDGE_CD", "", ORAPARM_OUTPUT)
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Add("ls_RET_STATUS", "", ORAPARM_OUTPUT)
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'gv_Odb_USR1.Parameters("FC_STA").serverType = ORATYPE_NUMBER

                    Dim FC_STA As OracleParameter = New OracleParameter 'パラメーターを宣言
                    FC_STA.ParameterName = "FC_STA"                     'パラメーター名を設定
                    FC_STA.Direction = ParameterDirection.ReturnValue 'パラメーターの種別を設定
                    FC_STA.OracleDbType = OracleDbType.Decimal ' パラメーターの型をテーブルを参照して指定
                    FC_STA.Value = 0                            '値を設定
                    cmd.Parameters.Add(FC_STA)                  '値を追加

                    'GENKA_USR1,PACKAGE_BODY,GNKFP99のデータを参照してパラメータをセットしていく
                    'TypeごとにOracleDbTypeが異なるので注意。変換手順メモPL/SQLも参照する。
                    'Input 
                    Dim input1 As OracleParameter = New OracleParameter
                    input1.ParameterName = FAC_CD_GENKA
                    input1.Direction = ParameterDirection.Input
                    input1.OracleDbType = OracleDbType.Varchar2
                    input1.Value = FAC_CD_GENKA
                    cmd.Parameters.Add(input1)

                    Dim input2 As OracleParameter = New OracleParameter
                    input2.ParameterName = Trim(ls_SBNNO)
                    input2.Direction = ParameterDirection.Input
                    input2.OracleDbType = OracleDbType.Varchar2
                    input2.Value = Trim(ls_SBNNO)
                    cmd.Parameters.Add(input2)

                    Dim input3 As OracleParameter = New OracleParameter
                    input3.ParameterName = Trim(ls_DENDT)
                    input3.Direction = ParameterDirection.Input
                    input3.OracleDbType = OracleDbType.Varchar2
                    input3.Value = Trim(ls_DENDT)
                    cmd.Parameters.Add(input3)
                    'Input end 



                    Dim SEIBAN_KBN As OracleParameter = New OracleParameter
                    SEIBAN_KBN.ParameterName = "ls_SEIBAN_KBN"
                    SEIBAN_KBN.Direction = ParameterDirection.InputOutput 'パッケージ内で値を入出力しているため、InputOutputで指定
                    SEIBAN_KBN.OracleDbType = OracleDbType.Varchar2
                    SEIBAN_KBN.Value = " " & " " '半角スペースが一つしかない場合、値を取得することができないため、空白を二回入力。原因不明。以下、同様。
                    cmd.Parameters.Add(SEIBAN_KBN)

                    Dim SEIBAN_CLS As OracleParameter = New OracleParameter
                    SEIBAN_CLS.ParameterName = "ls_SEIBAN_CLS"
                    SEIBAN_CLS.Direction = ParameterDirection.InputOutput
                    SEIBAN_CLS.OracleDbType = OracleDbType.Varchar2
                    SEIBAN_CLS.Value = " " & " "
                    cmd.Parameters.Add(SEIBAN_CLS)

                    Dim JUDGE_CD As OracleParameter = New OracleParameter
                    JUDGE_CD.ParameterName = "ls_JUDGE_CD"
                    JUDGE_CD.Direction = ParameterDirection.InputOutput
                    JUDGE_CD.OracleDbType = OracleDbType.Varchar2
                    JUDGE_CD.Value = " " & " "
                    cmd.Parameters.Add(JUDGE_CD)

                    Dim RET_STATUS As OracleParameter = New OracleParameter
                    RET_STATUS.ParameterName = "ls_RET_STATUS"
                    RET_STATUS.Direction = ParameterDirection.InputOutput
                    RET_STATUS.OracleDbType = OracleDbType.Varchar2
                    RET_STATUS.Value = " " & " "
                    cmd.Parameters.Add(RET_STATUS)

                    'change end 20190904 kuwa
                    strSQL = ""
                    strSQL = "DECLARE "
                    strSQL = strSQL & " FC_STA NUMBER; "
                    strSQL = strSQL & " BEGIN "
                    strSQL = strSQL & " FC_STA := GNKFP99.CHK_SEIBAN_GENKA2("
                    strSQL = strSQL & "     '" & FAC_CD_GENKA & "', "
                    strSQL = strSQL & "     '" & Trim(ls_SBNNO) & "', "
                    strSQL = strSQL & "     '" & Trim(ls_DENDT) & "', "
                    strSQL = strSQL & "     :ls_SEIBAN_KBN, "
                    strSQL = strSQL & "     :ls_SEIBAN_CLS, "
                    strSQL = strSQL & "     :ls_JUDGE_CD, "
                    strSQL = strSQL & "     :ls_RET_STATUS "
                    strSQL = strSQL & " ); "
                    strSQL = strSQL & " :FC_STA := FC_STA; "
                    strSQL = strSQL & " END; "

                    'add test 20190904
                    cmd.CommandText = "GNKFP99.CHK_SEIBAN_GENKA2"

                    cmd.ExecuteNonQuery()

                    'add test end 20190904

                    'SQL実行
                    'delete test start 20190904 kuwa
                    'bolRet = CF_Ora_Execute(gv_Odb_USR1, strSQL)
                    'If bolRet = False Then
                    '    GoTo F_Chk_Sbnno_err
                    'End If
                    'delete test end 20190904 kuwa

                    'ﾊﾞｲﾝﾄﾞ変数取得
                    'change start 20190905 kuwa
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'li_FC_STA = gv_Odb_USR1.Parameters("FC_STA").Value
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'ls_SEIBAN_KBN = gv_Odb_USR1.Parameters("ls_SEIBAN_KBN").Value
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'ls_SEIBAN_CLS = gv_Odb_USR1.Parameters("ls_SEIBAN_CLS").Value
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'ls_JUDGE_CD = gv_Odb_USR1.Parameters("ls_JUDGE_CD").Value
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'ls_RET_STATUS = gv_Odb_USR1.Parameters("ls_RET_STATUS").Value

                    li_FC_STA = CInt(cmd.Parameters("FC_STA").Value.ToString) 'li_FC_STAはShort型なので、Int型にキャストする
                    ls_SEIBAN_KBN = cmd.Parameters("ls_SEIBAN_KBN").Value.ToString
                    ls_SEIBAN_CLS = cmd.Parameters("ls_SEIBAN_CLS").Value.ToString
                    ls_JUDGE_CD = cmd.Parameters("ls_JUDGE_CD").Value.ToString
                    ls_RET_STATUS = cmd.Parameters("ls_RET_STATUS").Value.ToString

                    'change end 20190905 kuwa

                    If li_FC_STA = 0 Then
                        If CDbl(ls_RET_STATUS) = 3 Then
                            'MOD 20150330 START
                            '                    Execute_Flg = 1
                            strSQL = ""
                            strSQL = strSQL & " SELECT C_STATUS_CLS "
                            strSQL = strSQL & " FROM C_G101M "
                            strSQL = strSQL & " WHERE C_FAC_CD = '" & FAC_CD_GENKA & "'"
                            strSQL = strSQL & "   AND C_SEIBAN = '" & Left(Trim(ls_SBNNO), 6) & "'"
                            'change start 20190909 kuwa
                            'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
                            Dim dt As DataTable = DB_GetTable(strSQL)
                            'change end 20190909 kuwa
                            ...
                            If dt Is Nothing OrElse dt.Rows.Count <= 0 Then
                                ...
                                'UPD 20150618 START
                                '                        '原価未連携の見込製番(L～)を通す
                                '                        If Left(ls_SBNNO, 1) = "L" Then
                                '原価未連携の見込製番(L～)、無償製番(F～)を通す
                                If Left(ls_SBNNO, 1) = "L" Or Left(ls_SBNNO, 1) = "F" Then
                                    'UPD 20150618 END
                                    If DSPSBNM_SEARCH(ls_SBNNO, DB_SBNMTA_W) = 0 Then
                                        F_Chk_Sbnno = 0
                                    End If
                                End If

                                '原価未連携の本製番(R～)を通す（受注に存在する製番のみ）
                                If Left(ls_SBNNO, 1) = "R" Then
                                    If F_JDNTRA_SBNNO_SEARCH(ls_SBNNO) = 0 Then
                                        F_Chk_Sbnno = 0
                                    End If
                                End If
                            ...
                            Else
                                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                                'change start 20190909 kuwa
                                'ls_STATUS = Trim(CF_Ora_GetDyn(Usr_Ody, "C_STATUS_CLS", ""))
                                ls_STATUS = Trim(DB_NullReplace(dt.Rows(0)("C_STATUS_CLS"), ""))
                                'change end 20190909 kuwa
                                '親が完了準備中、完了、取消準備中、取消、抜取の場合
                                If ls_STATUS >= "40" Then
                                    '親が取消準備中の場合、警告
                        ...
                        Else
                            ...
                            strSQL = strSQL & " FROM C_G101M "
                            strSQL = strSQL & " WHERE C_FAC_CD = '" & FAC_CD_GENKA & "'"
                            strSQL = strSQL & "   AND C_SEIBAN = '" & Trim(ls_SBNNO) & "'"
                            'change start 20190909 kuwa
                            'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
                            Dim dt As DataTable = DB_GetTable(strSQL)
                            'change end 20190909 kuwa
                    ...
                    'change start 20190905 kuwa
                    'UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Remove("FC_STA")
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Remove("ls_SEIBAN_KBN")
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Remove("ls_SEIBAN_CLS")
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Remove("ls_JUDGE_CD")
                    ''UPGRADE_WARNING: オブジェクト gv_Odb_USR1.Parameters の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'Call gv_Odb_USR1.Parameters.Remove("ls_RET_STATUS")
                    cmd.Parameters.Clear() 'パラメーターをすべてクリア
                    'change end 20190905 kuwa
...
F_Chk_Sbnno_err:
            'add start 20190905 kuwa 'Try~Catchに変更したため、Go toを削除して、Catch句を追加
        Catch ex As Exception
            MsgBox("DSPBMNCD_SEARCH" & Constants.vbCrLf & ex.Message.ToString, MsgBoxStyle.Critical, "エラー")
        End Try
        F_Chk_Sbnno = 9
        'add end 20190905 kuwa
        'delete start 20190905 kuwa
        'GoTo F_Chk_Sbnno_end
        'delete end 20190905 kuwa
    End Function
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
    ...
    Private Function F_Chk_Sbnno_2(ByVal p_SBNNO As String) As Short
        ...
        For i As Integer = 0 To dsList.Tables("tableName").Rows.Count - 1
            lw_cnt = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("CNT"), "")
        Next
		...
		Select Case ls_Check_cd
			Case "1"
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    ll_cnt = DB_NullReplace(row("CNT"), 0)
                    'change end 20190909 kuwa
                    'delete stat 20190909
                    'Call CF_Ora_MoveNext(Usr_Ody)
                    'change start 20190909
                    'Loop
                Next
                'change end 20190909 kuwa
                If Dyn_Open = True Then
			...
			Case "2"
                ...
                Else
                    ...
                    For Each row As DataRow In dt.Rows
                        ...
                        ll_OUTZMISU = DB_NullReplace(dt.Rows(0)("OUTZMISU_SUM"), 0)
                        'change end 20190909 kuwa
                        F_Chk_Shikyu = 2
                        'delete start 20190909 kuwa
                        'Call CF_Ora_MoveNext(Usr_Ody)
	...
	Public Function F_Chk_BD_HINCD(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			Else
				...
				strSQL = strSQL & " WHERE DATKB = '1'"
				strSQL = strSQL & "   AND SOUCD = '" & IDOET52_SBNTRA_Inf.SOUCD & "'"
                strSQL = strSQL & "   AND HINCD = '" & pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.HINCD & "'"
                'change start 20190909 kuwa
                'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
                Dim dt As DataTable = DB_GetTable(strSQL)
                'change end 20190909 kuwa
                ...
                If dt Is Nothing OrElse dt.Rows.Count > 0 Then
                    ...
                    'change end 20190909 kuwa
                    M_HINMTB_MOTO_Inf.SOUCD = Trim(IDOET52_SBNTRA_Inf.SOUCD)
                    M_HINMTB_MOTO_Inf.HINCD = Trim(pm_All.Dsp_Body_Inf.Row_Inf(Bd_Index).Bus_Inf.HINCD)
                Else
                    M_HINMTB_MOTO_Inf.HIKSU = ""
					M_HINMTB_MOTO_Inf.SOUCD = ""
					M_HINMTB_MOTO_Inf.HINCD = ""
...
F_Chk_BD_HINNMB_END: 
	...
	Public Function F_Chk_BD_UODSU(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		'DEL START FKS)INABA 2008/10/29 *********************************************************************
		'連絡票№434
		'    'チェック実行判定
		'    Rtn_Cd = F_Chk_Jge_Action(pm_Chk_Dsp_Sub_Inf, Retn_Code, Msg_Flg, pm_Chk_Move)
		'    If Rtn_Cd = CHK_STOP Then
		...
		Else
			...
			'CHG START FKS)INABA 2006/11/30 ***************************************************************
			intRet = F_Chk_Relzaisu(strSOUCD, strHINCD, CDec(Trim(CStr(Val(Input_Value)))), pm_All)
			Select Case intRet
				Case 1 '在庫管理しない製品コード
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgIDOET52_E_064
		...
		strSQL = strSQL & "   FROM HINMTB "
		strSQL = strSQL & " WHERE DATKB = '1'"
		strSQL = strSQL & "   AND SOUCD = '" & strSOUCD & "'"
        strSQL = strSQL & "   AND HINCD = '" & strHINCD & "'"
        'change start 20190909 kuwa
        'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
        Dim dt As DataTable = DB_GetTable(strSQL)
        ...
        If dt Is Nothing OrElse dt.Rows.Count > 0 Then
            'change end 20190909 kuwa
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'change start 2019909 kuwa
            'M_HINMTB_MOTO_Inf.HIKSU = Trim(CF_Ora_GetDyn(Usr_Ody, "HIKSU", ""))
            M_HINMTB_MOTO_Inf.HIKSU = Trim(DB_NullReplace(dt.Rows(0)("HIKSU"), ""))
            'change end 20190909 kuwa
            M_HINMTB_MOTO_Inf.SOUCD = Trim(strSOUCD)
            M_HINMTB_MOTO_Inf.HINCD = Trim(strHINCD)
        Else
            M_HINMTB_MOTO_Inf.HIKSU = ""
			M_HINMTB_MOTO_Inf.SOUCD = ""
...
F_Ctl_Tail_RelChk_END: 
	...
	Public Function F_Ctl_CS_REF_SBN(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			If WLS_IDO1_DATNO <> "" Then
				...
				'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				If DSPNHSCD_SEARCH(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSCD.Tag)).Detail.Dsp_Value, Mst_Inf) = 0 Then
				End If
	...
	Public Function F_Ctl_CS_UODSU(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Act_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		If LL_KKOUT <> 1 Then
			...
			Wk_Index = CShort(FR_SSSMAIN.TL_KKOUT.Tag)
			Trg_Index = CF_Get_Idex_Same_Bd_Ctl(pm_All.Dsp_Sub_Inf(Wk_Index), pm_Act_Dsp_Sub_Inf.Detail.Body_Index, pm_All)
			If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			End If
			Exit Function
		...
		strSQL = strSQL & "   AND PGID     = '" & SSS_PrgId & "'"
        ...
        '      Full_Nm = SSS_INIDAT(2) & "EXE\" & "SRAET61" & " /RPTCLTID:" & LS_CLTID & " /PGID:" & LS_PGID & " /SBNNO:" & Trim(ls_SBNNO.Value) & " /HINCD:" & Trim(LS_HINCD.Value) & " /URISU:" & LL_URISU
        ...
        ' === 20080815 === INSERT S - RISE)Izumi
        Dim M_SRAET61_inf() As M_TYPE_SRAET61_MOTO
		...
		strSQL = strSQL & " WHERE RPTCLTID = '" & SSS_CLTID.Value & "'"
		strSQL = strSQL & "   AND PGID     = '" & SSS_PrgId & "'"
		strSQL = strSQL & "   AND SBNNO     = '" & Trim(ls_SBNNO.Value) & "'"
		strSQL = strSQL & "   AND HINCD     = '" & Trim(LS_HINCD.Value) & "'"
		strSQL = strSQL & " ORDER BY SRALINNO ASC "
        'change start 20190909 kuwa
        'Call CF_Ora_CreateDyn(gv_Odb_USR9, Usr_Ody, strSQL)
        Dim dt As DataTable = DB_GetTable(strSQL)
        'change end 20190909 kuwa
        Erase M_SRAET61_inf
        ReDim M_SRAET61_inf(0)
        'change start 20190909 kuwa
        'Do Until CF_Ora_EOF(Usr_Ody) = True
        For Each row As DataRow In dt.Rows
            'change end 20190909 kuwa
            lngCnt = lngCnt + 1
            ReDim Preserve M_SRAET61_inf(lngCnt)
            'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'change start 20190909 kuwa
            'M_SRAET61_inf(lngCnt).SRANO = CF_Ora_GetDyn(Usr_Ody, "SRANO", "")
            M_SRAET61_inf(lngCnt).SRANO = DB_NullReplace(row("SRANO"), "")
            'change end 20190909 kuwa
            'delete start 20190909 kuwa
            'Call CF_Ora_MoveNext(Usr_Ody)
            'change start 20190909 kuwa
        ...
        '退避しているシリアル管理テーブルの内容を初期化する
        Erase M_SRACNTTB_MOTO_inf
		ReDim M_SRACNTTB_MOTO_inf(0)
		
		'シリアル番号を元にシリアル管理テーブルのタイムスタンプを取得
		If UBound(M_SRAET61_inf) > 0 Then
			For lngCnt = 1 To UBound(M_SRAET61_inf)
				strSQL = ""
				strSQL = strSQL & "SELECT"
				strSQL = strSQL & "  SRANO "
				strSQL = strSQL & ", OPEID "
				...
				strSQL = strSQL & "WHERE"
				strSQL = strSQL & "  SRANO = " & "'" & M_SRAET61_inf(lngCnt).SRANO & "' "
                'change start 20190909 kuwa
                'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
                dt = DB_GetTable(strSQL)
                'change end 20190909 kuwa
                'change start 20190909 kuwa
                'If Not CF_Ora_EOF(Usr_Ody) = True Then
                If Not dt Is Nothing OrElse dt.Rows.Count <= 0 Then
                    'change end 20190909 kuwa
                    ReDim Preserve M_SRACNTTB_MOTO_inf(lngCnt)
                    With M_SRACNTTB_MOTO_inf(lngCnt)
                        'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                        'change start 20190909 kuwa
                        '                  .SRANO = CF_Ora_GetDyn(Usr_Ody, "SRANO", "")
                        ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                        ...
                        .SRANO = DB_NullReplace(dt.Rows(0)("SRANO"), "")
	...
	Public Function F_Ctl_Upd_Process(ByRef pm_All As Cls_All) As Short
		...
		''H.Y.(9/22)    If Inp_Inf.InpJDNUPDKB <> gc_strJDNUPDKB_OK Then
		''        Exit Function
		''H.Y.(9/22)    End If
		...
		If intRet <> CHK_OK Then
			...
			GoTo End_F_Ctl_Upd_Process
		End If
		'ADD START FKS)INABA 2007/08/15 ********************************************************************
		Dim Mst_Inf_HINMTA As TYPE_DB_HINMTA
		Dim strHINCD As String
		Dim strSBNNO As String
		Dim ll_CNT_1 As Integer
		Dim ll_su_1 As Integer
		'UPGRADE_WARNING: 構造体 Usr_Ody の配列は、使用する前に初期化する必要があります。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="814DF224-76BD-4BB4-BFFB-EA359CB9FC48"' をクリックしてください。
		Dim Usr_Ody As U_Ody
		Dim strSQL As String
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		...
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSBNNO = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_SBNNO.Tag)).Detail.Dsp_Value, 10)
		If DSPHINCD_SEARCH(strHINCD, Mst_Inf_HINMTA) = 0 Then
			'CHG START FKS)INABA 2009/12/21 **********************************************************
			'連絡票№766
			'CHG START FWEST)KOROYASU 2015/02/28 **********************************************************
			'会社統合対応
			'        If IDOET52_SBNTRA_Inf.KKOUT = 1 Then
			If IDOET52_SBNTRA_Inf.KKOUT = 1 And Mst_Inf_HINMTA.SERIKB = "1" Then
				'CHG  END FWEST)KOROYASU 2015/02/28 **********************************************************
				'        If IDOET52_SBNTRA_Inf.KKOUT = 1 And Mst_Inf_HINMTA.SERIKB = "1" Then
				...
				strSQL = strSQL & " WHERE RPTCLTID = '" & SSS_CLTID.Value & "'"
				strSQL = strSQL & "   AND PGID     = '" & SSS_PrgId & "'"
				strSQL = strSQL & "   AND SBNNO    = '" & Trim(strSBNNO) & "'"
				strSQL = strSQL & "   AND HINCD    = '" & Trim(strHINCD) & "'"
                strSQL = strSQL & " ORDER BY SRALINNO "
                'change start 20190909 kuwa
                'Call CF_Ora_CreateDyn(gv_Odb_USR9, Usr_Ody, strSQL)
                Dim dt As DataTable = DB_GetTable(strSQL)
                'change end 20190909 kuwa
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'change start 20190909 kuwa
                    'll_CNT_1 = CF_Ora_GetDyn(Usr_Ody, "CNT", "")
                    ll_CNT_1 = DB_NullReplace(row("CNT"), "")
                    'delete start 20190909 kuwa
                    'Call CF_Ora_MoveNext(Usr_Ody)
                    'delete end 20190909 kuwa
                    'change start 20190909 kuwa
                ...
                'change end 20190909 kuwa
                'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                ll_su_1 = Val(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value)
				If ll_su_1 <> ll_CNT_1 Then
					'CHG START FKS)INABA 2008/01/23 **********************************************************
					'入力したシリアル(棚番)件数と数量が不一致の場合、エラーとする
					intRet = AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_018, pm_All)
					GoTo End_F_Ctl_Upd_Process
					'                intRet = AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_A_016, pm_All)
					'                If intRet <> vbYes Then
					'                    GoTo End_F_Ctl_Upd_Process
					'                End If
					'CHG  END  FKS)INABA 2008/01/23 **********************************************************
				End If
			End If
		End If
        ...
        '2019/06/13 ADD START <C2-20190528-02> CIS)山口
        Dim Trg_Index As Short
        Dim strJODRSNKB As String '受注理由
        Dim strUODSU As Integer '指示数量
        Trg_Index = CShort(FR_SSSMAIN.HD_OUTRYCD.Tag)
        'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        strJODRSNKB = CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(Trg_Index))
        'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        strUODSU = CInt(Val(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value))
        If strJODRSNKB = "50" And strUODSU < 0 Then
            strSQL = ""
            strSQL = " SELECT ORGSBNNO "
            strSQL = strSQL & " FROM SBNTRA "
            strSQL = strSQL & " WHERE DATNO = (SELECT MAX(DATNO) "
            strSQL = strSQL & "                            FROM SBNTRA "
            strSQL = strSQL & "                            WHERE DATKB = '1' "
            strSQL = strSQL & "                            AND OUTKB = '2' "
            strSQL = strSQL & "                            AND SBNNO = '" & Trim(strSBNNO) & "') "
            'change start 20190909 kuwa
            'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
            Dim dt As DataTable = DB_GetTable(strSQL)
            ...
            If Not dt Is Nothing OrElse dt.Rows.Count <= 0 Then
                'change end 20190909 kuwa
                strSQL = ""
                strSQL = " SELECT  NVL(SUM(OTPSU),0) OTPSU "
                strSQL = strSQL & " FROM  ODNTRA "
                strSQL = strSQL & " WHERE  SBNNO   =  '" & Trim(strSBNNO) & "' "
                'change start 20190909 kuwa
                'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
                dt = DB_GetTable(strSQL)
                'change end 20190909 kuwa
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn(Usr_Ody, OTPSU, ) の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                'change start 20190909 kuwa
                'If CF_Ora_GetDyn(Usr_Ody, "OTPSU", "") + strUODSU < 0 Then
                If DB_NullReplace(dt.Rows(0)("OTPSU"), "") + strUODSU < 0 Then
                    'change end 20190909 kuwa
                    intRet = AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_105, pm_All)
                    GoTo End_F_Ctl_Upd_Process
                End If
            End If
        End If
        '2019/06/13 ADD END <C2-20190528-02> CIS)山口
...
Err_F_Ctl_Upd_Process: 
	...
	Public Function F_Update_Main(ByRef pot_strJdnNo As String, ByRef pm_All As Cls_All) As Short
		...
		''H.Y.(9/21)    '参照見積番号入力チェック
		''    bolMitNo = False
		''    If IsNumeric(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_JDNNO.Tag)))) = True Then
		''        If CLng(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_JDNNO.Tag)))) <> 0 Then
		''            bolMitNo = True
		''        End If
		''H.Y.(9/21)    End If
		...
		'ADD START FKS)INABA 2007/02/20 **************************************************
		If RunMode = RUNMODE_IDOET53 And Trim(IDOET52_SBNTRA_Inf.DATNO) <> "" Then
			strPUDLNO(1) = IDOET52_SBNTRA_Inf.PUDLNO
		Else
			'ADD START FKS)INABA 2007/02/20 **************************************************
			'入出庫番号採番処理
			intRet = F_Get_SYSTBC(SYSTBC_PUDLNO, strPUDLNO(1))
			If intRet <> 0 Then
				If intRet = 2 Then
					Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_036, pm_All)
				End If
		...
		End If
		'納品書番号採番処理
		intRet = F_Get_SYSTBC(SYSTBC_FDNNO, strFDNNO(1))
		If intRet <> 0 Then
			If intRet = 2 Then
				Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_036, pm_All)
			End If
		...
		Dim ret As Boolean
		'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		LS_HINCD = CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(CShort(FR_SSSMAIN.BD_HINCD(1).Tag)))
		'UPGRADE_WARNING: オブジェクト FR_SSSMAIN.SYSDT.Tag の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strJDNDT = CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(CShort(FR_SSSMAIN.SYSDT.Tag)))
		ret = DSPHINCD_SEARCH(LS_HINCD, Mst_Inf_HINMTA, strJDNDT)
		
		'ADD START FKS)INABA 2007/03/26 **********************************************************************
		'ADD START FKS)INABA 2007/03/26 ****************************************
		'保留
		'    If Trim$(IDOET52_SBNTRA_Inf.OUTRYKB3) = "9" Then
		'        If Trim$(IDOET52_SBNTRA_Inf.SOUCD) <> Trim$(Mst_Inf_HINMTA.TNACM) Then
		'            If DSPHINCD_SEARCH(ls_HINCD, Mst_Inf_HINMTA, strJDNDT) = 0 Then
		'                IDOET52_SBNTRA_Inf.SOUCD = Mst_Inf_HINMTA.TNACM    '標準倉庫コード
		'                Trg_Index = CInt(FR_SSSMAIN.HD_SOUCD.Tag)
		'                Dsp_Value = CF_Cnv_Dsp_Item(IDOET52_SBNTRA_Inf.SOUCD, pm_All.Dsp_Sub_Inf(Trg_Index), False)
		'                Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
		'                If DSPSOUCD_SEARCH(IDOET52_SBNTRA_Inf.SOUCD, Mst_Inf_SOUMTA) = 0 Then
		'                    IDOET52_SBNTRA_Inf.SOUCD = Mst_Inf_SOUMTA.SOUCD       '倉庫コード
		'                    IDOET52_SBNTRA_Inf.SOUNM = Mst_Inf_SOUMTA.SOUNM       '倉庫名
		'                    IDOET52_SBNTRA_Inf.SOUBSCD = Mst_Inf_SOUMTA.SOUBSCD   '場所コード
		...
		If Trim(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_SOUCD.Tag)).Detail.Dsp_Value) = "" Then
			...
			End If
		Else
			If Trim(IDOET52_SBNTRA_Inf.SOUCD) <> Trim(Mst_Inf_HINMTA.TNACM) Then '標準倉庫コード
				'画面で入力された倉庫コードと商品マスタに設定されている標準倉庫が違う場合
				'メッセージを表示する。
				If AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_A_099, pm_All) = MsgBoxResult.No Then
					'入力した倉庫と標準倉庫が違いますが、よろしいですか
					If AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_A_100, pm_All) = MsgBoxResult.Yes Then
						'標準倉庫を設定しますか？
						If DSPHINCD_SEARCH(LS_HINCD, Mst_Inf_HINMTA, strJDNDT) = 0 Then
							IDOET52_SBNTRA_Inf.SOUCD = Mst_Inf_HINMTA.TNACM '標準倉庫コード
							Trg_Index = CShort(FR_SSSMAIN.HD_SOUCD.Tag)
		...
		lngCnt = 0
		'シリアル管理テーブルの排他確認
		If UBound(M_SRACNTTB_MOTO_inf) > 0 Then
			For lngCnt = 1 To UBound(M_SRACNTTB_MOTO_inf)
				strSQL = ""
				strSQL = strSQL & "SELECT"
				strSQL = strSQL & "  SRANO "
				strSQL = strSQL & ", OPEID "
				...
				strSQL = strSQL & "  SRACNTTB "
				strSQL = strSQL & "WHERE"
				strSQL = strSQL & "  SRANO = " & "'" & M_SRACNTTB_MOTO_inf(lngCnt).SRANO & "' "
				strSQL = strSQL & "FOR UPDATE"
                'change start 20190909 kuwa
                'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
                Dim dt As DataTable = DB_GetTable(strSQL)
                ...
                If Not dt Is Nothing OrElse dt.Rows.Count <= 0 Then
                    ...
                    With M_SRACNTTB_MOTO_inf(lngCnt)
                        ...
                        'change start 20190909 kuwa
                        'If .OPEID <> CF_Ora_GetDyn(Usr_Ody, "OPEID", "") Or .CLTID <> CF_Ora_GetDyn(Usr_Ody, "CLTID", "") Or .WRTTM <> CF_Ora_GetDyn(Usr_Ody, "WRTTM", "") Or .WRTDT <> CF_Ora_GetDyn(Usr_Ody, "WRTDT", "") Or .UOPEID <> CF_Ora_GetDyn(Usr_Ody, "UOPEID", "") Or .UCLTID <> CF_Ora_GetDyn(Usr_Ody, "UCLTID", "") Or .UWRTTM <> CF_Ora_GetDyn(Usr_Ody, "UWRTTM", "") Or .UWRTDT <> CF_Ora_GetDyn(Usr_Ody, "UWRTDT", "") Then
                        If .OPEID <> DB_NullReplace(dt.Rows(0)("OPEID"), "") Or .CLTID <> DB_NullReplace(dt.Rows(0)("CLTID"), "") Or .WRTTM <> DB_NullReplace(dt.Rows(0)("WRTTM"), "") Or .WRTDT <> DB_NullReplace(dt.Rows(0)("WRTDT"), "") Or .UOPEID <> DB_NullReplace(dt.Rows(0)("UOPEID"), "") Or .UCLTID <> DB_NullReplace(dt.Rows(0)("UCLTID"), "") Or .UWRTTM <> DB_NullReplace(dt.Rows(0)("UWRTTM"), "") Or .UWRTDT <> DB_NullReplace(dt.Rows(0)("UWRTDT"), "") Then
                            'change end 20190909 kuwa
                            'メッセージ出力
                            Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_901, pm_All)
                            'ロールバック
                            '2019/06/14 CHG START
                            'Call CF_Ora_RollbackTrans(gv_Oss_USR1)
                            Call DB_Rollback()
		...
		Dim strHIKSU As String
		
		' 緊急出庫時
		If IDOET52_SBNTRA_Inf.KKOUT = 1 And RunMode = RUNMODE_IDOET52 Then
			'倉庫別在庫マスタの排他確認
			With M_HINMTB_MOTO_Inf
				If Trim(.HIKSU) <> "" And Trim(.SOUCD) <> "" And Trim(.HINCD) <> "" Then
					strSQL = ""
					strSQL = strSQL & " SELECT HIKSU "
					strSQL = strSQL & "   FROM HINMTB "
					strSQL = strSQL & " WHERE DATKB = '1'"
					strSQL = strSQL & "   AND SOUCD = '" & .SOUCD & "'"
                    ...
                    If dt Is Nothing OrElse dt.Rows.Count > 0 Then
                        ...
                        'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                        'change start 20190909 kuwa
                        'strHIKSU = CF_Ora_GetDyn(Usr_Ody, "HIKSU", "")
                        strHIKSU = DB_NullReplace(dt.Rows(0)("HIKSU"), "")
                        'change end 20190909 kuwa
                        If Trim(strHIKSU) <> Trim(.HIKSU) Then
                            'メッセージ出力
                            Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_901, pm_All)
                            'ロールバック
                            '2019/06/14 CHG START
		...
		'CHG START FKS)INABA 2007/10/02 ***************************************
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		ll_su = CInt(Val(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value))
		'    ll_su = CLng(Val(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UODSU))
		'CHG  END  FKS)INABA 2007/10/02 ***************************************
		'ADD  END  FKS)INABA 2006/12/02 **********************
		
		'ADD 20141119 START
		Dim ls_NENDO As String
		Dim ls_SEI_TAI_CLS As String
		If Execute_Flg = 1 Then
			...
			strSQL = ""
			strSQL = strSQL & " SELECT * "
			strSQL = strSQL & "   From C_G121T "
			strSQL = strSQL & "  WHERE C_FAC_CD = '" & FAC_CD_GENKA & "' "
			'strSQL = strSQL & "    AND C_NENDO = (SELECT SUBSTR(C_KISYU_YM,1,4) "
			'strSQL = strSQL & "                    FROM C_G109T "
			'strSQL = strSQL & "                   WHERE C_FAC_CD = '" & FAC_CD_GENKA & "') "
			strSQL = strSQL & "    AND (C_SEIBAN_YUKO_KETA = LENGTH('" & Trim(CF_Ora_String(IDOET52_SBNTRA_Inf.SBNNO, 20)) & "') "
			strSQL = strSQL & "     OR C_SYUKEI_KETA = LENGTH('" & Trim(CF_Ora_String(IDOET52_SBNTRA_Inf.SBNNO, 20)) & "')) "
            strSQL = strSQL & "    AND C_SEIBAN_1_4 = SUBSTR('" & Trim(CF_Ora_String(IDOET52_SBNTRA_Inf.SBNNO, 20)) & "', 1, C_SEIBAN_KETA) "
            'change start 20190909 kuwa
            'Call CF_Ora_CreateDyn(gv_Odb_USR1, Usr_Ody, strSQL)
            Dim dt As DataTable = DB_GetTable(strSQL)
            'change end 20190909 kuwa
            ...
            For Each row As DataRow In dt.Rows
                ...
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                'change start 20190909 kuwa
                'ls_NENDO = Trim(CF_Ora_GetDyn(Usr_Ody, "C_NENDO", ""))
                ls_NENDO = Trim(DB_NullReplace(row("C_NENDO"), ""))
                'change end 20190909 kuwa
                'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                'change start 20190909 kuwa
                'ls_SEI_TAI_CLS = Trim(CF_Ora_GetDyn(Usr_Ody, "C_SEI_TAI_CLS", ""))
                ls_SEI_TAI_CLS = Trim(DB_NullReplace(row("C_SEI_TAI_CLS"), ""))
                'change end 20190909 kuwa
                'delete stat 20190909
                'Call CF_Ora_MoveNext(Usr_Ody)
                'change start 20190909 kuwa
			...
			strSQL = strSQL & "BEGIN FC_STA := GNKFP99.INS_C_G101M_7KETA("
			strSQL = strSQL & "'" & FAC_CD_GENKA & "',"
			'strSQL = strSQL & "'" & ls_NENDO & "',"
			strSQL = strSQL & "'" & Trim(CF_Ora_String(IDOET52_SBNTRA_Inf.SBNNO, 20)) & "',"
			strSQL = strSQL & "'" & ls_SEI_TAI_CLS & "'"
			strSQL = strSQL & "); END;"
			bolRet = CF_Ora_Execute(gv_Odb_USR1, strSQL)
			If bolRet = False Then
				GoTo F_Update_Main_err
		...
		'ADD START FKS)INABA 2006/12/01 ****************************************************************************************************
		'ADD START FKS)INABA 2007/12/05 *********************************************************
		Call PUT_LOG("001", pm_All, Trim(IDOET52_SBNTRA_Inf.OUTRYKB3), Trim(CStr(FR_SSSMAIN.HD_OPT2.Checked)), Trim(CStr(ll_su)))
		'ADD  END  FKS)INABA 2007/12/05 *********************************************************
		If Trim(IDOET52_SBNTRA_Inf.OUTRYKB3) = "9" And FR_SSSMAIN.HD_OPT2.Checked = True Then
			'支給品の戻しの場合は更新は出荷指示トラン＆出荷指示見出トラン更新
			intRet = F_FDNTRA_Insert(strDATNO(1), strPUDLNO(1), strFDNNO(1), strRECNO(1), pm_All)
			If intRet <> 0 Then
				GoTo F_Update_Main_err
			End If
			If ll_su > 0 Then
				FILE1_PATH = GP_GetIni(My.Application.Info.DirectoryPath & "\" & "IDOET52.ini", "FILEPATH", "FILE1")
			Else
				FILE1_PATH = GP_GetIni(My.Application.Info.DirectoryPath & "\" & "IDOET52.ini", "FILEPATH", "FILE2")
			End If
			lngFileNo1 = FreeFile
		...
		Else
			...
			'訂正のとき
			'ADD START FKS)INABA 2007/12/05 *********************************************************
			Call PUT_LOG("002", pm_All, Trim(CStr(RunMode)), Trim(IDOET52_SBNTRA_Inf.DATNO))
			'ADD  END  FKS)INABA 2007/12/05 *********************************************************
			If RunMode = RUNMODE_IDOET53 And Trim(IDOET52_SBNTRA_Inf.DATNO) <> "" Then
			...
			'ADD START FKS)INABA 2007/01/20 ****************************************************
			'ADD START FKS)INABA 2007/12/05 *********************************************************
			Call PUT_LOG("003", pm_All, Trim(CStr(RunMode)), Trim(IDOET52_SBNTRA_Inf.OUTRYKB1))
			'ADD  END  FKS)INABA 2007/12/05 *********************************************************
			
			If RunMode = RUNMODE_IDOET52 And IDOET52_SBNTRA_Inf.OUTRYKB1 = "1" Then
				'DEL START FKS)INABA 2007/01/20 ****************************************************
				''DEL  END  FKS)INABA 2007/01/20 ****************************************************
				intRet2 = SBNNO_Saiban(pm_All, IDOET52_SBNTRA_Inf.J_BMNCD, IDOET52_SBNTRA_Inf.CLMDL, IDOET52_SBNTRA_Inf.REGDT, IDOET52_SBNTRA_Inf.SBNNO)
				If intRet2 <> 0 Then
					GoTo F_Update_Main_err
				End If
			End If
			'ADD  END  FKS)INABA 2007/01/20 ****************************************************
			'製番出庫ファイル更新
			intRet = F_SBNTRA_Insert(strDATNO(1), strPUDLNO(1), pm_All)
			If intRet <> 0 Then
				GoTo F_Update_Main_err
			End If
			
			'ADD START FKS)INABA 2007/12/05 *********************************************************
			Call PUT_LOG("004", pm_All, Trim(CStr(IDOET52_SBNTRA_Inf.KKOUT)), Trim(CStr(RunMode)))
			'ADD  END  FKS)INABA 2007/12/05 *********************************************************
			' 緊急出庫時
			If IDOET52_SBNTRA_Inf.KKOUT = 1 And RunMode = RUNMODE_IDOET52 Then
				'出荷指示トラン＆出荷指示見出トラン更新
				...
				'倉庫別在庫マスタ更新
				intRet = F_HINMTB_Update(pm_All, strPUDLNO(1))
				If intRet <> 0 Then
					GoTo F_Update_Main_err
				End If
				
				'2008/08/25 ADD START FKS)NAKATA
				''処理理由が「50」番以上の場合、引当内訳ファイルを作成しない
				If Trim(IDOET52_SBNTRA_Inf.OUTRYKB3) <> "9" Then
					'2008/08/25 ADD E.N.D FKS)NAKATA
					
					'2008/08/22 ADD START FKS)NAKATA
					''引当内訳ファイルの作成
					intRet = F_DTLTRA_Insert(pm_All, strPUDLNO(1))
					If intRet <> 0 Then
						GoTo F_Update_Main_err
					End If
					'2008/08/22 ADD E.N.D FKS)NAKATA
				...
				'CHG START FKS)INABA 2006/11/20 ****************************************************
				FILE1_PATH = GP_GetIni(My.Application.Info.DirectoryPath & "\" & "IDOET52.ini", "FILEPATH", "FILE1")
				'        FILE1_PATH = GP_GetIni(AE_AppPath$ & "IDOET52.ini", "FILEPATH", "FILE1")
				'CHG  END  FKS)INABA 2006/11/20 ****************************************************
				lngFileNo1 = FreeFile
				FileOpen(lngFileNo1, FILE1_PATH, OpenMode.Output)
				FileClose(lngFileNo1)
				'2006/10/11 [ADD-END]
				'2006/10/19 [ADD-START]
			Else
				'ADD START FKS)INABA 2007/12/05 *********************************************************
				'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Call PUT_LOG("005", pm_All, Trim(CStr(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UODSU)), Trim(CStr(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value)))
				'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				If CInt(Val(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UODSU)) <> CInt(Val(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value)) Then
					MsgBox("数量の内部処理でエラーが発生しました。システム管理者に連絡してください。")
					GoTo F_Update_Main_err
				End If
				'ADD  END  FKS)INABA 2007/12/05 *********************************************************
				'CHG START FKS)INABA 2006/11/27 ***************************************
				'CHG START FKS)INABA 2007/10/02 ***************************************
				'           Debug.Print CLng(Val(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UODSU))
				'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				If CInt(Val(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value)) < 0 Then
					'            If CLng(Val(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UODSU)) < 0 Then
					'CHG  END  FKS)INABA 2007/10/02 ***************************************
					'        If pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UODSU < 0 Then
					...
					'INIファイル取得用関数
					'CHG START FKS)INABA 2006/11/20 ****************************************************
					FILE1_PATH = GP_GetIni(My.Application.Info.DirectoryPath & "\" & "IDOET53.ini", "FILEPATH", "FILE2")
					'                FILE1_PATH = GP_GetIni(App.Path & "\" & "IDOET53.ini", "FILEPATH", "FILE1")
					'            FILE1_PATH = GP_GetIni(AE_AppPath$ & "IDOET53.ini", "FILEPATH", "FILE1")
					'CHG  END  FKS)INABA 2006/11/20 ****************************************************
					lngFileNo1 = FreeFile
					FileOpen(lngFileNo1, FILE1_PATH, OpenMode.Output)
					FileClose(lngFileNo1)
		...
		'ADD START FKS)INABA 2007/01/20 ****************************************************
		If RunMode = RUNMODE_IDOET52 And IDOET52_SBNTRA_Inf.OUTRYKB1 = "1" Then
			'DEL START FKS)INABA 2007/01/26 ****************************************************
			'    And (Trim$(IDOET52_SBNTRA_Inf.OUTRYCD) = "01" Or Trim$(IDOET52_SBNTRA_Inf.OUTRYCD) = "02") Then
			'DEL  END  FKS)INABA 2007/01/26 ****************************************************
			'UPGRADE_WARNING: Screen プロパティ Screen.MousePointer には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
			System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
			'製番表示画面
			gv_strDLGMSG01_BNGNM = "製番"
			gv_strDLGMSG01_NO = IDOET52_SBNTRA_Inf.SBNNO
			'           'Windowsに処理を返す
			'            DoEvents
			DLGMSG01_ACE.ShowDialog()
		End If
		'ADD  END  FKS)INABA 2007/01/20 ****************************************************
		
		'Debug.Print "DATNO= " & strDATNO(1) & " で保存された"
		
		'返り値設定
		F_Update_Main = 0
		
		pot_strJdnNo = strJdnNo
...
F_Update_Main_err: 
	...
	'DEL START FKS)INABA 2006/11/14********************************************************
	'使用していない為削除
	'' === 20060809 === INSERT S - ACE)Nagasawa 自動発注処理の組み込み
	'    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'    '   名称：  Function F_Execute_UODFP52
	'    '   概要：  自動発注呼び出し処理
	...
	'    '   備考：
	'    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'Public Function F_Execute_UODFP52(ByVal pin_strJDNNO As String, _
	''                                  ByRef pm_All As Cls_All) As Integer
	'
	'    Dim intHINCD_Col            As Integer
	'    Dim intUODSU_Col            As Integer
	'    Dim strHINCD                As String
	'    Dim lngUODSU                As Long
	...
	'
	'    '製品コードの列番号取得
	'    intHINCD_Col = CF_Get_Col_Same_Bd_Ctl(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_HINCD(0).Tag)), 1, pm_All)
	'    '出庫数量の列番号取得
	'    intUODSU_Col = CF_Get_Col_Same_Bd_Ctl(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(0).Tag)), 1, pm_All)
	'
	'    For intCnt = 1 To intMeisaiCnt Step 1
	'        '製品コード取得
	'        strHINCD = pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(intHINCD_Col).Dsp_Value
	'        '出庫数量取得
	'        lngUODSU = CLng(CF_Get_CCurString(CStr(pm_All.Dsp_Body_Inf.Row_Inf(intCnt).Item_Detail(intUODSU_Col).Dsp_Value)))
	'
	'        '自動発注処理の呼び出し
	'        intRet = AE_Execute_PLSQL_PRC_UODFP53("1" _
	''                                            , pin_strJDNNO _
	''                                            , Format(intCnt, "000") _
	''                                            , pin_strJDNNO & Format(intCnt, "00") _
	''                                            , strHINCD _
	''                                            , 0 _
	''                                            , lngUODSU)
	'        If intRet <> 0 Then
	'            'エラーメッセージ表示
	'            Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_034, pm_All, "AE_Execute_PLSQL_PRC_UODFP53")
	'            Exit Function
	'        End If
	'    Next intCnt
	'
	...
	Public Function F_SBNTRA_Insert(ByVal pin_strDatNo As String, ByVal pin_strPUDLNO As String, ByRef pm_All As Cls_All) As Short
		...
		End If
		' 16
		'CHG START FKS)INABA 2009/07/24 *********************************************************
		'連絡票№732
		If Trim(IDOET52_SBNTRA_Inf.OUTRYKB3) = "9" Then
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strSQL = strSQL & "        , " & CF_Ora_Number(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value)
		...
		'''' UPD 2012/02/28  FKS) T.Yamamoto    Start    連絡票№FC12022801
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String("", 1) & "' "          '削除フラグ
		strSQL = strSQL & "        ,  '" & gc_strDLFLG_INS & "' " '削除フラグ
		'''' UPD 2012/02/28  FKS) T.Yamamoto    End
		'ADD START FKS)INABA 2006/11/20 ***************************************
		strSQL = strSQL & "        ) "
...
F_SBNTRA_Delete_err: 
	...
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_FDNTRA_Insert(ByVal pin_strDatNo As String, ByVal pin_strPUDLNO As String, ByVal pin_strFDNNO As String, ByVal pin_strRecNo As String, ByRef pm_All As Cls_All) As Short
		
		Dim strSQL As String
		Dim bolRet As Boolean
		Dim intRet As Short
		Dim Mst_Inf_MEIMTA As TYPE_DB_MEIMTA ' 名称マスタレコード
		Dim Mst_Inf_TOKMTA As TYPE_DB_TOKMTA ' 得意先マスタレコード
		Dim Mst_Inf_BMNMTA As TYPE_DB_BMNMTA ' 部門マスタレコード
		Dim Mst_Inf_NHSMTA As TYPE_DB_NHSMTA ' 納入先マスタレコード
		Dim Mst_Inf_SOUMTA As TYPE_DB_SOUMTA ' 倉庫マスタレコード
		Dim strSBNNO As String
		Dim strSOUCD As String
		Dim strOUTBSCD As String
		Dim strJANCD As String
		Dim strTOKCD As String ' 得意先コード
		Dim strTOKNMA As String
		Dim strTOKNMB As String
		Dim strTOKZP As String
		Dim strTOKADA As String
		Dim strTOKADB As String
		Dim strTOKADC As String
		Dim strTOKTL As String
		Dim strTOKFX As String
		Dim strNHSCD As String ' 納入先コード
		Dim strNHSNMA As String
		Dim strNHSNMB As String
		Dim strNHSZP As String
		Dim strNHSADA As String
		Dim strNHSADB As String
		Dim strNHSADC As String
		Dim strNHSTL As String
		Dim strNHSFX As String
		...
		'ADD  END  FKS)INABA 2007/06/07 **********************
		On Error GoTo F_FDNTRA_Insert_err
		
		F_FDNTRA_Insert = 9
		
		' 製番
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSBNNO = pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_SBNNO.Tag)).Detail.Dsp_Value
		
		'出荷場所 (F_Chk_HD_SOUCD()参照)
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSOUCD = pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_SOUCD.Tag)).Detail.Dsp_Value
		strOUTBSCD = IDOET52_SBNTRA_Inf.SOUBSCD
		
		'JANコード
		strJANCD = pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.JANCD
		
		'ADD START FKS)INABA 2007/06/07 **********************
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strDate = CF_Ora_Date(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_DENDT.Tag)).Detail.Dsp_Value)
		'ADD  END  FKS)INABA 2007/06/07 **********************
		
		'得意先コード
		'CHG START FKS)INABA 2006/12/26 ******************************************
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strTOKCD = pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_TOKCD.Tag)).Detail.Dsp_Value
		If Trim(strTOKCD) <> "" Then
			'    If IDOET52_SBNTRA_Inf.OUTRYKB3 = "1" Then        ' 区分３ = "1"
			'        strTOKCD = pm_All.Dsp_Sub_Inf(FR_SSSMAIN.HD_TOKCD.Tag).Detail.Dsp_Value
			'CHG  END  FKS)INABA 2006/12/26 ******************************************
			If DSPTOKCD_SEARCH(strTOKCD, Mst_Inf_TOKMTA) = 0 Then
				strTOKNMA = Mst_Inf_TOKMTA.TOKNMA
				strTOKNMB = Mst_Inf_TOKMTA.TOKNMB
				strTOKZP = Mst_Inf_TOKMTA.TOKZP
				strTOKADA = Mst_Inf_TOKMTA.TOKADA
				strTOKADB = Mst_Inf_TOKMTA.TOKADB
				strTOKADC = Mst_Inf_TOKMTA.TOKADC
				strTOKTL = Mst_Inf_TOKMTA.TOKTL
				strTOKFX = Mst_Inf_TOKMTA.TOKFX
			Else
				' エラー
			End If
		Else
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strTOKCD = pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_BUMCD.Tag)).Detail.Dsp_Value
			
			'CHG START FKS)INABA 2007/06/07 ***************************************
			If DSPBMNCD_SEARCH(strTOKCD, Mst_Inf_BMNMTA, strDate) = 0 Then
				'        If DSPBMNCD_SEARCH(strTOKCD, Mst_Inf_BMNMTA) = 0 Then
				'CHG  END  FKS)INABA 2007/06/07 ***************************************
				strTOKNMA = Mst_Inf_BMNMTA.BMNNM
				strTOKNMB = ""
				strTOKZP = Mst_Inf_BMNMTA.BMNZP
				strTOKADA = Mst_Inf_BMNMTA.BMNADA
				strTOKADB = Mst_Inf_BMNMTA.BMNADB
				strTOKADC = Mst_Inf_BMNMTA.BMNADC
				strTOKTL = Mst_Inf_BMNMTA.BMNTL
				strTOKFX = Mst_Inf_BMNMTA.BMNFX
			Else
				' エラー
			End If
		End If
		...
		'納入先コード
		If Trim(strTOKCD) <> "" Then
			'    If IDOET52_SBNTRA_Inf.OUTRYKB3 = "1" Then
			'CHG  END  FKS)INABA 2006/12/26 ******************************************
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSCD = pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSCD.Tag)).Detail.Dsp_Value
			If DSPNHSCD_SEARCH(strNHSCD, Mst_Inf_NHSMTA) = 0 Then
				strNHSNMA = Mst_Inf_NHSMTA.NHSNMA
				strNHSNMB = Mst_Inf_NHSMTA.NHSNMB
				strNHSZP = Mst_Inf_NHSMTA.NHSZP
				strNHSADA = Mst_Inf_NHSMTA.NHSADA
				strNHSADB = Mst_Inf_NHSMTA.NHSADB
				strNHSADC = Mst_Inf_NHSMTA.NHSADC
				strNHSTL = Mst_Inf_NHSMTA.NHSTL
				strNHSFX = Mst_Inf_NHSMTA.NHSFX
			Else
				' エラー
			End If
			
			'ADD START FKS)INABA 2006/12/26 ************************************************************************
			'納入先郵便番号、納入先住所１、２、３、納入先電話番号、納入先ＦＡＸ番号は
			'画面よりセットする。
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSNMA = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSNMA.Tag)).Detail.Dsp_Value, 20)
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSNMB = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSNMB.Tag)).Detail.Dsp_Value, 20)
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSZP = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSZIPCD.Tag)).Detail.Dsp_Value, 20)
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSADA = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSADA.Tag)).Detail.Dsp_Value, 60)
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSADB = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSADB.Tag)).Detail.Dsp_Value, 60)
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSADC = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSADC.Tag)).Detail.Dsp_Value, 60)
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSTL = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSTL.Tag)).Detail.Dsp_Value, 20)
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSFX = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_NHSFAX.Tag)).Detail.Dsp_Value, 20)
			'ADD  END  FKS)INABA 2006/12/26 ************************************************************************
		Else
			'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			strNHSCD = pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_BUMCD.Tag)).Detail.Dsp_Value
			
			'CHG START FKS)INABA 2007/06/07 ***************************************
			If DSPBMNCD_SEARCH(strNHSCD, Mst_Inf_BMNMTA, strDate) = 0 Then
				'        If DSPBMNCD_SEARCH(strNHSCD, Mst_Inf_BMNMTA) = 0 Then
				'CHG  END  FKS)INABA 2007/06/07 ***************************************
				strNHSNMA = Mst_Inf_BMNMTA.BMNNM
				strNHSNMB = ""
				strNHSZP = Mst_Inf_BMNMTA.BMNZP
				strNHSADA = Mst_Inf_BMNMTA.BMNADA
				strNHSADB = Mst_Inf_BMNMTA.BMNADB
				strNHSADC = Mst_Inf_BMNMTA.BMNADC
				strNHSTL = Mst_Inf_BMNMTA.BMNTL
				strNHSFX = Mst_Inf_BMNMTA.BMNFX
			Else
				' エラー
			End If
		End If
		...
		'出荷指示数量を変数にセット
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		ll_su = CF_Ora_Number(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value)
		'ADD  END  FKS)INABA 2006/12/02 **********************
		...
		strSQL = strSQL & "        , CANKB " '取消区分
		strSQL = strSQL & "        , WRKKB " '処理区分
		' 6
		strSQL = strSQL & "        , FDNNO " '納品書No
		strSQL = strSQL & "        , SHFDNNO " '納品書No（表示用）
		strSQL = strSQL & "        , LINNO " '納品書行No
		strSQL = strSQL & "        , INVNO " 'インボイスNo
		...
		strSQL = strSQL & "        , JDNNO " '受注No
		strSQL = strSQL & "        , JDNLINNO " '受注行番号
		strSQL = strSQL & "        , HINCD " '製品コード
		' 21
		strSQL = strSQL & "        , HINNMA " '型式
		strSQL = strSQL & "        , HINNMB " '商品名1
		strSQL = strSQL & "        , JANCD " 'JANコード
		strSQL = strSQL & "        , LOTNO " 'ロットNo
		strSQL = strSQL & "        , FRDYTSU " '出荷予定数
		' 26
		strSQL = strSQL & "        , HIKSU " '引当数
		strSQL = strSQL & "        , FRDSU " '出荷指示数量
		...
		strSQL = strSQL & "        , NHSTL " '納入先TEL
		' 56
		strSQL = strSQL & "        , NHSFX " '納入先FAX
		...
		strSQL = strSQL & "        , BMNTL " '出荷元TEL
		strSQL = strSQL & "        , BMNFX " '出荷元FAX
		strSQL = strSQL & "        , BMNURL " '出荷元URL
		...
		strSQL = strSQL & "        , FDNZMIFL " '出荷済フラグ
		' 71
		strSQL = strSQL & "        , BKTHKKB " '分割不可区分
		strSQL = strSQL & "        , SYKDATNO " '出荷予定管理No
		'CHG START FKS)INABA 2006/11/20 *******************************************************
		strSQL = strSQL & "        , DUMMY1 " 'ダミー１
		strSQL = strSQL & "        , DUMMY2 " 'ダミー２
		strSQL = strSQL & "        , FOPEID " '初回登録ユーザID
		strSQL = strSQL & "        , FCLTID " '初回登録クライアントID
		strSQL = strSQL & "        , WRTFSTTM " 'タイムスタンプ（登録時間)
		strSQL = strSQL & "        , WRTFSTDT " 'タイムスタンプ（登録日付)
		...
		strSQL = strSQL & "        (  '" & CF_Ora_String(pin_strDatNo, 10) & "' "
		strSQL = strSQL & "        ,  '" & CF_Ora_String(gc_strDATKB_USE, 1) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("1", 1) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("0", 1) & "'"
		'CHG START FKS)INABA 2006/12/02 *******************************************************
		'出荷指示数量がマイナスの場合は処理区分はゼロ
		If ll_su < 0 Then
			strSQL = strSQL & "        ,  '" & CF_Ora_String("0", 1) & "'"
		Else
			strSQL = strSQL & "        ,  '" & CF_Ora_String("5", 1) & "'"
		End If
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String("5", 1) & "'"
		'CHG  END  FKS)INABA 2006/12/02 *******************************************************
		' 6
		strSQL = strSQL & "        ,  '" & CF_Ora_String(pin_strFDNNO, 8) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("NH" & Mid(pin_strFDNNO, 2), 9) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("001", 3) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 8) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(pin_strRecNo, 10) & "'"
		' 11
		'CHG START FKS)INABA 2007/05/08 *******************************************************
		strSQL = strSQL & "        ,  '" & LTrim(CF_Ora_String(strSBNNO, 20)) & "' "
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String(strSBNNO, 20) & "' "
		'CHG  END  FKS)INABA 2007/05/08 *******************************************************
		'CHG START FKS)INABA 2007/10/26 **************************************************************
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSQL = strSQL & "        ,  '" & CF_Ora_String(IIf(Trim(CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_BINCD.Tag)).Detail.Dsp_Value, 2)) = "", "99", CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_BINCD.Tag)).Detail.Dsp_Value, 2)), 2) & "' "
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String("", 2) & "'"
		'CHG  END  FKS)INABA 2007/10/26 **************************************************************
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strOUTBSCD, 3) & "'"
		strSQL = strSQL & "        ,  '" & strSOUCD & "' "
		...
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String("", 8) & "'"
		'CHG  END  FKS)INABA 2007/06/07 *******************************************************
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 10) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 3) & "'"
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSQL = strSQL & "        ,  '" & CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_HINCD(1).Tag)).Detail.Dsp_Value, 10) & "' "
		' 21
		'CHG START FKS)INABA 2008/02/19 ***********************************************************************************************
		...
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.HINNMB, 50) & "' "
		'CHG  END  FKS)INABA 2008/02/19 ***********************************************************************************************
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strJANCD, 13) & "' "
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 20) & "'"
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSQL = strSQL & "        ,   " & CF_Ora_Number(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value)
		' 26
		...
		strSQL = strSQL & "        ,  0 "
		strSQL = strSQL & "        ,  0 "
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 23) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 3) & "'"
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSQL = strSQL & "        ,   '" & CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_LINCMA(1).Tag)).Detail.Dsp_Value, 20) & "'"
		' 36
		...
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 3) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 3) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strTOKCD, 10) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strTOKNMA, 60) & "'"
		' 41
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strTOKNMB, 60) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strTOKZP, 20) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strTOKADA, 60) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strTOKADB, 60) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strTOKADC, 60) & "'"
		' 46
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strTOKTL, 20) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strTOKFX, 20) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strNHSCD, 10) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strNHSNMA, 60) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strNHSNMB, 60) & "'"
		' 51
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strNHSZP, 20) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strNHSADA, 60) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strNHSADB, 60) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strNHSADC, 60) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strNHSTL, 20) & "'"
		' 56
		strSQL = strSQL & "        ,  '" & CF_Ora_String(strNHSFX, 20) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 40) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 20) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 60) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 60) & "'"
		' 61
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 60) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 20) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 20) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 50) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 40) & "'"
		' 66
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 40) & "'"
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 40) & "'"
		...
		strSQL = strSQL & "        ,  '" & CF_Ora_String("", 10) & "'"
		'CHG START FKS)INABA 2006/12/02 ****************************************
		'出荷指示数量がマイナスの場合は出荷済フラグは3
		If ll_su < 0 Then
			strSQL = strSQL & "        ,  '" & CF_Ora_String("3", 1) & "'"
		Else
			strSQL = strSQL & "        ,  '" & CF_Ora_String("1", 1) & "'"
		End If
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String("1", 1) & "'"
		...
		' 71
		strSQL = strSQL & "        ,  '" & CF_Ora_String("1", 1) & "'"
		'CHG START FKS)INABA 2007/12/07 *******************************************
		If IDOET52_SBNTRA_Inf.KKOUT = CDbl("1") Then
			strSQL = strSQL & "        ,  '" & CF_Ora_String(pin_strDatNo, 10) & "' "
		Else
			strSQL = strSQL & "        ,  '" & CF_Ora_String("", 10) & "'"
		End If
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String("", 10) & "'"
		'CHG  END  FKS)INABA 2007/12/07 *******************************************
		'CHG START FKS)INABA 2006/11/20 ************************************************************************
		...
		strSQL = strSQL & "        ,  '" & SSS_PrgId & "' " '更新PG
		'CHG START FKS)INABA 2007/06/07 ************************************************************
		strSQL = strSQL & "        ,  '" & CF_Ora_String("2", 1) & "' " '削除フラグ
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String("", 1) & "' "          '削除フラグ
		'CHG START FKS)INABA 2007/06/07 ************************************************************
		...
		strSQL = strSQL & "        , CASEMKA  " 'ケースマーク1
		strSQL = strSQL & "        , CASEMKB  " 'ケースマーク2
		' 51
		strSQL = strSQL & "        , CASEMKC  " 'ケースマーク3
		strSQL = strSQL & "        , CASEMKD  " 'ケースマーク4
		strSQL = strSQL & "        , CASEMKE  " 'ケースマーク5
		strSQL = strSQL & "        , RELFL  " '連携フラグ
		'CHG START FKS)INABA 2006/11/20 *******************************************************
		strSQL = strSQL & "        , FOPEID " '初回登録ユーザID
		strSQL = strSQL & "        , FCLTID " '初回登録クライアントID
		...
		strSQL = strSQL & "        ,  '" & GV_SysDate & "' " 'タイムスタンプ(日付)
		strSQL = strSQL & "        ,  '" & SSS_PrgId & "' " '更新PG
		'CHG STRAT FKS)INABA 2007/06/07 ******************************************************************
		strSQL = strSQL & "        ,  '" & CF_Ora_String("2", 1) & "' " '削除フラグ
		'    strSQL = strSQL & "        ,  '" & CF_Ora_String("", 1) & "' "          '削除フラグ
		'CHG  END  FKS)INABA 2007/06/07 ******************************************************************
		...
		'ADD START FKS)INABA 2007/08/02 ***********************************************************************************
		'緊急出荷でシリアル管理製品の場合、シリアルロケーションテーブル、シリアル管理テーブル、シリアル履歴テーブル更新
		Dim intData As Short
		'UPGRADE_WARNING: 構造体 Usr_Ody の配列は、使用する前に初期化する必要があります。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="814DF224-76BD-4BB4-BFFB-EA359CB9FC48"' をクリックしてください。
		Dim Usr_Ody As U_Ody
		Dim strSQL2 As String
		'CHG START FKS)INABA 2007/12/18 ********************************************
		Dim wk_SRANO As String
		...
		If DSPHINCD_SEARCH(strHINCD, Mst_Inf_HINMTA) = 0 Then
			...
			If IDOET52_SBNTRA_Inf.KKOUT = 1 Then
				...
				'CHG START FKS)INABA 2007/12/18 ********************************************
				strSQL = " SELECT  "
				strSQL = strSQL & " RPTCLTID "
				strSQL = strSQL & ",PGID  "
				strSQL = strSQL & ",SBNNO "
				strSQL = strSQL & ",HINCD "
				strSQL = strSQL & ",SUM(URISU) URISU_SUM "
				strSQL = strSQL & ",SRANO "
				strSQL = strSQL & ",LOCATION "
				strSQL = strSQL & " FROM SRAET61 "
				strSQL = strSQL & " WHERE RPTCLTID = '" & SSS_CLTID.Value & "'"
				strSQL = strSQL & "   AND PGID     = '" & SSS_PrgId & "'"
				strSQL = strSQL & "   AND SBNNO    = '" & strSBNNO & "'"
				strSQL = strSQL & "   AND HINCD    = '" & strHINCD & "'"
                ...
                For Each row As DataRow In dt.Rows
                    'change end 20190909 kuwa
                    'シリアルロケーションテーブルに追加する。
                    strSQL2 = "INSERT INTO SRLTRA VALUES ("
                    strSQL2 = strSQL2 & "  '1' " '伝票削除区分
                    strSQL2 = strSQL2 & " ,'0' " '取消区分
                    strSQL2 = strSQL2 & " ,'" & CF_Ora_String(pin_strRecNo, 10) & "'" 'レコード管理№(出荷指示)
                    strSQL2 = strSQL2 & " ,' ' " 'レコード管理№(売上)
                    strSQL2 = strSQL2 & " ,'5' " '処理区分
                    strSQL2 = strSQL2 & " ,'" & CF_Ora_String(pin_strFDNNO, 8) & "'" '納品書№
                    strSQL2 = strSQL2 & " ,'" & CF_Ora_String("001", 3) & "'" '納品書行№
                    strSQL2 = strSQL2 & " ,'" & strDate & "'" '出荷予定日
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'change start 20190909 kuwa
                    '               strSQL2 = strSQL2 & " ,'" & CF_Ora_GetDyn(Usr_Ody, "SRANO", "") & "'" 'シリアル№
                    '               'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    '               strSQL2 = strSQL2 & " ,'" & CF_Ora_GetDyn(Usr_Ody, "LOCATION", "") & "'" 'ロケーション
                    ''CHG START FKS)INABA 2007/12/18 ********************************************
                    ''UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'strSQL2 = strSQL2 & " ,'" & CF_Ora_GetDyn(Usr_Ody, "URISU_SUM", 0) & "'" '数量

                    strSQL2 = strSQL2 & " ,'" & DB_NullReplace(row("SRANO"), "") & "'" 'シリアル№
                    strSQL2 = strSQL2 & " ,'" & DB_NullReplace(row("LOCATION"), "") & "'" 'ロケーション
                    strSQL2 = strSQL2 & " ,'" & DB_NullReplace(row("URISU_SUM"), 0) & "'" '数量
                    'change end 20190909 kuwa
                    '                strSQL2 = strSQL2 & " ,1 "                                           '数量
                    'CHG  END  FKS)INABA 2007/12/18 ********************************************
                    strSQL2 = strSQL2 & " ,' '" '連携フラグ
                    strSQL2 = strSQL2 & " ,'" & CF_Ora_String(SSS_OPEID.Value, 8) & "' " '初回登録ユーザID
                    strSQL2 = strSQL2 & " ,'" & CF_Ora_String(SSS_CLTID.Value, 5) & "' " '初回登録クライアントID
                    strSQL2 = strSQL2 & " ,'" & GV_SysTime & "' " 'タイムスタンプ（登録時間)
                    strSQL2 = strSQL2 & " ,'" & GV_SysDate & "' " 'タイムスタンプ（登録日付)
                    strSQL2 = strSQL2 & " ,'" & CF_Ora_String(SSS_OPEID.Value, 8) & "' " 'ユーザID(訂正)
                    strSQL2 = strSQL2 & " ,'" & CF_Ora_String(SSS_CLTID.Value, 5) & "' " 'クライアントID(訂正)
                    strSQL2 = strSQL2 & " ,'" & GV_SysTime & "' " 'タイムスタンプ(訂正時間)
                    strSQL2 = strSQL2 & " ,'" & GV_SysDate & "' " 'タイムスタンプ(訂正日付)
                    strSQL2 = strSQL2 & " ,'" & CF_Ora_String(SSS_OPEID.Value, 8) & "' " 'ユーザID(バッチ)
                    strSQL2 = strSQL2 & " ,'" & CF_Ora_String(SSS_CLTID.Value, 5) & "' " 'クライアントID(バッチ)
                    strSQL2 = strSQL2 & " ,'" & GV_SysTime & "' " 'タイムスタンプ(時間)
                    strSQL2 = strSQL2 & " ,'" & GV_SysDate & "' " 'タイムスタンプ(日付)
                    strSQL2 = strSQL2 & " ,'" & SSS_PrgId & "' " '更新PG
                    strSQL2 = strSQL2 & ")"
                    bolRet = CF_Ora_Execute(gv_Odb_USR1, strSQL2)
                    Debug.Print(strSQL2)
                    If bolRet = False Then
                        GoTo F_FDNTRA_Insert_err
                    End If
                    ...
                    'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                    'change start 20190909 kuwa
                    'wk_SRANO = CF_Ora_GetDyn(Usr_Ody, "SRANO", "")
                    wk_SRANO = DB_NullReplace(row("SRANO"), "")
                    'change end 20190909 kuwa
                    If Trim(wk_SRANO) <> "" Then
                        'CHG  END  FKS)INABA 2007/12/18 ********************************************
                        'シリアル履歴テーブルを更新
                        strSQL2 = "INSERT INTO SRARSTTB "
                        strSQL2 = strSQL2 & " SELECT "
                        strSQL2 = strSQL2 & "         SRANO    "
                        strSQL2 = strSQL2 & "        ,LOTNO    "
                        strSQL2 = strSQL2 & "        ,RSTDT    "
                        strSQL2 = strSQL2 & "        ,RSTTM    "
                        strSQL2 = strSQL2 & "        ,HINCD    "
                        strSQL2 = strSQL2 & "        ,SBNNO    "
                        strSQL2 = strSQL2 & "        ,PUDLNO   "
                        strSQL2 = strSQL2 & "        ,SZTNM    "
                        strSQL2 = strSQL2 & "        ,SZTNM    "
                        strSQL2 = strSQL2 & "        ,ZAISYOBN "
                        strSQL2 = strSQL2 & "        ,RELFL    "
                        strSQL2 = strSQL2 & "        ,FOPEID   "
                        strSQL2 = strSQL2 & "        ,FCLTID   "
                        strSQL2 = strSQL2 & "        ,WRTFSTTM "
                        strSQL2 = strSQL2 & "        ,WRTFSTDT "
                        strSQL2 = strSQL2 & "        ,OPEID    "
                        strSQL2 = strSQL2 & "        ,CLTID    "
                        strSQL2 = strSQL2 & "        ,WRTTM    "
                        strSQL2 = strSQL2 & "        ,WRTDT    "
                        strSQL2 = strSQL2 & "        ,UOPEID   "
                        strSQL2 = strSQL2 & "        ,UCLTID   "
                        strSQL2 = strSQL2 & "        ,UWRTTM   "
                        strSQL2 = strSQL2 & "        ,UWRTDT   "
                        strSQL2 = strSQL2 & "        ,PGID     "
                        strSQL2 = strSQL2 & "  FROM  SRACNTTB  "
                        'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                        'change start 20190909 kuwa
                        'strSQL2 = strSQL2 & "  WHERE SRANO = '" & CF_Ora_GetDyn(Usr_Ody, "SRANO", "") & "'"
                        strSQL2 = strSQL2 & "  WHERE SRANO = '" & DB_NullReplace(row("SRANO"), "") & "'"
                        'change end 20190909 kuwa
                        bolRet = CF_Ora_Execute(gv_Odb_USR1, strSQL2)
                        If bolRet = False Then
                            GoTo F_FDNTRA_Insert_err
                        ...
                        'シリアル管理テーブルを更新
                        strSQL2 = "UPDATE  SRACNTTB SET "
                        'ADD START FKS)INABA 2008/07/28 **********************************************************************************
                        strSQL2 = strSQL2 & "       SBNNO  = '" & LTrim(strSBNNO) & "' " '製番
                        strSQL2 = strSQL2 & "      ,PUDLNO = '" & Trim(pin_strPUDLNO) & "' " '入出庫番号
                        strSQL2 = strSQL2 & "      ,WRTDT = '" & Trim(GV_SysDate) & "' " '更新日
                        strSQL2 = strSQL2 & "      ,WRTTM = '" & Trim(GV_SysTime) & "' " '更新時間
                        strSQL2 = strSQL2 & "      ,PGID = '" & Trim(SSS_PrgId) & "' " 'プログラムＩＤ
                        'ADD  END  FKS)INABA 2008/07/28 **********************************************************************************
                        strSQL2 = strSQL2 & "      ,RSTDT = '" & Trim(GV_SysDate) & "' " '実績日
                        strSQL2 = strSQL2 & "      ,RSTTM = '" & Trim(GV_SysTime) & "' " '時間
                        strSQL2 = strSQL2 & "      ,SZTNM = '" & Trim(CF_Ora_String(strNHSCD, 10)) & "'" '納入先コード
                        strSQL2 = strSQL2 & "      ,ZAISYOBN = '02'" '在庫処理分類
                        'UPGRADE_WARNING: オブジェクト CF_Ora_GetDyn() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                        'change start 20190909 kuwa
                        'strSQL2 = strSQL2 & " WHERE SRANO = '" & CF_Ora_GetDyn(Usr_Ody, "SRANO", "") & "'"
                        strSQL2 = strSQL2 & " WHERE SRANO = '" & DB_NullReplace(row("SRANO"), "") & "'"
		...
		Else
			GoTo F_FDNTRA_Insert_err
		End If
		'上記処理を行なったデータをワークより削除
		strSQL = " DELETE "
		strSQL = strSQL & " FROM SRAET61 "
		strSQL = strSQL & " WHERE RPTCLTID = '" & SSS_CLTID.Value & "'"
        strSQL = strSQL & "   AND PGID     = '" & SSS_PrgId & "'"
        'add start 20190909 kuwa 
        CON_USR9 = DB_START_USR9() '接続先がCNT_USR1にしか接続できないため、接続先を変更
        Call DB_Execute(strSQL, 0, CON_USR9)
        'add end 20190909 kuwa
        'delete test 20190909 kuwa
        'bolRet = CF_Ora_Execute(gv_Odb_USR9, strSQL)
        'If bolRet = False Then
        '      GoTo F_FDNTRA_Insert_err
        '       End If
        'delete end 2090909 kuwa
...
F_FDNTRA_Insert_err: 
	...
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_HINMTB_Update(ByRef pm_All As Cls_All, ByRef ps_PUDLNO As String) As Short
		Dim strSQL As String
		Dim bolRet As Boolean
		Dim intRet As Short
		'ADD START FKS)INABA 2006/11/30 ***********************************
		...
		LS_HINCD = CF_Ora_String(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.HINCD, 10)
		strSQL = ""
		...
		If ls_RowID = "" Then
			...
			strSQL = strSQL & ", " & CF_Ora_Number(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UODSU) '24　引当数
			...
			strSQL = strSQL & ", 0 " '32　第1回入庫日
			strSQL = strSQL & ", '" & GV_SysDate & "'" '33　第1回出庫日
			strSQL = strSQL & ", ' '" '34　最新入庫日
			strSQL = strSQL & ", '" & GV_SysDate & "'" '35　最新出庫日
		...
		Else
			...
			strSQL = strSQL & " set HIKSU = HIKSU + " & CF_Ora_Number(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.UODSU)
		...
		'    strSQL = strSQL & "   , WRTDT = '" & GV_SysDate & "'"
		'    strSQL = strSQL & " where DATKB = '1'"
		'    strSQL = strSQL & "   and SOUCD = '" & CF_Ora_String(pm_All.Dsp_Sub_Inf(FR_SSSMAIN.HD_SOUCD.Tag).Detail.Dsp_Value, 3) & "'"
		'    strSQL = strSQL & "   and HINCD = '" & CF_Ora_String(pm_All.Dsp_Body_Inf.Row_Inf(1).Bus_Inf.HINCD, 10) & "'"
		'
		'    'SQL実行(FDNTRA)
		'    bolRet = CF_Ora_Execute(gv_Odb_USR1, strSQL)
		'    If bolRet = False Then
		...
		'CHG  END  FKS)INABA 2006/11/30 ***********************************
		''ADD START FKS)INABA 2008/07/30 ***********************************
		'MAKE途中
		'        strSQL = ""
		'        strSQL = strSQL & " INSERT INTO DTLTRA VALUES("
		'        strSQL = strSQL & " '4'"   '1トラン種別
		'        strSQL = strSQL & ",'" & Trim$(pm_All.Dsp_Sub_Inf(FR_SSSMAIN.HD_SBNNO.Tag).Detail.Dsp_Value) & "'"   '2トラン番号
		'        strSQL = strSQL & ",' '"   '3版数
		'        strSQL = strSQL & " ' '"   '4行番号
		'        strSQL = strSQL & ",'" & ps_PUDLNO & "'"  '5入出庫番号
		'        strSQL = strSQL & ",'" & CF_Ora_String(pm_All.Dsp_Sub_Inf(FR_SSSMAIN.HD_DENDT.Tag).Detail.Dsp_Value, 3) & "'"  '6トラン日付
		'        strSQL = strSQL & ",'00001'"   '7引当番号
		'        strSQL = strSQL & ",'M'"   '8自動手動引当区分
		'        strSQL = strSQL & ",'" & CF_Ora_String(pm_All.Dsp_Sub_Inf(FR_SSSMAIN.BD_HINCD(1).Tag).Detail.Dsp_Value, 10) & "' "   '9製品コード
		'        strSQL = strSQL & ",' '"   '10入庫予定日
		'        strSQL = strSQL & ",' ' "   '11ロット番号
		'        strSQL = strSQL & ",'" & CF_Ora_String(pm_All.Dsp_Sub_Inf(FR_SSSMAIN.HD_SOUCD.Tag).Detail.Dsp_Value, 3) & "'" '12倉庫コード
		'        strSQL = strSQL & ",'0'"   '13資産元区分
		'        strSQL = strSQL & " "   '14取引先コード
		'        strSQL = strSQL & " "   '15倉庫区分
		'        strSQL = strSQL & " "   '16引当数
		'        strSQL = strSQL & " "   '17最終作業者コード
		'        strSQL = strSQL & " "   '18クライアントＩＤ
		'        strSQL = strSQL & " "   '19タイムスタンプ（時間）
		'        strSQL = strSQL & " "   '20タイムスタンプ（日付）
		'        strSQL = strSQL & " "   '21タイムスタンプ（登録時間）
		'        strSQL = strSQL & " "   '22タイムスタンプ（登録日）
		'        strSQL = strSQL & " )"
		'
		''ADD  END  FKS)INABA 2008/07/30 ***********************************
...
F_HINMTB_Update_err: 
	...
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_DTLTRA_Insert(ByRef pm_All As Cls_All, ByRef ps_PUDLNO As String) As Short
		
		Dim strSQL As String
		Dim bolRet As Boolean
		
		''引当内訳ファイル(DTLTRA)用変数
		Dim strTRAKB As String '1. トラン種別
		Dim strTRANO As String '2. トラン番号
		Dim strMITNOV As String '3. 版数
		Dim strLINNO As String '4. 行番号
		Dim strPUDLNO As String '5. 入出庫番号
		Dim strTRADT As String '6. トラン日付
		Dim strHIKNO As String '7. 引当番号
		Dim strATMNKB As String '8. 自動手動引当区分
		Dim strHINCD As String '9. 製品コード
		Dim strINPYTDT As String '10. 入庫予定日
		Dim strLOTNO As String '11. ロット番号
		Dim strSOUCD As String '12. 倉庫コード
		'未使用
		''    Dim strSISNKIB      As String   '13. 資源元区分
		''    Dim strSOURTICD     As String   '14. 取引先コード
		''    Dim strSOUKOKB      As String   '15. 倉庫区分
		Dim lngHIKSU As Integer '16: 引当数
		...
		'2. トラン番号(製番)
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strTRANO = pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_SBNNO.Tag)).Detail.Dsp_Value
		...
		'6. トラン日付
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strTRADT = VB6.Format(Trim(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_DENDT.Tag)).Detail.Dsp_Value), "YYYYMMDD")
		...
		'12. 倉庫コード
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		strSOUCD = CF_Ora_String(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.HD_SOUCD.Tag)).Detail.Dsp_Value, 3)
		
		'16. 引当数
		'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
		lngHIKSU = CF_Ora_Number(pm_All.Dsp_Sub_Inf(CInt(FR_SSSMAIN.BD_UODSU(1).Tag)).Detail.Dsp_Value)
		...
		strSQL = strSQL & "  SELECT "
		strSQL = strSQL & "     '" & Trim(strTRAKB) & "' " '1
		strSQL = strSQL & "   , '" & Trim(strTRANO) & "' " '2
		strSQL = strSQL & "   , '" & strMITNOV & "' " '3
		strSQL = strSQL & "   , '" & strLINNO & "' " '4
		strSQL = strSQL & "   , '" & strPUDLNO & "' " '5
		strSQL = strSQL & "   , '" & Trim(strTRADT) & "' " '6
		strSQL = strSQL & "   , '" & Trim(strHIKNO) & "' " '7
		strSQL = strSQL & "   , '" & Trim(strATMNKB) & "' " '8
		strSQL = strSQL & "   , '" & Trim(strHINCD) & "' " '9
		strSQL = strSQL & "   , '" & strINPYTDT & "' " '10
		strSQL = strSQL & "   , '" & strLOTNO & "' " '11
		strSQL = strSQL & "   , '" & Trim(strSOUCD) & "' " '12
		strSQL = strSQL & "   , SOUMTA.SISNKB " '13
		strSQL = strSQL & "   , SOUMTA.SOUTRICD " '14
		strSQL = strSQL & "   , SOUMTA.SOUKOKB " '15
		strSQL = strSQL & "   ,  " & lngHIKSU '16
		strSQL = strSQL & "   , '" & CF_Ora_String(SSS_OPEID.Value, 8) & "' " '最終作業者ｺｰﾄﾞ
		strSQL = strSQL & "   , '" & CF_Ora_String(SSS_CLTID.Value, 5) & "' " 'ｸﾗｲｱﾝﾄID
		strSQL = strSQL & "   , '" & CF_Ora_String(GV_SysTime, 6) & "' " 'ﾀｲﾑｽﾀﾝﾌﾟ(時間)
		strSQL = strSQL & "   , '" & CF_Ora_String(GV_SysDate, 8) & "' " 'ﾀｲﾑｽﾀﾝﾌﾟ(日付)
		strSQL = strSQL & "   , '" & CF_Ora_String(GV_SysTime, 6) & "' " 'ﾀｲﾑｽﾀﾝﾌﾟ(登録時間)
		strSQL = strSQL & "   , '" & CF_Ora_String(GV_SysDate, 8) & "' " 'ﾀｲﾑｽﾀﾝﾌﾟ(登録日)
		strSQL = strSQL & "  FROM "
		strSQL = strSQL & "        SOUMTA "
		strSQL = strSQL & "  WHERE "
		strSQL = strSQL & "        SOUCD = '" & Trim(strSOUCD) & "' "
...
F_DTLTRA_Insert_err: 
		Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgIDOET52_E_034, pm_All, "F_DTLTRA_Insert")
	...
	Public Function F_Ctl_MN_APPENDC_Click(ByRef pm_All As Cls_All) As Short
        ...
        Call CF_Set_Frm_IN_TANCD_IDOET52(FR_SSSMAIN, pm_All)
        '2019/06/12 CHG END
...
F_Get_HINCD_END: 
	...
	Private Function F_DSPJDNTRA_SEARCH(ByVal pin_strJDNNO_LINNO As Object, ByRef pot_DB_JDNTRA As TYPE_DB_JDNTRA, ByRef pot_DB_JDNTHA As TYPE_DB_JDNTHA, Optional ByVal pin_strDATKB As String = gc_strDATKB_USE) As Short
		...
		strSQL = strSQL & "     AND JH.DATNO IN(SELECT MAX(DATNO) FROM JDNTHA GROUP BY JDNNO )"
...
ERR_DSPJDNTRA_SEARCH: 
	...
	Private Function F_DSPSBNTRA_SEARCH(ByVal pin_strDatNo As String, ByRef pm_All As Cls_All) As Short
        ...
        If dt Is Nothing OrElse dt.Rows.Count > 0 Then
            ...
            Dsp_Value = CF_Cnv_Dsp_Item(DB_NullReplace(dt.Rows(0)("FRDYTSU"), ""), pm_All.Dsp_Sub_Inf(Trg_Index), False)
            'change end 20190909 kuwa
            Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
            ...
            If Trim(Dsp_Value) = "1" Then
                Call CF_Set_Item_Direct(True, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
            End If
...
ERR_DSPHINMTB_SEARCH: 
	...
	Public Function F_Get_SYSTBC(ByVal Pin_strDKBSB As Object, ByRef pin_strDENNO As String) As Short
        ...
        'add test 20190909 kuwa
        Dim updSQL As String
        Dim newWRTTM As String
        Dim newWRTDT As String
        'add test 20190909 kuwa

        'delete test 20190906 kuwa Try Catchでデバッグ中のため
        ...
        '''''      'add test start 20190906  kuwa 'On Error Go To とTryは重複できないため、On Error　も一時コメントアウト
        '''''      Try
        '''''          Dim tst As Object
        '''''          tst = usrOdy
        '''''          'add test end 20190906
        '''''          usrOdy.Obj_Ody.Edit()
        '''''          'add test start 20190906 kuwa
        '''''      Catch ex As Exception
        '''''          MsgBox("フォームKeyDownエラー" & Constants.vbCrLf & ex.Message.ToString, MsgBoxStyle.Critical, "エラー")
        '''''      End Try
        ...
        'add test 20190909 kuwa obj_odyがエラーを吐くので、Update文を作成

        'Updateに必要な引数を作成
        If Trim(GV_SysTime) <> "" Then
            newWRTTM = GV_SysTime
        Else
            newWRTTM = CStr(VB6.Format(Now, "hhmmss"))
        End If
        If Trim(GV_SysDate) <> "" Then
            newWRTDT = GV_SysDate
        Else
            newWRTDT = CStr(VB6.Format(Now, "yyyymmdd"))
        End If

        'Update文を作成する
        updSQL = ""
        updSQL = updSQL & " UPDATE "
        updSQL = updSQL & "        SYSTBC "
        updSQL = updSQL & " SET "
        updSQL = updSQL & " DENNO		=	'" & strNewDENNO & "' "
        updSQL = updSQL & ",WRTTM		=	'" & newWRTTM & "' "
        updSQL = updSQL & ",WRTDT		=	'" & newWRTDT & "' "
        updSQL = updSQL & "  WHERE "
        updSQL = updSQL & "        DKBSB     = '" & Pin_strDKBSB & "' "
        DB_Execute(updSQL)
        'add test end 20190909 kuwa
...
#End If
    ...
    'add start 20190910 IDOET55と合体させる上で必要なコードをコピーしてファンクション名等を変えて貼り付け
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
    ...
    Public Function F_Ctl_Upd_Process2(ByRef pm_All As Cls_All) As Short
        ...
        Select Case intRet
            Case MsgBoxResult.Yes
                ...
                End If
                '画面内容初期化
                'delete start 20190910 kuwa IDOET55では以下がコメントアウトされてる 
                'Call SSSMAIN0001.F_Init_Clr_Dsp(-1, pm_All)
                'delete end 20190910 kuwa
...
Err_F_Ctl_Upd_Process:
    ...
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
    '   名称：  Function F_Init_BodyOnly2
    '   概要：  明細部のみ初期化制御
    '   引数：　pm_All     : 画面情報
    '   戻値：　なし
    '   備考：  全画面ローカル共通処理
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
    Public Function F_Init_BodyOnly2(ByRef pm_All As Cls_All) As Short
        ...
        'delete start 20190910 kuwa IDOET55では下記を削除
        ''出庫理由クリア
        'Trg_Index = CShort(FR_SSSMAIN.HD_OUTRYCD.Tag)
        'Call CF_Init_Clr_Dsp(pm_All.Dsp_Sub_Inf(Trg_Index), ITM_ALL_ONLY, pm_All)
        ...
        'change start 20190910 kuwa IDOET55と同じように修正
        'テイル部クリア
        'For intCnt = pm_All.Dsp_Base.Foot_Fst_Idx To pm_All.Dsp_Base.Item_Cnt
        For intCnt = pm_All.Dsp_Base.Foot_Fst_Idx + 1 To pm_All.Dsp_Base.Item_Cnt
