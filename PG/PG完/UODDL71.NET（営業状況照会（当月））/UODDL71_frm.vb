...
Friend Class FR_SSSMAIN
    ...
    '2019/03/27 ADD START saiki
    Private FORM_LOAD_FLG As Boolean = False
    '2019/03/27 ADD E N D saiki
	...
	Private Function Init_Def_Dsp() As Short
        ...
        Main_Inf.Dsp_IM_Denkyu = Me._IM_Denkyu_0
        Main_Inf.Off_IM_Denkyu = Me._IM_Denkyu_1
        Main_Inf.On_IM_Denkyu = Me._IM_Denkyu_2
		...
		With Main_Inf.Dsp_Base
			...
			.Dsp_Body_Cnt = 18 '画面表示明細数（０：明細なし、１～：表示時明細数）
		...
		Index_Wk = Index_Wk + 1
		'仕切
		BD_SIKKN(1).Tag = Index_Wk
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Ctl = BD_SIKKN(1)
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Area = IN_AREA_DSP_BD
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Typ = IN_TYP_NUM
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Str_Typ = IN_STR_TYP_KIN
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.MaxLengthB = 12
		...
		Index_Wk = Index_Wk + 1
		'売差
		BD_BAISA(1).Tag = Index_Wk
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Ctl = BD_BAISA(1)
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Area = IN_AREA_DSP_BD
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Typ = IN_TYP_NUM
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Str_Typ = IN_STR_TYP_KIN
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.MaxLengthB = 12
		...
		Index_Wk = Index_Wk + 1
		'売差率
		BD_BSART(1).Tag = Index_Wk
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Ctl = BD_BSART(1)
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Area = IN_AREA_DSP_BD
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Typ = IN_TYP_NUM
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.In_Str_Typ = IN_STR_TYP_KIN
		Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.MaxLengthB = 6
		...
		For BD_Cnt = 2 To Main_Inf.Dsp_Base.Dsp_Body_Cnt
			BD_SELECTB.Load(BD_Cnt) '選択明細オプションボタン
			BD_MEISYO.Load(BD_Cnt) '名称
			BD_UODSU.Load(BD_Cnt) '受注数
			BD_UODKN.Load(BD_Cnt) '受注金額
			BD_SIKKN.Load(BD_Cnt) '仕切
			BD_BAISA.Load(BD_Cnt) '売差
			BD_BSART.Load(BD_Cnt) '売差率
			...
			Index_Wk = Index_Wk + 1
			'仕切
			BD_SIKKN(BD_Cnt).Tag = Index_Wk
			Main_Inf.Dsp_Sub_Inf(Index_Wk).Ctl = BD_SIKKN(BD_Cnt)
			Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.Body_Index = BD_Cnt
			'明細部の１行上の情報を設定
			Call CF_Copy_Def_Dsp_Body(Index_Wk, Main_Inf.Dsp_Base.Body_Col_Cnt, Main_Inf)
			...
			Index_Wk = Index_Wk + 1
			'売差
			BD_BAISA(BD_Cnt).Tag = Index_Wk
			Main_Inf.Dsp_Sub_Inf(Index_Wk).Ctl = BD_BAISA(BD_Cnt)
			Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.Body_Index = BD_Cnt
			'明細部の１行上の情報を設定
			Call CF_Copy_Def_Dsp_Body(Index_Wk, Main_Inf.Dsp_Base.Body_Col_Cnt, Main_Inf)
			...
			Index_Wk = Index_Wk + 1
			'売差率
			BD_BSART(BD_Cnt).Tag = Index_Wk
			Main_Inf.Dsp_Sub_Inf(Index_Wk).Ctl = BD_BSART(BD_Cnt)
			Main_Inf.Dsp_Sub_Inf(Index_Wk).Detail.Body_Index = BD_Cnt
			'明細部の１行上の情報を設定
			Call CF_Copy_Def_Dsp_Body(Index_Wk, Main_Inf.Dsp_Base.Body_Col_Cnt, Main_Inf)
		Next 
	...
	Private Function Ctl_Item_Click(ByRef pm_Ctl As System.Windows.Forms.Control) As Short
        ...
        Select Case Trg_Index
            ...
            Case CShort(btnF1.Tag)
                'change 20190806 kuwahara
                Call Ctl_MN_APPENDC_Click()
                '受注／売上画面呼出
                Call F_Ctl_CS_JUC_URI_BMN(Main_Inf.Dsp_Sub_Inf(Trg_Index), Main_Inf)

                'change start 20190806 kuwahara
                'Case CShort(CS_GETU_KI.Tag)
            Case CShort(btnF11.Tag)
                'change change end 20190806 kuwahara
                Call Ctl_MN_APPENDC_Click()
                '当月／当期画面呼出
                Call F_Ctl_CS_GETU_KI_BMN(Main_Inf.Dsp_Sub_Inf(Trg_Index), Main_Inf)
            ...
            Case CShort(btnF7.Tag)
                'change 20190806 kuwahara
                '機種別総括表画面呼出
                Call F_Ctl_CS_SOUKATU_BMN(Main_Inf.Dsp_Sub_Inf(Trg_Index), Main_Inf)
            ...
            Case CShort(btnF2.Tag)
                ...
                Call Ctl_MN_APPENDC_Click()
                '再読込
                Call F_Ctl_CS_SAIYOMI_BMN(Main_Inf.Dsp_Sub_Inf(Trg_Index), Main_Inf)

            Case CShort(btnF12.Tag)
                '2019/04/04 CHG E N D
                '終了
	...
	Private Function Ctl_MN_Oprt_Click() As Short
		...
		Dim Act_Index As Short
		' 2006/11/28  ADD START  KUMEDA
		'UPGRADE_WARNING: オブジェクト Me.ActiveControl の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        'delete 20190325 START saiki
        'If Me.ActiveControl Is Nothing Or CDbl(Me.ActiveControl) = 0 Then
        '    Exit Function
        'End If
        'delete 20190325 END saiki
		' 2006/11/28  ADD END
	...
	Private Function Ctl_CM_PREV_Click(ByRef pm_Act_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Object
		...
		Dim Body_Cnt As Short
		' 2007/01/18  ADD END
		' 2007/01/21  ADD START
		Dim Bk_Kind As String
		Dim Bk_Code As String
		...
		If NowPageNum > MinPageNum Then
			...
			For Index_Cnt = pm_All.Dsp_Base.Body_Fst_Idx To pm_All.Dsp_Base.Foot_Fst_Idx - 1
				...
				' 2007/01/21  ADD START  KUMEDA   *** 明細選択チェックボックスの制御
				Bk_Kind = "99"
				Bk_Code = "Z"
				If Trim(gv_UODDL71_BMNCD) <> "" Then
					Bk_Kind = "1"
					Bk_Code = Trim(gv_UODDL71_BMNCD)
				ElseIf Trim(gv_UODDL71_TIKCD) <> "" Then 
					Bk_Kind = "2"
					Bk_Code = Trim(gv_UODDL71_TIKCD)
				ElseIf Trim(gv_UODDL71_EIGCD) <> "" Then 
					Bk_Kind = "3"
					Bk_Code = Trim(gv_UODDL71_EIGCD)
				End If
				
				For intCnt = CInt("1") To Main_Inf.Dsp_Base.Dsp_Body_Cnt * MaxPageNum
					With Main_Inf.Dsp_Body_Inf.Row_Inf(intCnt).Bus_Inf
						Div_Kind = .DIVISION
						Div_Code = .DIVCODE
						.Selected = CStr(False)
						If Div_Kind = Bk_Kind And Div_Code = Bk_Code Then
							.Selected = CStr(True)
						End If
					End With
				Next intCnt
				...
				' 2007/01/18  ADD START  KUMEDA
				For Body_Cnt = 1 To Main_Inf.Dsp_Base.Dsp_Body_Cnt
					Data_Row = (NowPageNum - 1) * Main_Inf.Dsp_Base.Dsp_Body_Cnt + Body_Cnt
                    'delete 20190325 START saiki
                    'If CBool(pm_All.Dsp_Body_Inf.Row_Inf(Data_Row).Bus_Inf.Selected) = True Then
                    '	'UPGRADE_ISSUE: Control BD_SELECTB は、汎用名前空間 Form 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
                    '	pm_All.Dsp_Base.FormCtl.BD_SELECTB(Body_Cnt).Value = True
                    'Else
                    '	'UPGRADE_ISSUE: Control BD_SELECTB は、汎用名前空間 Form 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
                    '	pm_All.Dsp_Base.FormCtl.BD_SELECTB(Body_Cnt).Value = False
                    'End If
                    'delete 20190325 END saiki
				Next Body_Cnt
				' 2007/01/18  ADD END
	...
	Private Function Ctl_CM_NEXTCM_Click(ByRef pm_Act_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Object
		...
		If NowPageNum > 0 Then
			If NowPageNum < MaxPageNum Then
				...
				' 2007/01/18  ADD START  KUMEDA
				For Index_Cnt = 1 To Main_Inf.Dsp_Base.Dsp_Body_Cnt
					Data_Row = (NowPageNum - 1) * Main_Inf.Dsp_Base.Dsp_Body_Cnt + Index_Cnt
                    'delete 20190325 START saiki
                    'If CBool(pm_All.Dsp_Body_Inf.Row_Inf(Data_Row).Bus_Inf.Selected) = True Then
                    '	'UPGRADE_ISSUE: Control BD_SELECTB は、汎用名前空間 Form 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
                    '	pm_All.Dsp_Base.FormCtl.BD_SELECTB(Index_Cnt).Value = True
                    'Else
                    '	'UPGRADE_ISSUE: Control BD_SELECTB は、汎用名前空間 Form 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
                    '	pm_All.Dsp_Base.FormCtl.BD_SELECTB(Index_Cnt).Value = False
                    'End If
                    'delete 20190325 END saiki
				Next Index_Cnt
				' 2007/01/18  ADD END
			...
			Else
				Call AE_CmnMsgLibrary(SSS_PrgNm, gc_strMsgUODDL71_E_007, pm_All)
			End If
		End If
	...
	Private Function Edi_Dsp_Def() As Short
		...
		'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
		'フォームタイトル
		Me.Text = RTrim(SSS_PrgNm) & "　－部門別総括表－"
	...
	Private Function Set_Body_Location() As Short
		...
		For Bd_Index = 2 To Main_Inf.Dsp_Base.Dsp_Body_Cnt
			'配置
			'選択明細オプションボタン
			BD_SELECTB(Bd_Index).Top = VB6.TwipsToPixelsY(BD_MEISYO_Top + BD_MEISYO_Height * (Bd_Index - 1))
			'名称
			BD_MEISYO(Bd_Index).Top = VB6.TwipsToPixelsY(BD_MEISYO_Top + BD_MEISYO_Height * (Bd_Index - 1))
			'受注数
			BD_UODSU(Bd_Index).Top = VB6.TwipsToPixelsY(BD_MEISYO_Top + BD_MEISYO_Height * (Bd_Index - 1))
			'受注金額
			BD_UODKN(Bd_Index).Top = VB6.TwipsToPixelsY(BD_MEISYO_Top + BD_MEISYO_Height * (Bd_Index - 1))
			'仕切
			BD_SIKKN(Bd_Index).Top = VB6.TwipsToPixelsY(BD_MEISYO_Top + BD_MEISYO_Height * (Bd_Index - 1))
			'売差
			BD_BAISA(Bd_Index).Top = VB6.TwipsToPixelsY(BD_MEISYO_Top + BD_MEISYO_Height * (Bd_Index - 1))
			'売差率
			BD_BSART(Bd_Index).Top = VB6.TwipsToPixelsY(BD_MEISYO_Top + BD_MEISYO_Height * (Bd_Index - 1))
			...
			'仕切
			BD_SIKKN(Bd_Index).Visible = True
			'売差
			BD_BAISA(Bd_Index).Visible = True
			'売差率
			BD_BSART(Bd_Index).Visible = True
	...
	Private Sub FR_SSSMAIN_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        ...
        If gv_bolUODDL71_Active = True Then
			' 2007/01/18  CHG START  KUMEDA
			'        'データ取得
			'        RtnCode = F_GET_BD_DATA_BMN_SOUKATU_JUC(gv_UODDL71_GETU_KI, Main_Inf)
            ...
            If gv_UODDL71_JUC_URI = "1" Then
                ...
                'UPGRADE_WARNING: オブジェクト FM_Panel3D1().Caption の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                _BD_UODSU_1.Text = "受注数"
                'UPGRADE_WARNING: オブジェクト FM_Panel3D1().Caption の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                _BD_UODKN_1.Text = "受注金額"
                'UPGRADE_WARNING: オブジェクト CS_JUC_URI.Caption の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            ...
            Else
                ...
                UODDL71_fpr = Me
                'ADD 20190404 END saiki
                RtnCode = F_GET_BD_DATA_BMN_SOUKATU_URI(gv_UODDL71_GETU_KI, Main_Inf)
            End If
			' 2007/01/18  CHG END
			
			' 2007/01/18  ADD START  KUMEDA   *** 明細選択チェックボックスの制御
			Bk_Kind = "99"
			Bk_Code = "Z"
			If Trim(gv_UODDL71_BMNCD) <> "" Then
				Bk_Kind = "1"
			...
			' 2007/01/18  ADD START  KUMEDA
			For intCnt = 1 To Main_Inf.Dsp_Base.Dsp_Body_Cnt
				Data_Row = (NowPageNum - 1) * Main_Inf.Dsp_Base.Dsp_Body_Cnt + intCnt
                'delete 20190325 START saiki
                'If CBool(Main_Inf.Dsp_Body_Inf.Row_Inf(Data_Row).Bus_Inf.Selected) = True Then
                '	'UPGRADE_ISSUE: Control BD_SELECTB は、汎用名前空間 Form 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
                '	Main_Inf.Dsp_Base.FormCtl.BD_SELECTB(intCnt).Value = True
                'Else
                '	'UPGRADE_ISSUE: Control BD_SELECTB は、汎用名前空間 Form 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
                '	Main_Inf.Dsp_Base.FormCtl.BD_SELECTB(intCnt).Value = False
                'End If
                'delete 20190325 END saiki
			Next intCnt
			' 2007/01/18  ADD END
			
			' 2007/01/18  DLT START  KUMEDA
			''' 2007/01/16  ADD START  KUMEDA
			''        '年月の設定
			''        Wk_NENGETU = F_GET_FIRSTDAY(gv_UODDL71_GETU_KI, GV_UNYDate)
			''        If gv_UODDL71_GETU_KI = "1" Then
		...
		Else
			'明細ページ数 設定
			MinPageNum = 1
			MaxPageNum = UBound(Main_Inf.Dsp_Body_Inf.Row_Inf) / Main_Inf.Dsp_Base.Dsp_Body_Cnt
			NowPageNum = (Main_Inf.Dsp_Body_Inf.Cur_Top_Index - 1) / Main_Inf.Dsp_Base.Dsp_Body_Cnt + 1

            '受注／売上 設定
            'UPGRADE_WARNING: オブジェクト CS_JUC_URI.Caption の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'change 20190325 START saiki
            'If CS_JUC_URI.Caption = "受　注" Then
            'change start 20190805 kuwahara
            'If btnF6.Text = "(F6)" & vbCrLf & "受　注" Then
            If Judge1 = 0 Then
            ...
            'UPGRADE_WARNING: オブジェクト CS_GETU_KI.Caption の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            'change 20190325 START saiki
            'If CS_GETU_KI.Caption = "当　月" Then
            'change start 20190805 kuwahara
            'If btnF7.Text = "(F7)" & vbCrLf & "当　月" Then
            If Judge2 = 0 Then
                'change end 20190805 kuwahara
                'change 20190328 END saiki
            ...
            Div_Kind = ""
            Div_Code = ""
            For Row_Cnt = 1 To Main_Inf.Dsp_Base.Dsp_Body_Cnt
                'オプションボタンが選択されている場合
                'UPGRADE_ISSUE: Control BD_SELECTB は、汎用名前空間 Form 内にあるため、解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="084D22AD-ECB1-400F-B4C7-418ECEC5E36E"' をクリックしてください。
                'delete 20190325 START saiki
                'If Main_Inf.Dsp_Base.FormCtl.BD_SELECTB(Row_Cnt).Value = True Then
                '    'Dsp_Body_Inf.Row_Infの行ＮＯへ変換
                '    Row_Index = Main_Inf.Dsp_Body_Inf.Cur_Top_Index + Row_Cnt - 1

                '    '選択データのコードを取得
                '    With Main_Inf.Dsp_Body_Inf.Row_Inf(Row_Index).Bus_Inf
                '        Div_Kind = .DIVISION
                '        Div_Code = .DIVCODE
                '    End With
            ...
            Select Case Div_Kind
                Case "1"
                    gv_UODDL71_BMNCD = Div_Code '部門コード
                    gv_UODDL71_TIKCD = ""
                    gv_UODDL71_EIGCD = ""
                Case "2"
                    gv_UODDL71_BMNCD = ""
                    gv_UODDL71_TIKCD = Div_Code '地区区分
                    gv_UODDL71_EIGCD = ""
                Case "3"
                    gv_UODDL71_BMNCD = ""
                    gv_UODDL71_TIKCD = ""
                    gv_UODDL71_EIGCD = Div_Code '営業所コード
            End Select
    ...
    Private Sub FR_SSSMAIN_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        '終了処理実行制御
        gv_bolUODDL71_EndFlg = False

        '変数初期化
        gv_UODDL71_JUC_URI = "1" '受注
        gv_UODDL71_GETU_KI = "1" '当月
    ...
    Private Sub btnF8_GotFocus()
        Debug.Print("btnF8_GotFocus")
	...
	Private Sub BD_SIKKN_MouseDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles BD_SIKKN.MouseDown
		Dim Button As Short = eventArgs.Button \ &H100000
		Dim Shift As Short = System.Windows.Forms.Control.ModifierKeys \ &H10000
		Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
		Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
		Dim Index As Short = BD_SIKKN.GetIndex(eventSender)
		Debug.Print("BD_SIKKN_MouseDown")
		Call Ctl_Item_MouseDown(BD_SIKKN(Index), Button, Shift, X, Y)
	End Sub
	
	Private Sub BD_BAISA_MouseDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles BD_BAISA.MouseDown
		Dim Button As Short = eventArgs.Button \ &H100000
		Dim Shift As Short = System.Windows.Forms.Control.ModifierKeys \ &H10000
		Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
		Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
		Dim Index As Short = BD_BAISA.GetIndex(eventSender)
		Debug.Print("BD_BAISA_MouseDown")
		Call Ctl_Item_MouseDown(BD_BAISA(Index), Button, Shift, X, Y)
	End Sub
	
	Private Sub BD_BSART_MouseDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles BD_BSART.MouseDown
		Dim Button As Short = eventArgs.Button \ &H100000
		Dim Shift As Short = System.Windows.Forms.Control.ModifierKeys \ &H10000
		Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
		Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
		Dim Index As Short = BD_BSART.GetIndex(eventSender)
		Debug.Print("BD_BSART_MouseDown")
		Call Ctl_Item_MouseDown(BD_BSART(Index), Button, Shift, X, Y)
	End Sub
	...
	Private Sub BD_SIKKN_MouseUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles BD_SIKKN.MouseUp
		Dim Button As Short = eventArgs.Button \ &H100000
		Dim Shift As Short = System.Windows.Forms.Control.ModifierKeys \ &H10000
		Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
		Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
		Dim Index As Short = BD_SIKKN.GetIndex(eventSender)
		Debug.Print("BD_SIKKN_MouseUp")
		Call Ctl_Item_MouseUp(BD_SIKKN(Index), Button, Shift, X, Y)
	End Sub
	
	Private Sub BD_BAISA_MouseUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles BD_BAISA.MouseUp
		Dim Button As Short = eventArgs.Button \ &H100000
		Dim Shift As Short = System.Windows.Forms.Control.ModifierKeys \ &H10000
		Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
		Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
		Dim Index As Short = BD_BAISA.GetIndex(eventSender)
		Debug.Print("BD_BAISA_MouseUp")
		Call Ctl_Item_MouseUp(BD_BAISA(Index), Button, Shift, X, Y)
	End Sub
	
	Private Sub BD_BSART_MouseUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles BD_BSART.MouseUp
		Dim Button As Short = eventArgs.Button \ &H100000
		Dim Shift As Short = System.Windows.Forms.Control.ModifierKeys \ &H10000
		Dim X As Single = VB6.PixelsToTwipsX(eventArgs.X)
		Dim Y As Single = VB6.PixelsToTwipsY(eventArgs.Y)
		Dim Index As Short = BD_BSART.GetIndex(eventSender)
		Debug.Print("BD_BSART_MouseUp")
		Call Ctl_Item_MouseUp(BD_BSART(Index), Button, Shift, X, Y)
	End Sub
	...
	Private Sub BD_SIKKN_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles BD_SIKKN.KeyDown
		Dim KEYCODE As Short = eventArgs.KeyCode
		Dim Shift As Short = eventArgs.KeyData \ &H10000
		Dim Index As Short = BD_SIKKN.GetIndex(eventSender)
		Debug.Print("BD_SIKKN_KeyDown")
		Call Ctl_Item_KeyDown(BD_SIKKN(Index), KEYCODE, Shift)
	End Sub
	
	Private Sub BD_BAISA_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles BD_BAISA.KeyDown
		Dim KEYCODE As Short = eventArgs.KeyCode
		Dim Shift As Short = eventArgs.KeyData \ &H10000
		Dim Index As Short = BD_BAISA.GetIndex(eventSender)
		Debug.Print("BD_BAISA_KeyDown")
		Call Ctl_Item_KeyDown(BD_BAISA(Index), KEYCODE, Shift)
	End Sub
	
	Private Sub BD_BSART_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles BD_BSART.KeyDown
		Dim KEYCODE As Short = eventArgs.KeyCode
		Dim Shift As Short = eventArgs.KeyData \ &H10000
		Dim Index As Short = BD_BSART.GetIndex(eventSender)
		Debug.Print("BD_BSART_KeyDown")
		Call Ctl_Item_KeyDown(BD_BSART(Index), KEYCODE, Shift)
	End Sub
	...
	Private Sub BD_SIKKN_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles BD_SIKKN.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		Dim Index As Short = BD_SIKKN.GetIndex(eventSender)
		Debug.Print("BD_SIKKN_KeyPress")
		Call Ctl_Item_KeyPress(BD_SIKKN(Index), KeyAscii)
		eventArgs.KeyChar = Chr(KeyAscii)
		If KeyAscii = 0 Then
			eventArgs.Handled = True
		End If
	...
	Private Sub BD_BAISA_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles BD_BAISA.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		Dim Index As Short = BD_BAISA.GetIndex(eventSender)
		Debug.Print("BD_BAISA_KeyPress")
		Call Ctl_Item_KeyPress(BD_BAISA(Index), KeyAscii)
		eventArgs.KeyChar = Chr(KeyAscii)
		If KeyAscii = 0 Then
			eventArgs.Handled = True
		End If
	...
	Private Sub BD_BSART_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles BD_BSART.KeyPress
		Dim KeyAscii As Short = Asc(eventArgs.KeyChar)
		Dim Index As Short = BD_BSART.GetIndex(eventSender)
		Debug.Print("BD_BSART_KeyPress")
		Call Ctl_Item_KeyPress(BD_BSART(Index), KeyAscii)
		eventArgs.KeyChar = Chr(KeyAscii)
		If KeyAscii = 0 Then
			eventArgs.Handled = True
		End If
	...
	Private Sub BD_SIKKN_KeyUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles BD_SIKKN.KeyUp
		Dim KEYCODE As Short = eventArgs.KeyCode
		Dim Shift As Short = eventArgs.KeyData \ &H10000
		Dim Index As Short = BD_SIKKN.GetIndex(eventSender)
		Debug.Print("BD_SIKKN_KeyUp")
		Call Ctl_Item_KeyUp(BD_SIKKN(Index))
	End Sub
	
	Private Sub BD_BAISA_KeyUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles BD_BAISA.KeyUp
		Dim KEYCODE As Short = eventArgs.KeyCode
		Dim Shift As Short = eventArgs.KeyData \ &H10000
		Dim Index As Short = BD_BAISA.GetIndex(eventSender)
		Debug.Print("BD_BAISA_KeyUp")
		Call Ctl_Item_KeyUp(BD_BAISA(Index))
	End Sub
	
	Private Sub BD_BSART_KeyUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles BD_BSART.KeyUp
		Dim KEYCODE As Short = eventArgs.KeyCode
		Dim Shift As Short = eventArgs.KeyData \ &H10000
		Dim Index As Short = BD_BSART.GetIndex(eventSender)
		Debug.Print("BD_BSART_KeyUp")
		Call Ctl_Item_KeyUp(BD_BSART(Index))
	End Sub
	...
	Private Sub BD_SIKKN_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_SIKKN.Enter
		Dim Index As Short = BD_SIKKN.GetIndex(eventSender)
		Debug.Print("BD_SIKKN_GotFocus")
		Call Ctl_Item_GotFocus(BD_SIKKN(Index))
	End Sub
	
	Private Sub BD_BAISA_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_BAISA.Enter
		Dim Index As Short = BD_BAISA.GetIndex(eventSender)
		Debug.Print("BD_BAISA_GotFocus")
		Call Ctl_Item_GotFocus(BD_BAISA(Index))
	End Sub
	
	Private Sub BD_BSART_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_BSART.Enter
		Dim Index As Short = BD_BSART.GetIndex(eventSender)
		Debug.Print("BD_BSART_GotFocus")
		Call Ctl_Item_GotFocus(BD_BSART(Index))
	End Sub
	...
	Private Sub BD_SIKKN_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_SIKKN.Leave
		Dim Index As Short = BD_SIKKN.GetIndex(eventSender)
		Debug.Print("BD_SIKKN_LostFocus")
		Call Ctl_Item_LostFocus(BD_SIKKN(Index))
	End Sub
	
	Private Sub BD_BAISA_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_BAISA.Leave
		Dim Index As Short = BD_BAISA.GetIndex(eventSender)
		Debug.Print("BD_BAISA_LostFocus")
		Call Ctl_Item_LostFocus(BD_BAISA(Index))
	End Sub
	
	Private Sub BD_BSART_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_BSART.Leave
		Dim Index As Short = BD_BSART.GetIndex(eventSender)
		Debug.Print("BD_BSART_LostFocus")
		Call Ctl_Item_LostFocus(BD_BSART(Index))
	End Sub
	...
	'UPGRADE_WARNING: イベント BD_SIKKN.TextChanged は、フォームが初期化されたときに発生します。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="88B12AE1-6DE0-48A0-86F1-60C0686C026A"' をクリックしてください。
	Private Sub BD_SIKKN_TextChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_SIKKN.TextChanged
		Dim Index As Short = BD_SIKKN.GetIndex(eventSender)
		Debug.Print("BD_SIKKN_Change")
		Call Ctl_Item_Change(BD_SIKKN(Index))
	End Sub
	
	'UPGRADE_WARNING: イベント BD_BAISA.TextChanged は、フォームが初期化されたときに発生します。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="88B12AE1-6DE0-48A0-86F1-60C0686C026A"' をクリックしてください。
	Private Sub BD_BAISA_TextChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_BAISA.TextChanged
		Dim Index As Short = BD_BAISA.GetIndex(eventSender)
		Debug.Print("BD_BAISA_Change")
		Call Ctl_Item_Change(BD_BAISA(Index))
	End Sub
	
	'UPGRADE_WARNING: イベント BD_BSART.TextChanged は、フォームが初期化されたときに発生します。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="88B12AE1-6DE0-48A0-86F1-60C0686C026A"' をクリックしてください。
	Private Sub BD_BSART_TextChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_BSART.TextChanged
		Dim Index As Short = BD_BSART.GetIndex(eventSender)
		Debug.Print("BD_BSART_Change")
		Call Ctl_Item_Change(BD_BSART(Index))
	End Sub
    ...
    ' 2007/01/18  ADD START  KUMEDA   *** 特殊処理追加
    ...
    Private Sub BD_SELECTB_CheckedChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles BD_SELECTB.CheckedChanged
        If eventSender.Checked Then
            ...
            Call setSELECTB(Index, Main_Inf)
    ...
    Private Sub FR_SSSMAIN_LocationChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.LocationChanged

    End Sub

    '2019/03/28 ADD saiki START
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
