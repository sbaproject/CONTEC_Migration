	...
	Private Function Ctl_Item_Click(ByRef pm_Ctl As System.Windows.Forms.Control) As Short
		...
		Select Case Trg_Index
            ...
            Case CShort(btnF9.Tag)
                ...
                '2019/10/09 ADD START
                If _BD_OUTYTDT_1.Text.Trim.Length > 0 Then
                ...
                HD_HINCD.BackColor = COLOR_WHITE
                HD_SBNNO.Select()
                HD_SBNNO.BackColor = COLOR_YELLOW
...
CF_Chk_Lock_EXCTBZ_Err:
	...
	Private Function Set_Body_Location() As Short
        ...
        '１行目のNoのTopとHeightを基準とする
        BD_OUTYTDT_Top = VB6.PixelsToTwipsY(BD_OUTYTDT(1).Top)
        BD_OUTYTDT_Height = VB6.PixelsToTwipsY(BD_OUTYTDT(1).Height) + Hosei_Value

        '１行目｢入出庫日｣から｢数量｣までの相対位置を取得
        BD_OUTYTSU_Top = VB6.PixelsToTwipsY(BD_OUTYTSU(1).Top) - BD_OUTYTDT_Top
        '１行目｢入出庫日｣から｢元製番｣までの相対位置を取得
        BD_ORGSBNNO_Top = VB6.PixelsToTwipsY(BD_ORGSBNNO(1).Top) - BD_OUTYTDT_Top
        '１行目｢入出庫日｣から｢出庫理由名｣までの相対位置を取得
        BD_OUTRSNNM_Top = VB6.PixelsToTwipsY(BD_OUTRSNNM(1).Top) - BD_OUTYTDT_Top
        '１行目｢入出庫日｣から｢得意先略称｣までの相対位置を取得
        BD_TOKRN_Top = VB6.PixelsToTwipsY(BD_TOKRN(1).Top) - BD_OUTYTDT_Top
        '１行目｢入出庫日｣から｢仕入先略称｣までの相対位置を取得
        BD_SIRRN_Top = VB6.PixelsToTwipsY(BD_SIRRN(1).Top) - BD_OUTYTDT_Top
        '１行目｢入出庫日｣から｢登録日｣までの相対位置を取得
        BD_WRTFSTDT_Top = VB6.PixelsToTwipsY(BD_WRTFSTDT(1).Top) - BD_OUTYTDT_Top
        '１行目｢入出庫日｣から｢倉庫名｣までの相対位置を取得
        BD_SOUNM_Top = VB6.PixelsToTwipsY(BD_SOUNM(1).Top) - BD_OUTYTDT_Top
        ...
        Main_Inf.Bd_Vs_Scrl.Top = VB6.TwipsToPixelsY(BD_OUTYTDT_Top)
        Main_Inf.Bd_Vs_Scrl.Height = VB6.TwipsToPixelsY(BD_OUTYTDT_Height * Main_Inf.Dsp_Base.Dsp_Body_Cnt)
        '2019/10/09 CHG END
	...
	Private Sub BD_SELECTB_MouseDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles BD_SELECTB.MouseDown
        ...
        SSSMAIN0001.F_Ctl_Head_Chk(Main_Inf)
    End Sub
	...
	Private Sub HD_HINCD_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles HD_HINCD.KeyDown
        ...
        '2019/10/09 ADD START
        If isKeyDown Then
            If Trim(HD_SBNNO.Text).Length = 0 Or _BD_OUTYTDT_1.Text.Trim.Length = 0 Then
                HD_HINCD.BackColor = COLOR_WHITE
            End If
        End If
        '2019/10/09 ADD START
	...
	Private Sub HD_HINCD_KeyUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles HD_HINCD.KeyUp
        ...
        '2019/10/09 ADD START
        HD_SBNNO.BackColor = COLOR_WHITE
        '2019/10/09 ADD END
    End Sub
    ...
    '   備考：
    ' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
    Public Function CF_Set_Frm_IN_TANCD_HIKET54(ByRef pm_Form As FR_SSSMAIN, ByRef pm_All As Cls_All) As Short
