...
Module SOUMTA_M51
	...
	Sub UPDMST()
		...
		Do While I < PP_SSSMAIN.LastDe
            ...
            pWhere = "WHERE SOUCD = '" & DB_SOUMTA.SOUCD & "'"
            GetRowsCommon(DBN_SOUMTA, pWhere)
            '2019/10/10 CHG END
            If DBSTAT = 0 Then
				' === 20080829 === INSERT S - RISE)Izumi チェック項目追加
				...
				If updkb = "削除" Then
                    ...
                    If Not dt Is Nothing Then
                        DB_SOUMTA.OPEID = DB_NullReplace(dt.Rows(0)("OPEID"), "")
                        DB_SOUMTA.CLTID = DB_NullReplace(dt.Rows(0)("CLTID"), "")
                        DB_SOUMTA.UOPEID = DB_NullReplace(dt.Rows(0)("UOPEID"), "")
                        DB_SOUMTA.UCLTID = DB_NullReplace(dt.Rows(0)("UCLTID"), "")
                        DB_SOUMTA.WRTDT = DB_NullReplace(dt.Rows(0)("WRTDT"), "")
                        DB_SOUMTA.WRTTM = DB_NullReplace(dt.Rows(0)("WRTTM"), "")
                        DB_SOUMTA.UWRTDT = DB_NullReplace(dt.Rows(0)("UWRTDT"), "")
                        DB_SOUMTA.UWRTTM = DB_NullReplace(dt.Rows(0)("UWRTTM"), "")
                    End If
                    '2019/10/10 CHG END
		...
		Do While I < PP_SSSMAIN.LastDe
            ...
            If DBSTAT = 0 Then
				...
				If updkb = "削除" Then
                    ...
                    updSQL = updSQL & "        SOUMTA "
                    updSQL = updSQL & " SET "

                    updSQL = updSQL & " DATKB		=	'" & DB_SOUMTA.DATKB & "' "
                    updSQL = updSQL & ",SALPALKB    =	'" & DB_SOUMTA.SALPALKB & "' "
                    updSQL = updSQL & ",HIKKB	    =	'" & DB_SOUMTA.HIKKB & "' "
                    updSQL = updSQL & ",SOUKOKB		=	'" & DB_SOUMTA.SOUKOKB & "' "
                    updSQL = updSQL & ",SOUTRICD    =	'" & DB_SOUMTA.SOUTRICD & "' "
                    updSQL = updSQL & ",SISNKB		=	'" & DB_SOUMTA.SISNKB & "' "
                    updSQL = updSQL & ",SRSCNKB		=	'" & DB_SOUMTA.SRSCNKB & "' "
                    updSQL = updSQL & ",SOUKB		=	'" & DB_SOUMTA.SOUKB & "' "
                    updSQL = updSQL & ",SOUBSCD		=	'" & DB_SOUMTA.SOUBSCD & "' "
                    updSQL = updSQL & ",SOUFX		=	'" & DB_SOUMTA.SOUFX & "' "
                    updSQL = updSQL & ",SOUTL		=	'" & DB_SOUMTA.SOUTL & "' "
                    updSQL = updSQL & ",SOUADC		=	'" & DB_SOUMTA.SOUADC & "' "
                    updSQL = updSQL & ",SOUADB		=	'" & DB_SOUMTA.SOUADB & "' "
                    updSQL = updSQL & ",SOUADA		=	'" & DB_SOUMTA.SOUADA & "' "
                    updSQL = updSQL & ",SOUZP		=	'" & DB_SOUMTA.SOUZP & "' "
                    updSQL = updSQL & ",SOUNM		=	'" & DB_SOUMTA.SOUNM & "' "
                    updSQL = updSQL & ",RELFL		=	'" & DB_SOUMTA.RELFL & "' "
                    updSQL = updSQL & ",FOPEID		=	'" & DB_SOUMTA.FOPEID & "' "
                    updSQL = updSQL & ",FCLTID		=	'" & DB_SOUMTA.FCLTID & "' "
                    updSQL = updSQL & ",WRTFSTTM	=	'" & DB_SOUMTA.WRTFSTTM & "' "
                    updSQL = updSQL & ",WRTFSTDT	=	'" & DB_SOUMTA.WRTFSTDT & "' "
                    updSQL = updSQL & ",OPEID		=	'" & DB_SOUMTA.OPEID & "' "
                    updSQL = updSQL & ",CLTID		=	'" & DB_SOUMTA.CLTID & "' "
                    updSQL = updSQL & ",WRTTM		=	'" & DB_SOUMTA.WRTTM & "' "
                    updSQL = updSQL & ",WRTDT		=	'" & DB_SOUMTA.WRTDT & "' "
                    updSQL = updSQL & ",UOPEID		=	'" & DB_SOUMTA.UOPEID & "' "
                    updSQL = updSQL & ",UCLTID		=	'" & DB_SOUMTA.UCLTID & "' "
                    updSQL = updSQL & ",UWRTTM		=	'" & DB_SOUMTA.UWRTTM & "' "
                    updSQL = updSQL & ",UWRTDT		=	'" & DB_SOUMTA.UWRTDT & "' "
                    updSQL = updSQL & ",PGID		=	'" & DB_SOUMTA.PGID & "' "

                    updSQL = updSQL & "  WHERE "
                    updSQL = updSQL & "        SOUCD     = '" & DB_SOUMTA.SOUCD & "' "
			...
			Else
                ...
                updSQL = ""
                updSQL = updSQL & " '" & DB_SOUMTA.DATKB & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUCD & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUNM & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUZP & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUADA & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUADB & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUADC & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUTL & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUFX & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUBSCD & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUKB & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SRSCNKB & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SISNKB & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUTRICD & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SOUKOKB & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.HIKKB & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.SALPALKB & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.RELFL & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.FOPEID & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.FCLTID & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.WRTFSTTM & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.WRTFSTDT & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.OPEID & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.CLTID & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.WRTTM & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.WRTDT & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.UOPEID & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.UCLTID & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.UWRTTM & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.UWRTDT & "' "
                updSQL = updSQL & ",'" & DB_SOUMTA.PGID & "' "

                updSQL = DB_InsertSQL(DBN_SOUMTA, updSQL)
                DB_Execute(updSQL)
                '2019/10/10 CHG END
            End If
			I = I + 1
