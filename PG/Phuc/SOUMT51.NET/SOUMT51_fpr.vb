...
Module SSSMAIN0001
	...
	Sub AE_Check_SSSMAIN_HIKKB(ByVal CC_NewVal As Object, ByVal pm_Status As Short, ByVal pm_MoveCursor As Boolean, ByVal pm_HandIn As Boolean) 'Generated.
		...
		With CP_SSSMAIN(wk_Px)
            ...
            If (.CheckRtnCode = 0) And pm_HandIn And (DB_NullReplace(CC_NewVal, "").ToString = DB_NullReplace(.CuVal, "").ToString Or (AE_IsNull_SSSMAIN(CC_NewVal) And AE_IsNull_SSSMAIN(.CuVal))) Then
	...
	Sub AE_Derived_SSSMAIN_bd_HIKKB(ByVal De_Index As Short) 'Generated.
		...
		If PP_SSSMAIN.DerivedOrigin <> "" Then
            ...
            If DB_NullReplace(CC_NewVal, "").ToString = DB_NullReplace(CP_SSSMAIN(wk_Px), "").ToString And CP_SSSMAIN(wk_Px).StatusC >= Cn_Status6 Then
                '2019/10/10 CHG END
                Call AE_ColorSub(PP_SSSMAIN, CP_SSSMAIN(wk_Px), CL_SSSMAIN(wk_Px))
                'UPGRADE_WARNING: Null/IsNull() の使用が見つかりました。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="2EED02CB-5C0E-4DC1-AE94-4FAA3A30F51A"' をクリックしてください。
