...
Module SSSMAIN0001
	...
	Public CP_SSSMAIN(2 + 0 + 0 + 1) As clsCP
	Public CL_SSSMAIN(2) As Short
	Public CQ_SSSMAIN(2) As String
	...
	Sub AE_ClearItm_SSSMAIN(ByVal pm_HandIn As Boolean) 'Generated.
		...
		If PP_SSSMAIN.Tx < 0 Or PP_SSSMAIN.Tx >= 2 Then Exit Sub
		PP_SSSMAIN.MaskMode = True
	...
	Sub AE_CursorCurrent_SSSMAIN() 'Generated.
		...
		If PP_SSSMAIN.Tx >= 0 And PP_SSSMAIN.Tx < 2 Then
	...
	Sub AE_CursorMove_SSSMAIN(ByVal pm_Tx As Short) 'Generated.
		...
		If wk_Tx < 0 Or wk_Tx >= 2 Then Call AE_CursorRestSub_SSSMAIN(Cn_CursorToRest) : Exit Sub
	...
	Function AE_CursorSkip_SSSMAIN() As Boolean 'Generated.
		...
		Select Case PP_SSSMAIN.CursorDest
			...
			Case Cn_Dest3
				If Not AE_CursorPrev_SSSMAIN(2) Then Call AE_CursorRestSub_SSSMAIN(Cn_CursorToRest)
			Case Cn_Dest4
			...
			Case Else
				Select Case PP_SSSMAIN.CursorDirection
					...
					Case Cn_Direction2
						...
						If PP_SSSMAIN.CursorDest = Cn_Dest1 And wk_Bool = False Then wk_Bool = AE_CursorPrev_SSSMAIN(2)
						PP_SSSMAIN.CursorDirection = wk_CursorDirection
					Case Cn_Direction3
...
#End If
	...
	Function AE_Execute_SSSMAIN() As Short 'Generated.
		...
		With PP_SSSMAIN
			...
			If .Mode = Cn_Mode1 Then
				'// ↓ ADD 2008-12-26 RISE)Morita
				If FR_SSSMAIN.HD_ALLDEL.CheckState = 1 Then
					'チェックがある場合は、確認メッセージ
					If DSP_MsgBox(SSS_CONFRM, "URKFP51_001", 0) <> 6 Then
						AE_Execute_SSSMAIN = Cn_CuCurrent
						Exit Function
					End If
				End If
				'// ↑ ADD 2008-12-26 RISE)Morita
	...
	Sub AE_InOutModeM_SSSMAIN(ByVal pm_ItemName As String, ByVal pm_Mode As String) 'Generated.
		...
		wk_Qx = 0
		Do While wk_Qx < 2 And UCase(Mid(CQ_SSSMAIN(wk_Qx), Cn_AfterPrfx)) <> wk_UCaseObjA
			wk_Qx = wk_Qx + 1
		Loop 
	...
	Sub AE_InOutModeN_SSSMAIN(ByVal pm_ItemName As String, ByVal pm_Mode As String, Optional ByVal pm_De As Object = Nothing) 'Generated.
		...
		wk_UCaseObjA = UCase(pm_ItemName)
		wk_Qx = 0
		Do While wk_Qx < 2 And Mid(CQ_SSSMAIN(wk_Qx), Cn_AfterPrfx) <> wk_UCaseObjA
			wk_Qx = wk_Qx + 1
		Loop 
		If UCase(Mid(CQ_SSSMAIN(wk_Qx), Cn_AfterPrfx)) <> wk_UCaseObjA Then
...
CheckOrSkip: 
		...
		Select Case pm_NewMode
			Case Cn_Mode1
				If PP_SSSMAIN.Mode <> Cn_Mode1 Then
					...
					Call AE_TabStop_SSSMAIN(0, 1, False)
					AE_CursorRest(PP_SSSMAIN.ScX).TabStop = False
				End If
			Case Cn_Mode2
	...
	Sub AE_TabStop_SSSMAIN(ByVal pm_FromTx As Short, ByVal pm_ToTx As Short, ByVal pm_SetInOut As Boolean) 'Generated.
		...
		Do While wk_Tx <= pm_ToTx
			If wk_Tx >= PP_SSSMAIN.NrBodyTx And wk_Tx < 2 Then
			Else
				wk_Px = AE_Px(PP_SSSMAIN, wk_Tx)
				wk_InOutMode = (CP_SSSMAIN(wk_Px).InOutMode \ 256) And &HFFs
