...
Module SSSMAIN0001
	...
	Public CP_SSSMAIN(5 + 0 + 0 + 1) As clsCP
	Public CL_SSSMAIN(5) As Short
	Public CQ_SSSMAIN(5) As String
	...
	Sub AE_Check_SSSMAIN_DENDT(ByVal CC_NewVal As Object, ByVal pm_Status As Short, ByVal pm_MoveCursor As Boolean, ByVal pm_HandIn As Boolean) 'Generated.
		...
		With CP_SSSMAIN(3)
			...
			If (.CheckRtnCode = 0) And pm_HandIn And (CC_NewVal = .CuVal Or (AE_IsNull_SSSMAIN(CC_NewVal) And AE_IsNull_SSSMAIN(.CuVal))) Then
				If Not PP_SSSMAIN.RecalcMode Then
					...
					Else
						...
						Call AE_SetSel(PP_SSSMAIN, CP_SSSMAIN(3), AE_Controls(PP_SSSMAIN.CtB + 3))
					End If
				End If
				Exit Sub
			End If
			...
			ElseIf (.CheckRtnCode <> 0) Then 
				...
				'UPGRADE_WARNING: オブジェクト Ck_Error の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Ck_Error = DENDT_CheckC(CC_NewVal, PP_SSSMAIN.De2)
			Else
				'UPGRADE_WARNING: オブジェクト DENDT_CheckC() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Ck_Error の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Ck_Error = DENDT_CheckC(CC_NewVal, PP_SSSMAIN.De2)
	...
	Sub AE_Check_SSSMAIN_PRTSB(ByVal CC_NewVal As Object, ByVal pm_Status As Short, ByVal pm_MoveCursor As Boolean, ByVal pm_HandIn As Boolean) 'Generated.
		Dim wk_SaveMask As Boolean
		Dim RC_ErrorC As Short
		Dim wk_RecalcSw As Boolean
		Dim wk_Equal As Boolean
		...
		With CP_SSSMAIN(2)
			...
			If (.CheckRtnCode <> 0) Then
				...
				'UPGRADE_WARNING: オブジェクト Ck_Error の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Ck_Error = PRTSB_Check(CC_NewVal)
			Else
				'UPGRADE_WARNING: オブジェクト PRTSB_Check() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Ck_Error の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Ck_Error = PRTSB_Check(CC_NewVal)
	...
	Sub AE_ClearItm_SSSMAIN(ByVal pm_HandIn As Boolean) 'Generated.
		...
		If PP_SSSMAIN.Tx < 0 Or PP_SSSMAIN.Tx >= 5 Then Exit Sub
		PP_SSSMAIN.MaskMode = True
		If PP_SSSMAIN.Tx < 5 Then
	...
	Sub AE_CursorCurrent_SSSMAIN() 'Generated.
		...
		If PP_SSSMAIN.Tx >= 0 And PP_SSSMAIN.Tx < 5 Then
	...
	Sub AE_CursorMove_SSSMAIN(ByVal pm_Tx As Short) 'Generated.
		...
		If wk_Tx < 0 Or wk_Tx >= 5 Then Call AE_CursorRestSub_SSSMAIN(Cn_CursorToRest) : Exit Sub
	...
	Function AE_CursorSkip_SSSMAIN() As Boolean 'Generated.
		...
		Select Case PP_SSSMAIN.CursorDest
			...
			Case Cn_Dest3
				If Not AE_CursorPrev_SSSMAIN(5) Then Call AE_CursorRestSub_SSSMAIN(Cn_CursorToRest)
			Case Cn_Dest4
			...
			Case Else
				Select Case PP_SSSMAIN.CursorDirection
					...
					Case Cn_Direction2
						...
						If PP_SSSMAIN.CursorDest = Cn_Dest1 And wk_Bool = False Then wk_Bool = AE_CursorPrev_SSSMAIN(5)
						PP_SSSMAIN.CursorDirection = wk_CursorDirection
					Case Cn_Direction3
...
#End If
	...
	Sub AE_InitValHd_SSSMAIN(ByVal pm_Px As Short, ByVal pm_SetInOut As Short, ByVal pm_Status As Short) 'Generated.
		...
		Dim wk_ww As Short
		If pm_Px = -2 Then
			Call AE_TabStop_SSSMAIN(0, 4, pm_SetInOut)
		ElseIf pm_Px >= 0 Then 
		...
		End If
		If pm_Px = -2 Or pm_Px = 2 Then 'PRTSB
			Call AE_InitVal_SSSMAIN(CP_SSSMAIN(2), PRTSB_InitVal(), pm_Status)
		End If
		If pm_Px = -2 Or pm_Px = 3 Then 'DENDT
			Call AE_InitVal_SSSMAIN(CP_SSSMAIN(3), DENDT_InitVal(AE_NullCnv2_SSSMAIN(CP_SSSMAIN(3).CuVal)), pm_Status)
		End If
		If pm_Px = -2 Or pm_Px = 4 Then 'PRTKB
			Call AE_InitVal_SSSMAIN(CP_SSSMAIN(4), PRTKB_InitVal(), pm_Status)
		End If
	End Sub
	...
	Sub AE_InOutModeM_SSSMAIN(ByVal pm_ItemName As String, ByVal pm_Mode As String) 'Generated.
		...
		wk_Qx = 0
		Do While wk_Qx < 5 And UCase(Mid(CQ_SSSMAIN(wk_Qx), Cn_AfterPrfx)) <> wk_UCaseObjA
			wk_Qx = wk_Qx + 1
		Loop 
	...
	Sub AE_InOutModeN_SSSMAIN(ByVal pm_ItemName As String, ByVal pm_Mode As String, Optional ByVal pm_De As Object = Nothing) 'Generated.
		...
		wk_UCaseObjA = UCase(pm_ItemName)
		wk_Qx = 0
		Do While wk_Qx < 5 And Mid(CQ_SSSMAIN(wk_Qx), Cn_AfterPrfx) <> wk_UCaseObjA
			wk_Qx = wk_Qx + 1
		Loop 
		If UCase(Mid(CQ_SSSMAIN(wk_Qx), Cn_AfterPrfx)) <> wk_UCaseObjA Then
...
CheckOrSkip: 
		...
		Select Case pm_NewMode
			Case Cn_Mode1
				If PP_SSSMAIN.Mode <> Cn_Mode1 Then
					...
					Call AE_TabStop_SSSMAIN(0, 4, False)
					AE_CursorRest(PP_SSSMAIN.ScX).TabStop = False
				End If
			Case Cn_Mode2
	...
	Sub AE_RecalcHd_SSSMAIN() 'Generated.
		...
		If AE_GetInOutMode(CP_SSSMAIN(2).InOutMode, Cn_Mode1) >= Cn_InOutMode2 Or AE_GetInOutMode(CP_SSSMAIN(2).InOutMode, Cn_Mode4) >= Cn_InOutMode2 Then
			If CP_SSSMAIN(2).StatusC >= Cn_Status2 Then Call AE_Check_SSSMAIN_PRTSB(AE_Val2(CP_SSSMAIN(2)), CP_SSSMAIN(2).StatusF, False, False)
		End If
		If AE_GetInOutMode(CP_SSSMAIN(3).InOutMode, Cn_Mode1) >= Cn_InOutMode2 Or AE_GetInOutMode(CP_SSSMAIN(3).InOutMode, Cn_Mode4) >= Cn_InOutMode2 Then
			If CP_SSSMAIN(3).StatusC >= Cn_Status2 Then Call AE_Check_SSSMAIN_DENDT(AE_Val2(CP_SSSMAIN(3)), CP_SSSMAIN(3).StatusF, False, False)
		End If
		If AE_GetInOutMode(CP_SSSMAIN(4).InOutMode, Cn_Mode1) >= Cn_InOutMode2 Or AE_GetInOutMode(CP_SSSMAIN(4).InOutMode, Cn_Mode4) >= Cn_InOutMode2 Then
			If CP_SSSMAIN(4).StatusC >= Cn_Status2 Then Call AE_Check_SSSMAIN_PRTKB(AE_Val2(CP_SSSMAIN(4)), CP_SSSMAIN(4).StatusF, False, False)
		End If
		...
		ElseIf PP_SSSMAIN.Tx < 5 Then 
			Select Case PP_SSSMAIN.Px
				...
				Case 1
					...
					End If
				Case 2
					Call AE_Check_SSSMAIN_PRTSB(CC_NewVal, pm_Status, False, pm_HandIn)
					If AE_ErrorToInteger(Ck_Error) = 0 Then
					End If
				Case 3
		...
		ElseIf PP_SSSMAIN.Tx = 3 Then 
			...
			If Not IsDbNull(wk_Slisted) Then
				...
				If PP_SSSMAIN.Mode <> Cn_Mode3 Then
					...
					Call AE_Check_SSSMAIN_DENDT(AE_Val3(CP_SSSMAIN(3), AE_Controls(PP_SSSMAIN.CtB + 3).ToString()), Cn_Status6, True, True)
				End If
			Else
				PP_SSSMAIN.CursorDest = Cn_Dest0
	...
	Sub AE_TabStop_SSSMAIN(ByVal pm_FromTx As Short, ByVal pm_ToTx As Short, ByVal pm_SetInOut As Boolean) 'Generated.
		...
		Do While wk_Tx <= pm_ToTx
			If wk_Tx >= PP_SSSMAIN.NrBodyTx And wk_Tx < 5 Then
			Else
				wk_Px = AE_Px(PP_SSSMAIN, wk_Tx)
				wk_InOutMode = (CP_SSSMAIN(wk_Px).InOutMode \ 256) And &HFFs
	...
	Sub DP_SSSMAIN_PRTSB(ByVal pm_De As Short, ByRef DBItem As Object) 'Generated.
		Dim V As Object
		Dim wk_SaveMask As Boolean
		wk_SaveMask = PP_SSSMAIN.MaskMode
		PP_SSSMAIN.MaskMode = True
	...
	Function RD_SSSMAIN_DENDT(ByVal De As Short) As Object 'Generated.
		...
		Else
			'UPGRADE_WARNING: オブジェクト CP_SSSMAIN().CuVal の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			RD_SSSMAIN_DENDT = VB6.Format(CP_SSSMAIN(3).CuVal, "YYYYMMDD")
		End If
	End Function
	...
	Function RD_SSSMAIN_PRTKB(ByVal De As Short) As Object 'Generated.
		...
		Else
			'UPGRADE_WARNING: オブジェクト CP_SSSMAIN().CuVal の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_PRTKB の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			RD_SSSMAIN_PRTKB = CP_SSSMAIN(4).CuVal
		End If
	End Function
	
	Function RD_SSSMAIN_PRTSB(ByVal De As Short) As Object 'Generated.
		Dim st_Work As String
		'UPGRADE_WARNING: IsEmpty は、IsNothing にアップグレードされ、新しい動作が指定されました。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"' をクリックしてください。
		If IsNothing(CP_SSSMAIN(2).CuVal) Then
			...
			RD_SSSMAIN_PRTSB = 0@
			'UPGRADE_WARNING: Null/IsNull() の使用が見つかりました。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="2EED02CB-5C0E-4DC1-AE94-4FAA3A30F51A"' をクリックしてください。
		ElseIf IsDbNull(CP_SSSMAIN(2).CuVal) Then 
			'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_PRTSB の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			RD_SSSMAIN_PRTSB = 0@
		Else
			'UPGRADE_WARNING: オブジェクト CP_SSSMAIN().CuVal の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			'UPGRADE_WARNING: オブジェクト RD_SSSMAIN_PRTSB の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
			RD_SSSMAIN_PRTSB = CP_SSSMAIN(2).CuVal
		End If
	End Function
End Module