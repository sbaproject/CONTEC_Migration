...
Module AE_CONST
	...
	' === 20070127 === INSERT S - ACE)Nagasawa 行番号は製番と同期をとるよう修正
	'明細最大行番号
	Public Const gc_intMAXLINNO As Short = 99
	' === 20070127 === INSERT E -
	...
	Public Const gc_strDKBSB_FDN As String = "020" '納品書番号取得用
	' === 20070331 === INSERT E -
	' === 20070327 === INSERT S - ACE)Nagasawa 売上訂正返品対応
	Public Const gc_strDKBSB_ODN As String = "030" '出荷伝票番号取得用
	Public Const gc_strDKBSB_UDN As String = "040" '売上伝票番号取得用
	' === 20070327 === INSERT E -
	...
	Public Const gc_strDKBSB_ENDUSRCD As String = "700" 'エンドユーザコード取得用
	' === 20130719 === INSERT E -
	...
	'    Public Const gc_curSIKRT_Max                As Currency = 999.9         '最大仕切率
	Public Const gc_curSIKRT_Max As Decimal = 9999.9 '最大仕切率
	' === 20070131 === UPDATE E -
	' === 20061020 === INSERT E -
	
	' === 20070219 === INSERT S - ACE)Nagasawa 仕切率1000％以上警告対応
	Public Const gc_curSIKRT_WARNING_MAX As Decimal = 1000 '警告基準
	' === 20070219 === INSERT E -
	...
	'トラン種別（引当内訳ファイル）
	Public Const gc_strDTLTRA_TRAKB_MIT As String = "1" '見積
	Public Const gc_strDTLTRA_TRAKB_JDN As String = "2" '受注
	Public Const gc_strDTLTRA_TRAKB_SKY As String = "3" '支給品
	Public Const gc_strDTLTRA_TRAKB_SBN As String = "4" '製番出庫
	' === 20070126 === INSERT E -
	...
	Public Const gc_strKEYCD_MRPKB As String = "092" '請求書出力区分
	' === 20071213 === UPDATE E -
	'20130701 ADD START 新通販連携対応
	Public Const gc_strKEYCD_KSHTK As String = "113" '決済方法得意先
	'20130701 ADD END
	' === 20130719 === INSERT S - FWEST)Koroyasu ｴﾝﾄﾞﾕｰｻﾞ対応
	Public Const gc_strKEYCD_ENDUSRKB As String = "114" 'エンドユーザ区分
	' === 20130719 === INSERT E -
	
	' === 20130719 === INSERT S - FWEST)Koroyasu ｴﾝﾄﾞﾕｰｻﾞ対応
	'名称マスタ（項目名）
	Public Const gc_strMEIKMKNM_ENDUSR As String = "エンドユーザ" 'エンドユーザ
	' === 20130719 === INSERT E -
	...
	' === 20130719 === INSERT S - FWEST)Koroyasu ｴﾝﾄﾞﾕｰｻﾞ対応
	Public Const gc_strTNKKB_RPT_NM As String = "リ" 'リピート特価
	' === 20130719 === INSERT E -
	...
	'20130701 ADD START 新通販連携対応
	Public Const gc_strPRCKB_EC1 As String = "E0001" '注文情報（EC通販:クレジット）
	Public Const gc_strPRCKB_EC2 As String = "E0002" '注文情報（EC通販:代引）
	Public Const gc_strPRCKB_EC3 As String = "E0003" '注文情報（EC通販:振込前払い）
	Public Const gc_strPRCKB_EC4 As String = "E0004" '注文情報（EC通販:振込後払い）
	Public Const gc_strPRCKB_EC5 As String = "E0005" '注文情報（EC通販:ペイジー）
	'20130701 ADD END
	...
	'C2-20170803-01 ADD START 富士通)橋本 2017/08/03
	'単位コード
	Public Const gc_strUNTCD_01 As String = "01" 'PC
	'C2-20170803-01 ADD END   富士通)橋本 2017/08/03
	...
	'請求書出力区分
	Public Const gc_strMRPKB_PRT As String = "1" '要
	Public Const gc_strMRPKB_NOPRT As String = "9" '不要
	' === 20071213 === INSERT E -
	
	' === 20130719 === INSERT S - FWEST)Koroyasu ｴﾝﾄﾞﾕｰｻﾞ対応
	'リピート特価区分
	Public Const gc_strRPTTKA_ON As String = "1" 'あり
	Public Const gc_strRPTTKA_OFF As String = "9" 'なし
	' === 20130719 === INSERT E -
	
	
	'固定値マスタ
	' === 20070309 === INSERT S - ACE)Nagasawa 売上後の入力可否制御の変更
	Public Const gc_strCTLCD_SSAKB As String = "101" '休日請求時の処理
	' === 20070309 === INSERT E -
	...
	' === 20130705 === INSERT S - FWEST)Koroyasu 連絡票№HAN20130705-01
	Public Const gc_strCTLCD_TKASTTDT As String = "306" '見積特価分類開始日
	' === 20130705 === INSERT E
	...
	' === 20130719 === INSERT S - FWEST)Koroyasu ｴﾝﾄﾞﾕｰｻﾞ対応
	Public Const gc_strCTLCD_ENDUSR_KINSIMOJI As String = "702" 'エンドユーザ禁止文字
	' === 20130719 === INSERT E -
	...
	'機器区分(コンピュータ型式)
	Public Const gc_strCMPKTCD_KIKI As String = "1" '機器受注
	Public Const gc_strCMPKTCD_KIKI_Else As String = "0" '機器受注以外
	
	'機器区分(生産担当)
	Public Const gc_strPRDTBMCD_KIKI As String = "1" '機器受注
	Public Const gc_strPRDTBMCD_KIKI_Else As String = "0" '機器受注以外
	' === 20070207 === INSERT E -
	
	' === 20070309 === INSERT S - ACE)Nagasawa
	'曜日
	Public Const gc_strCLDWKKB_SUN As String = "1" '日曜日
	Public Const gc_strCLDWKKB_MON As String = "2" '月曜日
	Public Const gc_strCLDWKKB_TUE As String = "3" '火曜日
	Public Const gc_strCLDWKKB_WED As String = "4" '水曜日
	Public Const gc_strCLDWKKB_THU As String = "5" '木曜日
	Public Const gc_strCLDWKKB_FRI As String = "6" '金曜日
	Public Const gc_strCLDWKKB_SUT As String = "7" '土曜日
	' === 20070309 === INSERT E -
	
	' === 20070327 === INSERT S - ACE)Nagasawa 売上訂正返品対応
	'取引区分コード
	Public Const gc_strDKBID_HP As String = "02" '返品
	Public Const gc_strDKBID_JHP As String = "06" '自返品
	
	'消込区分
	Public Const gc_strKESIKB_NOT As String = "9" '未消込
	' === 20070327 === INSERT E -
	...
	'20130701 ADD START 新通販連携対応
	'20130930 DEL START THN20130910-06
	'    '分納可否区分
	'    Public Const gc_strEBKTHKKB_ACD             As String = "1"             'アカデミック
	'    Public Const gc_strEBKTHKKB_ACD_Else        As String = "0"             'アカデミック外
	'
	'    '単価種別
	'    Public Const gc_strTNKKB_ACD_NEW            As String = "4"             'ア１（アカデミック－新規）
	'20130930 DEL END
	
	'ステータス情報トランのステータス
	Public Const gc_strSTSTRA_STS_ORD As String = "1" '注文受付完了
	Public Const gc_strSTSTRA_STS_UPD As String = "2" '変更
	Public Const gc_strSTSTRA_STS_ODN As String = "3" '出荷処理済
	
	'連携フラグ
	Public Const gc_strRELFL_NO As String = "0" '連携不要
	Public Const gc_strRELFL_YES As String = "1" '連携必要
	Public Const gc_strRELFL_NOW As String = "2" '連携処理中
	
	'通販データ検索のステータス
	Public Const gc_strSTS_MSK As String = "1" '未出荷
	Public Const gc_strSTS_ZUA As String = "2" '売上済
	Public Const gc_strSTS_ZNK As String = "3" '入金済
	Public Const gc_strSTS_ALL As String = "9" '全件
	'20130701 ADD END
