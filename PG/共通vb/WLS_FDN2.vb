...
Friend Class WLSFDN
	...
	Private Sub COM_HINCD_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles COM_HINCD.Click
        ...
        '20190620 DEL END
        WLSHIN4.ShowDialog() '0:入力候補一覧は入力後に残す指定。
		''98/09/25 追加
		WLSHIN4.Close()
        ...
        If IsDBNull(WLSHIN_RTNCODE) Then
            '20190606 CHG END
            DB_HINMTA.HINCD = ""
        Else
            ...
            'Call HINMTA_GetFirstRecByHINCD(WLSHIN_RTNCODE)
            '20190606 CHG END
	...
	Private Sub COM_SOUCD_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles COM_SOUCD.Click
        ...
        '20190606 CHG START
        'If IsDBNull(PP_SSSMAIN.SlistCom) Then
        If IsDBNull(WLSSOU_RTNCODE) Then
            '20190606 CHG END
            DB_SOUMTA.SOUCD = ""
        Else
            '20190606 CHG START
            'Call DB_GetEq(DBN_SOUMTA, 1, PP_SSSMAIN.SlistCom, BtrNormal)
            Call DSPSOUCD_SEARCH(WLSSOU_RTNCODE, DB_SOUMTA)
            '20190606 CHG END
            If DBSTAT = 0 Then
	...
	Private Sub COM_TOKCD_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles COM_TOKCD.Click
        ...
        '20190620 DEL END
        WLSTOK1.ShowDialog() '0:入力候補一覧は入力後に残す指定。
        ''98/09/25 追加
        WLSTOK1.Close()
        System.Windows.Forms.Application.DoEvents()
        'UPGRADE_WARNING: Null/IsNull() の使用が見つかりました。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="2EED02CB-5C0E-4DC1-AE94-4FAA3A30F51A"' をクリックしてください。
        '20190606 CHG START
        'If IsDBNull(PP_SSSMAIN.SlistCom) Then
        If IsDBNull(WLSTOK_RTNCODE) Then
            '20190606 CHG END
            DB_TOKMTA.TOKCD = ""
        Else
            'UPGRADE_WARNING: オブジェクト PP_SSSMAIN.SlistCom の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
            ...
            'Call TOKMTA_GetFirst(WLSTOK_RTNCODE)
            GetRowsCommon("TOKMTA", "Where TOKCD = '" & WLSTOK_RTNCODE & "'")
            '20190619 CHG END
            '20190606 CHG END
            If DBSTAT = 0 Then
    ...
    '20190625 ADD START
    Private Sub WLSFDN_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
	...
	Private Sub WLS_DISPLAY()
        ...
        'Call DB_GetData("SOUMTA", "Where SOUCD = '" & DB_SYKTRA.OUTSOUCD & "'", "")
        'DB_SOUMTA = SOUMTA_GetNext(0)

        Call GetRowsCommon("SOUMTA", "Where SOUCD = '" & DB_SYKTRA.OUTSOUCD & "'")
        '20190606 CHG END
	...
	Private Sub WLS_FORM_LOAD()
        ...
        WLSLABEL.Text = "製番        出荷指示日 型 式                          得意先　　　　　　　　　　　           　倉庫名"
    ...
    Private Sub btnF2_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles btnF2.Click
        ...
        Try
            If Me.WLSHINNMA.Focused Then
                Call WLSHINNMA_KeyDown(WLSHINNMA, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            ElseIf Me.WLSSOUCD.Focused Then
                Call WLSSOUCD_KeyDown(WLSSOUCD, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            ElseIf Me.WLSTOKCD.Focused Then
                Call WLSTOKCD_KeyDown(WLSTOKCD, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            Else
                Call WLSHINCD_KeyDown(WLSHINCD, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            End If
    ...
    Private Sub WLSSSS_DSP()
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            DB_SYKTRA.ODNYTDT = DB_NullReplace(dt.Rows(i).Item("ODNYTDT"), "")
            DB_SYKTRA.OUTSOUCD = DB_NullReplace(dt.Rows(i).Item("OUTSOUCD"), "")
            DB_SYKTRA.SBNNO = DB_NullReplace(dt.Rows(i).Item("SBNNO"), "")
            DB_SYKTRA.HINNMA = DB_NullReplace(dt.Rows(i).Item("HINNMA"), "")
            DB_SYKTRA.TOKNMA = DB_NullReplace(dt.Rows(i).Item("TOKNMA"), "")
            DB_SYKTRA.JDNNO = DB_NullReplace(dt.Rows(i).Item("JDNNO"), "")
	...
	Private Function WLSSSS_DSP_CHECK() As Object
		...
		If Not IsDbNull(WM_WLS_ENDKEY) Then
            ...
            '	Exit Function
            'End If
            If LeftWid(FDN_PARA1, LenWid(WM_WLS_ENDKEY)) > WM_WLS_ENDKEY Then
                WLSSSS_DSP_CHECK = SSS_END
                Exit Function
            End If
            '20190620 CHG END
	...
	Private Sub WLSSSS_FORM_INIT()
        ...
        Select Case MidWid(FDN_PARA1, 14, 1)
            '20190620 CHG END
            Case "2"
                HD_WRKNM.Text = "通販"
            Case "3"
        ...
        HD_WRKKB.Text = MidWid(FDN_PARA1, 14, 1)
        HD_FDNDT.Text = MidWid(FDN_PARA1, 15, 10)
        '20190620 CHG END
	...
	Private Function WLSSSS_SET_KEYBAK() As Object
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            DB_SYKTRA.CLTID = DB_NullReplace(dt.Rows(i).Item("CLTID"), "")
            DB_SYKTRA.PGID = DB_NullReplace(dt.Rows(i).Item("PGID"), "")
            DB_SYKTRA.DATKB = DB_NullReplace(dt.Rows(i).Item("DATKB"), "")
            DB_SYKTRA.WRKKB = DB_NullReplace(dt.Rows(i).Item("WRKKB"), "")
            DB_SYKTRA.ODNYTDT = DB_NullReplace(dt.Rows(i).Item("ODNYTDT"), "")
            DB_SYKTRA.TOKCD = DB_NullReplace(dt.Rows(i).Item("TOKCD"), "")
            DB_SYKTRA.HINCD = DB_NullReplace(dt.Rows(i).Item("HINCD"), "")
            DB_SYKTRA.OUTSOUCD = DB_NullReplace(dt.Rows(i).Item("OUTSOUCD"), "")
            DB_SYKTRA.HINNMA = DB_NullReplace(dt.Rows(i).Item("HINNMA"), "")
