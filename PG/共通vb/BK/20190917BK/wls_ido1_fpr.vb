Option Strict Off
Option Explicit On
Module WLS_IDO1_0001
	'Copyright 1994-2006 by AppliTech, Inc. All Rights Reserved.
	
	'□□□□□□□□ プログラム単位の共通処理 Start □□□□□□□□□□□□□□□□
	'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
	'製番出庫伝票検索引数
	Public WLS_IDO1_PARAM As String '受注取引区分初期値
	Public WLS_IDO1_SYSDT As String 'システム日付
	'製番出庫伝票検索戻り値
	Public WLS_IDO1_DATNO As String '伝票管理番号
	
	'初期処理時チェック実行フラグ
	'画面初期化フラグ
	Public gv_bolWLS_IDO1_LF_Enable As Boolean 'LF処理実行フラグ(True：実行する）
	
	Private Const WLS_PrgNm As String = "製番出庫伝票検索"
	...
	Public Function F_Ctl_Item_Chk(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Process As String, ByRef pm_Chk_Move_Flg As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
			' F_Chk_***関数はこのファイル内で定義されていること。
			' このファイル内に無くて親フォームにあると、そちらが呼ばれてしまう。
			Case WLS_IDO1.HD_JDNDT.Name
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Dsp_HD_NHSCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Dsp_HD_SBNNO_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Ctl_CS_JDNDT(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_IDO1.HD_JDNDT.Tag)
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			gv_bolWLS_IDO1_LF_Enable = False
			...
			If WLSDATE_RTNCODE <> "" Then
				...
				If Chk_Move_Flg = True Then
					'ﾁｪｯｸ後移動あり
					Call F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRIGHT, Move_Flg, pm_All)
				Else
	...
	Public Function F_Ctl_CS_HINCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_IDO1.HD_HINCD.Tag)
		Next_Focus = Trg_Index
		
		'ﾌｫｰｶｽを営業担当者コードへ移動
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			'        '現在のActiveコントロールの選択状態解除
			'        Call CF_Set_Item_Color(pm_All.Dsp_Sub_Inf(CInt(WLS_IDO1.ActiveControl.Tag)), ITEM_NORMAL_STATUS, pm_All)
			'ﾌｫｰｶｽ移動
			Call CF_Set_Item_SetFocus(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
			'選択状態の設定（初期選択）
			Call CF_Set_Sel_Ini(pm_All.Dsp_Sub_Inf(Trg_Index), SEL_INI_MODE_2)
	...
	Public Function F_Ctl_CS_TOKCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_IDO1.HD_TOKCD.Tag)
		Next_Focus = Trg_Index
	...
	Public Function F_Ctl_CS_NHSCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_IDO1.HD_NHSCD.Tag)
		Next_Focus = Trg_Index
	...
	Public Function F_Ctl_CS_JDNTRKB(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_IDO1.HD_JDNTRKB.Tag)
		Next_Focus = Trg_Index
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
            ...
            'システム日付を基準日とする
            '20190604 ADD START
            WLS_IDO1_SYSDT = DB_NullReplace(CNV_DATE(DB_UNYMTA.UNYDT), Format(Now(), "yyyy/MM/dd"))
            '20190604 ADD END
            WLSBMN_KJNDT = WLS_IDO1_SYSDT
	...
	Public Function F_Init_Clr_Dsp(ByRef pm_Index As Short, ByRef pm_All As Cls_All) As Short
		...
		For Index_Wk = Wk_Index_S To Wk_Index_E
			...
			Select Case Index_Wk
				Case CShort(WLS_IDO1.HD_JDNDT.Tag)
					' 出庫指示日付
					
				Case CShort(WLS_IDO1.LST.Tag)
					'ListBox
					WLS_IDO1.LST.Items.Clear()
					WLS_IDO1.KEYBAK.Items.Clear()
				Case Else
			End Select
			'Ｅ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｅ
