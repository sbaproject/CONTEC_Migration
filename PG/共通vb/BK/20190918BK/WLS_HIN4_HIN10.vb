...
Friend Class WLSHIN4
    ...
    Private Sub WLS_FORM_INIT()
        ...
        '''' UPD 2009/02/17  FKS) S.Nakajima    Start
        '    WM_WLS_LEN = 8
        WM_WLS_LEN = 10
        '''' UPD 2009/02/17  FKS) S.Nakajima    End
        ...
        '20190704 CHG START
        'WLSLABEL = "製品ｺｰﾄﾞ   型　　式                       品　　名                                          "
        WLSLABEL.Text = "製品ｺｰﾄﾞ   型　　式                       品　　名                                          "
        '20190704 CHG END
        'XXXXXXXX10 XXXXXXXXX1XXXXXXXXX2XXXXXXXXX3 MMMMMMMMM1MMMMMMMMM2MMMMMMMMM3MMMMMMMMM4MMMMMMMMM5
        '''' UPD 2009/02/17  FKS) S.Nakajima    End
    ...
    Private Sub WLS_SetArray(ByVal ArrayCnt As Short)
        ...
        WM_WLS_DSPArray(ArrayCnt) = LeftWid(DB_HINMTA.HINCD, 10) & " " & LeftWid(DB_HINMTA.HINNMA, 30) & " " & LeftWid(DB_HINMTA.HINNMB, 50)
        '''' UPD 2009/02/17  FKS) S.Nakajima    End
    End Sub
    ...
    Sub WLS_KbSQL()
        ...
        ' === 20081205 === UPDATE S - ACE)Nagasawa 「'」入力対応
        'D    WlsFromWhere = "From HINMTA Where HINKB = '" & WM_WLS_STTKEY & "'"
        'UPGRADE_WARNING: オブジェクト WM_WLS_STTKEY の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
    ...
    Sub WLS_TextSQL()
        ...
        If WM_WLS_STTKEY = "" Then WM_WLS_STTKEY = " "
        ' === 20081205 === UPDATE S - ACE)Nagasawa 「'」入力対応
        'D    WlsFromWhere = "From HINMTA Where HINCD >= '" & WM_WLS_STTKEY & "'"
        'UPGRADE_WARNING: オブジェクト WM_WLS_STTKEY の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
        WlsFromWhere = "From HINMTA Where HINCD >= '" & AE_EditSQLText(WM_WLS_STTKEY) & "'"
        ' === 20081205 === UPDATE E
        If Trim(WLSHINKB.Text) <> "" Then
            ' === 20081205 === UPDATE S - ACE)Nagasawa 「'」入力対応
            'D        WlsFromWhere = WlsFromWhere & " and HINKB = '" & WLSHINKB.Text & "'"
    ...
    Sub WLS_KanaSQL()
        ...
        If WM_WLS_STTKEY = "" Then WM_WLS_STTKEY = " "
        ' === 20081205 === UPDATE S - ACE)Nagasawa レスポンス対応
        'D    WlsFromWhere = "From HINMTA Where HINNK >= '" & WM_WLS_STTKEY & "' And HINNK < '" & WM_WLS_ENDKEY & "'"
    ...
    Private Sub WLS_DspNew()
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            DB_HINMTA.HINCD = DB_NullReplace(dsList.Tables(WM_WLS_MFIL).Rows(i).Item("HINCD"), "")
            DB_HINMTA.HINNMA = DB_NullReplace(dsList.Tables(WM_WLS_MFIL).Rows(i).Item("HINNMA"), "")
            DB_HINMTA.HINNMB = DB_NullReplace(dsList.Tables(WM_WLS_MFIL).Rows(i).Item("HINNMB"), "")
    ...
    Private Sub COM_HINKB_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles COM_HINKB.Click
        ...
        Else
            ...
            Call DSPMEIM_SEARCH("077", wkHINKB, DB_MEIMTA)
            '20190708 CHG END
    ...
    Private Sub WLSHIN_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        If WM_WLS_INIT = True Then
            ...
            strSQL = strSQL & "  from MEIMTA "
            strSQL = strSQL & "  Where KEYCD  = '077' "
            strSQL = strSQL & "  and   MEICDA  = '" & WLSHINKB.Text & "'"
            strSQL = strSQL & "  Order By MEICDA "

            Dim dt As DataTable = DB_GetTable(strSQL)
            Call Set_DB_MEIMTA(dt, DB_MEIMTA, 0)
            '20190708 CHG END
    ...
    '20190708 ADD START
    Private Sub WLSHIN_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
...
EventExitSub:
    ...
    Private Sub btnF2_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles btnF2.Click
        ...
        Try
            If Me.HD_NMA.Focused Then
                Call HD_NMA_KeyDown(HD_NMA, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            ElseIf Me.HD_Kana.Focused Then
                Call HD_Kana_KeyDown(HD_Kana, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            ElseIf Me.HD_TEXT.Focused Then
                Call HD_TEXT_KeyDown(HD_TEXT, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            Else
                Call WLSHINKB_KeyDown(WLSHINKB, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            End If
