...
Module WLSMIT0001
	...
	'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
	'見積情報検索引数
	Public WLSMIT_KKTFL As String '確定区分初期値("1":確定 "9":未確定 以外は全件）
	Public WLSMIT_JDNTRKB As String '受注取引区分初期値
	'見積情報検索戻り値
	Public WLSMIT_RTNMITNO As String '見積番号
	Public WLSMIT_RTNMITNOV As String '版数
	
	'初期処理時チェック実行フラグ
	'画面初期化フラグ
	Public gv_bolWLSMIT_LF_Enable As Boolean 'LF処理実行フラグ(True：実行する）
	Public pv_strInit_KKTFL As String '確定フラグ初期値
	Public pv_strInit_JDNTRKB As String '受注取引区分初期値
	' === 20060824 === INSERT S - ACE)Sejima 諸口対応
	Public WLSMIT0001_SKCHKB As String '諸口区分
	' === 20060824 === INSERT E
	' === 20061006 === INSERT S - ACE)Nagasawa
	Public WLSMIT0001_JNDTRFLG As String '受注取込フラグ("":受注に取り込まれた見積は表示しない)
	' === 20061006 === INSERT E -
	
	Private Const WLS_PrgNm As String = "見積情報検索"
	
	'メッセージコード
	Private Const gc_strMsgWLSMIT_E_001 As String = "2WLSMIT_001" '入力値が許容範囲外です。
	Private Const gc_strMsgWLSMIT_E_002 As String = "2WLSMIT_002" '日付に誤りがあります。修正してください。
	
	Private pv_strTANNM As String '担当者名
	Private pv_strJDNTRKBNM As String '受注取引区分名
	
	Public Const gc_strWLSMIT_KKTFL_KKT As String = "1" '確定
	Public Const gc_strWLSMIT_KKTFL_MKK As String = "9" '未確定
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Chk_HD_KKTFL(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Chk_HD_MITDT(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Ctl_Item_Chk(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Process As String, ByRef pm_Chk_Move_Flg As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			...
			Case WLS_MTMET61.HD_JDNTRKB.Name
			...
			Case WLS_MTMET61.HD_JDNTRKBNM.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'受注取引区分名ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_JDNTRKBNM(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_MTMET61.HD_KENNMA.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'件名ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_KENNMA(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_MTMET61.HD_KKTFL.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'確定区分ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_KKTFL(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_MTMET61.HD_MITDT.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'見積日ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_MITDT(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_MTMET61.HD_MITNO.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'見積番号ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_MITNO(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_MTMET61.HD_MITNOV.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'見積番号版数ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_MITNOV(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_MTMET61.HD_TANCD.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'担当者コードﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_TANCD(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_MTMET61.HD_TANNM.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'担当者名ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_TANNM(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_MTMET61.HD_TOKCD.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'得意先コードﾁｪｯｸ
	...
	Public Function F_Dsp_HD_JDNTRKB_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
				'【受注取引区分名】
				Trg_Index = CShort(WLS_MTMET61.HD_JDNTRKBNM.Tag)
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_KKTFL_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_MITDT_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Dsp_HD_TANCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
				'【担当者名】
				Trg_Index = CShort(WLS_MTMET61.HD_TANNM.Tag)
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Public Function F_Dsp_Item_Detail(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			...
			Case WLS_MTMET61.HD_KKTFL.Name
				'確定区分による画面表示
				Call F_Dsp_HD_KKTFL_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
				
			Case WLS_MTMET61.HD_MITDT.Name
				'見積日による画面表示
				Call F_Dsp_HD_MITDT_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Ctl_CS_MITDT
	'   概要：  対象項目のカレンダ検索ﾎﾞﾀﾝの制御
	'   引数：  pm_Mode : 呼出元項目判定用コード
	'   戻値：　なし
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Ctl_CS_MITDT(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_MTMET61.HD_MITDT.Tag)
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			If WLSDATE_RTNCODE <> "" Then
				...
				If Chk_Move_Flg = True Then
					'ﾁｪｯｸ後移動あり
					Call WLSMIT0001.F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRIGHT, Move_Flg, pm_All)
				Else
	...
	Public Function F_Ctl_CS_TANCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_MTMET61.HD_TANCD.Tag)
		Next_Focus = Trg_Index
		
		'ﾌｫｰｶｽを営業担当者コードへ移動
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			'        '現在のActiveコントロールの選択状態解除
			'        Call CF_Set_Item_Color(pm_All.Dsp_Sub_Inf(CInt(WLS_MTMET61.ActiveControl.Tag)), ITEM_NORMAL_STATUS, pm_All)
			'ﾌｫｰｶｽ移動
			Call CF_Set_Item_SetFocus(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
			'選択状態の設定（初期選択）
			Call CF_Set_Sel_Ini(pm_All.Dsp_Sub_Inf(Trg_Index), SEL_INI_MODE_2)
			...
			' === 20061206 === INSERT S - ACE)Nagasawa 見積/受注では営業担当者のみ入力
			...
			If WLSTAN_RTNCODE <> "" Then
				...
				If Chk_Move_Flg = True Then
					'ﾁｪｯｸ後移動あり
					Call WLSMIT0001.F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRETURN, Move_Flg, pm_All)
				Else
					' === 20060801 === UPDATE S - ACE)Nagasawa  検索ボタンを押したように見せるように対応
					'                'ﾁｪｯｸ後移動なし
					'                Call CF_Set_Item_SetFocus(pm_Dsp_Sub_Inf, pm_All)
	...
	Public Function F_Ctl_CS_TOKCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_MTMET61.HD_TOKCD.Tag)
		Next_Focus = Trg_Index
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			'担当者検索画面を呼び出す
			' === 20060824 === INSERT S - ACE)Sejima 諸口対応
			WLSTOK_SKCHKB = WLSMIT0001_SKCHKB
			' === 20060824 === INSERT E
            WLSTOK1.ShowDialog()
            WLSTOK1.Close()
	...
	Public Function F_Ctl_CS_JDNTRKB(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_MTMET61.HD_JDNTRKB.Tag)
		Next_Focus = Trg_Index
	...
	Public Function F_Init_Clr_Dsp(ByRef pm_Index As Short, ByRef pm_All As Cls_All) As Short
		...
		For Index_Wk = Wk_Index_S To Wk_Index_E
			...
			Select Case Index_Wk
				Case CShort(WLS_MTMET61.HD_KKTFL.Tag)
					'確定フラグ
					Call CF_Set_Item_Direct(CF_Cnv_Dsp_Item(pv_strInit_KKTFL, pm_All.Dsp_Sub_Inf(Index_Wk), False), pm_All.Dsp_Sub_Inf(Index_Wk), pm_All)
					...
					pm_All.Dsp_Sub_Inf(Index_Wk).Detail.Bef_Chk_Value = pv_strInit_KKTFL
					
				Case CShort(WLS_MTMET61.HD_JDNTRKB.Tag)
					'受注取引区分
					Call CF_Set_Item_Direct(CF_Cnv_Dsp_Item(pv_strInit_JDNTRKB, pm_All.Dsp_Sub_Inf(Index_Wk), False), pm_All.Dsp_Sub_Inf(Index_Wk), pm_All)
					'UPGRADE_WARNING: オブジェクト pm_All.Dsp_Sub_Inf().Detail.Bef_Chk_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
					pm_All.Dsp_Sub_Inf(Index_Wk).Detail.Bef_Chk_Value = pv_strInit_JDNTRKB
					Call F_Chk_HD_JDNTRKB(pm_All.Dsp_Sub_Inf(Index_Wk), Chk_Move_Flg, pm_All)
					
				Case CShort(WLS_MTMET61.HD_JDNTRKBNM.Tag)
					'受注取引区分名称
				...
				Case WLS_MTMET61.LST.TabIndex
					WLS_MTMET61.LST.Items.Clear()
				Case Else
			End Select
			'Ｅ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｅ
