...
Module WLSANID0001
	...
	'初期処理時チェック実行フラグ
	'画面初期化フラグ
	Public gv_bolWLSANID_LF_Enable As Boolean 'LF処理実行フラグ(True：実行する）
	
	Private Const WLS_PrgNm As String = "案件情報検索"
	
	'メッセージコード
	Private Const gc_strMsgWLSANID_E_001 As String = "2WLSANID_001" '入力値が許容範囲外です。
	Private Const gc_strMsgWLSANID_E_002 As String = "2WLSANID_002" '日付に誤りがあります。修正してください。
	...
	Public Function F_Set_Next_Focus(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_Move_Flg As Boolean, ByRef pm_All As Cls_All, Optional ByRef pm_Run_Flg As Boolean = True) As Short
		...
		Else
			...
			'検索開始は対象の項目の次
			Sta_Index = CShort(pm_Dsp_Sub_Inf.Ctl.Tag) + 1
			If pm_Dsp_Sub_Inf.Ctl.Tag = WLS_ANID.WLSCANCEL.Tag Then
				Sta_Index = CShort(WLS_ANID.HD_TANCD.Tag)
			End If
		End If
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_JDNYTDT(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			If CF_Chk_Item_Base(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf, Input_Value) <> CHK_BASE_OK Then
				...
				Err_Cd = gc_strMsgWLSANID_E_002 '入力範囲外
			Else
				' === 20060817 === DELETE S - ACE)Nagasawa 一時的に削除
				'' === 20060810 === INSERT S - ACE)Nagasawa
				'            If IsDate(Input_Value) = False Then
				'                Retn_Code = CHK_ERR_ELSE
				'                Err_Cd = gc_strMsgWLSANID_E_002              '入力範囲外
				'            Else
				'' === 20060810 === INSERT E -
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_OAKNID(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			If CF_Chk_Item_Base(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf, Input_Value) <> CHK_BASE_OK Then
				Retn_Code = CHK_ERR_ELSE
				Err_Cd = gc_strMsgWLSANID_E_001 '入力範囲外
			Else
				'ＯＫ
				Retn_Code = CHK_OK
				pm_Chk_Move = True
	...
	Public Function F_Ctl_Item_Chk(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Process As String, ByRef pm_Chk_Move_Flg As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			...
			Case WLS_ANID.HD_JDNYTDT.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'受注予定日ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_JDNYTDT(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_ANID.HD_OAKNID.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'親案件IDﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_OAKNID(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_ANID.HD_TANCD.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'担当者コードﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_TANCD(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_ANID.HD_TANNM.Name
				'担当者名ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'ﾁｪｯｸ
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Dsp_HD_JDNYTDT_Inf
	'   概要：  受注予定日による画面表示
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_JDNYTDT_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Dsp_HD_OAKNID_Inf
	'   概要：  親案件IDによる画面表示
	'   引数：  pm_Dsp_Sub_Inf   : 画面項目情報
	'           pm_Mode          : 画面表示モード
	'           pm_All           : 画面情報
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_OAKNID_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Dsp_HD_TANCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
				'【担当者名】
				Trg_Index = CShort(WLS_ANID.HD_TANNM.Tag)
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	Public Function F_Dsp_Item_Detail(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			...
			Case WLS_ANID.HD_JDNYTDT.Name
				'画面表示
				Call F_Dsp_HD_JDNYTDT_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
				
			Case WLS_ANID.HD_OAKNID.Name
				'画面表示
				Call F_Dsp_HD_OAKNID_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Ctl_CS_JDNYTDT
	'   概要：  対象項目のカレンダ検索ﾎﾞﾀﾝの制御
	'   引数：  pm_Mode : 呼出元項目判定用コード
	'   戻値：　なし
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Ctl_CS_JDNYTDT(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_ANID.HD_JDNYTDT.Tag)
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			If WLSDATE_RTNCODE <> "" Then
				...
				If Chk_Move_Flg = True Then
					'ﾁｪｯｸ後移動あり
					Call WLSANID0001.F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRIGHT, Move_Flg, pm_All)
				Else
					' === 20060810 === UPDATE S - ACE)Nagasawa
					'                'ﾁｪｯｸ後移動なし
	...
	Public Function F_Ctl_CS_TANCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_ANID.HD_TANCD.Tag)
		Next_Focus = Trg_Index
		
		'ﾌｫｰｶｽを営業担当者コードへ移動
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			'        '現在のActiveコントロールの選択状態解除
			'        Call CF_Set_Item_Color(pm_All.Dsp_Sub_Inf(CInt(WLS_ANID.ActiveControl.Tag)), ITEM_NORMAL_STATUS, pm_All)
			'ﾌｫｰｶｽ移動
			Call CF_Set_Item_SetFocus(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
			'選択状態の設定（初期選択）
			Call CF_Set_Sel_Ini(pm_All.Dsp_Sub_Inf(Trg_Index), SEL_INI_MODE_2)
			...
			If WLSTAN_RTNCODE <> "" Then
				...
				If Chk_Move_Flg = True Then
					'ﾁｪｯｸ後移動あり
					Call WLSANID0001.F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRETURN, Move_Flg, pm_All)
				Else
					' === 20060810 === UPDATE S - ACE)Nagasawa
					'                'ﾁｪｯｸ後移動なし
					'                Call CF_Set_Item_SetFocus(pm_Dsp_Sub_Inf, pm_All)
	...
	Public Function F_Init_Clr_Dsp(ByRef pm_Index As Short, ByRef pm_All As Cls_All) As Short
		...
		For Index_Wk = Wk_Index_S To Wk_Index_E
			...
			Select Case Index_Wk
				Case WLS_ANID.LST.TabIndex
					WLS_ANID.LST.Items.Clear()
				Case Else
			End Select
			'Ｅ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｅ
