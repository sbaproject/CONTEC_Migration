...
Module WLS_JDN2_0001
    ...
    Public WLS_JDN2_0001_SKCHKB As String '諸口区分
	' === 20060824 === INSERT E
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_NHTCD(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Ctl_Item_Chk(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Process As String, ByRef pm_Chk_Move_Flg As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			...
			Case WLS_JDN2.HD_JDNTRKBNM.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'受注取引区分名ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_JDNTRKBNM(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_JDN2.HD_KENNMA.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'件名ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_KENNMA(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_JDN2.HD_JDNNO.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'受注番号チェック
				Rtn_Chk = F_Chk_HD_JDNNO(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_JDN2.HD_NHTCD.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'受注日付チェック
				Rtn_Chk = F_Chk_HD_NHTCD(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_JDN2.HD_TOKCD.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'得意先コードﾁｪｯｸ
	...
	Public Function F_Dsp_HD_JDNTRKB_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				...
				'Ｓ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｓ
				'【受注取引区分名】
				Trg_Index = CShort(WLS_JDN2.HD_JDNTRKBNM.Tag)
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Dsp_HD_NHTCD_Inf
	'   概要：  受注日による画面表示
	'   引数：  pm_Dsp_Sub_Inf   :
	'           pm_Mode          : 画面表示モード
	'           pm_All           : 画面情報
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_NHTCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Dsp_Item_Detail(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			...
			Case WLS_JDN2.HD_NHTCD.Name
				'受注日付による画面表示
				Call F_Dsp_HD_NHTCD_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
	...
	Public Function F_Ctl_CS_JDNDT(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
        ...
        '割当ｲﾝﾃﾞｯｸｽ取得
        Trg_Index = CShort(WLS_JDN2.HD_NHTCD.Tag)
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			If WLSDATE_RTNCODE <> "" Then
				...
				If Chk_Move_Flg = True Then
					'ﾁｪｯｸ後移動あり
					Call WLS_JDN2_0001.F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRIGHT, Move_Flg, pm_All)
				Else
	...
	Public Function F_Ctl_CS_TOKCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_JDN2.HD_TOKCD.Tag)
		Next_Focus = Trg_Index
		
		'ﾌｫｰｶｽを得意先コードへ移動
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			'        '現在のActiveコントロールの選択状態解除
			'        Call CF_Set_Item_Color(pm_All.Dsp_Sub_Inf(CInt(WLS_JDN2.ActiveControl.Tag)), ITEM_NORMAL_STATUS, pm_All)
			'ﾌｫｰｶｽ移動
			Call CF_Set_Item_SetFocus(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
			'選択状態の設定（初期選択）
			Call CF_Set_Sel_Ini(pm_All.Dsp_Sub_Inf(Trg_Index), SEL_INI_MODE_2)
            ...
            '得意先検索画面を呼び出す
            ' === 20060824 === INSERT S - ACE)Sejima 諸口対応
            WLSTOK_SKCHKB = WLS_JDN2_0001_SKCHKB
            ' === 20060824 === INSERT E
            WLSTOK1.ShowDialog()
            WLSTOK1.Close()
            ...
            If WLSTOK_RTNCODE <> "" Then
                ...
                If Chk_Move_Flg = True Then
                    'ﾁｪｯｸ後移動あり
                    Call WLS_JDN2_0001.F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRETURN, Move_Flg, pm_All)
                Else
                    ' === 20060801 === UPDATE S - ACE)Nagasawa  検索ボタンを押したように見せるように対応
                    '                'ﾁｪｯｸ後移動なし
                    '                Call CF_Set_Item_SetFocus(pm_Dsp_Sub_Inf, pm_All)
	...
	Public Function F_Ctl_CS_NHSCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		If WLS_JDN2.ActiveControl Is Nothing Then
			Exit Function
		End If
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_JDN2.HD_NHTCD.Tag)
		' === 20060823 === UPDATE S - ACE)Nagasawa
		'    Next_Focus = Trg_Index + 1
		Next_Focus = Trg_Index
		' === 20060823 === UPDATE E -
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			Call CF_Set_Item_Color(pm_All.Dsp_Sub_Inf(CShort(WLS_JDN2.ActiveControl.Tag)), ITEM_NORMAL_STATUS, pm_All)
			'ﾌｫｰｶｽ移動
			Call CF_Set_Item_SetFocus(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
			'選択状態の設定（初期選択）
			Call CF_Set_Sel_Ini(pm_All.Dsp_Sub_Inf(Trg_Index), SEL_INI_MODE_2)
            ...
            If WLSNHSMTA_RTNCODE <> "" Then
                ...
                End If
            End If
            ' === 20060731 === INSERT S - ACE)Nagasawa　検索Wボタン対応
        Else
			'ﾁｪｯｸ後移動なし
			Call CF_Set_Item_SetFocus(pm_Dsp_Sub_Inf, pm_All)
			'項目色設定(エラー時はﾌｫｰｶｽなしの色設定！！)
	...
	Public Function F_Ctl_CS_JDNTRKB(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_JDN2.HD_JDNTRKB.Tag)
		Next_Focus = Trg_Index
	...
	Public Function F_Init_Clr_Dsp(ByRef pm_Index As Short, ByRef pm_All As Cls_All) As Short
		...
		For Index_Wk = Wk_Index_S To Wk_Index_E
			...
			Select Case Index_Wk
				Case CShort(WLS_JDN2.HD_JDNTRKB.Tag)
					'受注取引区分
				...
				Case CShort(WLS_JDN2.HD_JDNTRKBNM.Tag)
					'受注取引区分名称
				...
				Case CShort(WLS_JDN2.LST.Tag)
					'ListBox
					WLS_JDN2.LST.Items.Clear()
				Case Else
			End Select
			'Ｅ★★★★★★★★★★★★★★★★★★★★★★★★★★★★Ｅ
