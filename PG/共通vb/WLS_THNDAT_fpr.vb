Option Strict Off
Option Explicit On
Module WLSTHNDAT0001
	
	'通販ﾃﾞｰﾀ検索戻り値
	Public WLSTHNDAT_RTNJDNNO As String '受注番号
	
	'初期処理時チェック実行フラグ
	'画面初期化フラグ
	Public gv_bolWLSTHNDAT_LF_Enable As Boolean 'LF処理実行フラグ(True：実行する）
	
	Private Const WLS_PrgNm As String = "通販データ検索"
	
	'メッセージコード
	Private Const gc_strMsgWLSTHNDAT_E_001 As String = "2WLSTHNDAT_001" '入力値が許容範囲外です。
	Private Const gc_strMsgWLSTHNDAT_E_002 As String = "2WLSTHNDAT_002" '日付に誤りがあります。修正してください。
	
	Private pv_strKSHNM As String '決済方法名
			...
			If CF_Jge_Input_Str(pm_Dsp_Sub_Inf, wk_Moji) = 1 Then
				...
				Else
					...
					Else
                        ...
                        '20190829 CHG START
                        'pm_Dsp_Sub_Inf.Ctl.SelStart = Wk_SelStart
                        DirectCast(pm_Dsp_Sub_Inf.Ctl, TextBox).SelectionStart = Wk_SelStart                        '編集後のSelLengthを決定
                        'UPGRADE_WARNING: オブジェクト pm_Dsp_Sub_Inf.Ctl.SelLength の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
                        'pm_Dsp_Sub_Inf.Ctl.SelLength = Wk_SelLength
                        DirectCast(pm_Dsp_Sub_Inf.Ctl, TextBox).SelectionLength = Wk_SelLength
                        '20190829 CHG END
	...
	Public Function F_Set_Next_Focus(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_Move_Flg As Boolean, ByRef pm_All As Cls_All, Optional ByRef pm_Run_Flg As Boolean = True) As Short
		...
		Else
			...
			'検索開始は対象の項目の次
			Sta_Index = CShort(pm_Dsp_Sub_Inf.Ctl.Tag) + 1
			If pm_Dsp_Sub_Inf.Ctl.Tag = WLS_THNDAT.WLSCANCEL.Tag Then
				Sta_Index = CShort(WLS_THNDAT.HD_STS.Tag)
			End If
		End If
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_STS(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			If CF_Chk_Item_Base(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf, Input_Value) <> CHK_BASE_OK Then
				...
				Err_Cd = gc_strMsgWLSTHNDAT_E_001 '入力範囲外
			Else
				If Input_Value <> gc_strSTS_MSK And Input_Value <> gc_strSTS_ZUA And Input_Value <> gc_strSTS_ZNK And Input_Value <> gc_strSTS_ALL Then
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgWLSTHNDAT_E_001 '入力範囲外
				Else
					'ＯＫ
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Chk_HD_ODNYTDT_ST
	'   概要：  出荷予定日（開始）のﾁｪｯｸ
	'   引数：　pm_Chk_Dsp_Sub_Inf    :チェック項目
	'           pm_Chk_Move　　　　　 :チェック後移動フラグ（T：移動OK、F：移動NG）
	'           pm_All                :画面情報
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_ODNYTDT_ST(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			If CF_Chk_Item_Base(CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf), pm_Chk_Dsp_Sub_Inf, Input_Value) <> CHK_BASE_OK Then
				Retn_Code = CHK_ERR_ELSE
				Err_Cd = gc_strMsgWLSTHNDAT_E_002 '入力範囲外
			Else
				
				'出荷予定日（終了）の取得
				Trg_Index = CShort(WLS_THNDAT.HD_ODNYTDT_ED.Tag)
				'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				wk_ODNYTDT_ED = CF_Trim_Item(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(Trg_Index)), pm_All.Dsp_Sub_Inf(Trg_Index))
				
				' 出荷予定日（開始）が出荷予定日（終了）より後日付の場合
				'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf) の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				If wk_ODNYTDT_ED <> "" And wk_ODNYTDT_ED < CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf) Then
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgWLSTHNDAT_E_002 '入力範囲外
				Else
					'ＯＫ
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Chk_HD_ODNYTDT_ED
	'   概要：  出荷予定日（終了）のﾁｪｯｸ
	'   引数：　pm_Chk_Dsp_Sub_Inf    :チェック項目
	'           pm_Chk_Move　　　　　 :チェック後移動フラグ（T：移動OK、F：移動NG）
	'           pm_All                :画面情報
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_ODNYTDT_ED(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			Else
				...
				'出荷予定日（開始）の取得
				Trg_Index = CShort(WLS_THNDAT.HD_ODNYTDT_ST.Tag)
				'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				wk_ODNYTDT_ST = CF_Trim_Item(CF_Get_Item_Value(pm_All.Dsp_Sub_Inf(Trg_Index)), pm_All.Dsp_Sub_Inf(Trg_Index))
				
				' 出荷予定日（終了）が出荷予定日（開始）より前日付の場合
				'UPGRADE_WARNING: オブジェクト CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf) の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				If wk_ODNYTDT_ST <> "" And wk_ODNYTDT_ST > CF_Get_Item_Value(pm_Chk_Dsp_Sub_Inf) Then
					Retn_Code = CHK_ERR_ELSE
					Err_Cd = gc_strMsgWLSTHNDAT_E_002 '入力範囲外
				Else
					'ＯＫ
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_TOKCD(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Else
			...
			Else
				'マスタチェック
				If DSPMEINMB_SEARCH(gc_strKEYCD_KSHTK, Input_Value, Mst_Inf) = 0 Then
					If Mst_Inf.DATKB = gc_strDATKB_USE Then
						'取得項目格納
						pv_strKSHNM = Mst_Inf.MEINMA
					Else
						pv_strKSHNM = ""
					End If
				Else
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_KSHNM(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_HINCD(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Private Function F_Chk_HD_TOKJDNNO(ByRef pm_Chk_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Chk_Move As Boolean, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Ctl_Item_Chk(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Process As String, ByRef pm_Chk_Move_Flg As Boolean, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			...
			Case WLS_THNDAT.HD_STS.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'ステータスﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_STS(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_THNDAT.HD_ODNYTDT_ST.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'出荷予定日（開始）ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_ODNYTDT_ST(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_THNDAT.HD_ODNYTDT_ED.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'出荷予定日（終了）ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_ODNYTDT_ED(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_THNDAT.HD_TOKCD.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
                '決済方法（得意先コード）ﾁｪｯｸ
                '20190904 CHG START
                'Rtn_Chk = SSSMAIN0001.F_Chk_HD_TOKCD(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
                Rtn_Chk = F_Chk_HD_TOKCD(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
                '20190904 CHG END
            Case WLS_THNDAT.HD_KSHNM.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'決済方法名ﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_KSHNM(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_THNDAT.HD_HINCD.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'製品コードﾁｪｯｸ
				Rtn_Chk = F_Chk_HD_HINCD(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
				
			Case WLS_THNDAT.HD_TOKJDNNO.Name
				'ﾁｪｯｸ前処理(ﾁｪｯｸ関数の前で必須処理)
				Call CF_Set_Chk_From_Process(pm_Dsp_Sub_Inf, pm_Process, pm_All)
				'客先注文番号ﾁｪｯｸ
				Rtn_Chk = SSSMAIN0001.F_Chk_HD_TOKJDNNO(pm_Dsp_Sub_Inf, pm_Chk_Move_Flg, pm_All)
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_STS_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Dsp_HD_ODNYTDT_ST_Inf
	'   概要：  出荷予定日（開始）による画面表示
	'   引数：  pm_Dsp_Sub_Inf   : 画面項目情報
	'           pm_Mode          : 画面表示モード
	'           pm_All           : 画面情報
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_ODNYTDT_ST_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Dsp_HD_ODNYTDT_ED_Inf
	'   概要：  出荷予定日（終了）による画面表示
	'   引数：  pm_Dsp_Sub_Inf   : 画面項目情報
	'           pm_Mode          : 画面表示モード
	'           pm_All           : 画面情報
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_ODNYTDT_ED_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Dsp_HD_TOKCD_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		If pm_Mode = DSP_SET Then
			...
			If CF_Get_Item_Value(pm_Dsp_Sub_Inf) <> pm_Dsp_Sub_Inf.Detail.Bef_Chk_Value Then
				'【決済方法名】
				Trg_Index = CShort(WLS_THNDAT.HD_KSHNM.Tag)
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				...
				Dsp_Value = CF_Cnv_Dsp_Item(pv_strKSHNM, pm_All.Dsp_Sub_Inf(Trg_Index), False)
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Dsp_HD_KSHNM_Inf
	'   概要：  決済方法名による画面表示
	'   引数：  pm_Dsp_Sub_Inf   : 画面項目情報
	'           pm_Mode          : 画面表示モード
	'           pm_All           : 画面情報
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_KSHNM_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Dsp_HD_TOKJDNNO_Inf
	'   概要：  客先注文番号による画面表示
	'   引数：  pm_Dsp_Sub_Inf   : 画面項目情報
	'           pm_Mode          : 画面表示モード
	'           pm_All           : 画面情報
	...
	'   備考：  プログラム単位の共通処理
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Dsp_HD_TOKJDNNO_Inf(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Dsp_Item_Detail(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_Mode As Short, ByRef pm_All As Cls_All) As Short
		...
		Select Case pm_Dsp_Sub_Inf.Ctl.Name
			Case WLS_THNDAT.HD_STS.Name
				'ステータス
				Call F_Dsp_HD_STS_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
				
			Case WLS_THNDAT.HD_ODNYTDT_ST.Name
				'出荷予定日（開始）
				Call F_Dsp_HD_ODNYTDT_ST_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
				
			Case WLS_THNDAT.HD_ODNYTDT_ED.Name
				'出荷予定日（終了）
				Call F_Dsp_HD_ODNYTDT_ED_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
			...
			Case WLS_THNDAT.HD_KSHNM.Name
				'決済方法名
				Call F_Dsp_HD_KSHNM_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
			...
			Case WLS_THNDAT.HD_TOKJDNNO.Name
				'客先注文番号
				Call F_Dsp_HD_TOKJDNNO_Inf(pm_Dsp_Sub_Inf, pm_Mode, pm_All)
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Ctl_CS_ODNYTDT_ST
	'   概要：  対象項目のカレンダ検索ﾎﾞﾀﾝの制御
	'   引数：  pm_Mode : 呼出元項目判定用コード
	'   戻値：　なし
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Ctl_CS_ODNYTDT_ST(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			...
			gv_bolWLSTHNDAT_LF_Enable = False
			...
			gv_bolWLSTHNDAT_LF_Enable = True
			...
			If WLSDATE_RTNCODE <> "" Then
				...
				If Chk_Move_Flg = True Then
					'ﾁｪｯｸ後移動あり
					Call WLSTHNDAT0001.F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRIGHT, Move_Flg, pm_All)
				Else
					'ﾌｫｰｶｽ移動
					Call CF_Set_Item_SetFocus(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
				End If
	...
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	'   名称：  Function F_Ctl_CS_ODNYTDT_ED
	'   概要：  対象項目のカレンダ検索ﾎﾞﾀﾝの制御
	'   引数：  pm_Mode : 呼出元項目判定用コード
	'   戻値：　なし
	'   備考：
	' ======+=======+=======+=======+=======+=======+=======+=======+=======+=======+
	Public Function F_Ctl_CS_ODNYTDT_ED(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
	...
	Public Function F_Ctl_CS_TOKCD(ByRef pm_Dsp_Sub_Inf As Cls_Dsp_Sub_Inf, ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_THNDAT.HD_TOKCD.Tag)
		Next_Focus = Trg_Index
		
		'ﾌｫｰｶｽを決済方法（得意先コード）へ移動
		If CF_Set_Focus_Ctl(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All) = True Then
			'ﾌｫｰｶｽ移動
			Call CF_Set_Item_SetFocus(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
			'選択状態の設定（初期選択）
			...
			'名称マスタを検索するキー
			WLSMEI_KEYCD = gc_strKEYCD_KSHTK
			''' DSPMEINMB_SEARCH
			...
			WLS_KSH.ShowDialog()
            WLS_KSH.Close()
            'UPGRADE_NOTE: オブジェクト WLS_KSH をガベージ コレクトするまでこのオブジェクトを破棄することはできません。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"' をクリックしてください。
            WLS_KSH = Nothing
			
			gv_bolWLSTHNDAT_LF_Enable = True
			
			If WLSMEI_RTNMEINMB <> "" Then
				'検索ＯＫ
				'画面に編集
				'UPGRADE_WARNING: オブジェクト CF_Cnv_Dsp_Item() の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				'UPGRADE_WARNING: オブジェクト Dsp_Value の既定プロパティを解決できませんでした。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"' をクリックしてください。
				Dsp_Value = CF_Cnv_Dsp_Item(WLSMEI_RTNMEINMB, pm_All.Dsp_Sub_Inf(Trg_Index), False)
				Call CF_Set_Item_Direct(Dsp_Value, pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
				...
				If Chk_Move_Flg = True Then
					'ﾁｪｯｸ後移動あり
					Call WLSTHNDAT0001.F_Set_Next_Focus(pm_All.Dsp_Sub_Inf(Next_Focus), NEXT_FOCUS_MODE_KEYRETURN, Move_Flg, pm_All)
				Else
					'ﾌｫｰｶｽ移動
					Call CF_Set_Item_SetFocus(pm_All.Dsp_Sub_Inf(Trg_Index), pm_All)
				End If
	...
	Public Function F_Init_Clr_Dsp(ByRef pm_Index As Short, ByRef pm_All As Cls_All) As Short
		...
		For Index_Wk = Wk_Index_S To Wk_Index_E
			...
			Select Case Index_Wk
				Case WLS_THNDAT.LST.TabIndex
					WLS_THNDAT.LST.Items.Clear()
				Case Else
			End Select
	...
	Public Function F_Init_Cursor_Set(ByRef pm_All As Cls_All) As Short
		...
		'割当ｲﾝﾃﾞｯｸｽ取得
		Trg_Index = CShort(WLS_THNDAT.HD_STS.Tag)
