...
Friend Class WLSTOK6
	Inherits System.Windows.Forms.Form
	'********************************************************************************
	'*  システム名　　　：  新総合情報システム
	'*  サブシステム名　：　販売システム
	'*  機能　　　　　　：　検索ウィンドウ
	'*  プログラム名　　：　得意先検索（請求先より呼ばれる）
	'*  プログラムＩＤ　：  WLSTOK6
	'*  作成者　　　　　：　RISE)宮島
	'*  作成日　　　　　：  2008.08.26
	'*-------------------------------------------------------------------------------
	...
	Private Sub WLS_SetArray(ByVal ArrayCnt As Short)
		...
		End Select
		'
		WM_WLS_DSPArray(ArrayCnt) = LeftWid(DB_TOKMAT_W.TOKCD, 10) & Space(1) & LeftWid(DB_TOKMAT_W.TOKRN, 40) & Space(3) & WK_SMENM & WK_KESNM & Space(1) & WK_ZEINM & Space(1) & LeftWid(DB_TOKMAT_W.TOKTL, 13) & Space(1) & DB_TOKMAT_W.TOKSEICD
	...
	Sub WLS_TextSQL()
		...
		If Trim(WM_WLS_CODE) <> "" Then
			strSQL = strSQL & "  Where TOKCD >=   '" & WM_WLS_CODE & "'"
		Else
			strSQL = strSQL & "  Where TOKCD >=   ' '"
		End If
	...
	Private Sub WLS_DspNew()
        ...
        For i As Integer = 0 To dsList.Tables("tableName").Rows.Count - 1
            ...
            DB_TOKMAT_W.TOKRN = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKRN"), "") '得意先略所
            DB_TOKMAT_W.TOKZEIKB = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKZEIKB"), "") '消費税区分
            DB_TOKMAT_W.TOKSMEKB = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKSMEKB"), "") '締区分
            DB_TOKMAT_W.TOKSMEDD = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKSMEDD"), "") '締初期日付（売上）
            DB_TOKMAT_W.TOKKESCC = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKKESCC"), "") '回収サイクル締初期日付（売上）
            DB_TOKMAT_W.TOKKESDD = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKKESDD"), "") '回収日付
            DB_TOKMAT_W.TOKSDWKB = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKSDWKB"), "") '回収曜日
            DB_TOKMAT_W.TOKKDWKB = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKKDWKB"), "") '締め曜日
            DB_TOKMAT_W.TOKTL = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKTL"), "") '得意先電話番号
	...
	'以下は画面イベント処理
	'
	'UPGRADE_WARNING: Form イベント WLSTOK6.Activate には新しい動作が含まれます。 詳細については、'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"' をクリックしてください。
	Private Sub WLSTOK6_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
    ...
    Private Sub WLSTOK6_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        'Window初期設定
        Call WLS_FORM_INIT()
    ...
    '20190627 ADD START
    Private Sub WLSTOK6_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
