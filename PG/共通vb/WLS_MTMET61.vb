...
Friend Class WLS_MTMET61
	...
	Private Sub WLS_DspNew()
        ...
        For i As Integer = 0 To dsList.Tables("tableName").Rows.Count - 1
            '取得内容退避
            DB_MITTHA_W.MITNO = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("MITNO"), "") '見積番号
            DB_MITTHA_W.MITNOV = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("MITNOV"), "") '版数
            DB_MITTHA_W.JDNTRKB = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("JDNTRKB"), "") '受注取引区分
            DB_MITTHA_W.JDNTRKBNM = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("JDNTRKBNM"), "") '受注取引区分名

            If IsDate(VB6.Format(DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("MITDT"), ""), "@@@@/@@/@@")) = True Then
                DB_MITTHA_W.MITDT = VB6.Format(DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("MITDT"), ""), "@@@@/@@/@@")
            Else
                DB_MITTHA_W.MITDT = Space(10)
            End If

            DB_MITTHA_W.TOKRN = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("TOKRN"), "") '得意先略称
            DB_MITTHA_W.KENNMA = DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("KENNMA"), "") '件名１

            If DB_NullReplace(dsList.Tables("tableName").Rows(i).Item("KKTMTFL"), "") = "1" Then
                DB_MITTHA_W.KKTMTFL = "確定" '確定見積区分
            Else
                DB_MITTHA_W.KKTMTFL = "未確定" '確定見積区分
            End If
   ...
   '         Case CShort(CM_PrevCm.Tag)
    ...
    '20190521 ADD START
    Private Sub WLS_MTMET61_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub btnF2_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles btnF2.Click
        ...
        Try
            If Me.HD_MITNO.Focused Then
                Call HD_MITNO_KeyDown(HD_MITNO, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            ElseIf Me.HD_MITNOV.Focused Then
                Call HD_MITNOV_KeyDown(HD_MITNOV, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            ElseIf Me.HD_JDNTRKB.Focused Then
                Call HD_JDNTRKB_KeyDown(HD_JDNTRKB, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            ElseIf Me.HD_KKTFL.Focused Then
                Call HD_KKTFL_KeyDown(HD_KKTFL, New System.Windows.Forms.KeyEventArgs(Keys.Return))
            ElseIf Me.HD_MITDT.Focused Then
                Call HD_MITDT_KeyDown(HD_MITDT, New System.Windows.Forms.KeyEventArgs(Keys.Return))
    ...
    '20190517 ADD START
    Private Sub CS_MITDT_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CS_MITDT.Click
        Call Ctl_Item_Click(CS_MITDT)
    End Sub
    '20190517 ADD END
